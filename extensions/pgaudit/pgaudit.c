...
stack_push()
{
  ...
  DBUG_PRINT("pgaudit", "push new item onto the stack(stackId:%ld)", stackItem->stackId);
...
{
  DBUG_TRACE;
  DBUG_PRINT("pgaudit", "pop an audit event from the stack(stackId:%ld)", stackId);
...
stack_valid(int64 stackId)
{
  ...
  if (nextItem == NULL) {
    ...
    DBUG_PRINT("pgaudit", "something went wrong");
  } else {
    DBUG_PRINT("pgaudit", "the item(stackId:%ld) is on the stack", stackId);
  }
}
                 ...
                 int commandLoc, int commandLen)
{
  ...
  if (auditLogStatement) {
    ...
    DBUG_PRINT("pgaudit", "assign final command text:'%s' and length:%d", commandText, commandLen);
  }
}
...
log_audit_event(AuditEventStackItem *stackItem)
{
  ...
  if (creating_extension) {
    DBUG_PRINT("pgaudit", "skip logging script statements if an extension is currently being created or altered");
    return;
  }

  /* If this event has already been logged don't log it again */
  if (stackItem->auditEvent.logged) {
    DBUG_PRINT("pgaudit", "if this event has already been logged, don't log it again");
    return;
  }
  ...
  switch (stackItem->auditEvent.logStmtLevel) {
    ...
    case LOGSTMT_MOD:
      ...
      class = LOG_WRITE;
      DBUG_PRINT("pgaudit", "all mods go in WRITE class, except EXECUTE");

      switch (stackItem->auditEvent.commandTag) {
        /* Currently, only EXECUTE is different */
        case T_ExecuteStmt:
          DBUG_PRINT("pgaudit", "currently, only EXECUTE is different");
    ...
    case LOGSTMT_DDL:
      DBUG_PRINT("pgaudit", "these are DDL unless they are ROLE");
      className = CLASS_DDL;
      class = LOG_DDL;
      ...
      switch (stackItem->auditEvent.commandTag) {
        ...
        case T_AlterUserMappingStmt:
          ...
          if (stackItem->auditEvent.commandText != NULL) {
            ...
            DBUG_PRINT("pgaudit", "find index of password token");
    ...
    case LOGSTMT_ALL:
      switch (stackItem->auditEvent.commandTag) {
        ...
        case T_PrepareStmt:
        case T_PlannedStmt:
          DBUG_PRINT("pgaudit", "read statements");
        ...
        /* FUNCTION statements */
        case T_DoStmt:
          DBUG_PRINT("pgaudit", "function statements");
        ...
        case T_VariableSetStmt:
          DBUG_PRINT("pgaudit", "set statements reported as MISC but filtered by MISC_SET flags to maintain existing functionality");
          className = CLASS_MISC;
          class = LOG_MISC_SET;
          break;
  ...
  DBUG_PRINT("pgaudit", "only log the statement if:");
  DBUG_PRINT("pgaudit", "1. The object was selected for audit logging (granted), or");
  DBUG_PRINT("pgaudit", "2. The statement belongs to a class that is being logged");
  ...
  if (!stackItem->auditEvent.granted && !(auditLogBitmap & class)) {
    DBUG_PRINT("pgaudit", "if neither of these is true, return");
    return;
  }
  ...
  if (stackItem->auditEvent.statementId == 0) {
    ...
    if (!statementLogged) {
      DBUG_PRINT("pgaudit", "if nothing has been logged yet, then create a new statement Id");
      statementTotal++;
  ...
  if (auditLogStatement && !(stackItem->auditEvent.statementLogged && auditLogStatementOnce)) {
    ...
    char *commandStr = pnstrdup(stackItem->auditEvent.commandText,
                                stackItem->auditEvent.commandLen);
    DBUG_PRINT("pgaudit", "log the command:%s", commandStr);
    ...
    if (auditLogParameter) {
      ...
      DBUG_PRINT("pgaudit", "handle parameter logging, if enabled");
  ...
  /* we were asked to not log it */
    ...
    DBUG_PRINT("pgaudit", "we were asked not to log it");
    appendStringInfoString(&auditStr,
                           "<not logged>,<not logged>");
  } else {
    DBUG_PRINT("pgaudit", "previously logged");
    appendStringInfoString(&auditStr,
  ...
  /* Log rows affected */
  if (auditLogRows) {
    DBUG_PRINT("pgaudit", "log rows affected:%ld", stackItem->auditEvent.rows);
   ...
   * pgaudit anyway.
   */
  DBUG_PRINT("pgaudit", "log the audit entry");
             ...
             AclMode mask)
{
  ...
  if (result) {
    DBUG_PRINT("pgaudit", "return true");
  } else {
    DBUG_PRINT("pgaudit", "return false");
  }
                       ...
                       AclMode mode)
{
  ...
  if (bms_is_empty(attributeSet)) {
    ...
    DBUG_PRINT("pgaudit", "get relation to determine total columns:%d", nattrs);
...
log_select_dml(Oid auditOid, List *rangeTabls, List *permInfos)
{
  ...
  if (internalStatement) {
    DBUG_PRINT("pgaudit", "do not log if this is an internal statement");
    return;
  }
  ...
  foreach(lr, rangeTabls) {
    ...
    if (!is_member_of_role(GetSessionUserId(), GetUserId())) {
      DBUG_PRINT("pgaudit", "don't log if the session user is not a member of the current role");
      return;
    }
    ...
    if (first && !auditLogRelation) {
      DBUG_PRINT("pgaudit", "if this is the first RTE, then session log unless auditLogRelation is set");
    ...
    if (auditEventStack->auditEvent.granted) {
      ...
      DBUG_PRINT("pgaudit", "do relation level logging if a grant was found");
      log_audit_event(auditEventStack);
    }
    ...
    if (auditLogRelation) {
      auditEventStack->auditEvent.logged = false;
      auditEventStack->auditEvent.granted = false;
      DBUG_PRINT("pgaudit", "do relation level logging if auditLogRelation is set");
      log_audit_event(auditEventStack);
    }
  ...
  if (!found) {
    ...
    DBUG_PRINT("pgaudit", "if no tables were found that means that RangeTbls was empty or all");
    DBUG_PRINT("pgaudit", "relations were in the system schema.  In that case still log a session record");
    log_audit_event(auditEventStack);
  }
}
...
log_function_execute(Oid objectId)
{
  ...
  DBUG_PRINT("pgaudit", "get info about the function(%u)", objectId);
  ...
  if (IsCatalogNamespace(proc->pronamespace)) {
    DBUG_PRINT("pgaudit", "logging execution of all pg_catalog functions would make the log unusably noisy");
    ReleaseSysCache(proctup);
    return;
  }

  /* Push audit event onto the stack */
  DBUG_PRINT("pgaudit", "push audit event onto the stack");
  ...
  DBUG_PRINT("pgaudit", "generate the fully-qualified function name:'%s'", stackItem->auditEvent.objectName);
  ...
  DBUG_PRINT("pgaudit", "log the function call");
...
pgaudit_ExecutorStart_hook(QueryDesc *queryDesc, int eflags)
{
  ...
  if (stackItem) {
    ...
    if (auditLogRows) {
      DBUG_PRINT("pgaudit", "set query context for tracking rows processed");
...
pgaudit_ExecutorRun_hook(QueryDesc *queryDesc, ScanDirection direction, uint64 count)
{
  ...
  if (next_ExecutorRun_hook) {
    DBUG_PRINT("pgaudit", "call the previous hook");
    next_ExecutorRun_hook(queryDesc, direction, count);
  } else {
    DBUG_PRINT("pgaudit", "call standard function");
  ...
  if (auditLogRows && !internalStatement && !IsParallelWorker()) {
    ...
    if (stackItem != NULL) {
      STACK_NOT_EMPTY();
      DBUG_PRINT("pgaudit", "tuples processed during one ExecutorRun() call:%ld", queryDesc->estate->es_processed);
      stackItem->auditEvent.rows += queryDesc->estate->es_processed;
      DBUG_PRINT("pgaudit", "accumulate the number of rows processed:%ld", stackItem->auditEvent.rows);
    }
  } else {
    DBUG_PRINT("pgaudit", "skip audit");
  }
}
                            ...
                            QueryCompletion *qc)
{
  ...
  DBUG_PRINT("pgaudit", "hook ProcessUtility to do session auditing for DDL and utility commands");
  /*
  ...
  if (context <= PROCESS_UTILITY_QUERY && !IsAbortedTransactionBlockState()) {
    ...
    if (context == PROCESS_UTILITY_TOPLEVEL) {
      DBUG_PRINT("pgaudit", "process top level utility statement");
      ...
      if (auditEventStack != NULL) {
        ...
        do {
          if (nextItem->auditEvent.commandTag != T_SelectStmt &&
            ...
            DBUG_PRINT("pgaudit", "pgaudit stack is not empty");
    ...
    if (auditLogBitmap & LOG_FUNCTION &&
      ...
      DBUG_PRINT("pgaudit", "if this is a DO block, log it before calling the next ProcessUtility hook");
      log_audit_event(stackItem);
    }
    ...
    if (auditLogBitmap & LOG_DDL &&
        ...
        !IsAbortedTransactionBlockState()) {
      DBUG_PRINT("pgaudit", "if this is a create/alter extension command log it before calling the next ProcessUtility hook");
      log_audit_event(stackItem);
    }
    ...
    if (stackItem->auditEvent.commandTag == T_ClosePortalStmt) {
      DBUG_PRINT("pgaudit", "a close will free the open cursor which will also free the close audit entry");

      if (auditLogBitmap & LOG_MISC && !IsAbortedTransactionBlockState()) {
        DBUG_PRINT("pgaudit", "immediately log the close and set stackItem to NULL so it won't be logged later");
        log_audit_event(stackItem);
      }
  ...
  if (next_ProcessUtility_hook) {
    DBUG_PRINT("pgaudit", "call the standard process utility chain");
    (*next_ProcessUtility_hook) (pstmt, queryString, readOnlyTree, context,
                                 params, queryEnv, dest, qc);
  } else {
    DBUG_PRINT("pgaudit", "call standard_ProcessUtility");
  ...
  if (stackItem && !IsAbortedTransactionBlockState()) {
    DBUG_PRINT("pgaudit", "process the audit event if there is one");
    /*
    ...
    if (auditLogBitmap != 0 && !stackItem->auditEvent.logged) {
      DBUG_PRINT("pgaudit", "log the utility command if logging is on");
      log_audit_event(stackItem);
    }
  }
}
                           ...
                           void *arg)
{
  ...
  DBUG_PRINT("pgaudit", "classId:%u, objectId:%u, subId:%d", classId, objectId, subId);

  if (auditLogBitmap & LOG_FUNCTION && access == OAT_FUNCTION_EXECUTE &&
      auditEventStack && !IsAbortedTransactionBlockState()) {
    DBUG_PRINT("pgaudit", "hook object_access_hook to provide fully-qualified object names for function calls");
    log_function_execute(objectId);
  }

  if (next_object_access_hook) {
    DBUG_PRINT("pgaudit", "call next_object_access_hook");
...
pgaudit_ddl_command_end(PG_FUNCTION_ARGS)
{
  ...
  for (row = 0; row < SPI_processed; row++) {
    ...
    if (auditEventStack->auditEvent.command == CMDTAG_GRANT ||
      ...
      DBUG_PRINT("pgaudit", "identify grant/revoke commands");
      log_audit_event(auditEventStack);
  ...
  DBUG_PRINT("pgaudit", "iterate returned rows:%d", row);
  /* Complete the query */
  SPI_finish();
...
pgaudit_sql_drop(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgaudit", "supply additional data for drop statements that have event trigger support");
...
check_pgaudit_log(char **newVal, void **extra, GucSource source)
{
  ...
  if (!SplitIdentifierString(rawVal, ',', &flagRawList)) {
    ...
    DBUG_PRINT("pgaudit", "list syntax is invalid");
...
check_pgaudit_log_level(char **newVal, void **extra, GucSource source)
{
  ...
  if (pg_strcasecmp(*newVal, "debug") == 0)
    ...
    *logLevel = DEBUG1;
  else if (pg_strcasecmp(*newVal, "pgaudit") == 0)
    *logLevel = INFO;
...
_PG_init(void)
{
   ...
   * preserve the chains.
   */
  DBUG_PRINT("pgaudit", "install our hook functions after saving the existing pointers to preserve the chains");
