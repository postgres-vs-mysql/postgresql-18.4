  ...
  *pc = '\0';
  ...
  if (names) {
    for (i = 0;  names[i] != NULL;  i++) {
      DBUG_PRINT("cron", "get_num, compare(%s,%s)", names[i], temp);
      Debug(DPARS | DEXT,
            ("get_num, compare(%s,%s)\n", names[i], temp))
...
/* int    number; */
{
  ...
  DBUG_PRINT("cron", "set_element(?,%d,%d,%d)", low, high, number);
...
{
  DBUG_TRACE;
  DBUG_PRINT("cron", "set_range(?,%d,%d,%d,%d,%d)", low, high, start, stop, step);
