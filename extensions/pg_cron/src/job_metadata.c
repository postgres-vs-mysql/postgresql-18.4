...
GetCronJob(int64 jobId)
{
  ...
  DBUG_PRINT("cron", "get the cron job with the given id:%ld", jobId);
                ...
                text *usernameText, bool active, text *jobnameText)
{
  ...
  DBUG_PRINT("cron", "schedule a cron job with the given name");
...
GetRoleOidIfCanLogin(char *username)
{
  ...
  DBUG_PRINT("cron", "check user exist and can log in");
...
cron_schedule(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("cron", "schedule a job");
...
cron_schedule_named(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("cron", "schedule a named job");
  ...
  */
  ...
  DBUG_PRINT("cron", "jobId:%ld", jobId);
  PG_RETURN_INT64(jobId);
}
/*
...
NextRunId(void)
{
  ...
  DBUG_PRINT("cron", "draw a new run ID from cron.runid_seq:%ld", runId);
  return runId;
}
...
CronExtensionOwner(void)
{
  ...
  DBUG_PRINT("cron", "return the name of the user that owns the extension:%u", extensionOwner);
...
cron_unschedule(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("cron", "remove a cron job");
...
cron_unschedule_named(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("cron", "remove a cron job by name:%s", jobName);
  cronJobsTable = table_open(CronJobRelationId(), RowExclusiveLock);
...
cron_job_cache_invalidate(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("cron", "invalidate the job cache in response to a trigger");
...
CronJobRelationId(void)
{
  ...
  DBUG_PRINT("cron", "return cached oid of the cron.job relation:%u", CachedCronJobRelationId);
...
InsertJobRunDetail(int64 runId, int64 *jobId, char *database, char *username, char *command, char *status)
{
  ...
  if (jobId) {
    DBUG_PRINT("cron", "runId:%ld, jobId:%ld, database:%s, username:%s, command:'%s'", runId, *jobId, database, username, command);
  } else {
    DBUG_PRINT("cron", "runId:%ld, database:%s, username:%s, command:'%s'", runId, database, username, command);
  }
  ...
  /* Open SPI context. */
  DBUG_PRINT("cron", "open SPI context");
  if (SPI_connect() != SPI_OK_CONNECT)
    elog(ERROR, "SPI_connect failed");
                   ...
                   TimestampTz *end_time)
{
  ...
  DBUG_PRINT("cron", "SPI_exec:'%s'", querybuf.data);
  if (SPI_execute_with_args(querybuf.data,
...
MarkPendingRunsAsFailed(void)
{
  ...
  appendStringInfo(&querybuf,
      ...
      , CRON_SCHEMA_NAME, JOB_RUN_DETAILS_TABLE_NAME, 
      GetCronStatus(CRON_STATUS_FAILED), GetCronStatus(CRON_STATUS_STARTING), GetCronStatus(CRON_STATUS_RUNNING));
