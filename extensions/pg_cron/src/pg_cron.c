...
_PG_init(void)
{
  ...
  DBUG_PRINT("cron", "_PG_init is called");
  if (IsBinaryUpgrade) {
    return;
  }
...
PgCronLauncherMain(Datum arg)
{
  ...
  DBUG_PRINT("cron", "the main entry-point for the background worker that performs tasks");
  ...
  DBUG_PRINT("cron", "make pg_cron recognisable in pg_stat_activity");
  ...
  DBUG_PRINT("cron", "determine how many tasks we can run concurrently:%d", MaxRunningTasks);
  ...
  DBUG_PRINT("cron", "pg_cron scheduler started");
  ereport(LOG, (errmsg("pg_cron scheduler started")));

  /* set the desired log_min_messages */
  DBUG_PRINT("cron", "set the desired log_min_messages");
...
StartAllPendingRuns(List *taskList, TimestampTz currentTime)
{
  ...
  if (!RebootJobsScheduled) {
    DBUG_PRINT("cron", "find jobs with @reboot as a schedule");
  ...
  foreach(taskCell, taskList) {
    ...
    if (task->secondsInterval > 0 && task->isActive) {
      ...
      if (task->pendingRunCount == 0 &&
        ...
        DBUG_PRINT("cron", "for interval jobs, if a task takes longer than the interval, we only queue up once");
        task->pendingRunCount += 1;
      }
    }
  }
  ...
  if (minutesPassed == 0) {
    DBUG_PRINT("cron", "wait for new minute");
    /* wait for new minute */
    return;
  }
  ...
  if (minutesPassed > (3 * MINUTE_COUNT)) {
    ...
    DBUG_PRINT("cron", "clock jumped forward by more than 3 hours:%d", minutesPassed);
    clockProgress = CLOCK_CHANGE;
  } else if (minutesPassed > 5) {
    DBUG_PRINT("cron", "clock went forward by more than 5 minutes:%d", minutesPassed);
    /* clock went forward by more than 5 minutes (DST?) */
    clockProgress = CLOCK_JUMP_FORWARD;
  } else if (minutesPassed > 0) {
    DBUG_PRINT("cron", "clock went forward by 1-5 minutes:%d", minutesPassed);
    /* clock went forward by 1-5 minutes */
    clockProgress = CLOCK_PROGRESSED;
  } else if (minutesPassed > -(3 * MINUTE_COUNT)) {
    DBUG_PRINT("cron", "clock jumped backwards by less than 3 hours:%d", minutesPassed);
    /* clock jumped backwards by less than 3 hours (DST?) */
    clockProgress = CLOCK_JUMP_BACKWARD;
  } else {
    DBUG_PRINT("cron", "clock jumped backwards 3 hours or more:%d", minutesPassed);
  ...
  foreach(taskCell, taskList) {
    ...
    if (!task->isActive) {
       ...
       * to ManageCronTask.
       */
      DBUG_PRINT("cron", "the job has been unscheduled, so we should not schedule new runs");
      continue;
    }
  ...
  if (clockProgress != CLOCK_JUMP_BACKWARD) {
    DBUG_PRINT("cron", "if the clock jump backwards then we avoid repeating the fixed-time");
    DBUG_PRINT("cron", "tasks by preserving the last minute from before the clock jump, until the clock has caught up");
    lastMinute = TimestampMinuteStart(currentTime);
  }
}
                 ...
                 TimestampTz lastMinute, TimestampTz currentTime)
{
  ...
  DBUG_PRINT("cron", "kick off pending runs for a task if it should start, taking clock changes into consideration");
  ...
  switch (clockProgress) {
    case CLOCK_PROGRESSED: {
       ...
       */
      DBUG_PRINT("cron", "case 1: minutesPassed is a small positive number run jobs for each virtual minute until caught up");
    ...
    case CLOCK_JUMP_FORWARD: {
      ...
      DBUG_PRINT("cron", "run fixed-time jobs for each minute missed");
      ...
      DBUG_PRINT("cron", "run wildcard jobs for current minute");
    ...
    case CLOCK_JUMP_BACKWARD: {
      ...
      DBUG_PRINT("cron", "case 3: timeDiff is a small or medium-sized negative num");
    ...
    default: {
       ...
       * normal operation.
       */
      DBUG_PRINT("cron", "other: time has changed a lot, skip over any intermediate fixed-time jobs and go back t normal operation");
...
MinutesPassed(TimestampTz startTime, TimestampTz stopTime)
{
  ...
  DBUG_PRINT("cron", "return the number of minutes between startTime and topTime rounded down to the closest integer:%d", minutesPassed);
...
TimestampMinuteStart(TimestampTz time)
{
  ...
  DBUG_PRINT("cron", "return the timestamp at the start of the current minute for the given time:%ld", result);
  return result;
}
...
TimestampMinuteEnd(TimestampTz time)
{
  ...
  DBUG_PRINT("cron", "return the timestamp at the start of the next minute from the given time:%ld", result);
  return result;
}
              ...
              bool doNonWild)
{
  ...
  if (bit_test(schedule->minute, minute) &&
    ...
    if ((doNonWild && (schedule->flags & (MIN_STAR | HR_STAR)) == 0) ||
      ...
      DBUG_PRINT("cron", "return true");
      return true;
    }
  }

  DBUG_PRINT("cron", "return false");
  return false;
}
...
WaitForCronTasks(List *taskList)
{
  ...
  DBUG_PRINT("cron", "block waiting for any active task for at most 1 second");

  if (taskCount > 0) {
    DBUG_PRINT("cron", "task count:%d", taskCount);
...
PollForTasks(List *taskList)
{
  ...
  DBUG_PRINT("cron", "call poll() for the sockets of all tasks");
  ...
  foreach(taskCell, taskList) {
    ...
    if (activeTaskCount >= MaxRunningTasks) {
      DBUG_PRINT("cron", "already polling the maximum number(%d) of tasks", MaxRunningTasks);
    ...
    if (task->state == CRON_TASK_ERROR || task->state == CRON_TASK_DONE ||
        CanStartTask(task)) {
      DBUG_PRINT("cron", "there is work to be done(jobId:%ld), don't wait", task->jobId);
    ...
    if (task->state == CRON_TASK_WAITING && task->pendingRunCount == 0) {
      ...
      if (task->secondsInterval > 0) {
        ...
        DBUG_PRINT("cron", "make sure we do not wait past the next run time of an interval job");
      ...
      DBUG_PRINT("cron", "don't poll idle tasks");
    ...
    if (task->state == CRON_TASK_CONNECTING ||
      ...
      if (TimestampDifferenceExceeds(task->startDeadline, nextEventTime, 0)) {
        DBUG_PRINT("cron", "we need to wake up when a timeout expires");
        DBUG_PRINT("cron", "take the minimum of nextEventTime and task->startDeadline");
        nextEventTime = task->startDeadline;
      }
    }

    DBUG_PRINT("cron", "we plan to poll this task(jobId:%ld)", task->jobId);
    ...
    if (task->state == CRON_TASK_CONNECTING ||
       ...
       * Task is not running.
       */
      DBUG_PRINT("cron", "task is not running");
   ...
   * minute or a timeout.
   */
  DBUG_PRINT("cron", "find the first time-based event, which is either the start of a new minute or a timeout");
  ...
  if (pollTimeout <= 0) {
     ...
     */
    DBUG_PRINT("cron", "interval jobs might frequently be overdue, inject a small 1ms wait to avoid getting into a tight loop");
    pollTimeout = 1;
    ...
    DBUG_PRINT("cron", "we never wait more than 1 second, this gives us a chance to react");
    DBUG_PRINT("cron", "to external events like a TERM signal and job changes");
    pollTimeout = MaxWait;
  }

  if (activeTaskCount == 0) {
    /* turns out there's nothing to do, just wait for something to happen */
    DBUG_PRINT("cron", "turn out there's nothing to do, just wait for something to happen");
  ...
  DBUG_PRINT("cron", "poll is called, returning:%d", pollResult);
  ...
  if (pollResult < 0) {
     ...
     */
    DBUG_PRINT("cron", "this typically happens in case of a signal, though we should probably check errno in case something bad happened");
    pfree(polledTasks);
    pfree(pollFDs);
    return;
  }
...
CanStartTask(CronTask *task)
{
  DBUG_TRACE;
  bool result;
  result = task->state == CRON_TASK_WAITING && task->pendingRunCount > 0 &&
           RunningTaskCount < MaxRunningTasks;

  if (result) {
    DBUG_PRINT("cron", "task pendingRunCount:%d, RunningTaskCount:%d, MaxRunningTasks:%d", task->pendingRunCount, RunningTaskCount, MaxRunningTasks);
    DBUG_PRINT("cron", "CanStartTask returns true");
  } else {
    if (task->state == CRON_TASK_WAITING) {
      DBUG_PRINT("cron", "task state: CRON_TASK_WAITING");
      DBUG_PRINT("cron", "task pendingRunCount:%d, RunningTaskCount:%d, MaxRunningTasks:%d", task->pendingRunCount, RunningTaskCount, MaxRunningTasks);
    } else {
      DBUG_PRINT("cron", "task state is not CRON_TASK_WAITING, current state: %d", task->state);
    }
...
ManageCronTasks(List *taskList, TimestampTz currentTime)
{
  ...
  DBUG_PRINT("cron", "proceed the state machines of the given list of tasks");
...
ManageCronTask(CronTask *task, TimestampTz currentTime)
{
  ...
  DBUG_PRINT("cron", "implement the cron task state machine(jobId:%ld)", jobId);
  ...
  switch (checkState) {
    case CRON_TASK_WAITING: {
      ...
      if (!task->isActive) {
        /* remove task as well */
        DBUG_PRINT("cron", "remove task as well");
        RemoveTask(jobId);
        break;
      }

      DBUG_PRINT("cron", "checkState: CRON_TASK_WAITING");
      ...
      if (UseBackgroundWorkers) {
        ...
        DBUG_PRINT("cron", "set task state: CRON_TASK_BGW_START");
      } else {
        task->state = CRON_TASK_START;
        DBUG_PRINT("cron", "set task state: CRON_TASK_START");
      }
      ...
      DBUG_PRINT("cron", "add new entry to audit table");
    ...
    case CRON_TASK_START: {
      ...
      if (!UseBackgroundWorkers) {
        ...
        DBUG_PRINT("cron", "checkState: CRON_TASK_START");
        if (CronLogStatement) {
        ...
        DBUG_PRINT("cron", "begin the establishment of a connection to a postgres backend through the postmaster");
        ...
        DBUG_PRINT("cron", "set task state: CRON_TASK_CONNECTING");
    ...
    case CRON_TASK_BGW_START: {
       ...
       */
      DBUG_PRINT("cron", "checkState: CRON_TASK_BGW_START");
...
#define QUEUE_SIZE ((Size) 65536)
       ...
       *
       */
      DBUG_PRINT("cron", "prepare the background worker");
       ...
       */
      DBUG_PRINT("cron", "start the worker process");

      if (CronLogStatement) {
        ereport(LOG, (errmsg("cron job " INT64_FORMAT " %s: %s",
                             jobId, GetCronStatus(CRON_STATUS_STARTING), command)));
      }
      DBUG_PRINT("cron", "cron job " INT64_FORMAT " %s: %s", jobId, GetCronStatus(CRON_STATUS_STARTING), command);
      ...
      DBUG_PRINT("cron", "set task state: CRON_TASK_BGW_RUNNING");
      task->state = CRON_TASK_BGW_RUNNING;
      break;
    }
    ...
    case CRON_TASK_CONNECTING: {
      ...
      DBUG_PRINT("cron", "checkState: CRON_TASK_CONNECTING");
      /* check if job has been removed */
      if (jobCanceled(task)) {
        DBUG_PRINT("cron", "job has been removed");
        break;
      }

      /* check if timeout has been reached */
      if (jobStartupTimeout(task, currentTime)) {
        DBUG_PRINT("cron", "timeout has been reached");
        break;
      }
      ...
      if (connectionStatus == CONNECTION_BAD) {
        DBUG_PRINT("cron", "connection failed");
        task->errorMessage = "connection failed";
        task->pollingStatus = 0;
        task->state = CRON_TASK_ERROR;
        break;
      ...
      if (!task->isSocketReady) {
        DBUG_PRINT("cron", "socket is not ready to send");
        break;
      }

      /* check whether a connection has been established */
      DBUG_PRINT("cron", "check whether a connection has been established");
      pollingStatus = PQconnectPoll(connection);

      if (pollingStatus == PGRES_POLLING_OK) {
        pid_t pid;
        /* wait for socket to be ready to send a query */
        DBUG_PRINT("cron", "wait for socket to be ready to send a query");
        task->pollingStatus = PGRES_POLLING_WRITING;

        DBUG_PRINT("cron", "set task state: CRON_TASK_SENDING");
        task->state = CRON_TASK_SENDING;
         ...
         */
        DBUG_PRINT("cron", "connection is still being established");
    ...
    case CRON_TASK_SENDING: {
      ...
      DBUG_PRINT("cron", "checkState: CRON_TASK_SENDING");
      Assert(!UseBackgroundWorkers);
      ...
      if (sendResult == 1) {
        ...
        DBUG_PRINT("cron", "command is underway, stop using timeout");
        /* command is underway, stop using timeout */
        task->startDeadline = 0;
        DBUG_PRINT("cron", "set task state: CRON_TASK_RUNNING");
        task->state = CRON_TASK_RUNNING;
    ...
    case CRON_TASK_RUNNING: {
      ...
      DBUG_PRINT("cron", "checkState: CRON_TASK_RUNNING");
      /* check if connection is still alive */
      connectionStatus = PQstatus(connection);
      ...
      if (connectionBusy) {
        DBUG_PRINT("cron", "still waiting for results");
      ...
      DBUG_PRINT("cron", "set task state: CRON_TASK_DONE");
    ...
    case CRON_TASK_BGW_RUNNING: {
      ...
      DBUG_PRINT("cron", "checkState: CRON_TASK_BGW_RUNNING");
      /* check if job has been removed */
      if (jobCanceled(task)) {
    ...
    case CRON_TASK_ERROR: {
      ...
      DBUG_PRINT("cron", "checkState: CRON_TASK_ERROR");
      if (!task->isActive) {
        RemoveTask(jobId);
      }
    ...
    default: {
      ...
      DBUG_PRINT("cron", "checkState: CRON_TASK_DONE");
      ...
      if (job != NULL && job->active)
        ...
        DBUG_PRINT("cron", "set task state: CRON_TASK_WAITING");
      }
...
CronBackgroundWorker(Datum main_arg)
{
  ...
  DBUG_PRINT("cron", "background worker logic");
  ...
  DBUG_PRINT("cron", "prepare to execute the query");
  ...
  DBUG_PRINT("cron", "execute the query");
  /* Execute the query. */
...
ExecuteSqlString(const char *sql)
{
   ...
   */
  DBUG_PRINT("cron", "execute given SQL string without SPI or a libpq session:%s", sql);
