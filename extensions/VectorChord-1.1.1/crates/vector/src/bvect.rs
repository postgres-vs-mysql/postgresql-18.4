...
impl VectorOwned for BVectOwned {
    ...
    fn as_borrowed(&self) -> BVectBorrowed<'_> {
        let _guard = trace_guard!("VectorOwned(BVectOwned)::as_borrowed [rust]");
        BVectBorrowed {
            dim: self.dim,
            data: &self.data,
        }
...
impl<'a> BVectBorrowed<'a> {
    ...
    pub fn new_checked(dim: u32, data: &'a [u64]) -> Option<Self> {
        let _guard = trace_guard!("BVectBorrowed::new_checked [rust]");
        if !(1..=65535).contains(&dim) {
            return None;
        }
        if data.len() != dim.div_ceil(BVECTOR_WIDTH) as usize {
    ...
    pub fn iter(self) -> impl Iterator<Item = bool> + 'a {
        let _guard = trace_guard!("BVectBorrowed::iter [rust]");
        let mut index = 0_u32;
...
impl VectorBorrowed for BVectBorrowed<'_> {
    ...
    #[inline(always)]
    fn norm(&self) -> f32 {
        let _guard = trace_guard!("VectorBorrowed(BVectBorrowed)::norm [rust]");
        (simd::bit::reduce_sum_of_x(self.data) as f32).sqrt()
    }

    #[inline(always)]
    fn operator_dot(self, rhs: Self) -> Distance {
        let _guard = trace_guard!("VectorBorrowed(BVectBorrowed)::operator_dot [rust]");
    ...
    fn operator_hamming(self, rhs: Self) -> Distance {
        let _guard = trace_guard!("VectorBorrowed(BVectBorrowed)::operator_hamming [rust]");
        Distance::from(simd::bit::reduce_sum_of_xor(self.data, rhs.data) as f32)
    }

    #[inline(always)]
    fn operator_jaccard(self, rhs: Self) -> Distance {
        let _guard = trace_guard!("VectorBorrowed(BVectBorrowed)::operator_jaccard [rust]");
    ...
    fn operator_and(&self, rhs: Self) -> Self::Owned {
        let _guard = trace_guard!("VectorBorrowed(BVectBorrowed)::operator_and [rust]");
    ...
    fn operator_or(&self, rhs: Self) -> Self::Owned {
        let _guard = trace_guard!("VectorBorrowed(BVectBorrowed)::operator_or [rust]");
        assert_eq!(self.dim, rhs.dim);
    ...
    fn operator_xor(&self, rhs: Self) -> Self::Owned {
        let _guard = trace_guard!("VectorBorrowed(BVectBorrowed)::operator_xor [rust]");
        assert_eq!(self.dim, rhs.dim);
...
impl PartialEq for BVectBorrowed<'_> {
    fn eq(&self, other: &Self) -> bool {
        let _guard = trace_guard!("PartialEq(BVectBorrowed)::eq [rust]");
...
impl PartialOrd for BVectBorrowed<'_> {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        let _guard = trace_guard!("PartialOrd(BVectBorrowed)::partial_cmp [rust]");
