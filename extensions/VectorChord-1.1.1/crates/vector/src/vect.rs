...
impl<S: Floating> VectorBorrowed for VectBorrowed<'_, S> {
    ...
    #[inline(always)]
    fn norm(&self) -> f32 {
        let _guard = trace_guard!("VectorBorrowed(VectBorrowed)::norm [rust]");
        S::reduce_sum_of_x2(self.0).sqrt()
    }

    #[inline(always)]
    fn operator_dot(self, rhs: Self) -> Distance {
        let _guard = trace_guard!("VectorBorrowed(VectBorrowed)::operator_dot [rust]");
        Distance::from(-S::reduce_sum_of_xy(self.slice(), rhs.slice()))
    }

    #[inline(always)]
    fn operator_l2s(self, rhs: Self) -> Distance {
        let _guard = trace_guard!("VectorBorrowed(VectBorrowed)::operator_l2s [rust]");
        Distance::from(S::reduce_sum_of_d2(self.slice(), rhs.slice()))
    }

    #[inline(always)]
    fn operator_cos(self, rhs: Self) -> Distance {
        let _guard = trace_guard!("VectorBorrowed(VectBorrowed)::operator_cos [rust]");
    ...
    fn function_normalize(&self) -> VectOwned<S> {
        let _guard = trace_guard!("VectorBorrowed(VectBorrowed)::function_normalize [rust]");
