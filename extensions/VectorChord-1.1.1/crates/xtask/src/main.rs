...
fn pg_config(pg_config: impl AsRef<Path>) -> Result<HashMap<String, String>, Box<dyn Error>> {
    let _guard = trace_guard!("pg_config [rust]");
...
fn control_file(path: impl AsRef<Path>) -> Result<HashMap<String, String>, Box<dyn Error>> {
    let _guard = trace_guard!("control_file [rust]");
...
fn rustc_cfg(target: &str) -> Result<RustcCfg, Box<dyn Error>> {
    let _guard = trace_guard!("rustc_cfg [rust]");
...
fn cargo_metadata() -> Result<CargoMetadata, Box<dyn Error>> {
    let _guard = trace_guard!("cargo_metadata [rust]");
...
fn build(
    ...
    let _guard = trace_guard!("xtask::build [rust]");
    let mut command = Command::new("cargo");
    command.args(["rustc"]);
...
fn parse(rustc_cfg: &RustcCfg, obj: impl AsRef<Path>) -> Result<Vec<String>, Box<dyn Error>> {
    let _guard = trace_guard!("xtask::parse [rust]");
    let obj = obj.as_ref();
...
fn generate(
    ...
    let _guard = trace_guard!("xtask::generate [rust]");
...
fn install_by_copying(
    ...
    let _guard = trace_guard!("xtask::install_by_copying [rust]");
...
fn install_by_writing(
    ...
    let _guard = trace_guard!("xtask::install_by_writing [rust]");
...
fn clippy(
    ...
    let _guard = trace_guard!("xtask::clippy [rust]");
    let mut command = Command::new("cargo");
    command
        .args(["clippy"])
        .args(["--workspace"])
...
fn main() -> Result<(), Box<dyn Error>> {
    let _guard = trace_guard!("xtask::main [rust]");
    #[allow(unsafe_code)]
    unsafe {
