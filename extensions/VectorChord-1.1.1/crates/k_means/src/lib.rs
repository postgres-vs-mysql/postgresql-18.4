...
pub fn k_means_lookup(sample: &[f32], centroids: &Square) -> usize {
    let _guard = trace_guard!("k_means_lookup [rust]");
    use simd::Floating;
    ...
    trace_vchord_print!("centroids compared: {}, result: {}", count, result.1);
    result.1
}
...
pub fn lloyd_k_means<'a>(
    ...
    let _guard = trace_guard!("lloyd_k_means [rust]");
    assert!(d > 0 && c > 0);
    let n = samples.len();
    if n <= c {
...
pub fn hierarchical_k_means<'a>(
    ...
    let _guard = trace_guard!("hierarchical_k_means [rust]");
    assert!(d > 0 && c > 0);
