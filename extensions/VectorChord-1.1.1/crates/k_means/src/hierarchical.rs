...
impl<'a> KMeans for Hierarchical<'a> {
    ...
    fn assign(&mut self) {
        ...
        trace_vchord_print!("partitions num: {}", count);
    }
...
fn prefect_index(
    ...
    let _guard = trace_guard!("hierarchical::prefect_index [rust]");
...
fn index(
    ...
    let _guard = trace_guard!("hierarchical::index [rust]");
    let indexes = partitions.iter().map(|p| p.index()).collect::<Vec<_>>();
    move |sample| {
        use simd::Floating;
        let mut result = (f32::INFINITY, 0);
        trace_vchord_print!("coarse_centroids.len: {}", coarse_centroids.len());
...
pub fn new<'a>(
    ...
    let _guard = trace_guard!("hierarchical::new [rust]");
...
fn modified_webster_method(n: usize, weight: &[usize]) -> Vec<usize> {
    let _guard = trace_guard!("hierarchical::modified_webster_method [rust]");
    assert!(n >= weight.len());
...
fn partition<'a>(mut a: SquareMut<'a>, groups: &[impl AsRef<[usize]>]) -> Vec<SquareMut<'a>> {
    let _guard = trace_guard!("modified_webster_method::partition [rust]");
    let n = a.len();
