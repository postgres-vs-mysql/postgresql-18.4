...
impl<'a> KMeans for Flat<'a> {
    fn prefect_index(&self) -> Box<dyn Fn(&[f32]) -> (f32, usize) + Sync + '_> {
        let _guard = trace_guard!("k_means(Flat)::prefect_index [rust]");
        Box::new(prefect_index(&self.centroids))
    }

    fn assign(&mut self) {
        let _guard = trace_guard!("k_means(Flat)::assign [rust]");
        let this = &mut self.this;
        let samples = &mut self.samples;
        let centroids = &self.centroids;
        let targets = &mut self.targets;
        let index = index(centroids);
    ...
    fn update(&mut self) {
        let _guard = trace_guard!("k_means(Flat)::update [rust]");
        crate::index::update(
            &mut self.this,
            &self.samples,
            &self.targets,
    ...
    fn finish(self: Box<Self>) -> Square {
        let _guard = trace_guard!("k_means(Flat)::finish [rust]");
...
pub fn new<'a>(
    ...
    let _guard = trace_guard!("k_means(Flat)::new [rust]");
