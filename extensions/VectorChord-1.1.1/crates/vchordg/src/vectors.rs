...
pub fn by_prefetch<'b, R: RelationRead>(
    ...
    let _guard = trace_guard!("by_prefetch [rust]");
...
pub fn by_read<'b, R: RelationRead>(
    ...
    let _guard = trace_guard!("by_read [rust]");
...
pub fn copy_outs(x: &[OptionNeighbour]) -> VecDeque<(u32, u16)> {
    let _guard = trace_guard!("copy_outs [rust]");
    x.iter()
...
pub fn copy_all(x: &[OptionNeighbour]) -> VecDeque<((u32, u16), Distance)> {
    let _guard = trace_guard!("copy_all [rust]");
    x.iter()
        .flat_map(|neighbour| neighbour.into_inner())
        .collect::<VecDeque<_>>()
}
...
>(
    ...
    let _guard = trace_guard!("read [rust]");
...
pub fn read_without_accessor<R: RelationRead, O: Operator, Output>(
    ...
    let _guard = trace_guard!("read_without_accessor [rust]");
    let payload_u;
    let neighbours_u;
    let version_u;
    {
...
pub fn update<R: RelationWrite, O: Operator>(
    ...
    let _guard = trace_guard!("update [rust]");
