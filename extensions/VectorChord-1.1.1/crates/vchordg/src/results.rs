...
impl<T> Results<T> {
    pub fn new(ef: usize) -> Self {
        let _guard = trace_guard!("Results::new [rust]");
    ...
    pub fn push(&mut self, item: (Reverse<Distance>, AlwaysEqual<T>)) {
        let _guard = trace_guard!("Results::push [rust]");
    ...
    pub fn peek_ef_th(&mut self) -> Option<Distance> {
        let _guard = trace_guard!("Results::peek_ef_th [rust]");
        if self.front.len() < self.ef {
            None
        } else {
    ...
    pub fn pop_min(&mut self) -> Option<(Distance, AlwaysEqual<T>)> {
        let _guard = trace_guard!("Results::pop_min [rust]");
        if self.front.len() < self.ef {
            self.front.pop_min()
        } else {
    ...
    pub fn into_inner(
        ...
        let _guard = trace_guard!("Results::into_inner [rust]");
