...
impl Vector for VectOwned<f32> {
    ...
    fn unpack(vector: Self::Borrowed<'_>) -> (&[Self::Element], Self::Metadata) {
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f32>)::unpack [rust]");
        (vector.slice(), ())
    }
    ...
    fn split(
        ...
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f32>)::split [rust]");
    ...
    fn pack(_: u32, elements: Vec<Self::Element>, (): Self::Metadata) -> Self {
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f32>)::pack [rust]");
        VectOwned::new(elements)
    }

    fn code(bits: Bits, vector: Self::Borrowed<'_>) -> rabitq::bits::Code {
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f32>)::code [rust]");
        rabitq::bits::code(bits, vector.slice())
    }

    fn preprocess(vector: Self::Borrowed<'_>) -> rabitq::bits::binary::BinaryLut {
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f32>)::preprocess [rust]");
...
impl Vector for VectOwned<f16> {
    ...
    fn unpack(vector: Self::Borrowed<'_>) -> (&[Self::Element], Self::Metadata) {
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f16>)::unpack [rust]");
        (vector.slice(), ())
    }
    ...
    fn split(
        ...
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f16>)::split [rust]");
        let slice = vector.slice();
        let tailing = (size_of::<crate::tuples::OptionNeighbour>() * m)
            .next_multiple_of(crate::tuples::ALIGN);
        assert!(tailing <= 8000);
    ...
    fn pack(_: u32, elements: Vec<Self::Element>, (): Self::Metadata) -> Self {
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f16>)::pack [rust]");
        VectOwned::new(elements)
    }

    fn code(bits: Bits, vector: Self::Borrowed<'_>) -> rabitq::bits::Code {
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f16>)::code [rust]");
        rabitq::bits::code(bits, &f16::vector_to_f32(vector.slice()))
    }

    fn preprocess(vector: Self::Borrowed<'_>) -> rabitq::bits::binary::BinaryLut {
        let _guard = trace_guard!("vchordg::Vector(VectOwned<f16>)::preprocess [rust]");
...
impl Vector for Rabitq8Owned {
    ...
    fn unpack(vector: Self::Borrowed<'_>) -> (&[Self::Element], Self::Metadata) {
        let _guard = trace_guard!("vchordg::Vector(Rabitq8Owned)::unpack [rust]");
        (
            vector.packed_code(),
            [
                vector.sum_of_x2(),
    ...
    fn split(
        ...
        let _guard = trace_guard!("vchordg::Vector(Rabitq8Owned)::split [rust]");
        let metadata = [
            vector.sum_of_x2(),
            vector.norm_of_lattice(),
            vector.sum_of_code(),
        ...
        let slice = vector.packed_code();
        let tailing = (size_of::<crate::tuples::OptionNeighbour>() * m)
            .next_multiple_of(crate::tuples::ALIGN);
        assert!(tailing <= 8000);
        if slice.len() <= 8000 - tailing {
            return (vec![], (slice, metadata));
        }
        let (l, r) = slice.split_at(slice.len() - (8000 - tailing));
        (l.chunks(8000).collect::<Vec<_>>(), (r, metadata))
    }

    fn pack(dim: u32, code: Vec<Self::Element>, [_0, _1, _2, _3]: Self::Metadata) -> Self {
        let _guard = trace_guard!("vchordg::Vector(Rabitq8Owned)::pack [rust]");
        Rabitq8Owned::new(dim, _0, _1, _2, _3, code)
    }

    fn code(bits: Bits, vector: Self::Borrowed<'_>) -> rabitq::bits::Code {
        let _guard = trace_guard!("vchordg::Vector(Rabitq8Owned)::code [rust]");
    ...
    fn preprocess(vector: Self::Borrowed<'_>) -> rabitq::bits::binary::BinaryLut {
        let _guard = trace_guard!("vchordg::Vector(Rabitq8Owned)::preprocess [rust]");
        let scale = vector.sum_of_x2().sqrt() / vector.norm_of_lattice();
        let mut result = Vec::with_capacity(vector.dim() as _);
        for c in vector.unpacked_code() {
            let base = -0.5 * ((1 << 8) - 1) as f32;
...
impl Vector for Rabitq4Owned {
    ...
    fn unpack(vector: Self::Borrowed<'_>) -> (&[Self::Element], Self::Metadata) {
        let _guard = trace_guard!("vchordg::Vector(Rabitq4Owned)::unpack [rust]");
        (
            vector.packed_code(),
            [
                vector.sum_of_x2(),
    ...
    fn split(
        ...
        let _guard = trace_guard!("vchordg::Vector(Rabitq4Owned)::split [rust]");
        let metadata = [
            vector.sum_of_x2(),
            vector.norm_of_lattice(),
            vector.sum_of_code(),
    ...
    fn pack(dim: u32, code: Vec<Self::Element>, [_0, _1, _2, _3]: Self::Metadata) -> Self {
        let _guard = trace_guard!("vchordg::Vector(Rabitq4Owned)::pack [rust]");
        Rabitq4Owned::new(dim, _0, _1, _2, _3, code)
    }

    fn code(bits: Bits, vector: Self::Borrowed<'_>) -> rabitq::bits::Code {
        let _guard = trace_guard!("vchordg::Vector(Rabitq4Owned)::code [rust]");
        let scale = vector.sum_of_x2().sqrt() / vector.norm_of_lattice();
        let mut result = Vec::with_capacity(vector.dim() as _);
        for c in vector.unpacked_code() {
    ...
    fn preprocess(vector: Self::Borrowed<'_>) -> rabitq::bits::binary::BinaryLut {
        let _guard = trace_guard!("vchordg::Vector(Rabitq4Owned)::preprocess [rust]");
        let scale = vector.sum_of_x2().sqrt() / vector.norm_of_lattice();
        let mut result = Vec::with_capacity(vector.dim() as _);
        for c in vector.unpacked_code() {
            let base = -0.5 * ((1 << 4) - 1) as f32;
...
impl Operator for Op<VectOwned<f32>, L2S> {
    ...
    fn process(
        ...
        let _guard = trace_guard!("Operator(Op<VectOwned<f32>, L2S>)::process [rust]");
        use rabitq::bits::CodeMetadata;
        let sum = rabitq::bits::binary::accumulate(bits, code.1, &lut.1);
        let (distance,) = rabitq::bits::binary::half_process_l2s(
            bits,
            dim,
            sum,
    ...
    fn distance(
        ...
        let _guard = trace_guard!("Operator(Op<VectOwned<f32>, L2S>)::distance [rust]");
...
impl Operator for Op<VectOwned<f32>, Dot> {
    ...
    fn process(
        ...
        let _guard = trace_guard!("Operator(Op<VectOwned<f32>, Dot>)::process [rust]");
        use rabitq::bits::CodeMetadata;
        let sum = rabitq::bits::binary::accumulate(bits, code.1, &lut.1);
        ...
        trace_vchord_print!("distance: {}", distance);
        Distance::from_f32(distance)
    }
    ...
    fn distance(
        ...
        let _guard = trace_guard!("Operator(Op<VectOwned<f32>, Dot>)::distance [rust]");
...
impl Operator for Op<VectOwned<f16>, L2S> {
    ...
    fn process(
        ...
        let _guard = trace_guard!("Operator(Op<VectOwned<f16>, L2S>)::process [rust]");
        use rabitq::bits::CodeMetadata;
        let sum = rabitq::bits::binary::accumulate(bits, code.1, &lut.1);
        let (distance,) = rabitq::bits::binary::half_process_l2s(
            bits,
    ...
    fn distance(
        ...
        let _guard = trace_guard!("Operator(Op<VectOwned<f16>, L2S>)::distance [rust]");
        lhs.operator_l2s(rhs)
    }
}
...
impl Operator for Op<VectOwned<f16>, Dot> {
    ...
    fn process(
        ...
        let _guard = trace_guard!("Operator(Op<VectOwned<f16>, Dot>)::process [rust]");
        use rabitq::bits::CodeMetadata;
        let sum = rabitq::bits::binary::accumulate(bits, code.1, &lut.1);
        let (distance,) = rabitq::bits::binary::half_process_dot(
            bits,
    ...
    fn distance(
        ...
        let _guard = trace_guard!("Operator(Op<VectOwned<f16>, Dot>)::distance [rust]");
        lhs.operator_dot(rhs)
    }
}
...
impl Operator for Op<Rabitq8Owned, L2S> {
    ...
    fn process(
        ...
        let _guard = trace_guard!("Operator(Op<Rabitq8Owned, L2S>)::process [rust]");
        use rabitq::bits::CodeMetadata;
        let sum = rabitq::bits::binary::accumulate(bits, code.1, &lut.1);
        let (distance,) = rabitq::bits::binary::half_process_l2s(
            bits,
    ...
    fn distance(
        ...
        let _guard = trace_guard!("Operator(Op<Rabitq8Owned, L2S>)::distance [rust]");
        lhs.operator_l2s(rhs)
    }
}
...
impl Operator for Op<Rabitq8Owned, Dot> {
    ...
    fn process(
        ...
        let _guard = trace_guard!("Operator(Op<Rabitq8Owned, Dot>)::process [rust]");
        use rabitq::bits::CodeMetadata;
        let sum = rabitq::bits::binary::accumulate(bits, code.1, &lut.1);
        let (distance,) = rabitq::bits::binary::half_process_dot(
            bits,
    ...
    fn distance(
        ...
        let _guard = trace_guard!("Operator(Op<Rabitq8Owned, Dot>)::distance [rust]");
        lhs.operator_dot(rhs)
    }
}
...
impl Operator for Op<Rabitq4Owned, L2S> {
    ...
    fn process(
        ...
        let _guard = trace_guard!("Operator(Op<Rabitq4Owned, L2S>)::process [rust]");
        use rabitq::bits::CodeMetadata;
        let sum = rabitq::bits::binary::accumulate(bits, code.1, &lut.1);
        let (distance,) = rabitq::bits::binary::half_process_l2s(
            bits,
    ...
    fn distance(
        ...
        let _guard = trace_guard!("Operator(Op<Rabitq4Owned, L2S>)::distance [rust]");
        lhs.operator_l2s(rhs)
    }
}
...
impl Operator for Op<Rabitq4Owned, Dot> {
    ...
    fn process(
        ...
        let _guard = trace_guard!("Operator(Op<Rabitq4Owned, Dot>)::process [rust]");
        use rabitq::bits::CodeMetadata;
        let sum = rabitq::bits::binary::accumulate(bits, code.1, &lut.1);
        let (distance,) = rabitq::bits::binary::half_process_dot(
            bits,
    ...
    fn distance(
        ...
        let _guard = trace_guard!("Operator(Op<Rabitq4Owned, Dot>)::distance [rust]");
        lhs.operator_dot(rhs)
    }
}
...
impl<V: Vector> DefaultWithDimension for CloneAccessor<V> {
    fn default_with_dimension(dim: u32) -> Self {
        let _guard =
            trace_guard!("DefaultWithDimension(CloneAccessor)::default_with_dimension [rust]");
        Self(dim, Vec::new())
    }
}
...
impl<V: Vector> Accessor1<V::Element, V::Metadata> for CloneAccessor<V> {
    ...
    fn push(&mut self, input: &[V::Element]) {
        let _guard = trace_guard!("Accessor1(CloneAccessor)::push [rust]");
        self.1.extend(input);
    }

    fn finish(self, metadata: V::Metadata) -> Self::Output {
        let _guard = trace_guard!("Accessor1(CloneAccessor)::finish [rust]");
