...
impl VchordgIndexOptions {
    ...
    fn validate_alpha(alpha: &[f32]) -> Result<(), ValidationError> {
        let _guard = trace_guard!("VchordgIndexOptions::validate_alpha [rust]");
        if !alpha.is_sorted() {
...
impl Default for VchordgIndexOptions {
    fn default() -> Self {
        let _guard = trace_guard!("Default(VchordgIndexOptions)::default [rust]");
        Self {
...
impl VectorOptions {
    pub fn validate_self(&self) -> Result<(), ValidationError> {
        let _guard = trace_guard!("vchordg::VectorOptions::validate_self [rust]");
