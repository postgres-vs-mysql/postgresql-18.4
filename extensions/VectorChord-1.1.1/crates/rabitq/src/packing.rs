...
pub fn pack(x: [&[u8]; 32]) -> Vec<[u8; 16]> {
    let _guard = trace_guard!("packing::pack [rust]");
    let n = {
...
pub fn unpack(x: &[[u8; 16]]) -> [Vec<u8>; 32] {
    let _guard = trace_guard!("packing::unpack [rust]");
    let n = x.len();
...
pub fn padding_pack(x: impl IntoIterator<Item = impl AsRef<[u8]>>) -> Vec<[u8; 16]> {
    let _guard = trace_guard!("packing::padding_pack [rust]");
...
pub fn pack_to_u4(input: &[bool]) -> Vec<u8> {
    let _guard = trace_guard!("packing::pack_to_u4 [rust]");
