...
impl CodeMetadata {
    pub fn into_tuple(self) -> (f32, f32, f32) {
        let _guard = trace_guard!("extended::CodeMetadata::into_tuple [rust]");
        (self.dis_u_2, self.norm_of_lattice, self.sum_of_code)
    }
    pub fn into_array(self) -> [f32; 3] {
        let _guard = trace_guard!("extended::CodeMetadata::into_array [rust]");
        [self.dis_u_2, self.norm_of_lattice, self.sum_of_code]
    }
    pub fn from_tuple((dis_u_2, norm_of_lattice, sum_of_code): (f32, f32, f32)) -> Self {
        let _guard = trace_guard!("extended::CodeMetadata::from_tuple [rust]");
        Self {
            dis_u_2,
            norm_of_lattice,
            sum_of_code,
    ...
    pub fn from_array([dis_u_2, norm_of_lattice, sum_of_code]: [f32; 3]) -> Self {
        let _guard = trace_guard!("extended::CodeMetadata::from_array [rust]");
        Self {
            dis_u_2,
            norm_of_lattice,
            sum_of_code,
...
pub fn code<const BITS: usize>(vector: &[f32]) -> Code {
    let _guard = trace_guard!("code [rust]");
...
pub fn ugly_code<const BITS: usize>(vector: &[f32]) -> Code {
    let _guard = trace_guard!("ugly_code [rust]");
    assert!((1..=8).contains(&BITS));
...
pub fn half_process_l2s<const X: usize, const Y: usize>(
    ...
    let _guard = trace_guard!("half_process_l2s [rust]");
    ...
    let result = lhs.dis_u_2 + rhs.dis_u_2
        - 2.0
            * ip
            * (lhs.dis_u_2.sqrt() / lhs.norm_of_lattice)
            * (rhs.dis_u_2.sqrt() / rhs.norm_of_lattice);
    trace_vchord_print!("(extended) dim: {}, sum: {}, result: {}", dim, sum, result);
    result
}
...
pub fn half_process_dot<const X: usize, const Y: usize>(
    ...
    let _guard = trace_guard!("half_process_dot [rust]");
    assert!((1..=8).contains(&X));
    assert!((1..=8).contains(&Y));
    ...
    let result = -ip
        * (lhs.dis_u_2.sqrt() / lhs.norm_of_lattice)
        * (rhs.dis_u_2.sqrt() / rhs.norm_of_lattice);
    trace_vchord_print!("(exntended) dim: {}, sum: {}, result: {}", dim, sum, result);
    result
}
...
pub fn half_process_cos<const X: usize, const Y: usize>(
    ...
    let _guard = trace_guard!("half_process_cos [rust]");
    assert!((1..=8).contains(&X));
    assert!((1..=8).contains(&Y));
    ...
    let ip = sum as f32 - (c_y * lhs.sum_of_code + c_x * rhs.sum_of_code) + dim as f32 * c_x * c_y;
    let result = -ip / lhs.norm_of_lattice / rhs.norm_of_lattice;
    trace_vchord_print!("(exntended) dim: {}, sum: {}, result: {}", dim, sum, result);
    result
}

fn find_scale<const B: usize>(o: &[f32]) -> f32 {
    let _guard = trace_guard!("find_scale [rust]");
    ...
    let result = x_m as f32 + f32::EPSILON;
    trace_vchord_print!("(exntended) result: {}", result);
    result
}

fn ugly_find_scale<const B: usize>(o: &[f32]) -> f32 {
    let _guard = trace_guard!("ugly_find_scale [rust]");
    assert!((1..=8).contains(&B));

    let result = (1 << (B - 1)) as f32 / f32::reduce_min_max_of_x(o).1;
    trace_vchord_print!("(exntended) result: {}", result);
    result
}

pub fn pack_code<const BITS: usize>(input: &[u8]) -> [Vec<u64>; BITS] {
    let _guard = trace_guard!("pack_code [rust]");
    #[inline(always)]
...
pub fn accumulate<const X: usize, const Y: usize>(lhs: &[u64], rhs: &[Vec<u64>; Y]) -> u32 {
    let _guard = trace_guard!("accumulate [rust]");
