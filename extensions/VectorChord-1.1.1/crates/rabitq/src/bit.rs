...
impl CodeMetadata {
    ...
    pub fn into_tuple(self) -> (f32, f32, f32, f32) {
        let _guard = trace_guard!("bit::CodeMetadata::into_tuple [rust]");
        (
            self.dis_u_2,
            self.factor_cnt,
            self.factor_ip,
    ...
    pub fn into_array(self) -> [f32; 4] {
        let _guard = trace_guard!("bit::CodeMetadata::into_array [rust]");
        [
            self.dis_u_2,
            self.factor_cnt,
            self.factor_ip,
    ...
    pub fn from_tuple((dis_u_2, factor_cnt, factor_ip, factor_err): (f32, f32, f32, f32)) -> Self {
        let _guard = trace_guard!("bit::CodeMetadata::from_tuple [rust]");
        Self {
            dis_u_2,
            factor_cnt,
            factor_ip,
    ...
    pub fn from_array([dis_u_2, factor_cnt, factor_ip, factor_err]: [f32; 4]) -> Self {
        let _guard = trace_guard!("bit::CodeMetadata::from_array [rust]");
        Self {
            dis_u_2,
            factor_cnt,
            factor_ip,
...
pub fn code_metadata(vector: &[f32]) -> CodeMetadata {
    let _guard = trace_guard!("bit::code_metadata [rust]");
    let n = vector.len();
...
pub fn code_elements(vector: &[f32]) -> Vec<bool> {
    let _guard = trace_guard!("bit::code_elements [rust]");
    let n = vector.len();
...
pub fn code(vector: &[f32]) -> Code {
    let _guard = trace_guard!("bit::code [rust]");
    let n = vector.len();
    let sum_of_abs_x = f32::reduce_sum_of_abs_x(vector);
    let sum_of_x_2 = f32::reduce_sum_of_x2(vector);
    (
...
pub fn preprocess(vector: &[f32]) -> (BlockLut, BinaryLut) {
    let _guard = trace_guard!("bit::preprocess [rust]");
    let dis_v_2 = f32::reduce_sum_of_x2(vector);
    trace_vchord_print!("dis_v_2: {}", dis_v_2);
    (
...
pub mod binary {
    pub fn pack_code(input: &[bool]) -> Vec<u64> {
        let _guard = trace_guard!("bit::binary::pack_code [rust]");
        let f = |t: &[bool; 64]| {
    ...
    pub fn accumulate(x: &[u64], y: &[impl AsRef<[u64]>; BITS]) -> u32 {
        let _guard = trace_guard!("bit::binary::accumulate [rust]");
    ...
    pub fn half_process_l2s(
        ...
        let _guard = trace_guard!("bit::binary::half_process_l2s [rust]");
        ...
        trace_vchord_print!("rough: {}, err: {}", rough, err);
        (rough, err)
    }
    ...
    pub fn half_process_l2s_residual(
        ...
        let _guard = trace_guard!("bit::binary::half_process_l2s_residual [rust]");
        let e = k * ((2.0 * sum as f32) - qvector_sum) + b * factor_cnt;
        let rough = dis_u_2 + dis_f - 2.0 * e * factor_ip + delta;
        let err = 2.0 * factor_err * dis_f.sqrt();
        trace_vchord_print!("sum: {}, rough: {}, err: {}", sum, rough, err);
        (rough, err)
    }
    ...
    pub fn half_process_dot(
        ...
        let _guard = trace_guard!("bit::binary::half_process_dot [rust]");
        let e = k * ((2.0 * sum as f32) - qvector_sum) + b * factor_cnt;
        let rough = -e * factor_ip;
        let err = factor_err * dis_v_2.sqrt();
        trace_vchord_print!(
            "sum: {}, qvector_sum: {}, rough: {}, err: {}",
            sum,
            qvector_sum,
            rough,
            err
    ...
    pub fn half_process_dot_residual(
        ...
        let _guard = trace_guard!("bit::binary::half_process_dot_residual [rust]");
        let e = k * ((2.0 * sum as f32) - qvector_sum) + b * factor_cnt;
    ...
    pub(crate) fn binarize(vector: &[u8]) -> [Vec<u64>; BITS] {
        let _guard = trace_guard!("bit::binary::binarize [rust]");
        let n = vector.len();
...
pub mod block {
    ...
    pub fn preprocess(vector: &[f32]) -> BlockLut {
        let _guard = trace_guard!("bit::block::preprocess [rust]");
        let dis_v_2 = f32::reduce_sum_of_x2(vector);
        preprocess_with_distance(vector, dis_v_2)
    }

    pub(crate) fn preprocess_with_distance(vector: &[f32], dis_v_2: f32) -> BlockLut {
        let _guard = trace_guard!("bit::block::preprocess_with_distance [rust]");
    ...
    pub fn accumulate(t: &[[u8; 16]], s: &[[u8; 16]]) -> [u32; 32] {
        let _guard = trace_guard!("bit::block::accumulate [rust]");
        use std::iter::zip;
    ...
    pub fn full_process_l2s(
        ...
        let _guard = trace_guard!("bit::block::full_process_l2s [rust]");
        use std::iter::zip;
    ...
    pub fn full_process_dot(
        ...
        let _guard = trace_guard!("bit::block::full_process_dot [rust]");
        use std::iter::zip;
        let &(BlockLutMetadata { dis_v_2, k, c }, ref s) = lut;
        let mut sum = [0_u32; 32];
        for (t, s) in zip(t.chunks(STEP), s.chunks(STEP)) {
    ...
    pub fn half_process_l2s(
        ...
        let _guard = trace_guard!("bit::block::half_process_l2s [rust]");
    ...
    pub fn half_process_l2s_residual(
        ...
        let _guard = trace_guard!("bit::block::half_process_l2s_residual [rust]");
        let e = k * (sum as f32) + c;
        let rough = dis_u_2 + dis_f - 2.0 * e * factor_ip + delta;
        let err = 2.0 * factor_err * dis_f.sqrt();
        trace_vchord_print!("sum: {}, rough: {}, err: {}", sum, rough, err);
    ...
    pub fn half_process_dot(
        ...
        let _guard = trace_guard!("bit::block::half_process_dot [rust]");
        let e = k * (sum as f32) + c;
        let rough = -e * factor_ip;
        let err = factor_err * dis_v_2.sqrt();
        trace_vchord_print!("sum: {}, rough: {}, err: {}", sum, rough, err);
    ...
    pub fn half_process_dot_residual(
        ...
        let _guard = trace_guard!("bit::block::half_process_dot_residual [rust]");
        let e = k * (sum as f32) + c;
        let rough = -e * factor_ip + dis_f + delta;
        let err = factor_err * (dis_v_2 + norm * norm + 2.0 * dis_f).sqrt();
        trace_vchord_print!("sum: {}, rough: {}, err: {}", sum, rough, err);
    ...
    fn compress(vector: &[f32]) -> Vec<[f32; 16]> {
        let _guard = trace_guard!("bit::block::compress [rust]");
