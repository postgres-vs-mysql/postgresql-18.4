...
pub fn how(index: &impl RelationRead) -> RerankMethod {
    let _guard = trace_guard!("rerank::how [rust]");
    ...
    W: 'b + PackedRefMut<T = (NonZero<u64>, u16, BorrowedIter<'b>)>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        let _guard = trace_guard!("Iterator(Reranker)::next [rust]");
...
impl<T, F, P, W> Reranker<T, F, P, W> {
    pub fn finish(self) -> (P, impl Iterator<Item = Result>) {
        let _guard = trace_guard!("Reranker::finish [rust]");
...
>(
    ...
    let _guard = trace_guard!("rerank_index [rust]");
...
>(
    ...
    let _guard = trace_guard!("rerank_heap [rust]");
    let dim = vector.as_borrowed().dim();
    Reranker {
        prefetcher,
        cache: BinaryHeap::new(),
