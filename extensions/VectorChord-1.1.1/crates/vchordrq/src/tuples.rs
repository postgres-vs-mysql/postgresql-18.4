...
impl Tuple for MetaTuple {
    ...
    fn serialize(&self) -> Vec<u8> {
        let _guard = trace_guard!("Tuple(MetaTuple)::serialize [rust]");
...
impl WithReader for MetaTuple {
    ...
    fn deserialize_ref(source: &[u8]) -> MetaTupleReader<'_> {
        let _guard = trace_guard!("WithReader(MetaTuple)::deserialize_ref [rust]");
        let tag = tag(source);
        match tag {
            MAGIC => {
...
impl Tuple for FreepagesTuple {
    fn serialize(&self) -> Vec<u8> {
        let _guard = trace_guard!("Tuple(FreepagesTuple)::serialize [rust]");
...
impl WithWriter for FreepagesTuple {
    ...
    fn deserialize_mut(source: &mut [u8]) -> FreepagesTupleWriter<'_> {
        let _guard = trace_guard!("WithWriter(FreepagesTuple)::deserialize_mut [rust]");
...
impl<V: Vector> Tuple for CentroidTuple<V> {
    fn serialize(&self) -> Vec<u8> {
        let _guard = trace_guard!("Tuple(CentroidTuple)::serialize [rust]");
        let mut buffer = Vec::<u8>::new();
        match self {
            CentroidTuple::_0 { metadata, elements } => {
                trace_vchord_print!("CentroidTuple::_0 (metadata, elements)");
                ...
                trace_vchord_print!("elements: {}", elements_s);
            ...
            CentroidTuple::_1 { head, elements } => {
                trace_vchord_print!("CentroidTuple::_1 (head, elements)");
...
impl<V: Vector> WithReader for CentroidTuple<V> {
    ...
    fn deserialize_ref(source: &[u8]) -> CentroidTupleReader<'_, V> {
        let _guard = trace_guard!("WithReader(CentroidTuple)::deserialize_ref [rust]");
        let tag = tag(source);
        match tag {
            0 => {
                let checker = RefChecker::new(source);
...
impl<'a, V: Vector> CentroidTupleReader<'a, V> {
    pub fn elements(self) -> &'a [<V as Vector>::Element] {
        let _guard = trace_guard!("Vector(CentroidTupleReader)::elements [rust]");
        match self {
    ...
    pub fn metadata_or_head(self) -> Result<V::Metadata, u16> {
        let _guard = trace_guard!("Vector(CentroidTupleReader)::metadata_or_head [rust]");
        match self {
...
impl<V: Vector> Tuple for VectorTuple<V> {
    fn serialize(&self) -> Vec<u8> {
        let _guard = trace_guard!("Tuple(VectorTuple)::serialize [rust]");
        let mut buffer = Vec::<u8>::new();
        match self {
            VectorTuple::_0 {
                payload,
...
impl<V: Vector> WithReader for VectorTuple<V> {
    ...
    fn deserialize_ref(source: &[u8]) -> VectorTupleReader<'_, V> {
        let _guard = trace_guard!("WithReader(VectorTuple)::deserialize_ref [rust]");
        let tag = tag(source);
        match tag {
            0 => {
                let checker = RefChecker::new(source);
...
impl DirectoryTuple {
    pub fn estimate_size_0(elements: usize) -> usize {
        let _guard = trace_guard!("DirectoryTuple::estimate_size_0 [rust]");
        let mut size = 0_usize;
        size += size_of::<Tag>();
        size += size_of::<DirectoryTupleHeader0>();
        size += (elements * size_of::<u32>()).next_multiple_of(ALIGN);
        trace_vchord_print!("size: {})", size);
        size
    }
    pub fn fit_1(freespace: u16) -> Option<usize> {
        let _guard = trace_guard!("DirectoryTuple::fit_1 [rust]");
        ...
        freespace &= !(ALIGN - 1) as isize;
        if freespace >= 0 {
            trace_vchord_print!("freespace: {})", freespace);
...
impl Tuple for DirectoryTuple {
    fn serialize(&self) -> Vec<u8> {
        let _guard = trace_guard!("Tuple(DirectoryTuple)::serialize [rust]");
        let mut buffer = Vec::<u8>::new();
        match self {
            Self::_0 { elements } => {
                buffer.extend((0 as Tag).to_ne_bytes());
                ...
                buffer.extend(elements.as_bytes());
                let elements_e = buffer.len() as u16;
                trace_vchord_print!("elements_s: {}, elements_e: {})", elements_s, elements_e);
                while buffer.len() % ALIGN != 0 {
                    buffer.push(0);
                }
                // header
...
impl WithReader for DirectoryTuple {
    ...
    fn deserialize_ref(source: &[u8]) -> DirectoryTupleReader<'_> {
        let _guard = trace_guard!("WithReader(DirectoryTuple)::deserialize_ref [rust]");
        let tag = tag(source);
        match tag {
            0 => {
                let checker = RefChecker::new(source);
...
impl H1Tuple {
    pub fn estimate_size_0(prefetch: usize, elements: usize) -> usize {
        let _guard = trace_guard!("H1Tuple::estimate_size_0 [rust]");
        let mut size = 0_usize;
        size += size_of::<Tag>();
    ...
    pub fn fit_1(prefetch: usize, freespace: u16) -> Option<usize> {
        let _guard = trace_guard!("H1Tuple::fit_1 [rust]");
        let mut freespace = freespace as isize;
        freespace &= !(ALIGN - 1) as isize;
        freespace -= size_of::<Tag>() as isize;
        freespace &= !(ALIGN - 1) as isize;
...
impl Tuple for H1Tuple {
    fn serialize(&self) -> Vec<u8> {
        let _guard = trace_guard!("Tuple(H1Tuple)::serialize [rust]");
        let mut buffer = Vec::<u8>::new();
        match self {
            Self::_0 {
                head,
...
impl WithReader for H1Tuple {
    ...
    fn deserialize_ref(source: &[u8]) -> H1TupleReader<'_> {
        let _guard = trace_guard!("WithReader(H1Tuple)::deserialize_ref [rust]");
        let tag = tag(source);
        match tag {
            0 => {
                let checker = RefChecker::new(source);
...
impl Tuple for JumpTuple {
    fn serialize(&self) -> Vec<u8> {
        let _guard = trace_guard!("Tuple(JumpTuple)::serialize [rust]");
        let mut buffer = Vec::<u8>::new();
...
impl WithReader for JumpTuple {
    ...
    fn deserialize_ref(source: &[u8]) -> JumpTupleReader<'_> {
        let _guard = trace_guard!("WithReader(JumpTuple)::deserialize_ref [rust]");
        let checker = RefChecker::new(source);
...
impl WithWriter for JumpTuple {
    ...
    fn deserialize_mut(source: &mut [u8]) -> JumpTupleWriter<'_> {
        let _guard = trace_guard!("WithWriter(JumpTuple)::deserialize_mut [rust]");
        let mut checker = MutChecker::new(source);
...
impl FrozenTuple {
    pub fn estimate_size_0(prefetch: usize, elements: usize) -> usize {
        let _guard = trace_guard!("FrozenTuple::estimate_size_0 [rust]");
        let mut size = 0_usize;
        size += size_of::<Tag>();
    ...
    pub fn fit_1(prefetch: usize, freespace: u16) -> Option<usize> {
        let _guard = trace_guard!("FrozenTuple::fit_1 [rust]");
        let mut freespace = freespace as isize;
        freespace &= !(ALIGN - 1) as isize;
        freespace -= size_of::<Tag>() as isize;
        freespace &= !(ALIGN - 1) as isize;
...
impl Tuple for FrozenTuple {
    fn serialize(&self) -> Vec<u8> {
        let _guard = trace_guard!("Tuple(FrozenTuple)::serialize [rust]");
        let mut buffer = Vec::<u8>::new();
        match self {
            FrozenTuple::_0 {
                metadata,
...
impl WithReader for FrozenTuple {
    ...
    fn deserialize_ref(source: &[u8]) -> FrozenTupleReader<'_> {
        let _guard = trace_guard!("WithReader(FrozenTuple)::deserialize_ref [rust]");
        let tag = tag(source);
        match tag {
            0 => {
                let checker = RefChecker::new(source);
...
impl Tuple for AppendableTuple {
    fn serialize(&self) -> Vec<u8> {
        let _guard = trace_guard!("Tuple(AppendableTuple)::serialize [rust]");
        let mut buffer = Vec::<u8>::new();
...
impl WithReader for AppendableTuple {
    ...
    fn deserialize_ref(source: &[u8]) -> AppendableTupleReader<'_> {
        let _guard = trace_guard!("WithReader(AppendableTuple)::deserialize_ref [rust]");
        let checker = RefChecker::new(source);
...
impl WithWriter for AppendableTuple {
    ...
    fn deserialize_mut(source: &mut [u8]) -> AppendableTupleWriter<'_> {
        let _guard = trace_guard!("WithWriter(AppendableTuple)::deserialize_mut [rust]");
        let mut checker = MutChecker::new(source);
