...
{
    let _guard = trace_guard!("vchordrq::build [rust]");
    ...
    trace_vchord_print!("structures.len: {}", structures.len());
    for i in 0..structures.len() {
        let mut level = Vec::new();
        trace_vchord_print!("structures[{}].len: {}", i, structures[i].len());
        for j in 0..structures[i].len() {
            ...
            trace_vchord_print!("slices.len: {}", slices.len());
    ...
    trace_vchord_print!("dim: {}, height_of_root:{}", dim, structures.len());
    meta.push(MetaTuple {
        dim,
...
fn norm<V: Vector>(vector: V::Borrowed<'_>) -> f32 {
    let _guard = trace_guard!("vchordrq::norm [rust]");
