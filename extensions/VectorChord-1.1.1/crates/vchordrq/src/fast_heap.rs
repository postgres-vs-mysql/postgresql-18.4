...
impl<T: Ord> FastHeap<T> {
    pub fn from_vec(vec: Vec<T>) -> Self {
        let _guard = trace_guard!("FastHeap::from_vec [rust]");
        let n = vec.len();
    ...
    pub fn pop(&mut self) -> Option<T> {
        let _guard = trace_guard!("FastHeap::pop [rust]");
        match self {
    ...
    pub fn peek(&self) -> Option<&T> {
        let _guard = trace_guard!("FastHeap::peek [rust]");
        match self {
...
impl<T: Ord> Sequence for FastHeap<T> {
    ...
    fn next(&mut self) -> Option<T> {
        let _guard = trace_guard!("Sequence(FastHeap)::next [rust]");
        self.pop()
    }
    fn peek(&mut self) -> Option<&Self::Item> {
        let _guard = trace_guard!("Sequence(FastHeap)::peek [rust]");
        (self as &Self).peek()
    }
    fn into_inner(self) -> Self::Inner {
        let _guard = trace_guard!("Sequence(FastHeap)::into_inner [rust]");
        match self {
