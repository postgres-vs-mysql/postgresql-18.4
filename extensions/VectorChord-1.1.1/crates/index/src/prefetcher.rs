...
impl<T: Ord> Sequence for BinaryHeap<T> {
    ...
    fn next(&mut self) -> Option<T> {
        let _guard = trace_guard!("Sequence(BinaryHeap)::next [rust]");
        self.pop()
    }
    #[inline]
    fn peek(&mut self) -> Option<&T> {
        let _guard = trace_guard!("Sequence(BinaryHeap)::peek [rust]");
        (self as &Self).peek()
    }
    #[inline]
    fn into_inner(self) -> Self::Inner {
        let _guard = trace_guard!("Sequence(BinaryHeap)::into_inner [rust]");
...
impl<const N: usize, T: Ord> Sequence for DaryHeap<T, N> {
    ...
    #[inline]
    fn next(&mut self) -> Option<T> {
        let _guard = trace_guard!("Sequence(DaryHeap)::next [rust]");
        self.pop()
    }
    #[inline]
    fn peek(&mut self) -> Option<&T> {
        let _guard = trace_guard!("Sequence(DaryHeap)::peek [rust]");
        (self as &Self).peek()
    }
    #[inline]
    fn into_inner(self) -> Self::Inner {
        let _guard = trace_guard!("Sequence(DaryHeap)::into_inner [rust]");
        self.into_vec().into_iter()
    }
}
...
impl<I: Iterator> Sequence for std::iter::Peekable<I> {
    ...
    fn next(&mut self) -> Option<I::Item> {
        let _guard = trace_guard!("Sequence(Peekable)::next [rust]");
        Iterator::next(self)
    }
    #[inline]
    fn peek(&mut self) -> Option<&I::Item> {
        let _guard = trace_guard!("Sequence(Peekable)::peek [rust]");
        self.peek()
    }
    #[inline]
    fn into_inner(self) -> Self::Inner {
        let _guard = trace_guard!("Sequence(Peekable)::into_inner [rust]");
        self
    }
}
...
impl<T> Sequence for VecDeque<T> {
    ...
    #[inline]
    fn next(&mut self) -> Option<T> {
        let _guard = trace_guard!("Sequence(VecDeque)::next [rust]");
        self.pop_front()
    }
    #[inline]
    fn peek(&mut self) -> Option<&T> {
        let _guard = trace_guard!("Sequence(VecDeque)::peek [rust]");
        self.front()
    }
    #[inline]
    fn into_inner(self) -> Self::Inner {
        let _guard = trace_guard!("Sequence(VecDeque)::into_inner [rust]");
        self.into_iter()
    }
}
...
impl<'b, R, S: Sequence> IntoIterator for PlainPrefetcher<'b, R, S> {
    ...
    fn into_iter(self) -> Self::IntoIter {
        let _guard = trace_guard!("IntoIterator(PlainPrefetcher)::into_iter [rust]");
    ...
    S::Item: Fetch<'b>,
{
    ...
    fn next(
        ...
        let _guard = trace_guard!("Prefetcher(PlainPrefetcher)::next [rust]");
    ...
    fn next_if(
        ...
        let _guard = trace_guard!("Prefetcher(PlainPrefetcher)::next_if [rust]");
...
impl<'b, R: RelationRead, L: Iterator<Item = u32>> Iterator for PlainPrefetcherGuards<'b, R, L> {
    ...
    #[inline]
    fn next(&mut self) -> Option<Self::Item> {
        let _guard = trace_guard!("Iterator(PlainPrefetcherGuards)::next [rust]");
    ...
    fn size_hint(&self) -> (usize, Option<usize>) {
        let _guard = trace_guard!("Iterator(PlainPrefetcherGuards)::size_hint [rust]");
        self.list.size_hint()
    }
}
...
impl<'b, R, S: Sequence> SimplePrefetcher<'b, R, S> {
    #[inline]
    pub fn new(relation: &'b R, sequence: S) -> Self {
        let _guard = trace_guard!("SimplePrefetcher::new [rust]");
        Self {
            relation,
            window: VecDeque::new(),
            sequence,
...
impl<'b, R, S: Sequence> IntoIterator for SimplePrefetcher<'b, R, S> {
    ...
    #[inline]
    fn into_iter(self) -> Self::IntoIter {
        let _guard = trace_guard!("IntoIterator(SimplePrefetcher)::into_iter [rust]");
    ...
    S::Item: Fetch<'b>,
{
    ...
    fn next(
        ...
        let _guard = trace_guard!("Prefetcher(SimplePrefetcher)::next [rust]");
    ...
    fn next_if(
        ...
        let _guard = trace_guard!("Prefetcher(SimplePrefetcher)::next_if [rust]");
        while self.window.len() < WINDOW_SIZE
            && let Some(e) = self.sequence.next()
        {
            for id in e.fetch() {
...
impl<'b, R: RelationRead, L: Iterator<Item = u32>> Iterator for SimplePrefetcherGuards<'b, R, L> {
    ...
    #[inline]
    fn next(&mut self) -> Option<Self::Item> {
        let _guard = trace_guard!("Iterator(SimplePrefetcherGuards)::next [rust]");
        let id = self.list.next()?;
        Some(self.relation.read(id))
    }
    ...
    #[inline]
    fn size_hint(&self) -> (usize, Option<usize>) {
        let _guard = trace_guard!("Iterator(SimplePrefetcherGuards)::size_hint [rust]");
        self.list.size_hint()
    }
}
...
impl<S: Sequence> Iterator for StreamPrefetcherSequence<S> {
    ...
    #[inline]
    fn next(&mut self) -> Option<Self::Item> {
        let _guard = trace_guard!("Iterator(StreamPrefetcherSequence)::next [rust]");
        self.0.next()
    }
}
    ...
    S::Item: Fetch<'b>,
{
    ...
    #[inline]
    fn into_iter(self) -> Self::IntoIter {
        let _guard = trace_guard!("IntoIterator(StreamPrefetcher)::into_iter [rust]");
    ...
    S::Item: Fetch<'b>,
{
    ...
    fn next(&mut self) -> Option<(S::Item, Self::Guards)> {
        let _guard = trace_guard!("Prefetcher(StreamPrefetcher)::next [rust]");
        self.stream.next()
    }
    ...
    fn next_if(
        ...
        let _guard = trace_guard!("Prefetcher(StreamPrefetcher)::next_if [rust]");
