...
impl<'a> RefChecker<'a> {
    ...
    pub fn prefix<T: FromBytes + IntoBytes + KnownLayout + Immutable + Sized>(
        ...
        let _guard = trace_guard!("RefChecker::prefix [rust]");
        let start = Into::<usize>::into(s);
        let end = Into::<usize>::into(s) + size_of::<T>();
        let bytes = &self.bytes[start..end];
        trace_vchord_print!("deserialization: {} bytes", bytes.len());
    ...
    pub fn bytes<T: FromBytes + IntoBytes + KnownLayout + Immutable + ?Sized>(
        ...
        let _guard = trace_guard!("RefChecker::bytes [rust]");
        let start = Into::<usize>::into(s);
    ...
    pub unsafe fn bytes_slice_unchecked<T>(
        ...
        let _guard = trace_guard!("RefChecker::bytes_slice_unchecked [rust]");
        let start = Into::<usize>::into(s);
        let end = Into::<usize>::into(e);
        let bytes = &self.bytes[start..end];
        trace_vchord_print!("deserialization: {} bytes", bytes.len());
...
impl<'a> MutChecker<'a> {
    ...
    pub fn new(bytes: &'a mut [u8]) -> Self {
        let _guard = trace_guard!("MutChecker::new [rust]");
        Self {
            flag: 0,
            bytes,
            phantom: PhantomData,
    ...
    pub fn prefix<T: FromBytes + IntoBytes + KnownLayout + Sized>(
        ...
        let _guard = trace_guard!("MutChecker::prefix [rust]");
        let start = Into::<usize>::into(s);
        let end = Into::<usize>::into(s) + size_of::<T>();
    ...
    pub fn bytes<T: FromBytes + IntoBytes + KnownLayout + ?Sized>(
        ...
        let _guard = trace_guard!("MutChecker::bytes [rust]");
        let start = Into::<usize>::into(s);
        let end = Into::<usize>::into(e);
        if !(start <= end && end <= self.bytes.len()) {
            panic!("deserialization: bad bytes");
