...
impl Rabitq4Header {
    ...
    unsafe fn as_borrowed<'a>(this: NonNull<Self>) -> Rabitq4Borrowed<'a> {
        let _guard = trace_guard!("Rabitq4Header::as_borrowed [rust]");
        unsafe {
...
impl Rabitq4Input<'_> {
    unsafe fn from_ptr(p: NonNull<Rabitq4Header>) -> Self {
        let _guard = trace_guard!("Rabitq4Input::from_ptr [rust]");
        let q = unsafe {
...
impl Drop for Rabitq4Input<'_> {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(Rabitq4Input)::drop [rust]");
        if self.2 {
            unsafe {
...
impl Rabitq4Output {
    unsafe fn from_ptr(p: NonNull<Rabitq4Header>) -> Self {
        let _guard = trace_guard!("Rabitq4Output::from_ptr [rust]");
        let q = unsafe {
    ...
    pub fn new(vector: Rabitq4Borrowed<'_>) -> Self {
        let _guard = trace_guard!("Rabitq4Output::new [rust]");
        unsafe {
    ...
    pub fn as_borrowed(&self) -> Rabitq4Borrowed<'_> {
        let _guard = trace_guard!("Rabitq4Output::as_borrowed [rust]");
        unsafe { Rabitq4Header::as_borrowed(self.0) }
    }
    fn into_raw(self) -> *mut Rabitq4Header {
        let _guard = trace_guard!("Rabitq4Output::into_raw [rust]");
...
impl Drop for Rabitq4Output {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(Rabitq4Output)::drop [rust]");
        unsafe {
            pgrx::pg_sys::pfree(self.0.as_ptr().cast());
        }
    }
...
impl FromDatum for Rabitq4Input<'_> {
    unsafe fn from_polymorphic_datum(datum: Datum, is_null: bool, _typoid: Oid) -> Option<Self> {
        let _guard = trace_guard!("FromDatum(Rabitq4Input)::from_polymorphic_datum [rust]");
        if is_null {
            None
        } else {
...
impl FromDatum for Rabitq4Output {
    unsafe fn from_polymorphic_datum(datum: Datum, is_null: bool, _typoid: Oid) -> Option<Self> {
        let _guard = trace_guard!("FromDatum(Rabitq4Output)::from_polymorphic_datum [rust]");
        if is_null {
            None
        } else {
            let ptr = NonNull::new(datum.cast_mut_ptr()).unwrap();
...
impl IntoDatum for Rabitq4Output {
    fn into_datum(self) -> Option<Datum> {
        let _guard = trace_guard!("IntoDatum(Rabitq4Output)::into_datum [rust]");
...
unsafe impl<'a> pgrx::datum::UnboxDatum for Rabitq4Input<'a> {
    ...
    where
        Self: 'src,
    {
        let _guard = trace_guard!("UnboxDatum(Rabitq4Input)::unbox [rust]");
...
unsafe impl pgrx::datum::UnboxDatum for Rabitq4Output {
    ...
    where
        Self: 'src,
    {
        let _guard = trace_guard!("UnboxDatum(Rabitq4Output)::unbox [rust]");
        let datum = datum.sans_lifetime();
        let ptr = NonNull::new(datum.cast_mut_ptr()).unwrap();
        unsafe { Self::from_ptr(ptr) }
    }
...
unsafe impl<'fcx> pgrx::callconv::ArgAbi<'fcx> for Rabitq4Input<'fcx> {
    unsafe fn unbox_arg_unchecked(arg: pgrx::callconv::Arg<'_, 'fcx>) -> Self {
        let _guard = trace_guard!("ArgAbi(Rabitq4Input)::unbox_arg_unchecked [rust]");
        let index = arg.index();
        unsafe {
...
unsafe impl pgrx::callconv::BoxRet for Rabitq4Output {
    unsafe fn box_into<'fcx>(
        ...
        let _guard = trace_guard!("BoxRet(Rabitq4Output)::box_into [rust]");
