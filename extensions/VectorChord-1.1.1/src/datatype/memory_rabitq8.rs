...
impl Rabitq8Header {
    ...
    unsafe fn as_borrowed<'a>(this: NonNull<Self>) -> Rabitq8Borrowed<'a> {
        let _guard = trace_guard!("Rabitq8Header::as_borrowed [rust]");
        unsafe {
...
impl Rabitq8Input<'_> {
    unsafe fn from_ptr(p: NonNull<Rabitq8Header>) -> Self {
        let _guard = trace_guard!("Rabitq8Input::from_ptr [rust]");
        let q = unsafe {
    ...
    pub fn as_borrowed(&self) -> Rabitq8Borrowed<'_> {
        let _guard = trace_guard!("Rabitq8Input::as_borrowed [rust]");
...
impl Drop for Rabitq8Input<'_> {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(Rabitq8Input)::drop [rust]");
        if self.2 {
            unsafe {
...
impl Rabitq8Output {
    unsafe fn from_ptr(p: NonNull<Rabitq8Header>) -> Self {
        let _guard = trace_guard!("Rabitq8Output::from_ptr [rust]");
        let q = unsafe {
    ...
    pub fn new(vector: Rabitq8Borrowed<'_>) -> Self {
        let _guard = trace_guard!("Rabitq8Output::new [rust]");
        unsafe {
    ...
    pub fn as_borrowed(&self) -> Rabitq8Borrowed<'_> {
        let _guard = trace_guard!("Rabitq8Output::as_borrowed [rust]");
        unsafe { Rabitq8Header::as_borrowed(self.0) }
    }
    fn into_raw(self) -> *mut Rabitq8Header {
        let _guard = trace_guard!("Rabitq8Output::into_raw [rust]");
...
impl Drop for Rabitq8Output {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(Rabitq8Output)::drop [rust]");
        unsafe {
            pgrx::pg_sys::pfree(self.0.as_ptr().cast());
        }
    }
...
impl FromDatum for Rabitq8Input<'_> {
    unsafe fn from_polymorphic_datum(datum: Datum, is_null: bool, _typoid: Oid) -> Option<Self> {
        let _guard = trace_guard!("FromDatum(Rabitq8Input)::from_polymorphic_datum [rust]");
        if is_null {
            None
        } else {
...
impl FromDatum for Rabitq8Output {
    unsafe fn from_polymorphic_datum(datum: Datum, is_null: bool, _typoid: Oid) -> Option<Self> {
        let _guard = trace_guard!("FromDatum(Rabitq8Output)::from_polymorphic_datum [rust]");
        if is_null {
            None
        } else {
            let ptr = NonNull::new(datum.cast_mut_ptr()).unwrap();
...
impl IntoDatum for Rabitq8Output {
    fn into_datum(self) -> Option<Datum> {
        let _guard = trace_guard!("IntoDatum(Rabitq8Output)::into_datum [rust]");
...
unsafe impl<'a> pgrx::datum::UnboxDatum for Rabitq8Input<'a> {
    ...
    where
        Self: 'src,
    {
        let _guard = trace_guard!("UnboxDatum(Rabitq8Input)::unbox [rust]");
...
unsafe impl pgrx::datum::UnboxDatum for Rabitq8Output {
    ...
    where
        Self: 'src,
    {
        let _guard = trace_guard!("UnboxDatum(Rabitq8Output)::unbox [rust]");
        let datum = datum.sans_lifetime();
        let ptr = NonNull::new(datum.cast_mut_ptr()).unwrap();
        unsafe { Self::from_ptr(ptr) }
    }
...
unsafe impl<'fcx> pgrx::callconv::ArgAbi<'fcx> for Rabitq8Input<'fcx> {
    unsafe fn unbox_arg_unchecked(arg: pgrx::callconv::Arg<'_, 'fcx>) -> Self {
        let _guard = trace_guard!("ArgAbi(Rabitq8Input)::unbox_arg_unchecked [rust]");
        let index = arg.index();
        unsafe {
...
unsafe impl pgrx::callconv::BoxRet for Rabitq8Output {
    unsafe fn box_into<'fcx>(
        ...
        let _guard = trace_guard!("BoxRet(Rabitq8Output)::box_into [rust]");
