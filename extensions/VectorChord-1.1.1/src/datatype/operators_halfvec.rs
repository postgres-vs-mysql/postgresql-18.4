...
fn _vchord_halfvec_sphere_l2_in(
    ...
    let _guard = trace_guard!("_vchord_halfvec_sphere_l2_in [rust]");
...
fn _vchord_halfvec_sphere_ip_in(
    ...
    let _guard = trace_guard!("_vchord_halfvec_sphere_ip_in [rust]");
    let center: HalfvecOutput = match rhs.get_by_index(NonZero::new(1).unwrap()) {
        Ok(Some(s)) => s,
        Ok(None) => pgrx::error!("Bad input: empty center at sphere"),
        Err(_) => unreachable!(),
...
fn _vchord_halfvec_sphere_cosine_in(
    ...
    let _guard = trace_guard!("_vchord_halfvec_sphere_cosine_in [rust]");
    let center: HalfvecOutput = match rhs.get_by_index(NonZero::new(1).unwrap()) {
        Ok(Some(s)) => s,
        Ok(None) => pgrx::error!("Bad input: empty center at sphere"),
        Err(_) => unreachable!(),
...
fn _vchord_halfvec_operator_maxsim(
    ...
    let _guard = trace_guard!("_vchord_halfvec_operator_maxsim [rust]");
