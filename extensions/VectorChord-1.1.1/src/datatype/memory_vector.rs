...
impl VectorHeader {
    ...
    unsafe fn as_borrowed<'a>(this: NonNull<Self>) -> VectBorrowed<'a, f32> {
        let _guard = trace_guard!("VectorHeader::as_borrowed [rust]");
        unsafe {
...
impl VectorInput<'_> {
    unsafe fn from_ptr(p: NonNull<VectorHeader>) -> Self {
        let _guard = trace_guard!("VectorInput::from_ptr [rust]");
        let q = unsafe {
    ...
    pub fn as_borrowed(&self) -> VectBorrowed<'_, f32> {
        let _guard = trace_guard!("VectorInput::as_borrowed [rust]");
...
impl Drop for VectorInput<'_> {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(VectorInput)::drop [rust]");
        if self.2 {
            unsafe {
...
impl VectorOutput {
    unsafe fn from_ptr(p: NonNull<VectorHeader>) -> Self {
        let _guard = trace_guard!("VectorOutput::from_ptr [rust]");
        let q = unsafe {
    ...
    pub fn new(vector: VectBorrowed<'_, f32>) -> Self {
        let _guard = trace_guard!("VectorOutput::new [rust]");
        unsafe {
    ...
    pub fn as_borrowed(&self) -> VectBorrowed<'_, f32> {
        let _guard = trace_guard!("VectorOutput::as_borrowed [rust]");
        unsafe { VectorHeader::as_borrowed(self.0) }
    }
    fn into_raw(self) -> *mut VectorHeader {
        let _guard = trace_guard!("VectorOutput::into_raw [rust]");
...
impl Drop for VectorOutput {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(VectorOutput)::drop [rust]");
        unsafe {
            pgrx::pg_sys::pfree(self.0.as_ptr().cast());
        }
    }
...
impl FromDatum for VectorInput<'_> {
    unsafe fn from_polymorphic_datum(datum: Datum, is_null: bool, _typoid: Oid) -> Option<Self> {
        let _guard = trace_guard!("FromDatum(VectorInput)::from_polymorphic_datum [rust]");
        if is_null {
            None
        } else {
...
impl FromDatum for VectorOutput {
    unsafe fn from_polymorphic_datum(datum: Datum, is_null: bool, _typoid: Oid) -> Option<Self> {
        let _guard = trace_guard!("FromDatum(VectorOutput)::from_polymorphic_datum [rust]");
        if is_null {
            None
        } else {
            let ptr = NonNull::new(datum.cast_mut_ptr()).unwrap();
...
impl IntoDatum for VectorOutput {
    fn into_datum(self) -> Option<Datum> {
        let _guard = trace_guard!("IntoDatum(VectorOutput)::into_datum [rust]");
...
unsafe impl<'a> pgrx::datum::UnboxDatum for VectorInput<'a> {
    ...
    where
        Self: 'src,
    {
        let _guard = trace_guard!("UnboxDatum(VectorInput)::unbox [rust]");
...
unsafe impl pgrx::datum::UnboxDatum for VectorOutput {
    ...
    where
        Self: 'src,
    {
        let _guard = trace_guard!("UnboxDatum(VectorOutput)::unbox [rust]");
        let datum = datum.sans_lifetime();
        let ptr = NonNull::new(datum.cast_mut_ptr()).unwrap();
        unsafe { Self::from_ptr(ptr) }
    }
...
unsafe impl<'fcx> pgrx::callconv::ArgAbi<'fcx> for VectorInput<'fcx> {
    unsafe fn unbox_arg_unchecked(arg: pgrx::callconv::Arg<'_, 'fcx>) -> Self {
        let _guard = trace_guard!("ArgAbi(VectorInput)::unbox_arg_unchecked [rust]");
        let index = arg.index();
        unsafe {
...
unsafe impl pgrx::callconv::BoxRet for VectorOutput {
    unsafe fn box_into<'fcx>(
        ...
        let _guard = trace_guard!("BoxRet(VectorOutput)::box_into [rust]");
