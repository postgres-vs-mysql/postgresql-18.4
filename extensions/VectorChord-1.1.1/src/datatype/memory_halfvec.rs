...
impl HalfvecHeader {
    ...
    unsafe fn as_borrowed<'a>(this: NonNull<Self>) -> VectBorrowed<'a, f16> {
        let _guard = trace_guard!("HalfvecHeader::as_borrowed [rust]");
        unsafe {
...
impl HalfvecInput<'_> {
    unsafe fn from_ptr(p: NonNull<HalfvecHeader>) -> Self {
        let _guard = trace_guard!("HalfvecInput::from_ptr [rust]");
        let q = unsafe {
...
impl Drop for HalfvecInput<'_> {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(HalfvecInput)::drop [rust]");
        if self.2 {
            unsafe {
...
impl HalfvecOutput {
    unsafe fn from_ptr(p: NonNull<HalfvecHeader>) -> Self {
        let _guard = trace_guard!("HalfvecOutput::from_ptr [rust]");
        let q = unsafe {
    ...
    pub fn new(vector: VectBorrowed<'_, f16>) -> Self {
        let _guard = trace_guard!("HalfvecOutput::new [rust]");
        unsafe {
    ...
    pub fn as_borrowed(&self) -> VectBorrowed<'_, f16> {
        let _guard = trace_guard!("HalfvecOutput::as_borrowed [rust]");
        unsafe { HalfvecHeader::as_borrowed(self.0) }
    }
    fn into_raw(self) -> *mut HalfvecHeader {
        let _guard = trace_guard!("HalfvecOutput::into_raw [rust]");
...
impl Drop for HalfvecOutput {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(HalfvecOutput)::drop [rust]");
        unsafe {
            pgrx::pg_sys::pfree(self.0.as_ptr().cast());
        }
    }
...
impl FromDatum for HalfvecInput<'_> {
    unsafe fn from_polymorphic_datum(datum: Datum, is_null: bool, _typoid: Oid) -> Option<Self> {
        let _guard = trace_guard!("FromDatum(HalfvecInput)::from_polymorphic_datum [rust]");
        if is_null {
            None
        } else {
...
impl FromDatum for HalfvecOutput {
    unsafe fn from_polymorphic_datum(datum: Datum, is_null: bool, _typoid: Oid) -> Option<Self> {
        let _guard = trace_guard!("FromDatum(HalfvecOutput)::from_polymorphic_datum [rust]");
        if is_null {
            None
        } else {
            let ptr = NonNull::new(datum.cast_mut_ptr()).unwrap();
...
impl IntoDatum for HalfvecOutput {
    fn into_datum(self) -> Option<Datum> {
        let _guard = trace_guard!("IntoDatum(HalfvecOutput)::into_datum [rust]");
...
unsafe impl<'a> pgrx::datum::UnboxDatum for HalfvecInput<'a> {
    ...
    where
        Self: 'src,
    {
        let _guard = trace_guard!("UnboxDatum(HalfvecInput)::unbox [rust]");
...
unsafe impl pgrx::datum::UnboxDatum for HalfvecOutput {
    ...
    where
        Self: 'src,
    {
        let _guard = trace_guard!("UnboxDatum(HalfvecOutput)::unbox [rust]");
        let datum = datum.sans_lifetime();
        let ptr = NonNull::new(datum.cast_mut_ptr()).unwrap();
        unsafe { Self::from_ptr(ptr) }
    }
...
unsafe impl<'fcx> pgrx::callconv::ArgAbi<'fcx> for HalfvecInput<'fcx> {
    unsafe fn unbox_arg_unchecked(arg: pgrx::callconv::Arg<'_, 'fcx>) -> Self {
        let _guard = trace_guard!("ArgAbi(HalfvecInput)::unbox_arg_unchecked [rust]");
        let index = arg.index();
        unsafe {
...
unsafe impl pgrx::callconv::BoxRet for HalfvecOutput {
    unsafe fn box_into<'fcx>(
        ...
        let _guard = trace_guard!("BoxRet(HalfvecOutput)::box_into [rust]");
