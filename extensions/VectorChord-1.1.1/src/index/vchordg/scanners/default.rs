...
impl SearchBuilder for DefaultBuilder {
    ...
    unsafe fn add(&mut self, strategy: u16, datum: Option<pgrx::pg_sys::Datum>) {
        let _guard = trace_guard!("vchordg::SearchBuilder(DefaultBuilder)::add [rust]");
        match strategy {
            1 => {
    ...
    {
        let _guard = trace_guard!("vchordg::SearchBuilder(DefaultBuilder)::build [rust]");
        let mut vector = None;
        ...
        let iter: Box<dyn Iterator<Item = (Distance, NonZero<u64>)>> =
            match (opfamily.vector_kind(), opfamily.distance_kind()) {
                ...
                (VectorKind::Rabitq8, DistanceKind::L2S) => {
                    type Op = operator::Op<Rabitq8Owned, L2S>;
                ...
                (VectorKind::Rabitq8, DistanceKind::Dot) => {
                    type Op = operator::Op<Rabitq8Owned, Dot>;
                    let unprojected = if let OwnedVector::Rabitq8(vector) = vector.clone() {
                        let vector = vector.as_borrowed();
                        Rabitq8Borrowed::new(
                            vector.dim(),
