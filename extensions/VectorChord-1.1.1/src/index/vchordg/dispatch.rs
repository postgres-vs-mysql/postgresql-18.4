...
{
    let _guard = trace_guard!("vchordg::dispatch::prewarm [rust]");
    match (opfamily.vector_kind(), opfamily.distance_kind()) {
        ...
        (VectorKind::Rabitq8, DistanceKind::L2S) => {
            vchordg::prewarm::<_, Op<Rabitq8Owned, L2S>>(index)
        }
...
pub fn bulkdelete<R>(
    ...
    R::Page: Page<Opaque = vchordg::Opaque>,
{
    let _guard = trace_guard!("vchordg::dispatch::bulkdelete [rust]");
    match (opfamily.vector_kind(), opfamily.distance_kind()) {
        (VectorKind::Vecf32, DistanceKind::L2S) => {
        ...
        (VectorKind::Rabitq8, DistanceKind::L2S) => {
            vchordg::bulkdelete::<_, Op<Rabitq8Owned, L2S>>(index, &check, &callback);
        }
        (VectorKind::Rabitq8, DistanceKind::Dot) => {
...
where
    ...
    R::Page: Page<Opaque = vchordg::Opaque>,
{
    let _guard = trace_guard!("vchordg::dispatch::maintain [rust]");
    match (opfamily.vector_kind(), opfamily.distance_kind()) {
        (VectorKind::Vecf32, DistanceKind::L2S) => {
        ...
        (VectorKind::Rabitq8, DistanceKind::L2S) => {
            vchordg::maintain::<_, Op<Rabitq8Owned, L2S>>(index, &check);
        }
        (VectorKind::Rabitq8, DistanceKind::Dot) => {
...
where
    ...
    R::Page: Page<Opaque = vchordg::Opaque>,
{
    let _guard = trace_guard!("vchordg::dispatch::build [rust]");
    match (vector_options.v, vector_options.d) {
        ...
        (VectorKind::Rabitq8, DistanceKind::L2S) => {
            vchordg::build::<_, Op<Rabitq8Owned, L2S>>(vector_options, vchordg_options, index)
        }
        (VectorKind::Rabitq8, DistanceKind::Dot) => {
...
where
    ...
    R::Page: Page<Opaque = vchordg::Opaque>,
{
    let _guard = trace_guard!("vchordg::dispatch::insert [rust]");
    ...
    match (vector, opfamily.distance_kind()) {
        ...
        (OwnedVector::Rabitq8(unprojected), DistanceKind::L2S) => {
            assert!(opfamily.vector_kind() == VectorKind::Rabitq8);
            vchordg::insert::<_, Op<Rabitq8Owned, L2S>>(
                index,
...
impl RandomProject for VectBorrowed<'_, f32> {
    ...
    fn project(self) -> VectOwned<f32> {
        let _guard = trace_guard!("vchordg::RandomProject(VectBorrowed<'_, f32>)::project [rust]");
...
impl RandomProject for VectBorrowed<'_, f16> {
    ...
    fn project(self) -> VectOwned<f16> {
        let _guard = trace_guard!("vchordg::RandomProject(VectBorrowed<'_, f16>)::project [rust]");
        use rabitq::rotate::rotate;
        use simd::Floating;
...
impl<'b, R> Clone for MakePlainPrefetcher<'b, R> {
    fn clone(&self) -> Self {
        let _guard = trace_guard!("Clone(MakePlainPrefetcher)::clone [rust]");
...
impl<'b, R: RelationRead> PrefetcherSequenceFamily<'b, R> for MakePlainPrefetcher<'b, R> {
    ...
    where
        S::Item: Fetch<'b>,
    {
        let _guard = trace_guard!("PrefetcherSequenceFamily(MakePlainPrefetcher)::prefetch [rust]");
...
impl<'b, R> Clone for MakeSimplePrefetcher<'b, R> {
    fn clone(&self) -> Self {
        let _guard = trace_guard!("Clone(MakeSimplePrefetcher)::clone [rust]");
        Self { index: self.index }
    }
}
    ...
    for MakeSimplePrefetcher<'b, R>
{
    ...
    {
        let _guard =
            trace_guard!("PrefetcherSequenceFamily(MakeSimplePrefetcher)::prefetch [rust]");
...
impl<'b, R> Clone for MakeStreamPrefetcher<'b, R> {
    fn clone(&self) -> Self {
        let _guard = trace_guard!("Clone(MakeStreamPrefetcher)::clone [rust]");
        Self {
            index: self.index,
            hints: self.hints,
        }
    ...
    for MakeStreamPrefetcher<'b, R>
{
    ...
    {
        let _guard =
            trace_guard!("PrefetcherSequenceFamily(MakeStreamPrefetcher)::prefetch [rust]");
