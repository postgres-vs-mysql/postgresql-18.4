...
impl Opfamily {
    fn input(self, vector: BorrowedVector<'_>) -> OwnedVector {
        let _guard = trace_guard!("vchordg::Opfamily::input [rust]");
    ...
    pub unsafe fn store(self, datum: Datum) -> Option<Vec<(OwnedVector, u16)>> {
        let _guard = trace_guard!("vchordg::Opfamily::store [rust]");
        if datum.is_null() {
            return None;
        }
    ...
    pub unsafe fn input_sphere(self, datum: Datum) -> Option<Sphere<OwnedVector>> {
        let _guard = trace_guard!("vchordg::Opfamily::input_sphere [rust]");
        if datum.is_null() {
            return None;
        }
    ...
    pub unsafe fn input_vector(self, datum: Datum) -> Option<OwnedVector> {
        let _guard = trace_guard!("vchordg::Opfamily::input_vector [rust]");
        if datum.is_null() {
            return None;
        }
    ...
    pub fn output(self, x: Distance) -> f32 {
        let _guard = trace_guard!("vchordg::Opfamily::output [rust]");
        match self {
            Self::VectorCosine
            | Self::HalfvecCosine
            | Self::Rabitq8Cosine
...
pub unsafe fn opfamily(index_relation: pgrx::pg_sys::Relation) -> Opfamily {
    let _guard = trace_guard!("vchordg::opfamily [rust]");
    use pgrx::pg_sys::Oid;
