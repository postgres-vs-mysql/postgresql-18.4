...
pub fn init() {
    let _guard = trace_guard!("gucs::init [rust]");
...
unsafe fn check(p: *mut pgrx::pg_sys::config_generic, name: &CStr) -> bool {
    let _guard = trace_guard!("gucs::check [rust]");
    if p.is_null() {
        return false;
    }
...
pub fn vchordg_enable_scan() -> bool {
    let _guard = trace_guard!("gucs::vchordg_enable_scan [rust]");
    VCHORDG_ENABLE_SCAN.get()
}

pub fn vchordg_ef_search(index: pgrx::pg_sys::Relation) -> u32 {
    let _guard = trace_guard!("gucs::vchordg_ef_search [rust]");
    fn parse(x: i32) -> u32 {
        x as u32
    }
    assert!(crate::is_main());
...
pub fn vchordg_io_rerank() -> Io {
    let _guard = trace_guard!("gucs::vchordg_io_rerank [rust]");
...
pub fn vchordrq_enable_scan() -> bool {
    let _guard = trace_guard!("gucs::vchordrq_enable_scan [rust]");
    let result = VCHORDRQ_ENABLE_SCAN.get();
    if result {
        trace_vchord_print!("return true");
    } else {
        trace_vchord_print!("return false");
    }
    result
}

pub unsafe fn vchordrq_probes(index: pgrx::pg_sys::Relation) -> Vec<u32> {
    let _guard = trace_guard!("gucs::vchordrq_probes [rust]");
    fn parse(value: &CStr) -> Vec<u32> {
        ...
        if let Some(current) = current {
            trace_vchord_print!("current: {}", current);
            result.push(current);
        }
        result
    }
...
pub unsafe fn vchordrq_epsilon(index: pgrx::pg_sys::Relation) -> f32 {
    let _guard = trace_guard!("gucs::vchordrq_epsilon [rust]");
    fn parse(x: f64) -> f32 {
        x as f32
    }
    assert!(crate::is_main());
...
pub fn vchordrq_max_scan_tuples() -> Option<u32> {
    let _guard = trace_guard!("gucs::vchordrq_max_scan_tuples [rust]");
    let x = VCHORDRQ_MAX_SCAN_TUPLES.get();
    if x < 0 { None } else { Some(x as u32) }
}

pub fn vchordrq_maxsim_refine(index: pgrx::pg_sys::Relation) -> u32 {
    let _guard = trace_guard!("gucs::vchordrq_maxsim_refine [rust]");
    fn parse(x: i32) -> u32 {
        x as u32
    }
    assert!(crate::is_main());
...
pub fn vchordrq_maxsim_threshold(index: pgrx::pg_sys::Relation) -> u32 {
    let _guard = trace_guard!("gucs::vchordrq_maxsim_threshold [rust]");
    fn parse(x: i32) -> u32 {
        x as u32
    }
    assert!(crate::is_main());
...
pub fn vchordrq_prefilter() -> bool {
    let _guard = trace_guard!("gucs::vchordrq_prefilter [rust]");
    VCHORDRQ_PREFILTER.get()
}

pub fn vchordrq_io_search() -> Io {
    let _guard = trace_guard!("gucs::vchordrq_io_search [rust]");
...
pub fn vchordrq_io_rerank() -> Io {
    let _guard = trace_guard!("gucs::vchordrq_io_rerank [rust]");
...
pub fn vchordrq_query_sampling_enable() -> bool {
    let _guard = trace_guard!("gucs::vchordrq_query_sampling_enable [rust]");
    VCHORDRQ_QUERY_SAMPLING_ENABLE.get()
}

pub fn vchordrq_query_sampling_max_records() -> u32 {
    let _guard = trace_guard!("gucs::vchordrq_query_sampling_max_records [rust]");
    let result = VCHORDRQ_QUERY_SAMPLING_MAX_RECORDS.get() as u32;
    trace_vchord_print!("result: {}", result);
    result
}

pub fn vchordrq_query_sampling_rate() -> f64 {
    let _guard = trace_guard!("gucs::vchordrq_query_sampling_rate [rust]");
    let result = VCHORDRQ_QUERY_SAMPLING_RATE.get();
    trace_vchord_print!("result: {}", result);
    result
}
