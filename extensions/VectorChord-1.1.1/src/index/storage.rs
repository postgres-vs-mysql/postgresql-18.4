...
impl<O: Opaque> PostgresPage<O> {
    pub fn clone_into_boxed(&self) -> Box<Self> {
        let _guard = trace_guard!("storage::PostgresPage::clone_into_boxed [rust]");
...
unsafe fn page_init<O: Opaque>(this: *mut PostgresPage<O>, opaque: O) {
    unsafe {
        let _guard = trace_guard!("storage::page_init [rust]");
...
impl<Opaque> Deref for PostgresBufferReadGuard<Opaque> {
    ...
    fn deref(&self) -> &PostgresPage<Opaque> {
        let _guard = trace_guard!("storage::Deref(PostgresBufferReadGuard)::deref [rust]");
...
impl<Opaque> Drop for PostgresBufferReadGuard<Opaque> {
    fn drop(&mut self) {
        let _guard = trace_guard!("storage::Drop(PostgresBufferReadGuard)::drop [rust]");
        unsafe {
...
impl<O: Opaque> Deref for PostgresBufferWriteGuard<O> {
    ...
    fn deref(&self) -> &PostgresPage<O> {
        let _guard = trace_guard!("storage::Deref(PostgresBufferWriteGuard)::deref [rust]");
        unsafe { self.page.as_ref() }
    }
}
...
impl<O: Opaque> DerefMut for PostgresBufferWriteGuard<O> {
    fn deref_mut(&mut self) -> &mut PostgresPage<O> {
        let _guard = trace_guard!("storage::DerefMut(PostgresBufferWriteGuard)::deref_mut [rust]");
...
impl<O: Opaque> Drop for PostgresBufferWriteGuard<O> {
    fn drop(&mut self) {
        let _guard = trace_guard!("storage::Drop(PostgresBufferWriteGuard)::drop [rust]");
        unsafe {
...
impl<O: Opaque> RelationRead for PostgresRelation<O> {
    fn read(&self, id: u32) -> Self::ReadGuard<'_> {
        let _guard = trace_guard!("storage::RelationRead(PostgresRelation)::read [rust]");
...
impl<O: Opaque> RelationWrite for PostgresRelation<O> {
    fn write(&self, id: u32, tracking_freespace: bool) -> PostgresBufferWriteGuard<O> {
        let _guard = trace_guard!("storage::RelationWrite(PostgresRelation)::write [rust]");
        assert!(id != u32::MAX, "no such page");
        unsafe {
            use pgrx::pg_sys::{
...
impl<O: Opaque> RelationPrefetch for PostgresRelation<O> {
    fn prefetch(&self, id: u32) {
        let _guard = trace_guard!("storage::RelationPrefetch(PostgresRelation)::prefetch [rust]");
        assert!(id != u32::MAX, "no such page");
        unsafe {
...
impl<'b, I: Iterator> Default for Cache<'b, I> {
    fn default() -> Self {
        let _guard = trace_guard!("storage::Default(Cache)::default [rust]");
        Self {
    ...
    I::Item: Fetch<'b>,
{
    ...
    pub fn pop_id(&mut self) -> Option<u32> {
        let _guard = trace_guard!("Cache::pop_id [rust]");
        ...
        let result = self.tail.pop_front();
        trace_vchord_print!("pop_id result: {:?}", result);
        result
    }
    #[allow(dead_code)]
    ...
    L: Iterator<Item = u32>,
{
    ...
    #[cfg(any(feature = "pg17", feature = "pg18"))]
    fn next(&mut self) -> Option<Self::Item> {
        let _guard = trace_guard!("storage::Iterator(PostgresReadStreamGuards)::next [rust]");
    ...
    I::Item: Fetch<'b>,
{
    fn read<L: Iterator<Item = u32>>(&mut self, fetch: L) -> PostgresReadStreamGuards<O, I, L> {
        let _guard = trace_guard!("storage::PostgresReadStream::read [rust]");
    ...
    I::Item: Fetch<'b>,
{
    ...
    #[cfg(any(feature = "pg17", feature = "pg18"))]
    fn next(&mut self) -> Option<(I::Item, Self::Guards)> {
        let _guard = trace_guard!("storage::ReadStream(PostgresReadStream)::next [rust]");
    ...
    fn next_if<P: FnOnce(&I::Item) -> bool>(
        ...
        let _guard = trace_guard!("storage::ReadStream(PostgresReadStream)::next_if [rust]");
    ...
    fn into_inner(mut self) -> Self::Inner {
        let _guard = trace_guard!("storage::ReadStream(PostgresReadStream)::into_inner [rust]");
...
impl<'b, O: Opaque, I: Iterator> Drop for PostgresReadStream<'b, O, I> {
    fn drop(&mut self) {
        let _guard = trace_guard!("storage::Drop(PostgresReadStream)::drop [rust]");
        unsafe {
...
impl<O: Opaque> RelationReadStream for PostgresRelation<O> {
    ...
    {
        let _guard =
            trace_guard!("storage::RelationReadStream(PostgresRelation)::read_stream [rust]");
        #[pgrx::pg_guard]
