...
impl<T: Fetcher, F: FnOnce() -> T> Fetcher for LazyCell<T, F> {
    ...
    fn fetch(&mut self, key: [u16; 3]) -> Option<Self::Tuple<'_>> {
        let _guard = trace_guard!("Fetcher(LazyCell)::fetch [rust]");
...
impl Drop for HeapFetcher {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(HeapFetcher)::drop [rust]");
        unsafe {
...
impl Fetcher for HeapFetcher {
    ...
    fn fetch(&mut self, key: [u16; 3]) -> Option<Self::Tuple<'_>> {
        let _guard = trace_guard!("Fetcher(HeapFetcher)::fetch [rust]");
        unsafe {
...
impl Tuple for HeapTuple<'_> {
    fn build(&mut self) -> (&[Datum; 32], &[bool; 32]) {
        let _guard = trace_guard!("fetcher::Tuple(HeapTuple)::build [rust]");
        unsafe {
...
impl FilterableTuple for HeapTuple<'_> {
    fn filter(&mut self) -> bool {
        let _guard = trace_guard!("FilterableTuple(HeapTuple)::filter [rust]");
        unsafe {
            use pgrx::pg_sys::ffi::pg_guard_ffi_boundary;
            let this = &mut self.this;
