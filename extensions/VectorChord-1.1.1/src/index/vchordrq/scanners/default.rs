...
impl SearchBuilder for DefaultBuilder {
    ...
    unsafe fn add(&mut self, strategy: u16, datum: Option<pgrx::pg_sys::Datum>) {
        let _guard = trace_guard!("vchordrq::SearchBuilder(DefaultBuilder)::add [rust]");
        match strategy {
            1 => {
    ...
    {
        let _guard = trace_guard!("vchordrq::SearchBuilder(DefaultBuilder)::build [rust]");
        let mut vector = None;
        ...
        let iter: Box<dyn Iterator<Item = (f32, NonZero<u64>)>> =
            match (opfamily.vector_kind(), opfamily.distance_kind()) {
                (VectorKind::Vecf32, DistanceKind::L2S) => {
                    trace_vchord_print!("VectorKind::Vecf32, DistanceKind::L2S");
                ...
                (VectorKind::Vecf32, DistanceKind::Dot) => {
                    trace_vchord_print!("VectorKind::Vecf32, DistanceKind::Dot");
                    ...
                    trace_vchord_print!("here we check method, io_rerank and prefilter");
                    let method = how(index);
                    let sequence = Heap::from(results);
                    match (method, options.io_rerank, options.prefilter) {
                        (RerankMethod::Index, Io::Plain, false) => {
                            trace_vchord_print!("RerankMethod::Index, Io::Plain, false");
                            let prefetcher = PlainPrefetcher::new(index, sequence);
                            Box::new(rerank_index::<Op, _, _, _>(unprojected, prefetcher).map(f))
                        }
                        (RerankMethod::Index, Io::Plain, true) => {
                            trace_vchord_print!("RerankMethod::Index, Io::Plain, true");
                            let predicate =
                                id_0(move |(_, AlwaysEqual(PackedRefMut4((pointer, _, _))))| {
                                    let (key, _) = pointer_to_kv(*pointer);
                                    let Some(mut tuple) = fetcher.fetch(key) else {
                        ...
                        (RerankMethod::Index, Io::Simple, false) => {
                            trace_vchord_print!("RerankMethod::Index, Io::Simple, false");
                            let prefetcher = SimplePrefetcher::new(index, sequence);
                            Box::new(rerank_index::<Op, _, _, _>(unprojected, prefetcher).map(f))
                        }
                        (RerankMethod::Index, Io::Simple, true) => {
                            trace_vchord_print!("RerankMethod::Index, Io::Simple, true");
                            let predicate =
                                id_0(move |(_, AlwaysEqual(PackedRefMut4((pointer, _, _))))| {
                                    let (key, _) = pointer_to_kv(*pointer);
                                    let Some(mut tuple) = fetcher.fetch(key) else {
                        ...
                        (RerankMethod::Index, Io::Stream, false) => {
                            trace_vchord_print!("RerankMethod::Index, Io::Stream, false");
                            let prefetcher = StreamPrefetcher::new(index, sequence, rerank_hints);
                            Box::new(rerank_index::<Op, _, _, _>(unprojected, prefetcher).map(f))
                        }
                        (RerankMethod::Index, Io::Stream, true) => {
                            trace_vchord_print!("RerankMethod::Index, Io::Stream, true");
                            let predicate =
                                id_0(move |(_, AlwaysEqual(PackedRefMut4((pointer, _, _))))| {
                                    let (key, _) = pointer_to_kv(*pointer);
                                    let Some(mut tuple) = fetcher.fetch(key) else {
                        ...
                        (RerankMethod::Heap, _, false) => {
                            trace_vchord_print!("RerankMethod::Heap, _, false");
                            let fetch = move |payload| {
                                let (key, _) = pointer_to_kv(payload);
                                let mut tuple = fetcher.fetch(key)?;
                                let (datums, is_nulls) = tuple.build();
                        ...
                        (RerankMethod::Heap, _, true) => {
                            trace_vchord_print!("RerankMethod::Heap, _, true");
                            let fetch = move |payload| {
                                let (key, _) = pointer_to_kv(payload);
                                let mut tuple = fetcher.fetch(key)?;
                                if !tuple.filter() {
                ...
                (VectorKind::Vecf16, DistanceKind::L2S) => {
                    trace_vchord_print!("VectorKind::Vecf16, DistanceKind::L2S");
                ...
                (VectorKind::Vecf16, DistanceKind::Dot) => {
                    trace_vchord_print!("VectorKind::Vecf16, DistanceKind::Dot");
                ...
                (VectorKind::Rabitq8, DistanceKind::L2S) => {
                    trace_vchord_print!("VectorKind::Rabitq8, DistanceKind::L2S");
                ...
                (VectorKind::Rabitq8, DistanceKind::Dot) => {
                    trace_vchord_print!("VectorKind::Rabitq8, DistanceKind::Dot");
                ...
                (VectorKind::Rabitq4, DistanceKind::L2S) => {
                    trace_vchord_print!("VectorKind::Rabitq4, DistanceKind::L2S");
                ...
                (VectorKind::Rabitq4, DistanceKind::Dot) => {
                    trace_vchord_print!("VectorKind::Rabitq4, DistanceKind::Dot");
        ...
        let iter = if let Some(threshold) = threshold {
            trace_vchord_print!("threshold = {}", threshold);
            Box::new(iter.take_while(move |(x, _)| *x < threshold))
        } else {
            trace_vchord_print!("threshold = None");
            iter
        };
        let iter = if let Some(max_scan_tuples) = options.max_scan_tuples {
            trace_vchord_print!("max_scan_tuples = {}", max_scan_tuples);
            Box::new(iter.take(max_scan_tuples as _))
        } else {
            trace_vchord_print!("max_scan_tuples = None");
            iter
        };
