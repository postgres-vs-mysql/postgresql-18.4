...
impl Normalize for VectOwned<f32> {
    fn normalize(vector: Self) -> Normalized {
        let _guard = trace_guard!("Normalize(VectOwned<f32>)::normalize [rust]");
        vector.into_vec()
    }

    fn denormalize(x: Normalized) -> Self {
        let _guard = trace_guard!("Normalize(VectOwned<f32>)::denormalize [rust]");
        Self::new(x)
    }
}
...
impl Normalize for VectOwned<f16> {
    fn normalize(vector: Self) -> Normalized {
        let _guard = trace_guard!("Normalize(VectOwned<f16>)::normalize [rust]");
        f16::vector_to_f32(vector.slice())
    }

    fn denormalize(x: Normalized) -> Self {
        let _guard = trace_guard!("Normalize(VectOwned<f16>)::denormalize [rust]");
...
impl Normalize for Rabitq8Owned {
    fn normalize(vector: Self) -> Normalized {
        let _guard = trace_guard!("Normalize(Rabitq8Owned)::normalize [rust]");
    ...
    fn denormalize(vector: Normalized) -> Self {
        let _guard = trace_guard!("Normalize(Rabitq8Owned)::denormalize [rust]");
...
impl Normalize for Rabitq4Owned {
    fn normalize(vector: Self) -> Normalized {
        let _guard = trace_guard!("Normalize(Rabitq4Owned)::normalize [rust]");
        let vector = vector.as_borrowed();
        let scale = vector.sum_of_x2().sqrt() / vector.norm_of_lattice();
        let mut result = Vec::with_capacity(vector.dim() as _);
        for c in vector.unpacked_code() {
    ...
    fn denormalize(vector: Normalized) -> Self {
        let _guard = trace_guard!("Normalize(Rabitq4Owned)::denormalize [rust]");
        let dim = vector.len() as u32;
