...
{
    let _guard = trace_guard!("vchordrq::dispatch::prewarm [rust]");
...
pub fn bulkdelete<R>(
    ...
    R::Page: Page<Opaque = vchordrq::Opaque>,
{
    let _guard = trace_guard!("vchordrq::dispatch::bulkdelete [rust]");
    match (opfamily.vector_kind(), opfamily.distance_kind()) {
        (VectorKind::Vecf32, DistanceKind::L2S) => {
...
pub fn maintain<R>(
    ...
    R::Page: Page<Opaque = vchordrq::Opaque>,
{
    let _guard = trace_guard!("vchordrq::dispatch::maintain [rust]");
    let make_h0_plain_prefetcher = MakeH0PlainPrefetcher { index };
...
pub fn build<R>(
    ...
    R::Page: Page<Opaque = vchordrq::Opaque>,
{
    let _guard = trace_guard!("vchordrq::dispatch::build [rust]");
...
pub fn insert<R>(
    ...
    R::Page: Page<Opaque = vchordrq::Opaque>,
{
    let _guard = trace_guard!("vchordrq::dispatch::insert [rust]");
...
fn map_structures<T, U>(x: Vec<Structure<T>>, f: impl Fn(T) -> U + Copy) -> Vec<Structure<U>> {
    let _guard = trace_guard!("vchordrq::dispatch::map_structures [rust]");
    x.into_iter()
        .map(
            |Structure {
                 centroids,
...
impl RandomProject for VectBorrowed<'_, f32> {
    ...
    fn project(self) -> VectOwned<f32> {
        let _guard = trace_guard!("vchordrq::RandomProject(VectBorrowed<'_, f32>)::project [rust]");
...
impl RandomProject for VectBorrowed<'_, f16> {
    ...
    fn project(self) -> VectOwned<f16> {
        let _guard = trace_guard!("vchordrq::RandomProject(VectBorrowed<'_, f16>)::project [rust]");
        use rabitq::rotate::rotate;
        use simd::Floating;
