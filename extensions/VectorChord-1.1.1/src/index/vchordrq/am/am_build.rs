...
pub extern "C-unwind" fn ambuildphasename(x: i64) -> *mut core::ffi::c_char {
    let _guard = trace_guard!("vchordrq::ambuildphasename [rust]");
...
impl PostgresReporter {
    fn phase(&self, phase: BuildPhase) {
        let _guard = trace_guard!("vchordrq::PostgresReporter::phase [rust]");
        unsafe {
    ...
    fn tuples_total(&self, tuples_total: u64) {
        let _guard = trace_guard!("vchordrq::PostgresReporter::tuples_total [rust]");
        unsafe {
            trace_vchord_print!("tuples_total: {}", tuples_total);
            pgrx::pg_sys::pgstat_progress_update_param(
    ...
    fn tuples_done(&self, tuples_done: u64) {
        let _guard = trace_guard!("vchordrq::PostgresReporter::tuples_done [rust]");
        unsafe {
            trace_vchord_print!("tuples_done: {}", tuples_done);
            pgrx::pg_sys::pgstat_progress_update_param(
...
pub unsafe extern "C-unwind" fn ambuild(
    ...
    if vector_options.v == VectorKind::Rabitq8 && vchordrq_options.index.residual_quantization {
        trace_vchord_instant_print!("residual_quantization is not supported for rabitq8 type");
    ...
    if vector_options.v == VectorKind::Rabitq4 && vchordrq_options.index.residual_quantization {
        trace_vchord_instant_print!("residual_quantization is not supported for rabitq4 type");
    ...
    let mut structures = match vchordrq_options.build.source.clone() {
        VchordrqBuildSourceOptions::Default(default_build) => {
            trace_vchord_print!("VchordrqBuildSourceOptions::Default(default_build)");
        ...
        VchordrqBuildSourceOptions::Internal(internal_build) => {
            trace_vchord_print!("VchordrqBuildSourceOptions::Internal(internal_build)");
        ...
        VchordrqBuildSourceOptions::External(external_build) => {
            trace_vchord_print!("VchordrqBuildSourceOptions::External(external_build)");
    ...
    for structure in structures.iter_mut() {
        let mut centroid_len = 0;
        trace_vchord_print!("before the loop");
        for centroid in structure.centroids.iter_mut() {
            rabitq::rotate::rotate_inplace(centroid);
            centroid_len = centroid_len + 1;
        }
        trace_vchord_print!("centroid num: {}", centroid_len);
    }
...
mod vchordrq_cached {
    ...
    impl VchordrqCached {
        pub fn serialize(&self) -> Vec<u8> {
            let _guard = trace_guard!("VchordrqCached::serialize [rust]");
...
fn is_mvcc_snapshot(snapshot: *mut pgrx::pg_sys::SnapshotData) -> bool {
    let _guard = trace_guard!("vchordrq::is_mvcc_snapshot [rust]");
    matches!(
...
impl VchordrqLeader {
    pub unsafe fn enter(
        ...
        let _guard = trace_guard!("VchordrqLeader::enter [rust]");
        unsafe fn compute_parallel_workers(
            heap_relation: pgrx::pg_sys::Relation,
            index_relation: pgrx::pg_sys::Relation,
        ) -> i32 {
            let _guard = trace_guard!("compute_parallel_workers(internal) [rust]");
            unsafe {
...
impl Drop for VchordrqLeader {
    fn drop(&mut self) {
        let _guard = trace_guard!("Drop(VchordrqLeader)::drop [rust]");
...
pub unsafe extern "C-unwind" fn vchordrq_parallel_build_main(
    ...
    let _guard = trace_guard!("vchordrq_parallel_build_main [rust]");
    ...
    unsafe {
        parallel_build(
            ...
            || {
                #[allow(clippy::needless_late_init)]
                let _guard = trace_guard!("parallel_build(internal) [rust]");
                let order;
                // enter the barrier
...
unsafe fn parallel_build(
    ...
    let _guard = trace_guard!("vchordrq::parallel_build [rust]");
    ...
    trace_vchord_print!("begin to match cached");
    let mut trace_disabled_flag = 0;
    let mut total_count = 0;
    let max_trace_iterations = trace::max_trace_iterations();
    let trace_enabled_flag = trace::trace_enabled();
    match cached {
        VchordrqCachedReader::_0(_) => {
            trace_vchord_print!("VchordrqCachedReader::_0(_)");
            traverser.traverse(true, |tuple: &mut dyn crate::index::traverse::Tuple| {
                ...
                unsafe {
                    ...
                    if trace_enabled_flag {
                        if total_count > max_trace_iterations {
        ...
        VchordrqCachedReader::_1(cached) => {
            trace_vchord_print!("VchordrqCachedReader::_1(cached)");
    ...
    if trace_disabled_flag == 1 {
        trace::enable_trace();
        trace_vchord_print!("...");
        trace_disabled_flag = 0;
    }
    trace_vchord_print!("total_count: {}", total_count);
    trace_vchord_print!("enter index consum process");
    ...
    trace_vchord_print!("number of consume calls: {}", count);
...
unsafe fn sequential_build(
    ...
    let _guard = trace_guard!("vchordrq::sequential_build [rust]");
    use vchordrq_cached::VchordrqCachedReader;
    ...
    trace_vchord_print!("enter matching cached process");
    let mut indtuples = 0;
    let mut trace_disabled_flag = 0;
    let max_trace_iterations = trace::max_trace_iterations() as u64;
    let trace_enabled_flag = trace::trace_enabled();
    ...
    match cached {
        VchordrqCachedReader::_0(_) => {
            ...
            traverser.traverse(true, |tuple: &mut dyn crate::index::traverse::Tuple| {
                ...
                callback(indtuples);
                if trace_enabled_flag {
                    if indtuples > max_trace_iterations {
                        if trace_disabled_flag == 0 {
                            trace::disable_trace();
                            trace_disabled_flag = 1;
                        }
    ...
    trace_vchord_print!("indtuples: {}", indtuples);
    trace_vchord_print!("enter index consum process");
    let mut count = 0;
    let max_trace_iterations = trace::max_trace_iterations();
    ...
    trace_vchord_print!("enter index maintain process");
    crate::index::vchordrq::dispatch::maintain(opfamily, &index, &mut chooser, check);
...
unsafe fn options(
    ...
    let _guard = trace_guard!("vchordrq::options [rust]");
...
fn make_default_build(
    ...
    let _guard = trace_guard!("vchordrq::make_default_build [rust]");
    vec![Structure::<Normalized> {
...
fn make_internal_build(
    ...
    let _guard = trace_guard!("vchordrq::make_internal_build [rust]");
    ...
    trace_vchord_print!("clustering: using {} threads", pool.current_num_threads());
    ...
    for w in internal_build.lists.iter().rev().copied().chain(once(1)) {
        let mut input = if let Some(structure) = result.last() {
            ...
            trace_vchord_print!("centroids len: {}", structure.centroids.len());
        ...
        trace_vchord_print!(
            "num_points: {}, num_dim: {}, num_lists: {}, num_iterations: {}",
            num_points,
            num_dim,
            num_lists,
            num_iterations
        ...
        for i in 0..num_iterations {
            ...
            trace_vchord_print!("clustering: iteration {}", i + 1);
            if num_lists > 1 {
        ...
        if trace_thread_mode == 1 {
            trace::disable_trace_thread_mode();
            trace_vchord_print!(
                "parallel execution finished, and process trace mode is now enabled."
            );
        }
        if result.is_empty() {
            let percentage = 100;
        ...
        if let Some(structure) = result.last() {
            ...
            for i in 0..structure.len() as u32 {
                ...
                trace_vchord_print!("target children: {}, push i: {}", target, i);
            }
...
fn make_external_build(
    ...
    let _guard = trace_guard!("vchordrq::make_external_build [rust]");
    ...
    pgrx::spi::Spi::connect(|client| {
        ...
        trace_vchord_print!("rows in centroids: {}", count);
    });
    trace_vchord_print!("parenets.len: {}", parents.len());
    if parents.len() >= 2 && parents.values().all(|x| x.is_none()) {
        ...
        result.push(Structure {
            centroids: vec![{
                ...
                trace_vchord_print!("compute the vector on root, without normalizing it");
    ...
    fn dfs_for_heights(
        ...
        let _guard = trace_guard!("vchordrq::dfs_for_heights [rust]");
    ...
    fn extract(
        ...
        let _guard = trace_guard!("vchordrq::extract [rust]");
        labels
            .iter()
    ...
    for CachingRelation<'_, R>
{
    fn read(&self, id: u32) -> Self::ReadGuard<'_> {
        let _guard = trace_guard!("RelationRead(CachingRelation)::read [rust]");
    ...
    for CachingRelation<'_, R>
{
    fn write(&self, id: u32, tracking_freespace: bool) -> Self::WriteGuard<'_> {
        let _guard = trace_guard!("RelationWrite(CachingRelation)::write [rust]");
    ...
    fn extend(
        ...
        let _guard = trace_guard!("RelationWrite(CachingRelation)::extend [rust]");
        self.relation.extend(opaque, tracking_freespace)
    }

    fn search(&self, freespace: usize) -> Option<Self::WriteGuard<'_>> {
        let _guard = trace_guard!("RelationWrite(CachingRelation)::search [rust]");
