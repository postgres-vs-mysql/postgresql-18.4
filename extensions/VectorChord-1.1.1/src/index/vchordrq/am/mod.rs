...
impl Reloption {
    unsafe fn options<'a>(this: *const Self, default: &'static CStr) -> &'a CStr {
        let _guard = trace_guard!("vchordrq::Reloption::options [rust]");
        unsafe {
            if this.is_null() {
                return default;
            }
...
pub fn init() {
    let _guard = trace_guard!("vchordrq::init [rust]");
...
pub unsafe extern "C-unwind" fn amoptions(
    ...
    let _guard = trace_guard!("vchordrq::amoptions [rust]");
...
pub unsafe extern "C-unwind" fn amcostestimate(
    ...
    let _guard = trace_guard!("vchordrq::amcostestimate [rust]");
    unsafe {
        use pgrx::pg_sys::disable_cost;
        let index_opt_info = (*path).indexinfo;
        trace_vchord_print!("provide cost estimates");
        ...
        {
            trace_vchord_print!("do not use index, if there are no orderbys or clauses");
        ...
        trace_vchord_print!("selectivity: {}", selectivity);
        // index exists
        if !(*index_opt_info).hypothetical {
            trace_vchord_print!("index exists");
            ...
            trace_vchord_print!("node count: {}", node_count);
            let page_count = {
                let mut pages = 0_f64;
                pages += 1.0;
            ...
            trace_vchord_print!("page count: {}", page_count);
            let next_count =
            ...
            *index_pages = page_count;
            trace_vchord_print!(
                "index_startup_cost: {}, index_total_cost: {}",
                *index_startup_cost,
                *index_total_cost
            );
            return;
...
pub unsafe extern "C-unwind" fn aminsert(
    ...
    let _guard = trace_guard!("vchordrq::aminsert [rust]");
...
unsafe fn aminsertinner(
    ...
    let _guard = trace_guard!("vchordrq::aminsertinner [rust]");
...
pub unsafe extern "C-unwind" fn ambulkdelete(
    ...
    let _guard = trace_guard!("vchordrq::ambulkdelete [rust]");
...
pub unsafe extern "C-unwind" fn ambeginscan(
    ...
    let _guard = trace_guard!("vchordrq::ambeginscan [rust]");
...
pub unsafe extern "C-unwind" fn amrescan(
    ...
    let _guard = trace_guard!("vchordrq::amrescan [rust]");
    unsafe {
        ...
        trace_vchord_print!(
            "epsilon:{}, probes:{}, maxsim_refine:{}, maxsim_threshold:{}",
            options.epsilon,
            options.probes.len(),
            options.maxsim_refine,
            options.maxsim_threshold
        );
...
pub unsafe extern "C-unwind" fn amgettuple(
    ...
    let _guard = trace_guard!("vchordrq::amgettuple [rust]");
    if direction != pgrx::pg_sys::ScanDirection::ForwardScanDirection {
        trace_vchord_instant_print!(
            "vector search without a forward scan direction is not supported"
        );
    ...
    {
        trace_vchord_instant_print!("scanning with a non-MVCC-compliant snapshot is not supported");
...
pub unsafe extern "C-unwind" fn amendscan(scan: pgrx::pg_sys::IndexScanDesc) {
    let _guard = trace_guard!("vchordrq::amendscan [rust]");
...
impl Index {
    fn open(indexrelid: pgrx::pg_sys::Oid, lockmode: pgrx::pg_sys::LOCKMASK) -> Self {
        let _guard = trace_guard!("vchordrq::open [rust]");
        Self {
...
impl Drop for Index {
    fn drop(&mut self) {
        let _guard = trace_guard!("vchordrq::Drop(Index)::drop [rust]");
        unsafe {
