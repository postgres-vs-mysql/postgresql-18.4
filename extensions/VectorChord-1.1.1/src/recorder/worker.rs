...
fn get<'a>() -> Option<RefMut<'a, rusqlite::Connection>> {
    let _guard = trace_guard!("worker::get [rust]");
...
pub fn push(index: u32, sample: &str, max_records: u32) {
    let _guard = trace_guard!("worker::push [rust]");
...
pub fn delete_index(index: u32) {
    let _guard = trace_guard!("worker::delete_index [rust]");
...
pub fn delete_database(database_oid: u32) {
    let _guard = trace_guard!("worker::delete_database [rust]");
