                            ...
                            bool inhparent, RelOptInfo *rel)
{
  ...
  if (IsColumnarTableAmTable(relationObjectId)) {
    ...
    DBUG_PRINT("citus", "disable parallel query and index-only scan");
                      ...
                      IndexPath *indexPath)
{
  ...
  DBUG_PRINT("citus", "columnar table index scan costs estimated by indexAM: startup cost = %.2f, total cost = %.2f",
  ...
  ereport(DEBUG4, (errmsg("columnar table index scan costs re-estimated "
                          ...
                          indexPath->path.total_cost)));
  DBUG_PRINT("citus", "columnar table index scan costs re-estimated by columnarAM (including indexAM costs):");
  DBUG_PRINT("citus", "startup cost = %.2f, total cost = %.2f", indexPath->path.startup_cost, indexPath->path.total_cost);
}
...
CostColumnarSeqPath(RelOptInfo *rel, Oid relationId, Path *path)
{
  ...
  DBUG_PRINT("citus", "startup_cost:%.2f, total_cost:%.2f", path->startup_cost, path->total_cost);
}
