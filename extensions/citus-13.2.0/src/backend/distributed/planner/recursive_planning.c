                                     ...
                                     RouterPlanType routerPlan)
{
  ...
  DBUG_PRINT("citus", "plan subqueries and CTEs that cannot be pushed down");
  /*
  ...
  if (context.subPlanList && IsLoggableLevel(DEBUG1)) {
    ...
    DBUG_PRINT("citus", "Plan %ld query after replacing subqueries and CTEs: %s", planId, subPlanString->data);
...
RecursivelyPlanSubqueriesAndCTEs(Query *query, RecursivePlanningContext *context)
{
  ...
  DBUG_PRINT("citus", "descend into subqueries");
  ...
  if (query->havingQual != NULL) {
    if (NodeContainsSubqueryReferencingOuterQuery(query->havingQual)) {
      DBUG_PRINT("citus", "subqueries in HAVING cannot refer to outer query");
  ...
  if (ShouldRecursivelyPlanNonColocatedSubqueries(query, context)) {
    DBUG_PRINT("citus", "when the query doesn't have distribution key equality, recursively plan some of its subqueries");
  ...
  if (ShouldRecursivelyPlanSublinks(query)) {
    ...
    if (query->jointree && query->jointree->quals) {
      DBUG_PRINT("citus", "replace all subqueries in the WHERE clause");
      RecursivelyPlanAllSubqueries((Node *) query->jointree->quals, context);
    }

    /* replace all subqueries in the SELECT clause */
    DBUG_PRINT("citus", "replace all subqueries in the SELECT clause");
    ...
    RecursivePlanningContext *context)
{
  ...
  if (context->allDistributionKeysInQueryAreEqual) {
    DBUG_PRINT("citus", "when the input query already contains the equality, simply return");
    DBUG_PRINT("citus", "since it is not possible to find any non colocated subqueries");
    return false;
  }
  ...
  if (!ContainsSubquery(subquery)) {
    DBUG_PRINT("citus", "it does not include subqueries and return false");
    return false;
  }
  ...
  if (FindNodeMatchingCheckFunctionInRangeTableList(subquery->rtable,
    ...
    DBUG_PRINT("citus", "direct joins with local tables are not supported by any of Citus planners");
    return false;
  }
   ...
   */
  DBUG_PRINT("citus", "finally, check whether this subquery contains distribution key equality or not");
...
{
  DBUG_TRACE;
  bool result = JoinTreeContainsSubquery(query);

  if (!result) {
    result = WhereOrHavingClauseContainsSubquery(query);

    if (result) {
      DBUG_PRINT("citus", "WhereOrHavingClauseContainsSubquery returns true");
    }
  } else {
    DBUG_PRINT("citus", "JoinTreeContainsSubquery returns true");
  }

  if (result) {
    DBUG_PRINT("citus", "the input query contains some subqueries in the FROM or WHERE clauses");
  } else {
    DBUG_PRINT("citus", "the input query contains no subqueries in the FROM or WHERE clauses.");
  }
                                      ...
                                      RecursivePlanningContext *recursivePlanningContext)
{
  ...
  if (joinNode == NULL) {
    ...
    DBUG_PRINT("citus", "recurse into the left subtree");
    RecursivelyPlanNonColocatedJoinWalker(joinExpr->larg, colocatedJoinChecker,
                                          recursivePlanningContext);

    /* recurse into the right subtree */
    DBUG_PRINT("citus", "recurse into the right subtree");
    ...
    if (rte->rtekind != RTE_SUBQUERY) {
      DBUG_PRINT("citus", "we're only interested in subqueries for now");
      return;
    }
    ...
    if (!SubqueryColocated(subquery, colocatedJoinChecker)) {
      DBUG_PRINT("citus", "when the subquery is not colocated with the anchor subquery, recursively plan it");
  ...
  if (node == NULL) {
    ...
    switch (joinExpr->jointype) {
        ...
        if (leftNodeRecurs && !rightNodeRecurs) {
          if (recursivePlanningContext->forceRecursivelyPlanRecurringOuterJoins
              ...
              chainedJoin || !CanPushdownRecurringOuterJoin(joinExpr,
                  query)) {
            DBUG_PRINT("citus", "recursively planning right side of the left join since the outer side is a recurring rel");
            ...
            DBUG_PRINT("citus", "a push down safe left join with recurring left side");
      ...
      case JOIN_RIGHT: {
        ...
        if (!leftNodeRecurs && rightNodeRecurs) {
          if (recursivePlanningContext->forceRecursivelyPlanRecurringOuterJoins
              ...
              chainedJoin || !CanPushdownRecurringOuterJoin(joinExpr,
                  query)) {
            DBUG_PRINT("citus", "recursively planning left side of the right join since the outer side is a recurring rel");
            ...
            DBUG_PRINT("citus", "a push down safe right join with recurring left side");
            ereport(DEBUG3, (errmsg(
      ...
      case JOIN_FULL: {
         ...
         */
        if (leftNodeRecurs && !rightNodeRecurs) {
          DBUG_PRINT("citus", "recursively planning right side of the full join since the outer side is a recurring rel");
          ereport(DEBUG1, (errmsg("recursively planning right side of "
          ...
          DBUG_PRINT("citus", "recursively planning left side of the full join since the outer side is a recurring rel");
          ereport(DEBUG1, (errmsg("recursively planning left side of "
                                  "the full join since the other side "
                                  "is a recurring rel")));
          RecursivelyPlanDistributedJoinNode(leftNode, query,
                                   ...
                                   RecursivePlanningContext *recursivePlanningContext)
{
  ...
  if (distributedRte->rtekind == RTE_RELATION) {
    name_str = GetRelationNameAndAliasName(distributedRte);
    DBUG_PRINT("citus", "recursively planning distributed relation %s", name_str);
    DBUG_PRINT("citus", "since it is part of a distributed join node that is outer joined with a recurring rel");
    ...
    DBUG_PRINT("citus", "recursively planning the distributed subquery ");
    DBUG_PRINT("citus", "since it is part of a distributed join node that is outer joined with a recurring rel");
...
RecursivelyPlanCTEs(Query *query, RecursivePlanningContext *planningContext)
{
  ...
  if (query->hasRecursive) {
    DBUG_PRINT("citus", "recursive CTEs are only supported when they contain a filter on the distribution column");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  foreach(cteCell, query->cteList) {
    ...
    if (ContainsReferencesToOuterQuery(subquery)) {
      DBUG_PRINT("citus", "CTEs that refer to other subqueries are not supported in multi-shard queries");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
...

    {
      StringInfo subPlanString = makeStringInfo();
      pg_get_query_def(subquery, subPlanString);
      DBUG_PRINT("citus", "generating subplan (planId:%ld) _%u for CTE %s: %s", planId, subPlanId, cteName, subPlanString->data);
    }

    /* build a sub plan for the CTE */
    DBUG_PRINT("citus", "build a sub plan for the CTE");
    ...
    if (subquery->returningList) {
      ...
      DBUG_PRINT("citus", "modifying CTE with returning (resultId:%s)", resultId);
      cteTargetList = subquery->returningList;
    } else {
      DBUG_PRINT("citus", "regular SELECT CTE (resultId:%s)", resultId);
      /* regular SELECT CTE */
...
RecursivelyPlanSubqueryWalker(Node *node, RecursivePlanningContext *context)
{
  ...
  if (IsA(node, Query)) {
    ...
    if (ShouldRecursivelyPlanSubquery(query, context)) {
      DBUG_PRINT("citus", "recursively plan this subquery if it cannot be pushed down and is eligible for recursive planning");
      RecursivelyPlanSubquery(query, context);
    }

    DBUG_PRINT("citus", "we're done, no need to recurse anymore for this query");
...
RecursivelyPlanSubquery(Query *subquery, RecursivePlanningContext *planningContext)
{
  ...
  DBUG_PRINT("citus", "recursively plan a query, replace it with a result query and returns the subplan");

  if (ContainsReferencesToOuterQuery(subquery)) {
    DBUG_PRINT("citus", "skipping recursive planning for the subquery since it contains references to outer queries");
  ...
  DBUG_PRINT("citus", "create the subplan and append it to the list in the planning context(planId:%lu, subPlanId:%d)", planId, subPlanId);
  DistributedSubPlan *subPlan = CreateDistributedSubPlan(subPlanId, subquery);
  planningContext->subPlanList = lappend(planningContext->subPlanList, subPlan);
  ...
  if (IsLoggableLevel(DEBUG1)) {
    ...
    DBUG_PRINT("citus", "generating subplan %ld _%u for subquery %s", planId, subPlanId, subqueryString->data);
    ereport(DEBUG1, (errmsg("generating subplan " UINT64_FORMAT
                            "_%u for subquery %s", planId, subPlanId,
                            subqueryString->data)));
  }

  DBUG_PRINT("citus", "finally update the input subquery to point the result query");
                                  ...
                                  RTEPermissionInfo *perminfo)
{
  ...
  {
    char *relationAndAliasName = GetRelationNameAndAliasName(rangeTableEntry);
    DBUG_PRINT("citus", "wrapping relation %s to a subquery", relationAndAliasName);
  }

  /* as we created the subquery, now forcefully recursively plan it */
  DBUG_PRINT("citus", "as we created the subquery, now forcefully recursively plan it");
...
ShouldTransformRTE(RangeTblEntry *rangeTableEntry)
{
  ...
  DBUG_PRINT("citus", "the given RTE should be wrapped in a subquery");
  return true;
}
                                  ...
                                  bool useBinaryCopyFormat)
{
  ...
  DBUG_PRINT("citus", "build the target list and column definition list");
  ...
  DBUG_PRINT("citus", "build the call to read_intermediate_result");
  ...
  DBUG_PRINT("citus", "build the RTE for the call to read_intermediate_result");
  ...
  DBUG_PRINT("citus", "build the join tree using the read_intermediate_result RTE");
  RangeTblRef *rangeTableRef = makeNode(RangeTblRef);
  rangeTableRef->rtindex = 1;
  ...
  /* build the SELECT query */
  DBUG_PRINT("citus", "build the SELECT query");
  Query *resultQuery = makeNode(Query);
  resultQuery->commandType = CMD_SELECT;
...
CanPushdownRecurringOuterJoinOnOuterRTE(RangeTblEntry *rte)
{
  ...
  if (IsCitusTable(rte->relid) && IsCitusTableType(rte->relid, REFERENCE_TABLE)) {
    ...
    DBUG_PRINT("citus", "RTE type %d is not safe for pushdown", rte->rtekind);
                                      ...
                                      RangeTblEntry **distRte, int *attnum)
{
  ...
  if (IsA(joinExpr->rarg, JoinExpr) || IsA(joinExpr->larg, JoinExpr)) {
    DBUG_PRINT("citus", "one side is a join expression, pushdown is not supported in this path");
  ...
  if (!IsA(joinExpr->larg, RangeTblRef) || !IsA(joinExpr->rarg, RangeTblRef)) {
    DBUG_PRINT("citus", "one side is not a RangeTblRef, pushdown is not supported in this path");
    ereport(DEBUG5, (errmsg(
  ...
  *outerRte = rt_fetch(*outerRtIndex, query->rtable);
  ...
  if (JoinTreeContainsLateral(joinExpr->rarg, query->rtable) || JoinTreeContainsLateral(
    ...
    DBUG_PRINT("citus", "lateral join is not supported for pushdown in this path");
    ereport(DEBUG5, (errmsg(
