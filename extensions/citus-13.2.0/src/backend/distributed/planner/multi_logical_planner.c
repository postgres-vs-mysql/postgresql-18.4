...
MultiNodeTree(Query *queryTree)
{
  ...
  if (subqueryEntryList != NIL) {
    DBUG_PRINT("citus", "when we have a subquery, build a multi table node for the subquery");
    DBUG_PRINT("citus", "and add a collect node on top of the multi table node");
    ...
    DBUG_PRINT("citus", "we calculate the join order using the list of tables(len:%d) in the query", list_length(tableEntryList));
    DBUG_PRINT("citus", "and the join clauses between them");
    ...
    DBUG_PRINT("citus", "find best join order for commutative inner joins");
...
IsCitusExtraDataContainerRelation(RangeTblEntry *rte)
{
  ...
  bool result = FindNodeMatchingCheckFunction((Node *) rte->functions,
...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "determine whether a given node is a function call to the read_intermediate_result function");
...
{
  bool result = IsA(node, GroupingFunc);
  return result;
}
...
IsJoinClause(Node *clause)
{
  ...
  if (list_length(varList) <= 0) {
    ...
    DBUG_PRINT("citus", "no column references in query, not describing a join");
    return false;
  }
  ...
  foreach_declared_ptr(var, varList) {
    if (var->varno != initialVar->varno) {
       ...
       * join
       */
      DBUG_PRINT("citus", "this column reference comes from a different relation, hence describing a join");
      return true;
    }
  }

  DBUG_PRINT("citus", "all column references were to the same relation, no join");
