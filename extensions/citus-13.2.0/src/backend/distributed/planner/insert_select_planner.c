...
InsertSelectIntoCitusTable(Query *query)
{
  ...
  if (insertSelectQuery) {
    ...
    if (IsCitusTable(insertRte->relid)) {
      DBUG_PRINT("citus", "return true when the input query is an INSERT INTO ... SELECT kind of query and the target is a citus table");
      return true;
    }
  }
                               ...
                               ParamListInfo boundParams)
{
  ...
  if (distributedPlan->planningError != NULL) {
     ...
     * repartitioning.
     */
    DBUG_PRINT("citus", "if INSERT..SELECT cannot be distributed, pull to coordinator or use repartitioning");
                                  ...
                                  PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  DBUG_PRINT("citus", "create a DistributedPlan for distributed INSERT ... SELECT queries");
  ...
  DBUG_PRINT("citus", "plan select query for each shard in the target table(shard count:%d)", shardCount);
  ...
  for (int shardOffset = 0; shardOffset < shardCount; shardOffset++) {
    ...
    if (modifyTask != NULL) {
      DBUG_PRINT("citus", "add the task when it could be created");
  ...
  DBUG_PRINT("citus", "create the worker job");
  ...
  DBUG_PRINT("citus", "finally the multi plan");
                                     ...
                                     PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  DBUG_PRINT("citus", "create the plan for INSERT .. SELECT queries where the selected table is distributed and the inserted table is not");
                                 ...
                                 PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  if (!NeedsDistributedPlanning(subquery)) {
    DBUG_PRINT("citus", "distributed INSERT ... SELECT can only select from distributed tables");
  ...
  if (subqueryRteListProperties->hasDistributedTable &&
    ...
    DBUG_PRINT("citus", "distributed INSERT ... SELECT cannot select from distributed tables and local tables at the same time");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (subqueryRteListProperties->hasDistributedTable &&
    ...
    DBUG_PRINT("citus", "distributed INSERT ... SELECT cannot insert into a local table that is added to metadata");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (subqueryRteListProperties->hasPostgresLocalTable) {
    DBUG_PRINT("citus", "distributed INSERT ... SELECT cannot select from a local table");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "distributed INSERT ... SELECT cannot select from "
  ...
  foreach(rangeTableCell, queryTree->rtable) {
    ...
    if (rangeTableEntry->rtekind == RTE_RELATION &&
      ...
      DBUG_PRINT("citus", "cannot insert into view over distributed table");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (FindNodeMatchingCheckFunction((Node *) queryTree, CitusIsVolatileFunction)) {
    DBUG_PRINT("citus", "volatile functions are not allowed in distributed INSERT ... SELECT queries");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (IsCitusTableType(targetRelationId, CITUS_LOCAL_TABLE)) {
    ...
    if (!allReferenceTables) {
      DBUG_PRINT("citus", "only reference tables may be queried when targeting a reference table with distributed INSERT ... SELECT");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
    ...
    if (subqueryRteListProperties->hasCitusLocalTable ||
      ...
      DBUG_PRINT("citus", "distributed INSERT ... SELECT cannot select from a local relation when inserting into a distributed table");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                           "distributed INSERT ... SELECT cannot select from "
  ...
  if (!AllDistributedRelationsInListColocated(distributedRelationIdList)) {
    DBUG_PRINT("citus", "INSERT target relation and all source relations of the SELECT must be colocated in distributed INSERT ... SELECT");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                                 ...
                                 DeferredErrorMessage **routerPlannerError)
{
   ...
   */
  DBUG_PRINT("citus", "use router planner to decide on whether we can push down the query or not");
  ...
  if (list_length(selectPlacementList) == 0) {
    DBUG_PRINT("citus", "skipping target shard interval %ld since SELECT query for it pruned away", shardId);
                                   ...
                                   Oid *selectPartitionColumnTableId)
{
  ...
  foreach(targetEntryCell, query->targetList) {
    ...
    if (subqueryPartitionColumnRelationId == InvalidOid) {
      ...
      DBUG_PRINT("citus", "cannot perform distributed INSERT INTO ... SELECT because");
      DBUG_PRINT("citus", "the partition columns in the source table and subquery do not match");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
...
NonPushableInsertSelectSupported(Query *insertSelectQuery)
{
  ...
  if (IsCitusTableType(insertRte->relid, APPEND_DISTRIBUTED)) {
    DBUG_PRINT("citus", "INSERT ... SELECT into an append-distributed table is not supported");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
