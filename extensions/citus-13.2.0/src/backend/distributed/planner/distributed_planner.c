                    ...
                    ParamListInfo boundParams)
{
  ...
  DBUG_PRINT("citus", "distributed planner hook");
  ...
  if (cursorOptions & CURSOR_OPT_FORCE_DISTRIBUTED) {
    ...
    if (needsDistributedPlanning) {
      DBUG_PRINT("citus", "it needs distributed planning");
  ...
  if (needsDistributedPlanning) {
    ...
    if (!fastPathRouterQuery) {
      ...
      DBUG_PRINT("citus", "when there are partitioned tables (not applicable to fast path), pretend that");
      DBUG_PRINT("citus", "they are regular tables to avoid unnecessary work in standard_planner");
   ...
   */
  PlannerLevel++;
  DBUG_PRINT("citus", "we keep track of how many times we've recursed into the planner:%d", PlannerLevel);
  ...
  PG_TRY();
  {
    if (fastPathRouterQuery) {
       ...
       * postgres' planner.
       */
      DBUG_PRINT("citus", "call into standard_planner because the Citus planner relies on both the restriction information");
      DBUG_PRINT("citus", "per table and parse tree transformations made by postgres' planner");
...
NeedsDistributedPlanning(Query *query)
{
  ...
  if (!CitusHasBeenLoaded()) {
    DBUG_PRINT("citus", "return false when the Citus extension is not loaded");
    return false;
  }
  ...
  if (commandType != CMD_SELECT && commandType != CMD_INSERT &&
    ...
    DBUG_PRINT("citus", "return false when command type is not expected");
    return false;
  }

  List *allRTEs = ExtractRangeTableEntryList(query);

  result = ListContainsDistributedTableRTE(allRTEs, NULL);

  if (result) {
    DBUG_PRINT("citus", "return true when Citus extension is loaded and the query contains a distributed table");
  } else {
                                ...
                                bool *maybeHasForeignDistributedTable)
{
  ...
  foreach(rangeTableCell, rangeTableList) {
    ...
    if (IsCitusTable(rangeTableEntry->relid)) {
      ...
      DBUG_PRINT("citus", "return true when there is at least one distributed relation range table entry in the list");
      return true;
    }
  }
...
AssignRTEIdentities(List *rangeTableList, int rteIdCounter)
{
  ...
  DBUG_PRINT("citus", "modify query tree by adding RTE identities to the RTE_RELATIONs");
    ...
    bool setPartitionedTablesInherited)
{
  ...
  DBUG_PRINT("citus", "modify query tree by changing inh flag and relkind of partitioned tables");
...
GetQueryLockMode(Query *query)
{
  ...
  if (IsModifyCommand(query)) {
    DBUG_PRINT("citus", "return RowExclusiveLock to be acquired for the given query");
    return RowExclusiveLock;
  } else if (query->hasForUpdate) {
    DBUG_PRINT("citus", "return RowShareLock to be acquired for the given query");
    return RowShareLock;
  } else {
    DBUG_PRINT("citus", "return AccessShareLock to be acquired for the given query");
...
IsModifyCommand(Query *query)
{
  ...
  if (commandType == CMD_INSERT || commandType == CMD_UPDATE ||
    ...
    DBUG_PRINT("citus", "return true when the query performs modifications");
    return true;
  }

  DBUG_PRINT("citus", "return false when the query does not perform modifications");
  return false;
}
...
IsMultiTaskPlan(DistributedPlan *distributedPlan)
{
  ...
  if (workerJob != NULL && list_length(workerJob->taskList) > 1) {
    DBUG_PRINT("citus", "return true when job contains multiple tasks");
    return true;
  }

  DBUG_PRINT("citus", "return false when job does not contain multiple tasks");
  return false;
}
...
PlanFastPathDistributedStmt(DistributedPlanningContext *planContext)
{
  ...
  DBUG_PRINT("citus", "create a distributed planned statement using the FastPathPlanner");
                    ...
                    int rteIdCounter)
{
  ...
  DBUG_PRINT("citus", "create a distributed planned statement using the PG planner");
...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "try dissuade planner when planning a plan that potentially failed due to unresolved prepared statement parameters");
  /*
...
CreateDistributedPlannedStmt(DistributedPlanningContext *planContext)
{
  ...
  DBUG_PRINT("citus", "encapsulate the logic needed to transform a particular query into a distributed plan(planId:%lu)", planId);
  ...
  if (!distributedPlan) {
    ...
    DBUG_PRINT("citus", "could not create distributed plan");
    DBUG_PRINT("citus", "possibly this is caused by the use of parameters in SQL functions, which is not supported in citus");
  ...
  DBUG_PRINT("citus", "remember the plan's identifier(planId:%lu) for identifying subplans", planId);
  distributedPlan->planId = planId;

  DBUG_PRINT("citus", "create final plan by combining local plan with distributed plan");
    ...
    DistributedPlanningContext *planContext)
{
  ...
  DBUG_PRINT("citus", "we'll inline the CTEs and try distributed planning, preserve the original");
  DBUG_PRINT("citus", "query in case the planning fails and we fallback to recursive planning of CTEs");
...
GetRouterPlanType(Query *query, Query *originalQuery, bool hasUnresolvedParams)
{
  ...
  if (!IsModifyCommand(originalQuery)) {
    DBUG_PRINT("citus", "return SELECT_QUERY");
    return SELECT_QUERY;
  }
  ...
  if (InsertSelectIntoCitusTable(originalQuery)) {
    if (hasUnresolvedParams) {
      DBUG_PRINT("citus", "return REPLAN_WITH_BOUND_PARAMETERS");
      return REPLAN_WITH_BOUND_PARAMETERS;
    }

    DBUG_PRINT("citus", "return INSERT_SELECT_INTO_CITUS_TABLE");
    ...
    DBUG_PRINT("citus", "return INSERT_SELECT_INTO_LOCAL_TABLE");
    ...
    DBUG_PRINT("citus", "return MERGE_QUERY");
    return MERGE_QUERY;
  } else {
    DBUG_PRINT("citus", "return DML_QUERY");
    return DML_QUERY;
  }
}
                      ...
                      PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  DBUG_PRINT("citus", "generate a distributed plan(planId:%lu) for a query", planId);
  /* Step 1: Try router planner */

  DBUG_PRINT("citus", "try router planner");
  routerPlan = GetRouterPlanType(query, originalQuery,
                                 hasUnresolvedParams);

  switch (routerPlan) {
    case INSERT_SELECT_INTO_CITUS_TABLE: {
      DBUG_PRINT("citus", "insert select into citus table");
      distributedPlan =
    ...
    case INSERT_SELECT_INTO_LOCAL_TABLE: {
      DBUG_PRINT("citus", "insert select into local table");
      distributedPlan =
    ...
    case DML_QUERY: {
      DBUG_PRINT("citus", "modifications are always routed through the same planner/executor");
    ...
    case REPLAN_WITH_BOUND_PARAMETERS: {
      DBUG_PRINT("citus", "replan with bound parameters");
      /*
    ...
    case SELECT_QUERY: {
       ...
       */
      DBUG_PRINT("citus", "for select queries we, if router executor is enabled, first try to plan the query as a router query");
      distributedPlan =
  ...
  if (distributedPlan->planningError == NULL) {
    ...
    DBUG_PRINT("citus", "planning error");
  ...
  if (hasUnresolvedParams) {
     ...
     * bound parameters.
     */
    DBUG_PRINT("citus", "there are parameters that don't have a value in boundParams");
    return NULL;
  }
  ...
  DBUG_PRINT("citus", "generate subplans for CTEs and complex subqueries");
  DBUG_PRINT("citus", "that cannot be pushed down by recursively calling the planner and");
  DBUG_PRINT("citus", "return the resulting plans to subPlanList");
  ...
  if (list_length(subPlanList) > 0 || hasCtes) {
    DBUG_PRINT("citus", "when subqueries were recursively planned then we need to replan the query");
    DBUG_PRINT("citus", "to get the new planner restriction context and apply planner transformations");
     ...
     * to do it again here.
     */
    DBUG_PRINT("citus", "we force standard_planner to treat partitioned tables as regular tables");
     ...
     * being contiguous.
     */
    DBUG_PRINT("citus", "call standard_planner to get the transformed query");
    ...
    *query = *newQuery;
    ...
    DBUG_PRINT("citus", "recurse into CreateDistributedPlan with subqueries/CTEs replaced");
   ...
   */
  if (IsModifyCommand(originalQuery)) {
    DBUG_PRINT("citus", "DML command returns a planning error, even after recursive planning");
    return distributedPlan;
  }
  ...
  DBUG_PRINT("citus", "try logical planner");
  ...
  DBUG_PRINT("citus", "create the physical planner");
...
DeferErrorIfPartitionTableNotSingleReplicated(Oid relationId)
{
  ...
  if (PartitionTableNoLock(relationId) && !SingleReplicatedTable(relationId)) {
    ...
    DBUG_PRINT("citus", "modifications on partitions when replication factor is greater than 1 is not supported");
...
GetDistributedPlan(CustomScan *customScan)
{
  ...
  DBUG_PRINT("citus", "return the associated DistributedPlan for a CustomScan");
...
FinalizePlan(PlannedStmt *localPlan, DistributedPlan *distributedPlan)
{
  ...
  DBUG_PRINT("citus", "combine local plan with distributed plan and create a plan which can be run by the PostgreSQL executor");
  ...
  DBUG_PRINT("citus", "necessary to avoid extra Result node in PG15");
...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "it is a hook called by standard planner to notify us about various planning info regarding joins");
  DBUG_PRINT("citus", "we use it to learn about the joining column");
                                ...
                                Index restrictionIndex, RangeTblEntry *rte)
{
  ...
  DBUG_PRINT("citus", "it is a hook called by standard planner to notify us about various planning info regarding a relation");
  DBUG_PRINT("citus", "we use it to retrieve restrictions on relations");
  ...
  if (ReplaceCitusExtraDataContainer && IsCitusExtraDataContainerRelation(rte)) {
    ...
    DBUG_PRINT("citus", "replace all paths with our custom scan and recalculate cheapest");
    relOptInfo->pathlist = list_make1(path);
    set_cheapest(root, relOptInfo);
...
FindTargetAppendRelInfo(PlannerInfo *root, int relationRteIndex)
{
  ...
  DBUG_PRINT("citus", "find the target append rel info for the given relation rte index");
...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "adjust the row count and total cost of a read_intermediate_result call based on the file size");

  if (rangeTableEntry->rtekind != RTE_FUNCTION ||
      list_length(rangeTableEntry->functions) != 1) {
    DBUG_PRINT("citus", "avoid more expensive checks below for non-functions");
    /* avoid more expensive checks below for non-functions */
    return;
  }

  if (!CitusHasBeenLoaded() || !CheckCitusVersion(DEBUG5)) {
    DBUG_PRINT("citus", "read_intermediate_result may not exist");
  ...
  if (!IsA(resultIdConst, Const)) {
    DBUG_PRINT("citus", "not sure how to interpret non-const");
    ...
    Const *resultFormatConst)
{
  ...
  DBUG_PRINT("citus", "adjust the row count and total cost of reading intermediate results based on file sizes");
  ...
  for (int index = 0; index < resultIdCount; index++) {
    ...
    if (resultSize < 0) {
      DBUG_PRINT("citus", "result does not exist, will probably error out later on");
  ...
  DBUG_PRINT("citus", "start with the cost of evaluating quals:%g", rowCost);

  /* postgres' estimate for the width of the rows */
  rowSizeEstimate += reltarget->width;
  DBUG_PRINT("citus", "postgres' estimate for the width of the rows:%g", rowSizeEstimate);

  /* add 2 bytes for column count (binary) or line separator (text) */
  rowSizeEstimate += 2;
  DBUG_PRINT("citus", "add 2 bytes for column count (binary) or line separator (text): %g", rowSizeEstimate);
  ...
  DBUG_PRINT("citus", "estimate the number of rows based on the file size and estimated row size:%.2f", rowCountEstimate);
  /* cost of reading the data */
  ioCost = seq_page_cost * totalResultSize / BLCKSZ;
  DBUG_PRINT("citus", "cost of reading the data:%g", ioCost);

  Assert(pathList != NIL);

  DBUG_PRINT("citus", "tell the planner about the cost and row count of the function");
  ...
  DBUG_PRINT("citus", "startup_cost:%.2f, total_cost:%.2f, rows:%.2f", path->startup_cost, path->total_cost, path->rows);
}
