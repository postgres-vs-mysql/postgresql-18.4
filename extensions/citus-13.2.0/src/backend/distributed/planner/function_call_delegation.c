...
TryToDelegateFunctionCall(DistributedPlanningContext *planContext)
{
  ...
  if (joinTree->fromlist != NIL) {
    ...
    if (IsA(reference, RangeTblRef)) {
      ...
      if (rtentry->rtekind == RTE_FUNCTION) {
        ...
        DBUG_PRINT("citus", "FromList item is not empty");
        return NULL;
      }
    } else {
      /*
  ...
  if (procedure == NULL || !procedure->isDistributed) {
    ...
    DBUG_PRINT("citus", "function is distributed");
  ...
  if (PlanningInsertSelect()) {
    DBUG_PRINT("citus", "not pushing down function calls in INSERT ... SELECT");
  ...
  if (walkerParamContext.hasParam) {
    if (walkerParamContext.paramKind == PARAM_EXTERN) {
      ...
      DBUG_PRINT("citus", "arguments in a distributed function must not contain subqueries");
  ...
  if (IsMultiStatementTransaction()) {
    if (!procedure->forceDelegation) {
      DBUG_PRINT("citus", "not pushing down function calls in a multi-statement transaction");
      ...
      if (!IsA(partitionValueNode, Const)) {
        DBUG_PRINT("citus", "distribution argument value must be a constant when using force_delegation flag");
      ...
      if ((MaybeExecutingUDF() || DoBlockLevel > 0) &&
          ...
          !fromFuncExpr) {
        DBUG_PRINT("citus", "Skipping pushdown of function from a PL/PgSQL simple expression");
       ...
       */
      DBUG_PRINT("citus", "pushing down function call in a multi-statement transaction");
  ...
  if (contain_volatile_functions((Node *) funcExpr->args)) {
    DBUG_PRINT("citus", "arguments in a distributed function must be constant expressions");
    ereport(DEBUG1, (errmsg("arguments in a distributed function must "
  ...
  if (colocatedRelationId == InvalidOid) {
    DBUG_PRINT("citus", "function does not have co-located tables");
  ...
  if (GeneratingSubplans()) {
    DBUG_PRINT("citus", "not pushing down function calls in CTEs or Subqueries");
  ...
  if (workerNode == NULL || !workerNode->hasMetadata || !workerNode->metadataSynced) {
    DBUG_PRINT("citus", "the worker node does not have metadata");
     ...
     *      on the node itself
     */
    DBUG_PRINT("citus", "not pushing down function to the same node");
    ereport(DEBUG1, (errmsg("not pushing down function to the same node")));
    return NULL;
  }

  DBUG_PRINT("citus", "pushing down the function call");
    ...
    PlannedStmt *plan)
{
  ...
  if (procedure->distributionArgIndex < 0 ||
    ...
    DBUG_PRINT("citus", "cannot push down invalid distribution_argument_index");
  ...
  if (!IsA(partitionValueNode, Const)) {
    DBUG_PRINT("citus", "distribution argument value must be a constant");
  ...
  if (shardInterval == NULL) {
    DBUG_PRINT("citus", "cannot push down call, failed to find shard interval");
  ...
  if (list_length(placementList) != 1) {
    DBUG_PRINT("citus", "cannot push down function call for replicated distributed tables");
...
CheckDelegatedFunctionExecution(DistObjectCacheEntry *procedure, FuncExpr *funcExpr)
{
  ...
  if (IsA(partitionValueNode, Const)) {
    ...
    char *name_str = pretty_format_node_dump(nodeToString(partitionValueNode));

    DBUG_PRINT("citus", "pushdown argument: %s", name_str);
    ereport(DEBUG1, (errmsg("Pushdown argument: %s", name_str)));
...
EnableInForceDelegatedFuncExecution(Const *distArgument, uint32 colocationId)
{
  ...
  DBUG_PRINT("citus", "saving Distribution Argument: %s:%d", pretty_format_node_dump(nodeToString(distArgument)), colocationId);
...
CheckAndResetAllowedShardKeyValueIfNeeded(void)
{
  ...
  if (!AllowedDistributionColumnValue.isActive ||
    ...
    DBUG_PRINT("citus", "if no distribution argument is pinned or the pinned argument was");
    DBUG_PRINT("citus", "set by a nested-executor from upper level, nothing to reset");
    return;
  }
