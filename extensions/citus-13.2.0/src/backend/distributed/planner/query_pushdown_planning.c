                          ...
                          PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  if (JoinTreeContainsSubquery(rewrittenQuery)) {
    DBUG_PRINT("citus", "join tree contains subquery");
    return true;
  }
  ...
  if (WhereOrHavingClauseContainsSubquery(rewrittenQuery)) {
    DBUG_PRINT("citus", "the modified query contains subqueries in the WHERE and HAVING clauses");
    return true;
  }
  ...
  if (TargetListContainsSubquery(rewrittenQuery->targetList)) {
    DBUG_PRINT("citus", "the modified query contains subqueries in the SELECT clause");
    return true;
  }
  ...
  if (joinRestrictionContext->hasSemiJoin) {
    DBUG_PRINT("citus", "hasSemiJoin is true and return true");
    return true;
  }
  ...
  if (FindNodeMatchingCheckFunction((Node *) originalQuery, IsFunctionOrValuesRTE)) {
    DBUG_PRINT("citus", "we process function and VALUES RTEs as subqueries");
    return true;
  }
  ...
  if (FindNodeMatchingCheckFunction((Node *) originalQuery->jointree, IsOuterJoinExpr)) {
    DBUG_PRINT("citus", "we handle outer joins as subqueries");
    return true;
  }
  ...
  if (FindNodeMatchingCheckFunction((Node *) rewrittenQuery->jointree, IsOuterJoinExpr)) {
    DBUG_PRINT("citus", "original query may not have an outer join while rewritten query does");
    DBUG_PRINT("citus", "we should push down in this case");
    return true;
  }
  ...
  if (DeferErrorIfUnsupportedClause(qualifierList) != NULL) {
    DBUG_PRINT("citus", "some unsupported join clauses in logical planner may be supported by subquery pushdown planner");
    return true;
  }
  ...
  if (originalQuery->hasWindowFuncs &&
    ...
    DBUG_PRINT("citus", "the query has a window function and it is safe to pushdown");
    return true;
  }
...
JoinTreeContainsSubquery(Query *query)
{
  ...
  bool result = JoinTreeContainsSubqueryWalker((Node *) joinTree, query);

  if (result) {
    DBUG_PRINT("citus", "return true when the input query contains any subqueries in the join tree");
  } else {
    DBUG_PRINT("citus", "return false");
  }
...
IsFunctionOrValuesRTE(Node *node)
{
  if (IsA(node, RangeTblEntry)) {
    ...
    if (rangeTblEntry->rtekind == RTE_FUNCTION ||
      ...
      DBUG_PRINT("citus", "the given node is a function RTE");
      return true;
    }
  }
...
IsNodeSubquery(Node *node)
{
  ...
  if (IsA(node, Query) || IsA(node, SubPlan)) {
    DBUG_PRINT("citus", "the given node is a Query or SubPlan");
    return true;
  }
  ...
  bool result = ((Param *) node)->paramkind == PARAM_EXEC;

  if (result) {
    DBUG_PRINT("citus", "the given node is a Param node with paramkind PARAM_EXEC");
  }
...
IsOuterJoinExpr(Node *node)
{
  ...
  if (isOuterJoin) {
    DBUG_PRINT("citus", "the given node is an outer join expression");
  }
                                        ...
                                        bool plannerPhase)
{
  ...
  if (ContainsUnionSubquery(originalQuery)) {
    if (!SafeToPushdownUnionSubquery(originalQuery, plannerRestrictionContext)) {
      DBUG_PRINT("citus", "cannot pushdown the subquery since not all subqueries in the UNION have the partition column in the same position");
    ...
    DBUG_PRINT("citus", "%s", errorMessage->data);
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
...
DeferErrorIfFromClauseRecurs(Query *queryTree)
{
  ...
  if (recurType == RECURRING_TUPLES_REFERENCE_TABLE) {
    DBUG_PRINT("citus", "correlated subqueries are not supported when the FROM clause contains a reference table");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
    ...
    DBUG_PRINT("citus", "correlated subqueries are not supported when the FROM clause contains a set returning function");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "correlated subqueries are not supported when "
                         "the FROM clause contains a set returning function", NULL,
                         NULL);
  } else if (recurType == RECURRING_TUPLES_RESULT_FUNCTION) {
    DBUG_PRINT("citus", "correlated subqueries are not supported when the FROM clause contains a CTE or subquery");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "correlated subqueries are not supported when "
                         "the FROM clause contains a CTE or subquery", NULL, NULL);
  } else if (recurType == RECURRING_TUPLES_EMPTY_JOIN_TREE) {
    DBUG_PRINT("citus", "correlated subqueries are not supported when the FROM clause contains a subquery without FROM");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "correlated subqueries are not supported when "
                         "the FROM clause contains a subquery without FROM", NULL,
                         NULL);
  } else if (recurType == RECURRING_TUPLES_VALUES) {
    DBUG_PRINT("citus", "correlated subqueries are not supported when the FROM clause contains VALUES");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "correlated subqueries are not supported when "
                         "the FROM clause contains VALUES", NULL,
                         NULL);
  } else if (recurType == RECURRING_TUPLES_JSON_TABLE) {
    DBUG_PRINT("citus", "correlated subqueries are not supported when the FROM clause contains JSON_TABLE");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "correlated subqueries are not supported when "
...
{
  DBUG_TRACE;
  bool result = DeferErrorIfCannotPushdownSubquery(subqueryTree, outerMostQueryHasLimit) ==
                NULL;

  if (result) {
    DBUG_PRINT("citus", "we can push down the given subquery to worker nodes");
  } else {
    DBUG_PRINT("citus", "we can not push down the given subquery to worker nodes");
  }
...
DeferErrorIfCannotPushdownSubquery(Query *subqueryTree, bool outerMostQueryHasLimit)
{
  ...
  DBUG_PRINT("citus", "check if we can push down the given subquery to worker nodes");
  ...
  if (HasEmptyJoinTree(subqueryTree) &&
    ...
    DBUG_INSTANT_PRINT("citus", "subqueries without a FROM clause can only contain immutable functions");
  ...
  if (!ContainsReferencesToOuterQuery(subqueryTree)) {
    ...
    if (deferredError) {
      ...
      DBUG_PRINT("citus", "correlated subqueries are effectively functions that are repeatedly called");
      DBUG_PRINT("citus", "for the values of the vars that point to the outer query");
      DBUG_PRINT("citus", "we can liberally push down SQL features within such a function");
      DBUG_PRINT("citus", "as long as co-located join checks are applied");
    }
  }
  ...
  if (subqueryTree->limitCount && SubqueryPushdown && !outerMostQueryHasLimit) {
    DBUG_INSTANT_PRINT("citus", "Limit in subquery without limit in the outermost query is unsupported");
    preconditionsSatisfied = false;
  ...
  if (subqueryTree->hasRecursive) {
    ...
    DBUG_INSTANT_PRINT("citus", "recursive queries are currently unsupported");
  }

  if (subqueryTree->cteList) {
    preconditionsSatisfied = false;
    errorDetail = "Common Table Expressions are currently unsupported";
    DBUG_INSTANT_PRINT("citus", "Common Table Expressions are currently unsupported");
  }

  if (subqueryTree->hasForUpdate) {
    preconditionsSatisfied = false;
    errorDetail = "For Update/Share commands are currently unsupported";
    DBUG_INSTANT_PRINT("citus", "For Update/Share commands are currently unsupported");
  }
  ...
  if (subqueryTree->groupingSets) {
    ...
    errorDetail = "could not run distributed query with GROUPING SETS, CUBE, "
                  "or ROLLUP";
    DBUG_INSTANT_PRINT("citus", "could not run distributed query with GROUPING SETS, CUBE, or ROLLUP");
  }
  ...
  if (!preconditionsSatisfied) {
    DBUG_INSTANT_PRINT("citus", "cannot push down this subquery");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "cannot push down this subquery",
                         errorDetail, NULL);
  }

  DBUG_PRINT("citus", "we can push down a subquery when it follows rules");
  return NULL;
}
                                  ...
                                  char *referencedThing)
{
  ...
  /* finally check and return deferred if not satisfied */
  if (!preconditionsSatisfied) {
    DBUG_PRINT("citus", "cannot push down this subquery");
    DBUG_PRINT("citus", "%s", errorDetail);
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "cannot push down this subquery",
                         errorDetail, NULL);
  }
...
DeferErrorIfUnsupportedUnionQuery(Query *subqueryTree)
{
  ...
  foreach(setOperationStatmentCell, setOperationStatementList) {
    ...
    if (setOperation->op != SETOP_UNION) {
      DBUG_PRINT("citus", "cannot push down this subquery");
      DBUG_PRINT("citus", "intersect and Except are currently unsupported");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                           "cannot push down this subquery",
  ...
  if (recurType == RECURRING_TUPLES_REFERENCE_TABLE) {
    DBUG_PRINT("citus", "cannot push down this subquery");
    DBUG_PRINT("citus", "reference tables are not supported with union operator");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "cannot push down this subquery",
    ...
    DBUG_PRINT("citus", "cannot push down this subquery");
    DBUG_PRINT("citus", "table functions are not supported with union operator");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "cannot push down this subquery",
    ...
    DBUG_PRINT("citus", "cannot push down this subquery");
    DBUG_PRINT("citus", "subqueries without a FROM clause are not supported with union operator");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "cannot push down this subquery",
    ...
    DBUG_PRINT("citus", "cannot push down this subquery");
    DBUG_PRINT("citus", "complex subqueries and CTEs are not supported within a UNION");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "cannot push down this subquery",
    ...
    DBUG_PRINT("citus", "cannot push down this subquery");
    DBUG_PRINT("citus", "VALUES is not supported within a UNION");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "cannot push down this subquery",
    ...
    DBUG_PRINT("citus", "cannot push down this subquery");
    DBUG_PRINT("citus", "JSON_TABLE is not supported within a UNION");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                         "cannot push down this subquery",
