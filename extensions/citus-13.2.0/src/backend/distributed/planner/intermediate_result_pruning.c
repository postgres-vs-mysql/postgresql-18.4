                               ...
                               DistributedPlan *distributedPlan)
{
  ...
  foreach(subPlanCell, usedSubPlanNodeList) {
    ...
    if (list_length(entry->nodeIdList) == nodeCount && entry->writeLocalFile) {
      DBUG_PRINT("citus", "subplan %s is used in all workers", resultId);
    ...
    if (usedPlan->accessType == SUBPLAN_ACCESS_LOCAL) {
      ...
      DBUG_PRINT("citus", "subplan %s is used in %lu", resultId, distributedPlan->planId);
                                      ...
                                      List *workerNodeList)
{
  ...
  {
    if (entry->writeLocalFile) {
      DBUG_PRINT("citus", "subplan %s will be written to local file", resultId);
    }

    WorkerNode *workerNode = NULL;
    foreach_declared_ptr(workerNode, workerNodeList) {
      DBUG_PRINT("citus", "subplan %s will be sent to %s:%d",
                 resultId, workerNode->workerName, workerNode->workerPort);
    }
  }
