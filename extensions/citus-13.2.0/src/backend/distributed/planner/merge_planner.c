                      ...
                      PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  DBUG_PRINT("citus", "creating MERGE router plan");
                           ...
                           ParamListInfo boundParams)
{
  ...
  DBUG_PRINT("citus", "creating MERGE repartition plan");
    ...
    List *targetList, CmdType commandType)
{
  ...
  foreach(targetEntryCell, targetList) {
    ...
    if (targetEntryDistributionColumn &&
      ...
      DBUG_PRINT("citus", "updating the distribution column is not allowed in MERGE actions");
    ...
    if (FindNodeMatchingCheckFunction((Node *) targetEntry->expr,
      ...
      DBUG_PRINT("citus", "functions used in MERGE actions on distributed tables must not be VOLATILE");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (hasVarArgument) {
    DBUG_PRINT("citus", "STABLE functions used in MERGE queries cannot be called with column references");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (quals != NULL && nodeTag(quals) == T_CurrentOfExpr) {
    DBUG_PRINT("citus", "cannot run MERGE actions with cursors");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                                 ...
                                 PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  foreach(restrictionCell,
    ...
    if (ContainsFalseClause(restrictClauseList) ||
      ...
      DBUG_PRINT("citus", "routing query is not possible with no shards for target");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                                      ...
                                      Oid targetRelationId)
{
  ...
  if (list_length(distTablesList) > 0 && list_length(refTablesList) > 0) {
    DBUG_PRINT("citus", "a mix of distributed and reference table, try repartitioning");
  ...
  if (list_length(distTablesList) > 0 && list_length(localTablesList) > 0) {
    DBUG_PRINT("citus", "a mix of distributed and local table, try repartitioning");
    ereport(DEBUG1, (errmsg(
  ...
  if (list_length(distTablesList) == 1) {
    DBUG_PRINT("citus", "there is only one distributed table, merge is not pushable, try repartitioning");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (!AllDistributedRelationsInRTEListColocated(distTablesList)) {
    DBUG_PRINT("citus", "distributed tables are not co-located, try repartitioning");
  ...
  if (!MergeSourceHasRouterSelect(query, plannerRestrictionContext)) {
    ...
    if (deferredError) {
      DBUG_PRINT("citus", "sub-query is not pushable, try repartitioning");
    ...
    if (HasDangerousJoinUsing(query->rtable, (Node *) query->jointree)) {
      DBUG_PRINT("citus", "query has ambiguous joins, merge is not pushable, try repartitioning");
      ereport(DEBUG1, (errmsg(
  ...
  if (deferredError) {
    DBUG_PRINT("citus", "target relation has a filter of the form: false (AND ..), which results in empty shards");
    DBUG_PRINT("citus", ", but we still need to evaluate NOT-MATCHED clause, try repartitioning");
  ...
  if (insertVar &&
    ...
    DBUG_PRINT("citus", "MERGE INSERT must use the source table distribution column value for push down to workers");
    DBUG_PRINT("citus", "otherwise, repartitioning will be applied");
    ereport(DEBUG1, (errmsg(
                                 ...
                                 CitusTableCacheEntry *targetRelation)
{
  ...
  if (sourceResultRepartitionColumnIndex == -1) {
    ...
    DBUG_PRINT("citus", "using column - index:%d from the source list to redistribute", sourceResultRepartitionColumnIndex);
