                            ...
                            PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  if (!EquivalenceListContainsRelationsEquality(allAttributeEquivalenceList,
    ...
    DBUG_PRINT("citus", "cannot confirm equality for all distribution colums");
    return false;
  }
  ...
  if (!AllDistributedRelationsInRestrictionContextColocated(restrictionContext)) {
    ...
    DBUG_PRINT("citus", "distribution columns are equal, but tables are not co-located");
    return false;
  }
  ...
  if (!PartitionColumnSelectedForOuterJoin(originalQuery,
      restrictionContext,
      joinRestrictionContext)) {
    DBUG_PRINT("citus", "outer join does not select partition column of outer relation");
    /* outer join does not select partition column of outer relation */
    return false;
  }

  DBUG_PRINT("citus", "return true if all the relations are returns partition keys");
  DBUG_PRINT("citus", "in the same ordinal position and there is no reference table exists");
  return true;
}
                    ...
                    tableType)
{
  ...
  uint32 result = list_length(rteIdentityList);
  DBUG_PRINT("citus", "the unique relation count:%u", result);
  return result;
}
...
IsRelOptOuterJoin(PlannerInfo *root, int varNo)
{
  ...
  if (root->simple_rel_array_size <= varNo) {
    DBUG_PRINT("citus", "it is an outer join");
    return true;
  }
  ...
  if (rel == NULL) {
    DBUG_PRINT("citus", "must be an outer join");
