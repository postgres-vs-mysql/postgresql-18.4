                 ...
                 PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  DBUG_PRINT("citus", "creating router plan");
                                ...
                                CmdType commandType, List *returningList)
{
  ...
  foreach(targetEntryCell, targetList) {
    ...
    if (commandType == CMD_UPDATE &&
      ...
      DBUG_PRINT("citus", "functions used in UPDATE queries on distributed tables must not be VOLATILE");
    ...
    if (commandType == CMD_UPDATE && targetEntryPartitionColumn &&
      ...
      DBUG_PRINT("citus", "modifying the partition value of rows is not allowed");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
    ...
    if (FindNodeMatchingCheckFunction((Node *) targetEntry->expr,
      ...
      DBUG_PRINT("citus", "inserting or modifying composite type fields is not supported");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (joinTree != NULL) {
    if (FindNodeMatchingCheckFunction((Node *) quals,
                                      CitusIsVolatileFunction)) {
      DBUG_PRINT("citus", "functions used in the WHERE/ON/WHEN clause of modification queries on distributed tables must not be VOLATILE");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (hasVarArgument) {
    DBUG_PRINT("citus", "STABLE functions used in UPDATE queries cannot be called with column references");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (hasBadCoalesce) {
    DBUG_PRINT("citus", "non-IMMUTABLE functions are not allowed in CASE or COALESCE statements");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (contain_mutable_functions((Node *) returningList)) {
    DBUG_PRINT("citus", "non-IMMUTABLE functions are not allowed in the RETURNING clause");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (quals != NULL &&
    ...
    DBUG_PRINT("citus", "cannot run DML queries with cursors");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                            ...
                            Oid *distributedTableIdOutput)
{
  ...
  if (queryTree->hasSubLinks == true) {
    ...
    if (!UpdateOrDeleteOrMergeQuery(queryTree)) {
      ...
      DBUG_PRINT("citus", "subqueries are not supported within INSERT queries");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (queryTree->cteList != NIL) {
    ...
    if (queryTree->commandType == CMD_INSERT) {
      DBUG_PRINT("citus", "router planner doesn't support common table expressions with INSERT queries");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
    ...
    foreach(cteCell, queryTree->cteList) {
      ...
      if (cteQuery->commandType != CMD_SELECT) {
        ...
        if (multiShardQuery) {
          DBUG_PRINT("citus", "router planner doesn't support non-select common table expressions with multi shard queries");
          return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
        ...
        /* Modifying CTEs exclude both INSERT CTEs & INSERT queries. */
          ...
          DBUG_PRINT("citus", "router planner doesn't support INSERT common table expressions");
          return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
      ...
      if (cteQuery->hasForUpdate &&
          FindNodeMatchingCheckFunctionInRangeTableList(cteQuery->rtable,
              IsReferenceTableRTE)) {
        DBUG_PRINT("citus", "router planner doesn't support INSERT FOR UPDATE in common table expressions involving reference tables");
        return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
      ...
      if (FindNodeMatchingCheckFunction((Node *) cteQuery, CitusIsVolatileFunction)) {
        DBUG_PRINT("citus", "router planner doesn't support VOLATILE functions in common table expressions");
        return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
...
DeferErrorIfUnsupportedLocalTableJoin(List *rangeTableList)
{
  ...
  if (ModifiesLocalTableWithRemoteCitusLocalTable(rangeTableList)) {
    DBUG_PRINT("citus", "modifying local tables with remote local tables is not supported");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
...
IsLocallyAccessibleCitusLocalTable(Oid relationId)
{
  ...
  if (localShardPlacement != NULL) {
    DBUG_PRINT("citus", "return true when the given table is a citus local table that can be accessed using local execution");
  } else {
    DBUG_PRINT("citus", "return false");
  }
                     ...
                     PlannerRestrictionContext *plannerRestrictionContext)
{
  ...
  if (!fastPathRouterQuery &&
    ...
    if (hasTidColumn) {
      DBUG_PRINT("citus", "cannot perform distributed planning for the given modification");
      DBUG_PRINT("citus", "recursively planned distributed modifications with ctid on where clause are not supported");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  foreach_declared_ptr(rangeTableEntry, rangeTableList) {
    if (rangeTableEntry->rtekind == RTE_RELATION) {
      ...
      if (rangeTableEntry->relkind == RELKIND_VIEW) {
        ...
        DBUG_PRINT("citus", "materialized views in modify queries are not supported");
        return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
      ...
      /* for other kinds of relations, check if it's distributed */
        ...
        if (IsRelationLocalTableOrMatView(rangeTableEntry->relid) &&
          ...
          if (IsCitusTable(rangeTableEntry->relid)) {
            DBUG_PRINT("citus", "local table %s cannot be joined with these distributed tables", relationName);
            appendStringInfo(errorMessage,
                             "local table %s cannot be joined with these distributed tables",
                             relationName);
          } else {
            DBUG_PRINT("citus", "relation %s is not distributed", relationName);
      ...
      if (rangeTableEntry->rtekind == RTE_SUBQUERY) {
        ...
        DBUG_PRINT("citus", "subqueries are not supported in modifications across multiple shards");
      ...
      DBUG_PRINT("citus", "cannot perform distributed planning for the given modifications");
      DBUG_PRINT("citus", "%s", rangeTableEntryErrorDetail);
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                           "cannot perform distributed planning for the given "
                           "modifications",
  ...
  DBUG_PRINT("citus", "the query only contains supported features");
  return NULL;
}
...
DeferErrorIfModifyView(Query *queryTree)
{
  ...
  if (queryTree->rtable != NIL) {
    ...
    if (firstRangeTableElement->rtekind == RTE_RELATION &&
      ...
      DBUG_PRINT("citus", "cannot modify views when the query contains citus tables");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
...
ErrorIfOnConflictNotSupported(Query *queryTree)
{
  ...
  foreach(setTargetCell, onConflictSet) {
    ...
    if (setTargetEntryPartitionColumn) {
      ...
      if (IsA(setTargetEntry->expr, Var)) {
        ...
        DBUG_PRINT("citus", "functions used in the DO UPDATE SET clause of INSERTs on distributed tables must be marked IMMUTABLE");
        return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (contain_mutable_functions((Node *) arbiterWhere) ||
    ...
    DBUG_PRINT("citus", "functions used in the WHERE clause of the ON CONFLICT clause of INSERTs on distributed tables must be marked IMMUTABLE");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
...
{
  DBUG_TRACE;
  bool result =  (query->commandType == CMD_UPDATE ||
                  query->commandType == CMD_DELETE ||
                  query->commandType == CMD_MERGE);

  if (result) {
    DBUG_PRINT("citus", "the given query is an UPDATE or DELETE or MERGE command");
  } else {
    DBUG_PRINT("citus", "return false");
  }
...
{
  DBUG_TRACE;
  bool result = (query->commandType == CMD_MERGE);

  if (result) {
    DBUG_PRINT("citus", "the given query is a MERGE SQL command");
  } else {
    DBUG_PRINT("citus", "return false");
  }
                     ...
                     DeferredErrorMessage **planningError)
{
  ...
  DBUG_PRINT("citus", "generate a list of tasks for performing an INSERT on a distributed table via the router executor");
          ...
          DeferredErrorMessage **planningError)
{
  ...
  if (fastPathRestrictionContext->fastPathRouterQuery &&
    ...
    DBUG_PRINT("citus", "deferred pruning for a fast-path router query");
                                 ...
                                 DistributedPlan *plan)
{
  ...
  if (isLocalExecution) {
    ...
    if (canBuildLocalPlan) {
      ...
      DBUG_PRINT("citus", "plan the query with the new shard relation id:%u", relationShard->relationId);
      planContext->plan = standard_planner(planContext->query, NULL,
                                           planContext->cursorOptions,
                                           planContext->boundParams);
      SetTaskQueryPlan(task, job->jobQuery, planContext->plan);

      DBUG_PRINT("citus", "fast-path router query: created local execution plan");
      DBUG_PRINT("citus", "to avoid deparse and compile of shard query");
  ...
  DBUG_PRINT("citus", "fall back to fast path planner and generating SQL query on the shard");
  planContext->plan = FastPathPlanner(planContext->originalQuery,
                                      planContext->query,
                                      planContext->boundParams);
...
ConvertToQueryOnShard(Query *query, Oid citusTableOid, Oid shardId)
{
  ...
  if (!CheckAttributesMatch(citusTableOid, shardRelationId)) {
    ...
    DBUG_PRINT("citus", "router planner fast path cannot modify parse tree for local execution");
    DBUG_PRINT("citus", "shard table \"%s.%s\" does not match the distributed table \"%s.%s\"",
               schemaName, shardRelationName, schemaName, citusTableName);
    ...
    List *placementList)
{
  ...
  if (taskAssignmentPolicy == TASK_ASSIGNMENT_ROUND_ROBIN) {
    ...
    DBUG_PRINT("citus", "assigned task %u to node %s:%u", task->taskId, primaryPlacement->nodeName, primaryPlacement->nodePort);
                    ...
                    int colocationId, bool delayedFastPath)
{
  ...
  DBUG_PRINT("citus", "generate a task for single shard query and returns it as a list");
                ...
                bool *isLocalTableModification)
{
  ...
  DBUG_PRINT("citus", "run router pruning logic for SELECT, UPDATE, DELETE, and MERGE queries");
  *placementList = NIL;
  ...
  if (fastPathRouterQuery) {
    ...
    *prunedShardIntervalListList = shardIntervalList;
    DBUG_PRINT("citus", "distributed planning for a fast-path router query");
  ...
  if (isMultiShardQuery) {
    ...
    if (commandType == CMD_SELECT) {
      DBUG_PRINT("citus", "router planner cannot handle multi-shard select queries");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
    ...
    if (planningError != NULL) {
      ...
      DBUG_PRINT("citus", "it is a multi-shard modify query");
  ...
  *relationShardList =
  ...
  if (!EnableNonColocatedRouterQueryPushdown &&
    ...
    DBUG_PRINT("citus", "router planner does not support queries that reference non-colocated distributed tables");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (!shardsPresent && !replacePrunedQueryWithDummy) {
     ...
     */
    DBUG_PRINT("citus", "for INSERT ... SELECT, this query could be still a valid for some other target shard intervals");
    return NULL;
  }
...
RelationShardListForShardIntervalList(List *shardIntervalList, bool *shardsPresent)
{
  ...
  if (relationShardList) {
    DBUG_PRINT("citus", "relation shard list:%d", list_length(relationShardList));
  }
...
JoinConditionIsOnFalse(List *joinInfoList)
{
  ...
  if (joinFalseQuery) {
    DBUG_PRINT("citus", "return true for queries that have contradiction clauses");
  } else {
    DBUG_PRINT("citus", "return false");
  }
...
RelationPrunesToMultipleShards(List *relationShardList)
{
  ...
  foreach(relationShardCell, relationShardList) {
    ...
    if (previousRelationShard != NULL &&
      ...
      DBUG_PRINT("citus", "return true if the given list of relation-to-shard mappings contains at least");
      DBUG_PRINT("citus", "two mappings with the same relation, but different shards");
      return true;
    }
...
{
  DBUG_TRACE;
  bool result = ExtractDistributedInsertValuesRTE(query) != NULL;
  if (result) {
...
DeferErrorIfUnsupportedRouterPlannableSelectQuery(Query *query)
{
  ...
  if (query->commandType != CMD_SELECT) {
    DBUG_PRINT("citus", "only SELECT query types are supported in this path");
  ...
  if (!EnableRouterExecution) {
    DBUG_PRINT("citus", "router planner not enabled");
  ...
  foreach(rangeTableRelationCell, rangeTableRelationList) {
    ...
    if (rte->rtekind == RTE_RELATION) {
      ...
      if (!IsCitusTable(distributedTableId)) {
        ...
        hasPostgresOrCitusLocalTable = true;
        DBUG_PRINT("citus", "router planner finds a local table added to metadata");
      ...
      if (IsCitusTableType(distributedTableId, APPEND_DISTRIBUTED)) {
        DBUG_PRINT("citus", "router planner does not support append-partitioned tables");
        return DeferredError(
      ...
      if (query->hasForUpdate) {
        ...
        if (tableReplicationFactor > 1 && IsCitusTableType(distributedTableId,
            DISTRIBUTED_TABLE)) {
          DBUG_PRINT("citus", "SELECT FOR UPDATE with table replication factor > 1 not supported for non-reference tables");
          return DeferredError(
                   ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (contain_nextval_expression_walker((Node *) query->targetList, NULL) &&
    ...
    DBUG_PRINT("citus", "sequences cannot be used in router queries");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (hasPostgresOrCitusLocalTable && hasDistributedTable) {
    DBUG_PRINT("citus", "local tables cannot be used in distributed queries");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
...
ErrorIfQueryHasUnroutableModifyingCTE(Query *queryTree)
{
  ...
  foreach_declared_ptr(cte, queryTree->cteList) {
    ...
    if (cteQuery->commandType != CMD_SELECT &&
      ...
      DBUG_PRINT("citus", "only SELECT, UPDATE, or DELETE common table expressions may be router planned");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
    ...
    if (cteQuery->commandType != CMD_SELECT) {
      ...
      if (!IsCitusTableTypeCacheEntry(modificationTableCacheEntry,
                                      DISTRIBUTED_TABLE)) {
        DBUG_PRINT("citus", "cannot router plan modification of a non-distributed table");
        return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
      ...
      if (replicationModel &&
        ...
        DBUG_PRINT("citus", "cannot route mixed replication models");
        return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
...
ErrorIfQueryHasCTEWithSearchClause(Query *queryTree)
{
  if (ContainsSearchClauseWalker((Node *) queryTree, NULL)) {
    DBUG_PRINT("citus", "CTEs with search clauses are not supported");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
