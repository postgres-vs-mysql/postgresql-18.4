            ...
            Const **partitionValueConst)
{
  ...
  DBUG_PRINT("citus", "shard count:%d", shardCount);

  /* there are no shards to return */
  if (shardCount == 0) {
    DBUG_PRINT("citus", "there are no shards to return");
    return NIL;
  }

  /* always return empty result if WHERE clause is of the form: false (AND ..) */
  if (ContainsFalseClause(whereClauseList)) {
    DBUG_PRINT("citus", "always return empty result if WHERE clause is of the form: false (AND ..)");
    return NIL;
  }

  /* short circuit for non-distributed tables such as reference table */
  if (!HasDistributionKeyCacheEntry(cacheEntry)) {
    DBUG_PRINT("citus", "short circuit for non-distributed tables such as reference table");
  ...
  if (cacheEntry->shardIntervalCompareFunction) {
    DBUG_PRINT("citus", "initiate function call info once (allows comparators to cache metadata)");
  ...
  DBUG_PRINT("citus", "build logical tree of prunable restrictions and invalid restrictions");
  BuildPruningTree((Node *) whereClauseList, &treeBuildContext);

  DBUG_PRINT("citus", "simplify logic tree of prunable restrictions");
  /* Simplify logic tree of prunable restrictions */
  SimplifyPruningTree(tree, NULL);

  DBUG_PRINT("citus", "figure out what we can prune on");
   ...
   * together.
   */
  DBUG_PRINT("citus", "prune using each of the PrunableInstances we found, and OR results together");
  ...
  foreach(pruneCell, context.pruningInstances) {
    ...
    if (!prune->hasValidConstraint) {
      DBUG_PRINT("citus", "when the current instance has no prunable expressions, we'll have to return all shards");
    ...
    if (context.partitionMethod == DISTRIBUTE_BY_HASH) {
      if (!prune->evaluatesToFalse && !prune->equalConsts &&
        ...
        DBUG_PRINT("citus", "when hash-partitioned and no equals constraints, return all shards");
    ...
    if (pruneOneList) {
      DBUG_PRINT("citus", "prune one list length:%d", list_length(pruneOneList));
    }
  ...
  if (!foundRestriction) {
    DBUG_PRINT("citus", "found no valid restriction, build list of all shards");
    prunedList = ShardArrayToList(cacheEntry->sortedShardIntervalArray,
                                  cacheEntry->shardIntervalArrayLength);
  }
...
BuildPruningTree(Node *node, PruningTreeBuildContext *context)
{
  ...
  DBUG_PRINT("citus", "build a logical tree of constraints for pruning");
...
SimplifyPruningTree(PruningTreeNode *node, PruningTreeNode *parent)
{
  ...
  if (!parent) {
    DBUG_PRINT("citus", "root is always ANDed expressions");
  ...
  if (ConstraintCount(node) <= 1) {
    ...
    DBUG_PRINT("citus", "remove current node from parent");
         ...
         PruningInstance *prune)
{
  ...
  if (prune->evaluatesToFalse) {
    DBUG_PRINT("citus", "well, if life always were this easy...");
    return NIL;
  }
  ...
  if (prune->equalConsts &&
     ...
     */
    if (!shardInterval) {
      DBUG_PRINT("citus", "when pruned down to nothing, we're done");
      return NIL;
    }
  }
  ...
  if (shardInterval) {
    if (context->partitionMethod != DISTRIBUTE_BY_HASH &&
      ...
      DBUG_PRINT("citus", "no chance to prune further, return");
   ...
   */
  DBUG_PRINT("citus", "brute force: Check each shard");
                    ...
                    PruningInstance *prune)
{
  ...
  if (hasLowerBound) {
    ...
    lowerBoundIdx = LowerShardBoundary(lowerBound, sortedShardIntervalArray,
                                       ...
                                       lowerBoundInclusive);
    DBUG_PRINT("citus", "find lower bound:%d", lowerBoundIdx);
  } else {
    lowerBoundIdx = 0;
  }
  ...
  if (hasUpperBound) {
    upperBoundIdx = UpperShardBoundary(upperBound, sortedShardIntervalArray,
                                       shardCount, compareFunctionCall,
                                       upperBoundInclusive);
    DBUG_PRINT("citus", "find upper bound:%d", upperBoundIdx);
  } else {
  ...
  if (remainingShardList) {
    DBUG_PRINT("citus", "remaining shard list:%d", list_length(remainingShardList));
  }
...
{
  int result = list_length(node->childBooleanNodes) +
         list_length(node->validConstraints) +
         (node->hasInvalidConstraints ? 1 : 0);

  DBUG_PRINT("citus", "return how many arguments this node is taking:%d", result);
  return result;
}
