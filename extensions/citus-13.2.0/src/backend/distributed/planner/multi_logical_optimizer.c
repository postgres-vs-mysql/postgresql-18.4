...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "check whether given aggregate can be distributed across workers using worker_partial_agg & coord_combine_agg");
...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "check if any aggregates cannot be pushed down");
...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "check whether the expression can be pushed down to workers");

  if (contain_nextval_expression_walker(expression, NULL)) {
    DBUG_PRINT("citus", "nextval can only be evaluated on the coordinator");
  ...
  if (hasPushableWindowFunction) {
    DBUG_PRINT("citus", "aggregates inside pushed down window functions can be pushed down and return true");
    return true;
  }
...
DeferErrorIfUnsupportedArrayAggregate(Aggref *arrayAggregateExpression)
{
  ...
  if (arrayAggregateExpression->aggorder) {
    DBUG_PRINT("citus", "array_agg with order by is unsupported");
  ...
  if (arrayAggregateExpression->aggdistinct) {
    DBUG_PRINT("citus", "array_agg (distinct) is unsupported");
    return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                                     ...
                                     Aggref *aggregateExpression)
{
  ...
  if (aggregateExpression->aggdistinct || aggregateExpression->aggorder) {
    ...
    DBUG_PRINT("citus", "%s", errorDetail.data);
    ...
    MultiNode *logicalPlanNode)
{
  ...
  if (aggregateType == AGGREGATE_COUNT) {
    ...
    foreach_declared_ptr(column, columnList) {
      if (column->varattno <= 0) {
        DBUG_PRINT("citus", "cannot compute count (distinct)");
        DBUG_PRINT("citus", "non-column references are not supported yet");
        return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
    ...
    foreach_declared_ptr(multiTable, multiTableNodeList) {
      if (multiTable->relationId == SUBQUERY_RELATION_ID ||
        ...
        DBUG_PRINT("citus", "cannot compute aggregate (distinct)");
        DBUG_PRINT("citus", "only count(distinct) aggregate is supported in subqueries");
        return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  if (aggregateType == AGGREGATE_COUNT &&
    ...
    if (distinctExtensionId != InvalidOid) {
      ...
      DBUG_PRINT("citus", "cannot compute count (distinct) approximation");
      return DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
                 ...
                 orderByLimitReference)
{
  ...
  if (limitCount == NULL) {
    ...
    DBUG_PRINT("citus", "no limit node to push down");
    return NULL;
  }
  ...
  if (!IsA(limitCount, Const)) {
     ...
     */
    DBUG_PRINT("citus", "we only push down constant LIMIT clauses to make sure we get back the minimum number of rows");
    return NULL;
  }
  ...
  if (limitOffset != NULL && !IsA(limitOffset, Const)) {
     ...
     * push down.
     */
    DBUG_PRINT("citus", "when OFFSET is not a constant then we cannot calculate the LIMIT to push down");
    return NULL;
  }
  ...
  if (!orderByLimitReference.onlyPushableWindowFunctions) {
    DBUG_PRINT("citus", "when window functions are computed on coordinator, we cannot push down LIMIT");
    ...
    DBUG_PRINT("citus", "when we don't have group by clauses, or we have group by partition column,");
    DBUG_PRINT("citus", "we can push down the original limit");
    canPushDownLimit = LIMIT_CAN_PUSHDOWN;
  } else if (orderByLimitReference.sortClauseIsEmpty) {
    DBUG_PRINT("citus", "when sort clause is empty, we cannot push down LIMIT");
    canPushDownLimit = LIMIT_CANNOT_PUSHDOWN;
  } else if (!orderByLimitReference.hasOrderByAggregate) {
    DBUG_PRINT("citus", "when we have order by clauses without aggregates, we can push down the original limit");
    canPushDownLimit = LIMIT_CAN_PUSHDOWN;
  } else if (orderByLimitReference.canApproximate) {
    DBUG_PRINT("citus", "when we have order by clauses with commutative aggregates, we can push down approximate limits");
    canPushDownLimit = LIMIT_CAN_APPROXIMATE;
  }

  /* create the workerLimitNode according to the decisions above */
  DBUG_PRINT("citus", "create the workerLimitNode according to the decisions above");
  ...
  if (workerLimitNode != NULL && limitOffset != NULL) {
    ...
    if (!workerLimitConst->constisnull) {
      DBUG_PRINT("citus", "when offset clause is present and limit can be pushed down, add the offset value to limit on workers");
      DBUG_PRINT("citus", "only update the worker limit if the const is not null");
      ...
      DBUG_PRINT("citus", "worker limit count:%ld", workerLimitCount);
  ...
  if (workerLimitNode != NULL) {
    ...
    if (!workerLimitConst->constisnull) {
      ...
      DBUG_PRINT("citus", "push down of limit count:%ld", workerLimitCount);
      ereport(DEBUG1, (errmsg("push down of limit count: " INT64_FORMAT,
                              workerLimitCount)));
    } else {
      DBUG_PRINT("citus", "push down of limit count: ALL");
                     ...
                     OrderByLimitReference orderByLimitReference)
{
  ...
  if (limitCount == NULL && !orderByLimitReference.hasDistinctOn) {
    DBUG_PRINT("citus", "when no limit node and no hasDistinctOn, no need to push down sort clauses");
    return NIL;
  }

  /* If window functions are computed on coordinator, we cannot push down sorting. */
  if (!orderByLimitReference.onlyPushableWindowFunctions) {
    DBUG_PRINT("citus", "when window functions are computed on coordinator, we cannot push down sorting");
    return NIL;
  }
  ...
  if (orderByLimitReference.groupClauseIsEmpty ||
      orderByLimitReference.groupedByDisjointPartitionColumn) {
    DBUG_PRINT("citus", "when we are pushing down the limit, push down any order by clauses");
    ...
    if (orderByNonAggregates) {
      DBUG_PRINT("citus", "when we are pushing down the limit because the order by clauses don't have any aggregates");
      DBUG_PRINT("citus", "add group by clauses to the order by list");
      workerSortClauseList = sortClauseList;
      workerSortClauseList = list_concat(workerSortClauseList, groupClauseList);
    } else if (canApproximate) {
      DBUG_PRINT("citus", "when canApproximate is true, push down any order by clauses");
      workerSortClauseList = sortClauseList;
    }
  }
...
CanPushDownLimitApproximate(List *sortClauseList, List *targetList)
{
  ...
  if (LimitClauseRowFetchCount == DISABLE_LIMIT_APPROXIMATION) {
    DBUG_PRINT("citus", "user hasn't enabled the limit approximation");
    return false;
  }
  ...
  if (canApproximate) {
    DBUG_PRINT("citus", "the user has enabled the limit approximation and the query has order by clauses that are commutative");
    DBUG_PRINT("citus", "return true");
  } else {
    DBUG_PRINT("citus", "return false");
  }
...
HasOrderByNonCommutativeAggregate(List *sortClauseList, List *targetList)
{
  ...
  if (hasOrderByNonCommutativeAggregate) {
    DBUG_PRINT("citus", "we have an order by an aggregate which is not commutative");
  } else {
    DBUG_PRINT("citus", "return false");
  }
...
HasOrderByComplexExpression(List *sortClauseList, List *targetList)
{
  ...
  if (hasOrderByComplexExpression) {
    DBUG_PRINT("citus", "we have a nested expression that contains an aggregate function within it");
  } else {
    DBUG_PRINT("citus", "return false");
  }
...
HasOrderByHllType(List *sortClauseList, List *targetList)
{
  ...
  if (hasOrderByHllType)  {
    DBUG_PRINT("citus", "some of those clauses operate on hll data type");
  } else {
    DBUG_PRINT("citus", "return false");
  }
...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "check whether each clause in group clauses also exists in the distinct clauses");
