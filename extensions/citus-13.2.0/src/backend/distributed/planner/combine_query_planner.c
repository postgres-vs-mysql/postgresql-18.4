                          ...
                          CustomScan *remoteScan)
{
   ...
   */
  DBUG_PRINT("citus", "the 100k rows we put on the cost of the path is kind of arbitrary and");
  DBUG_PRINT("citus", "could be improved in accuracy to produce better plans");
                        ...
                        List *custom_plans)
{
  ...
  DBUG_PRINT("citus", "this function returns a Plan node, more specifically the CustomScan Plan node");
  DBUG_PRINT("citus", "that has the ability to execute the distributed part of the query");
...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "create a PlannedStmt where it combines the combineQuery and the remoteScan");
  DBUG_PRINT("citus", "it utilizes the standard_planner from postgres to create a plan based on the combineQuery");
  /*
...

  {
    StringInfo queryString = makeStringInfo();
    pg_get_query_def(combineQuery, queryString);
    DBUG_PRINT("citus", "combine query: %s", queryString->data);
  }
  ...
  PG_TRY();
  {
    ...
    DBUG_PRINT("citus", "call standard_planner");
