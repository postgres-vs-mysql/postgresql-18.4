...
RebuildQueryStrings(Job *workerJob)
{
  ...
  foreach_declared_ptr(task, taskList) {
    ...
    if (isQueryObjectOrText) {
      name_str = TaskQueryString(task);
    } else {
      name_str = "(null)";
    }
    DBUG_PRINT("citus", "query before rebuilding: %s", !isQueryObjectOrText ? "(null)" : name_str);
    ...
    name_str = TaskQueryString(task);
    DBUG_PRINT("citus", "query after rebuilding: %s", name_str);
...
UpdateWhereClauseToPushdownRecurringOuterJoin(Query *query, List *relationShardList)
{
  ...
  foreach(fromExprCell, fromExpr->fromlist) {
    ...
    DBUG_PRINT("citus", "distributed table from the inner part of the outer join: %s", innerRte->eref->aliasname);
...
TaskQueryString(Task *task)
{
  ...
  if (taskQueryType == TASK_QUERY_NULL) {
    ...
    DBUG_INSTANT_PRINT("citus", "unexpected task query state: task query type is null");
