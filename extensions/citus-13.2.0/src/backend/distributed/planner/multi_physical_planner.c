...
BuildJobTree(MultiTreeRoot *multiTree)
{
  ...
  DBUG_PRINT("citus", "build the physical job tree from the given logical plan tree");
  ...
  while (parentNode != NULL) {
    ...
    if (boundaryNodeJobType == JOIN_MAP_MERGE_JOB) {
      ...
      if (CitusIsA(leftChildNode, MultiPartition)) {
        ...
        DBUG_PRINT("citus", "build query and partition job");
      ...
      if (CitusIsA(rightChildNode, MultiPartition)) {
         ...
         */
        DBUG_PRINT("citus", "the right query and right partition job do not depend on any jobs since our logical plan tree is left deep");
                         ...
                         DeferredErrorMessage **planningError)
{
  ...
  DBUG_PRINT("citus", "create a list of SQL tasks to execute the given subquery pushdown job");
  ...
  if (list_length(relationRestrictionContext->relationRestrictionList) == 0) {
    DBUG_PRINT("citus", "cannot handle complex subqueries when the router executor is disabled");
  ...
  foreach_declared_ptr(relationRestriction,
    ...
    if (prevShardCount > 0 && prevShardCount != cacheEntry->shardIntervalArrayLength) {
      DBUG_PRINT("citus", "shard counts of co-located tables do not match");
      *planningError = DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
  ...
  forboth_ptr(prunedShardList, prunedRelationShardList,
    ...
    if (IsInnerTableOfOuterJoin(relationRestriction, distributedTableIndex,
      ...
      if (outerPartHasDistributedTable) {
        DBUG_PRINT("citus", "we can skip the shards from this relation restriction");
         ...
         */
        DBUG_PRINT("citus", "the outer part does not include distributed tables, we can not skip shards");
  ...
  if (!updateQualsForOuterJoin && FindNodeMatchingCheckFunction((Node *) query,
     ...
     */
    DBUG_PRINT("citus", "we have an outer join, so assume 'might' need to update quals");
    updateQualsForOuterJoin = true;
  }
  ...
  while ((shardOffset = bms_next_member(taskRequiredForShardIndex, shardOffset)) >= 0) {
    ...
    DBUG_PRINT("citus", "set job id:%lu", jobId);
  ...
  if (taskType == MODIFY_TASK && list_length(
    ...
    DBUG_PRINT("citus", "when it is a modify task with multiple tables, set modifyWithSubquery true");
                        ...
                        bool *outerPartHasDistributedTable)
{
  ...
  foreach_declared_ptr(joinInfo, relationRestriction->relOptInfo->joininfo) {
    ...
    if (!isInOuter) {
      ...
      *outerPartHasDistributedTable = bms_overlap(joinInfo->outer_relids,
      ...
      DBUG_PRINT("citus", "this is an inner table of an outer join");
  ...
  DBUG_PRINT("citus", "we have not found any join clause that satisfies both requirements");
                        ...
                        DeferredErrorMessage **planningError)
{
  ...
  DBUG_PRINT("citus", "create  a sql task by replacing the target shardInterval's boundary value");
  ...
  if (UpdateOrDeleteOrMergeQuery(originalQuery)) {
    DBUG_PRINT("citus", "when it is a modify query with sub-select, we need to set result relation shard's id as anchor shard id");
   ...
   */
  DBUG_PRINT("citus", "find the relevant shard out of each relation for this task");
  ...
  foreach(restrictionCell, restrictionContext->relationRestrictionList) {
    ...
    if (!HasDistributionKeyCacheEntry(cacheEntry)) {
      ...
      DBUG_PRINT("citus", "non-distributed tables have only one shard");
      ...
      if (anchorShardId == INVALID_SHARD_ID) {
        DBUG_PRINT("citus", "use as anchor shard only if we couldn't find any yet");
      ...
      if (!modifyWithSubselect || relationId == resultRelationOid) {
        DBUG_PRINT("citus", "for UPDATE/DELETE the shard in the result relation becomes the anchor shard");
  ...
  if (list_length(taskPlacementList) == 0) {
    DBUG_PRINT("citus", "cannot find a worker that has active placements for all shards in the query");
    *planningError = DeferredError(ERRCODE_FEATURE_NOT_SUPPORTED,
   ...
   */
  DBUG_PRINT("citus", "augment the relations in the query with the shard IDs");
  ...
  if ((taskType == MODIFY_TASK && !modifyRequiresCoordinatorEvaluation) ||
    ...
    DBUG_PRINT("citus", "distributed statement: %s", queryString->data);
...
SqlTaskList(Job *job)
{
  ...
  foreach(fragmentCombinationCell, fragmentCombinationList) {
    ...
    DBUG_PRINT("citus", "generated sql query for task %d", sqlTask->taskId);
    DBUG_PRINT("citus", "query string: \"%s\"", sqlQueryString->data);
...
NodeIsRangeTblRefReferenceTable(Node *node, List *rangeTableList)
{
  ...
  result = IsCitusTableType(rangeTableEntry->relid, REFERENCE_TABLE);
  if (result) {
    DBUG_PRINT("citus", "return true");
  } else {
    DBUG_PRINT("citus", "return false");
...
PartitionedOnColumn(Var *column, List *rangeTableList, List *dependentJobList)
{
  ...
  if (rangeTableType == CITUS_RTE_RELATION) {
    ...
    if (IsCitusTable(relationId) && !HasDistributionKey(relationId)) {
      DBUG_PRINT("citus", "non-distributed tables do not have partition columns");
      DBUG_PRINT("citus", "return false");
      return false;
    }
...
JoinPrunable(RangeTableFragment *leftFragment, RangeTableFragment *rightFragment)
{
  ...
  if (leftFragment->fragmentType == CITUS_RTE_REMOTE_QUERY &&
    ...
    if (leftMergeTask->partitionId != rightMergeTask->partitionId) {
      DBUG_PRINT("citus", "join prunable for task partitionId %u and %u", leftMergeTask->partitionId, rightMergeTask->partitionId);
  ...
  if (!overlap) {
    if (IsLoggableLevel(DEBUG2)) {
      ...
      DBUG_PRINT("citus", "join prunable for intervals %s and %s", leftString->data, rightString->data);
...
ShardIntervalsOverlap(ShardInterval *firstInterval, ShardInterval *secondInterval)
{
  ...
  result = ShardIntervalsOverlapWithParams(firstMin, firstMax, secondMin, secondMax,
...
AnchorShardId(List *fragmentList, uint32 anchorRangeTableId)
{
  ...
  Assert(anchorShardId != INVALID_SHARD_ID);
  DBUG_PRINT("citus", "return this ragment's shard identifier:%lu", anchorShardId);
  ...
  foreach(sqlTaskCell, sqlTaskList) {
    ...
    foreach(dependentTaskCell, dependentTaskList) {
      ...
      if (dataFetchTask->taskType == MERGE_FETCH_TASK) {
        ...
        DBUG_PRINT("citus", "pruning merge fetch taskId %d", dataFetchTask->taskId);
        DBUG_PRINT("citus", "creating dependency on merge taskId %d", mergeTaskReference->taskId);
...
AssignTaskList(List *sqlTaskList)
{
  ...
  foreach(constrainedSqlTaskCell, constrainedSqlTaskList) {
    ...
    foreach(mergeTaskCell, mergeTaskList) {
      ...
      DBUG_PRINT("citus", "propagating assignment from merge task %d to constrained sql task %d", mergeTask->taskId, sqlTask->taskId);
...
HasMergeTaskDependencies(List *sqlTaskList)
{
  ...
  if (hasMergeTaskDependencies) {
    DBUG_PRINT("citus", "check if sql tasks in the given sql task list have any dependencies on merge tasks ? Yes");
    DBUG_PRINT("citus", "return true");
  } else {
    DBUG_PRINT("citus", "return false");
  }
...
GreedyAssignTask(WorkerNode *workerNode, List *taskList, List *activeShardPlacementLists)
{
  ...
  if (assignedTask != NULL) {
    ...
    DBUG_PRINT("citus", "assigned task %u to node %s:%u", assignedTask->taskId,
...
FindDependentMergeTaskList(Task *sqlTask)
{
  ...
  DBUG_PRINT("citus", "return those found tasks in a new list(length:%d)", count);
...
TaskListHighestTaskId(List *taskList)
{
  ...
  DBUG_PRINT("citus", "return highestTaskId:%u", highestTaskId);
