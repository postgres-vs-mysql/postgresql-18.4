...
partition_column_id(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "return the column_id of its partition column:%d", (int16) partitionColumn->varattno);
...
partition_type(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "return the type of partitioning in use by that table: %d", partitionType);
...
is_distributed_table(PG_FUNCTION_ARGS)
{
  ...
  if (isCitusTable) {
    DBUG_PRINT("citus", "return whether a given table is distributed? Yes");
  } else {
    DBUG_PRINT("citus", "return whether a given table is distributed? No");
  }
...
create_monolithic_shard_row(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "return newShardId:%ld", newShardId);
...
relation_count_in_query(PG_FUNCTION_ARGS)
{
  ...
  foreach_declared_ptr(parsetree, parseTreeList) {
    ...
    foreach_declared_ptr(query, queryTreeList) {
      ...
      result = list_length(rangeTableList);
      DBUG_PRINT("citus", "return the first query's relation count:%d", result);
