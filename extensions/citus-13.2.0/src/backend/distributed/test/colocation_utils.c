...
get_table_colocation_id(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "return colocation id of given distributed table:%d", colocationId);
...
find_shard_interval_index(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "find index of given shard(shardId:%u) in sorted shard interval list: %u", shardId, shardIndex);
