...
get_adjacency_list_wait_graph(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "distributed deadlock detection only considers distributed txs");
  ...
  while ((transactionNode = (TransactionNode *) hash_seq_search(&status)) != 0) {
    ...
    DBUG_PRINT("citus", "transaction number:%lu, wait for %s", transactionNode->transactionId.transactionNumber,
