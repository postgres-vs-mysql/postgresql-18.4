...
IsFirstShard(CitusTableCacheEntry *tableEntry, uint64 shardId)
{
  ...
  if (tableEntry == NULL || tableEntry->sortedShardIntervalArray == NULL) {
    DBUG_PRINT("citus", "the given shardId is not the first shard");
    return false;
  }
  ...
  if (shardId == tableEntry->sortedShardIntervalArray[0]->shardId) {
    DBUG_PRINT("citus", "the given shardId is the first shard");
    return true;
  } else {
    DBUG_PRINT("citus", "the given shardId is not the first shard");
    return false;
...
HasDistributionKey(Oid relationId)
{
  ...
  bool result = HasDistributionKeyCacheEntry(tableEntry);

  if (result) {
    DBUG_PRINT("citus", "the given Citus table has a distribution key");
  } else {
    DBUG_PRINT("citus", "the given Citus table has no distribution key");
  }
...
IsCitusTable(Oid relationId)
{
  ...
  if (relationId < FirstNormalObjectId) {
    DBUG_PRINT("citus", "if the relationId is less than FirstNormalObjectId, we can immediately return false");
    return false;
  }

  bool result = LookupCitusTableCacheEntry(relationId) != NULL;

  if (result) {
    DBUG_PRINT("citus", "return whether relationId:%u is a distributed relation or not? Yes", relationId);
  } else {
    DBUG_PRINT("citus", "return whether relationId:%u is a distributed relation or not? No", relationId);
  }
  return result;
}
...
IsCitusTableViaCatalog(Oid relationId)
{
  ...
  if (heapTupleIsValid) {
    DBUG_PRINT("citus", "return whether the given relation(%u) is a distributed table or not ? Yes", relationId);
  } else {
    DBUG_PRINT("citus", "return whether the given relation(%u) is a distributed table or not ? No", relationId);
  }
...
RelationIdForShard(uint64 shardId)
{
  ...
  ShardIdCacheEntry *shardIdEntry = LookupShardIdCacheEntry(shardId, missingOk);
  CitusTableCacheEntry *tableEntry = shardIdEntry->tableEntry;
  DBUG_PRINT("citus", "return the relationId:%u of the given shardId:%lu", tableEntry->relationId, shardId);
...
ReferenceTableShardId(uint64 shardId)
{
  ...
  ShardIdCacheEntry *shardIdEntry = LookupShardIdCacheEntry(shardId, missingOk);
  CitusTableCacheEntry *tableEntry = shardIdEntry->tableEntry;
  bool result = IsCitusTableTypeCacheEntry(tableEntry, REFERENCE_TABLE);
  if (result) {
    DBUG_PRINT("citus", "return true if the given shardId belongs to a reference table");
  } else {
...
DistributedTableShardId(uint64 shardId)
{
  ...
  ShardIdCacheEntry *shardIdEntry = LookupShardIdCacheEntry(shardId, missingOk);
  CitusTableCacheEntry *tableEntry = shardIdEntry->tableEntry;
  bool result = IsCitusTableTypeCacheEntry(tableEntry, DISTRIBUTED_TABLE);
  if (result) {
    DBUG_PRINT("citus", "return true if the given shardId belongs to a distributed table");
  } else {
    DBUG_PRINT("citus", "return false");
  }
  return result;
                           ...
                           int shardIntervalArrayLength)
{
  ...
  if (shardIntervalArrayLength == 0) {
    DBUG_PRINT("citus", "if there are no shards, there is no uniform distribution");
    return false;
  }
...
CitusHasBeenLoadedInternal(void)
{
  ...
  if (IsBinaryUpgrade) {
    ...
    DBUG_PRINT("citus", "never use citus logic during pg_upgrade");
    return false;
  }
  ...
  if (citusExtensionOid == InvalidOid) {
    DBUG_PRINT("citus", "citus extension does not exist yet");
    /* Citus extension does not exist yet */
    return false;
  }

  /* citus extension exists and has been created */
  DBUG_PRINT("citus", "citus extension exists and has been created");
  return true;
}
...
GetLocalGroupId(void)
{
   ...
   */
  if (LocalGroupId != -1) {
    DBUG_PRINT("citus", "already set the group id, no need to read the heap again:%d", LocalGroupId);
    return LocalGroupId;
  }
  ...
  DBUG_PRINT("citus", "return the group identifier of the local node:%d", groupId);
  return groupId;
}
...
GetLocalNodeId(void)
{
   ...
   */
  if (LocalNodeId != -1) {
    DBUG_PRINT("citus", "already set the node id, no need to read the heap again:%d", LocalNodeId);
    return LocalNodeId;
  }
  ...
  DBUG_PRINT("citus", "return the node identifier of the local node:%d", nodeId);
  return nodeId;
}
...
RelationExists(Oid relationId)
{
  ...
  if (relationExists) {
    DBUG_PRINT("citus", "return whether a relation with the given OID:%u exists? Yes", relationId);
  } else {
    DBUG_PRINT("citus", "return whether a relation with the given OID:%u exists? No", relationId);
  }
