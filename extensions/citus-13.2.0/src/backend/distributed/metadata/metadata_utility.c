...
citus_total_relation_size(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "return a distributed table and its indexes' total relation size:%ld", relationSize);
...
citus_table_size(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "return a distributed table's total relation size:%ld", relationSize);
  PG_RETURN_INT64(relationSize);
}
...
citus_relation_size(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "return a relation's 'main' fork's size:%ld", relationSize);
  PG_RETURN_INT64(relationSize);
}
