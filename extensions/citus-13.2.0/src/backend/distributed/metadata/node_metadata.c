...
citus_activate_node(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "node name:%s, node port:%d", nodeNameString, nodePort);
  ...
  DBUG_PRINT("citus", "return workerNode->nodeId:%u", workerNode->nodeId);
...
get_shard_id_for_distribution_column(PG_FUNCTION_ARGS)
{
  ...
  if (shardInterval != NULL) {
    DBUG_PRINT("citus", "return shardId:%ld", shardInterval->shardId);
    PG_RETURN_INT64(shardInterval->shardId);
  }

  DBUG_PRINT("citus", "return shardId:0");
  PG_RETURN_INT64(0);
}
...
citus_nodename_for_nodeid(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "node id:%d, node name:%s", nodeId, node->workerName);
...
citus_nodeport_for_nodeid(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "return the node port for the node with given node id(%d): %d", nodeId, node->workerPort);
...
citus_coordinator_nodeid(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "return the node id for the coordinator node: %d", coordinatorNodeId);
...
FindWorkerNode(const char *nodeName, int32 nodePort)
{
  ...
  if (handleFound) {
    ...
    *workerNode = *cachedWorkerNode;
    DBUG_PRINT("citus", "nodeId:%u, workerPort:%u, workerName:'%s', groupId:%d", 
        workerNode->nodeId, workerNode->workerPort, workerNode->workerName, workerNode->groupId);
    return workerNode;
  }

  if (strcmp(LocalHostName, nodeName) == 0 && nodePort == PostPortNumber) {
     WorkerNode *workerNode = PrimaryNodeForGroup(GetLocalGroupId(), NULL);
     DBUG_PRINT("citus", "nodeId:%u, workerPort:%u, workerName:'%s', groupId:%d", 
        workerNode->nodeId, workerNode->workerPort, workerNode->workerName, workerNode->groupId);
    return workerNode;
  }
...
ReadDistNode(bool includeNodesFromOtherClusters)
{
  ...
  DBUG_PRINT("citus", "worker node list length:%d", count);
