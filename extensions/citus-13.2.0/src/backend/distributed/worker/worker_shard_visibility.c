...
ShouldHideShards(void)
{
  ...
  if (HideShards == CHECK_APPLICATION_NAME) {
    if (ShouldHideShardsInternal()) {
      ...
      DBUG_PRINT("citus", "return whether we should hide shards in the current session? Yes");
      return true;
    } else {
      HideShards = DO_NOT_HIDE_SHARDS;
      DBUG_PRINT("citus", "return whether we should hide shards in the current session? No");
      return false;
    }
  } else {
    result = HideShards == HIDE_SHARDS_FROM_APPLICATION;
