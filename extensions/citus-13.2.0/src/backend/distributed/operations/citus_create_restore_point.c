...
citus_create_restore_point(PG_FUNCTION_ARGS)
{
  ...
  if (RecoveryInProgress()) {
    DBUG_PRINT("citus", "recovery is in progress");
    DBUG_INSTANT_PRINT("citus", "WAL control functions cannot be executed during recovery");
    ereport(ERROR,
  ...
  if (!XLogIsNeeded()) {
    DBUG_INSTANT_PRINT("citus", "WAL level not sufficient for creating a restore point");
    ereport(ERROR,
            (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
  ...
  if (strlen(restoreNameString) >= MAXFNAMELEN) {
    DBUG_INSTANT_PRINT("citus", "value too long for restore point (maximum %d characters)", MAXFNAMELEN - 1);
    ereport(ERROR,
