                  ...
                  bool *cancellationReceived)
{
  ...
  DBUG_PRINT("citus", "process the received events from connections");
...
HasAnyConnectionFailure(WorkerPool *workerPool)
{
  ...
  if (workerPool->failureState == WORKER_POOL_FAILED ||
    ...
    DBUG_PRINT("citus", "connection pool failed");
    return true;
  }
  ...
  if (failedConnectionCount >= 1) {
    ...
    DBUG_PRINT("citus", "do not attempt to open more connections after one failed");
    return true;
  }
...
ShouldWaitForSlowStart(WorkerPool *workerPool)
{
  ...
  if (UseConnectionPerPlacement()) {
    DBUG_PRINT("citus", "if we can use a connection per placement, we don't need to wait for slowstart");
    return false;
  }

  /* if slow start is disabled, we can open new connections */
  if (ExecutorSlowStartInterval == SLOW_START_DISABLED) {
    DBUG_PRINT("citus", "if slow start is disabled, we can open new connections");
    return false;
  }
...
CalculateNewConnectionCount(WorkerPool *workerPool)
{
  ...
  if (UseConnectionPerPlacement()) {
    ...
    if (EnableCostBasedConnectionEstablishment && newConnectionCount > 0 &&
       ...
       */
      DBUG_PRINT("citus", "return the amount of connections that we can currently open:0");
      return 0;
    }
  }

  DBUG_PRINT("citus", "return the amount of connections that we can currently open:%d", newConnectionCount);
...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "opening %d new connections to %s:%d", newConnectionCount, workerPool->nodeName, workerPool->nodePort);
...
HasUnfinishedTaskForSession(WorkerSession *session)
{
  ...
  if (session->currentTask != NULL) {
    ...
    DBUG_PRINT("citus", "the session is executing a command right now");
    return true;
  }
  ...
  if (!dlist_is_empty(sessionReadyTaskQueue)) {
    DBUG_PRINT("citus", "session has an assigned task, which is ready for execution");
  ...
  if (!dlist_is_empty(poolReadyTaskQueue)) {
     ...
     */
    DBUG_PRINT("citus", "pool has unassigned tasks that can be executed by the input session");
    return true;
  }
...
TransactionStateMachine(WorkerSession *session)
{
  ...
  DBUG_PRINT("citus", "connectionId:%lu, sessionId:%lu", connection->connectionId, session->sessionId);

  do {
    currentState = transaction->transactionState;

    if (!CheckConnectionReady(session)) {
      DBUG_PRINT("citus", "connection is busy, no state transitions to make");
    ...
    switch (currentState) {
      case REMOTE_TRANS_NOT_STARTED: {
        if (useRemoteTransactionBlocks == TRANSACTION_BLOCKS_REQUIRED) {
          DBUG_PRINT("citus", "if we're expanding the nodes in a transaction, use 2PC");
          /* if we're expanding the nodes in a transaction, use 2PC */
          Activate2PCIfModifyingTransactionExpandsToNewNode(session);

          /* need to open a transaction block first */
          DBUG_PRINT("citus", "need to open a transaction block first");
          ...
          if (placementExecution == NULL) {
             ...
             */
            DBUG_PRINT("citus", "no tasks are ready to be executed at the moment");
            UpdateConnectionWaitFlags(session, WL_SOCKET_READABLE);
          ...
          if (!placementExecutionStarted) {
            DBUG_PRINT("citus", "no need to continue, connection is lost");
      ...
      case REMOTE_TRANS_CLEARING_RESULTS: {
        ...
        if (result != NULL) {
          if (!IsResponseOK(result)) {
            DBUG_INSTANT_PRINT("citus", "query failures are always hard errors");
          ...
          DBUG_PRINT("citus", "wake up WaitEventSetWait");
        ...
        if (session->currentTask != NULL) {
          ...
          DBUG_PRINT("citus", "connection is ready to use for executing commands");
          /* connection is ready to use for executing commands */
          workerPool->idleConnectionCount++;
        }

        /* connection needs to be writeable to send next command */
        DBUG_PRINT("citus", "connection needs to be writeable to send next command");
        UpdateConnectionWaitFlags(session,
                                  WL_SOCKET_READABLE | WL_SOCKET_WRITEABLE);
      ...
      case REMOTE_TRANS_STARTED: {
        DBUG_PRINT("citus", "remote transaction is started");
        TaskPlacementExecution *placementExecution = PopPlacementExecution(
              session);
                                 ...
                                 WorkerSession *session)
{
  ...
  DBUG_PRINT("citus", "connection(connectionId:%lu) is going to be in use", connection->connectionId);
  workerPool->idleConnectionCount--;
...
ReceiveResults(WorkerSession *session, bool storeRows)
{
  ...
  DBUG_PRINT("citus", "read the result of a command or query(sessionId:%lu, connectionId:%lu) and", session->sessionId, connection->connectionId);
  DBUG_PRINT("citus", "writes returned rows to the tuple store of the scan state");
  ...
  while (!PQisBusy(connection->pgConn)) {
    ...
    if (result == NULL) {
      ...
      DBUG_PRINT("citus", "no more results, break out of loop and free allocated memory");
      fetchDone = true;
      break;
    }
    ...
    if (resultStatus == PGRES_COMMAND_OK) {
      ...
      if (storeRows && *currentAffectedTupleString != '\0') {
        DBUG_PRINT("citus", "when there are multiple replicas, make sure to consider only one");
      ...
      DBUG_PRINT("citus", "task query might contain multiple queries, so fetch until we reach NULL");
       ...
       */
      DBUG_PRINT("citus", "we've already consumed all the tuples, no more results");
      ...
      DBUG_PRINT("citus", "query failures are always hard errors");
      /* query failures are always hard errors */
      ReportResultError(connection, result, ERROR);
    } else if (!storeRows) {
      /*
       ...
       */
      DBUG_PRINT("citus", "already received rows from executing on another shard placement or doesn't need at all");
      PQclear(result);
      continue;
    }
