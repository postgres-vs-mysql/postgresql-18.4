...
AssignDistributedTransactionIdCommand(void)
{
  ...
  DBUG_PRINT("citus", "command:%s", command);
  return command;
}
...
Assign2PCIdentifier(MultiConnection *connection)
{
  ...
  DBUG_PRINT("citus", "transaction number:%lu", transactionNumber);
                             ...
                             uint32 *connectionNumber)
{
  ...
  *connectionNumber = strtoul(currentCharPointer, NULL, 10);
  ...
  DBUG_PRINT("citus", "prepared transaction name:%s, transaction number:%lu", preparedTransactionName, *transactionNumber);
  return true;
}
