...
IsMultiStatementTransaction(void)
{
  ...
  if (IsTransactionBlock()) {
    ...
    DBUG_PRINT("citus", "in a BEGIN...END block");
    return true;
  } else if (DoBlockLevel > 0) {
    DBUG_PRINT("citus", "in (a transaction within) a do block");
    /* in (a transaction within) a do block */
    return true;
  } else if (StoredProcedureLevel > 0) {
    DBUG_PRINT("citus", "in (a transaction within) a stored procedure");
    /* in (a transaction within) a stored procedure */
    return true;
  } else if (MaybeExecutingUDF() && FunctionOpensTransactionBlock) {
    DBUG_PRINT("citus", "in a language-handler function call, open a transaction if configured to do so");
