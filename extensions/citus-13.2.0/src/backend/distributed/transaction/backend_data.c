...
assign_distributed_transaction_id(PG_FUNCTION_ARGS)
{
  ...
  if (MyBackendData->transactionId.transactionNumber != 0) {
    DBUG_INSTANT_PRINT("citus", "when an id is already assigned, release the lock and error");
  ...
  DBUG_PRINT("citus", "assign distributed transaction id:%lu", transactionNumber);
...
get_current_transaction_id(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "get current transaction id:%lu", distributedTransctionId->transactionNumber);
...
get_global_active_transactions(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("citus", "add active transactions for local node");
  /* add active transactions for local node */
  StoreAllActiveTransactions(tupleStore, tupleDescriptor);

  int32 localGroupId = GetLocalGroupId();

  DBUG_PRINT("citus", "open connections in parallel");
  ...
  DBUG_PRINT("citus", "send commands in parallel");
  ...
  DBUG_PRINT("citus", "receive query results");
  ...
  DBUG_PRINT("citus", "count:%d", count);
  PG_RETURN_VOID();
}
...
StoreAllActiveTransactions(Tuplestorestate *tupleStore, TupleDesc tupleDescriptor)
{
  ...
  DBUG_PRINT("citus", "we're reading all distributed transactions, prevent new backends");
  ...
  for (int backendIndex = 0; backendIndex < TotalProcCount(); ++backendIndex) {
    ...
    DBUG_PRINT("citus", "backend pid:%d, database id:%u, transaction number:%lu", backendPid, databaseId, transactionNumber);
...
TotalProcCount(void)
{
  ...
  DBUG_PRINT("citus", "total procs:%d", totalProcs);
  return totalProcs;
}
...
GetCurrentDistributedTransactionId(void)
{
  ...
  DBUG_PRINT("citus", "read the backend's distributed transaction id:%lu", currentDistributedTransactionId->transactionNumber);
...
AssignDistributedTransactionId(void)
{
  ...
  DBUG_PRINT("citus", "generate a new distributed transaction id:%lu", nextTransactionNumber);
...
SetBackendDataDatabaseId(void)
{
  ...
  SpinLockRelease(&MyBackendData->mutex);
  DBUG_PRINT("citus", "set the databaseId(%u) in the backend data", MyDatabaseId);
}
...
GetGlobalPID(void)
{
  ...
  DBUG_PRINT("citus", "global pid:%lu", globalPID);
  return globalPID;
}
...
citus_calculate_gpid(PG_FUNCTION_ARGS)
{
  ...
  uint64 result = CalculateGlobalPID(nodeId, pid);

  DBUG_PRINT("citus", "calculate the gpid:%lu for any given process(%d) on any given node(%d)", result, pid, nodeId);
...
CalculateGlobalPID(int32 nodeId, pid_t pid)
{
   ...
   */
  uint64 result = (((uint64) nodeId) * GLOBAL_PID_NODE_ID_MULTIPLIER) + pid;
  DBUG_PRINT("citus", "get a nodeId:%d and pid:%d, and return the global pid:%lu", nodeId, pid, result);
  return result;
}
...
citus_backend_gpid(PG_FUNCTION_ARGS)
{
  ...
  result = GetGlobalPID();
  DBUG_PRINT("citus", "return citus assigned gpid:%lu", result);
  PG_RETURN_UINT64(result);
}
...
citus_nodeid_for_gpid(PG_FUNCTION_ARGS)
{
  ...
  result = (ExtractNodeIdFromGlobalPID(globalPID, missingOk));

  DBUG_PRINT("citus", "return node id:%d for the global process with given global pid(%lu)", result, globalPID);
...
citus_pid_for_gpid(PG_FUNCTION_ARGS)
{
  ...
  result = (ExtractProcessIdFromGlobalPID(globalPID));
  DBUG_PRINT("citus", "return process id:%d for the global process with given global pid:%lu", result, globalPID);
  PG_RETURN_INT32(result);
}
...
ExtractGlobalPID(const char *applicationName)
{
  ...
  if (!applicationName) {
    DBUG_PRINT("citus", "return INVALID_CITUS_INTERNAL_BACKEND_GPID");
  ...
  for (int i = 0; i < lengthof(CitusBackendPrefixes); i++) {
    ...
    DBUG_PRINT("citus", "global pid:%lu for ExtractGlobalPID:'%s'", globalPID, applicationName);
    return globalPID;
  }
...
ExtractNodeIdFromGlobalPID(uint64 globalPID, bool missingOk)
{
  ...
  DBUG_PRINT("citus", "node id:%d for globalPID:%lu'", nodeId, globalPID);
  return nodeId;
}
...
{
  DBUG_TRACE;
  int result = (int) (globalPID % GLOBAL_PID_NODE_ID_MULTIPLIER);
  DBUG_PRINT("citus", "extract the process id:%d for globalPID:%lu'", result, globalPID);
  return result;
}
...
CurrentDistributedTransactionNumber(void)
{
  ...
  DBUG_PRINT("citus", "return the transaction number of the current distributed transaction:%lu", MyBackendData->transactionId.transactionNumber);
...
ActiveDistributedTransactionNumbers(void)
{
  ...
  for (int curBackend = 0; curBackend < MaxBackends; curBackend++) {
    ...
    *transactionNumber = currentBackendData.transactionId.transactionNumber;
    ...
    DBUG_PRINT("citus", "current backend index:%d, transaction number:%lu", curBackend, *transactionNumber);
...
IsCitusInternalBackend(void)
{
  ...
  result = CurrentBackendType == CITUS_INTERNAL_BACKEND;

  if (result == true) {
    DBUG_PRINT("citus", "return true if we are in a backend that citus initiated via remote connection");
  } else {
...
IsCitusRunCommandBackend(void)
{
  ...
  result = CurrentBackendType == CITUS_RUN_COMMAND_BACKEND;

  if (result == true) {
    DBUG_PRINT("citus", "return true if we are in a backend that one of the run_command_on_* functions initiated");
  } else {
    DBUG_PRINT("citus", "return false");
  }
...
IsExternalClientBackend(void)
{
  ...
  result = CurrentBackendType == EXTERNAL_CLIENT_BACKEND;
...
IsCitusShardTransferBackend(void)
{
  ...
  result = strncmp(application_name,
