...
CheckForDistributedDeadlocks(void)
{
  ...
  if (list_length(workerNodeList) == 0) {
    DBUG_PRINT("citus", "we don't need to do any distributed deadlock checking when there are no worker nodes");
    return false;
  }
  ...
  while ((transactionNode = (TransactionNode *) hash_seq_search(&status)) != 0) {
    ...
    if (deadlockFound) {
      ...
      if (youngestAliveTransaction) {
        ...
        DBUG_PRINT("citus", "we found the deadlock and its associated proc exists");
        return true;
      }
    }
  }
                                ...
                                List **deadlockPath)
{
  ...
  while (toBeVisitedNodes != NIL) {
    ...
    if (currentTransactionNode == startingTransactionNode) {
      ...
      DBUG_PRINT("citus", "cycle found, let the caller know about the cycle");
      return true;
    }
...
AssociateDistributedTransactionWithBackendProc(TransactionNode *transactionNode)
{
  ...
  DBUG_PRINT("citus", "get a transaction node and search the corresponding backend");
  ...
  for (int backendIndex = 0; backendIndex < MaxBackends; ++backendIndex) {
    ...
    if (currentProc->pid <= 0) {
      DBUG_PRINT("citus", "we're not interested in processes that are not active or waiting on a lock for backend:%d", backendIndex);
      continue;
    }
    ...
    if (!IsInDistributedTransaction(&currentBackendData)) {
      DBUG_PRINT("citus", "we're only interested in distributed transactions for backend:%d", backendIndex);
      continue;
    }
    ...
    if (currentTransactionId->transactionNumber !=
      ...
      DBUG_PRINT("citus", "current transaction number:%lu, transactionNode's transactionNumber:%lu for backend:%d",
                 currentTransactionId->transactionNumber, transactionNode->transactionId.transactionNumber, backendIndex);
      continue;
    }
    ...
    DBUG_PRINT("citus", "at the point we should only have transactions initiated by this node for backend:%d", backendIndex);
...
LogTransactionNode(TransactionNode *transactionNode)
{
  ...
  DBUG_PRINT("citus", "%s", logMessage->data);
  LogDistributedDeadlockDebugMessage(logMessage->data);
}
