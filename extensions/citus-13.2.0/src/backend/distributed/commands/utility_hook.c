...
{
  DBUG_TRACE;
  DBUG_PRINT("citus", "it is the main entry hook for implementing Citus-specific utility behavior");
  ...
  if (IsA(parsetree, CreateExtensionStmt)) {
     ...
     */
    DBUG_PRINT("citus", "postgres forbids creating/altering other extensions from within an extension script");
  ...
  if (IsA(parsetree, DropdbStmt)) {
    ...
    DBUG_PRINT("citus", "make sure that on DROP DATABASE we terminate the background daemon associated with it");
                             ...
                             QueryCompletion *completionTag)
{
  ...
  if (IsA(parsetree, ExplainStmt) &&
    ...
    if (IsTransactionBlock()) {
      ...
      if (analyze) {
         ...
         * cannot continue.
         */
        DBUG_PRINT("citus", "since we cannot execute EXPLAIN ANALYZE locally, we cannot continue");
  ...
  if (IsA(parsetree, AlterSubscriptionStmt)) {
    ...
    if (!superuser() &&
      ...
      DBUG_INSTANT_PRINT("citus", "only superusers can alter subscriptions that are created by citus");
      ereport(ERROR, (
  ...
  if (IsA(parsetree, DropSubscriptionStmt)) {
    ...
    if (!superuser() &&
      ...
      DBUG_INSTANT_PRINT("citus", "only superusers can drop subscriptions that are created by citus");
      ereport(ERROR, (
                errcode(ERRCODE_INSUFFICIENT_PRIVILEGE),
                errmsg(
  ...
  if (EnableDDLPropagation) {
    DBUG_PRINT("citus", "only generate worker DDLJobs when propagation is enabled");
  ...
  if (IsDropCitusExtensionStmt(parsetree)) {
    DBUG_PRINT("citus", "make sure that on DROP EXTENSION we terminate the background daemon associated with it");
   ...
   */
  DBUG_PRINT("citus", "post process for ddl statements");
...
IsDropSchemaOrDB(Node *parsetree)
{
  ...
  result = (dropStatement->removeType == OBJECT_SCHEMA) ||
        ...
        if (currentSearchPath != NULL) {
          commandList = lappend(commandList,
                                psprintf("SET search_path TO %s;",
                                         currentSearchPath));
          DBUG_PRINT("citus", "SET search_path TO '%s'", currentSearchPath);
        }
...
IncrementUtilityHookCountersIfNecessary(Node *parsetree)
{
  ...
  if (IsA(parsetree, AlterTableStmt)) {
    activeAlterTables++;
    DBUG_PRINT("citus", "activeAlterTables:%d", activeAlterTables);
  }

  if (IsDropSchemaOrDB(parsetree)) {
    activeDropSchemaOrDBs++;
    DBUG_PRINT("citus", "activeDropSchemaOrDBs:%d", activeDropSchemaOrDBs);
  }
}
