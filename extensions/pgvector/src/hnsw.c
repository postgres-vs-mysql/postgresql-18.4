...
HnswInitLockTranche(void)
{
  ...
  DBUG_PRINT("pgvector", "assign a tranche ID for our LWLocks");
...
HnswInit(void)
{
  ...
  DBUG_PRINT("pgvector", "initialize index options and variables");
...
hnswbuildphasename(int64 phasenum)
{
  ...
  switch (phasenum) {
    case PROGRESS_CREATEIDX_SUBPHASE_INITIALIZE:
      DBUG_PRINT("pgvector", "get the name of index build phase:initializing");
      return "initializing";

    case PROGRESS_HNSW_PHASE_LOAD:
      DBUG_PRINT("pgvector", "get the name of index build phase:loading tuples");
                 ...
                 double *indexPages)
{
  ...
  DBUG_PRINT("pgvector", "estimate the cost of an index scan");

  /* Never use index without order */
  if (path->indexorderbys == NIL) {
    DBUG_PRINT("pgvector", "never use index without order");
  ...
  if (path->indexinfo->tuples > 0) {
    ...
    double      optimal_ml_from_paper = HnswGetMl(m);
    ...
    DBUG_PRINT("pgvector", "path->indexinfo->tuples:%g, optimal ML from paper:%g", path->indexinfo->tuples, optimal_ml_from_paper);
    DBUG_PRINT("pgvector", "calculated entryLevel:%d, m:%d, layer0TuplesMax:%d, layer0Selectivity:%g, tuples:%g, ratio:%g", entryLevel, m, layer0TuplesMax,
               layer0Selectivity, path->indexinfo->tuples, ratio);
  ...
  DBUG_PRINT("pgvector", "costs.indexStartupCost = costs.indexTotalCost:%g * ratio:%g", costs.indexTotalCost, ratio);
  DBUG_PRINT("pgvector", "startup cost is cost before returning the first row:%g", costs.indexStartupCost);

  /* Adjust cost if needed since TOAST not included in seq scan cost */
  DBUG_PRINT("pgvector", "adjust cost if needed since TOAST not included in seq scan cost");
  startupPages = costs.numIndexPages * ratio;

  if (startupPages > path->indexinfo->rel->pages && ratio < 0.5) {
    DBUG_PRINT("pgvector", "startupPages:%g, path->indexinfo->rel->pages:%u", startupPages, path->indexinfo->rel->pages);
    /* Change all page cost from random to sequential */
    DBUG_PRINT("pgvector", "change all page cost from random to sequential");
    costs.indexStartupCost -= startupPages * (costs.spc_random_page_cost - spc_seq_page_cost);
    DBUG_PRINT("pgvector", "costs.spc_random_page_cost:%g, spc_seq_page_cost:%g, indexStartupCost:%g", costs.spc_random_page_cost, spc_seq_page_cost,
               costs.indexStartupCost);

    /* Remove cost of extra pages */
    costs.indexStartupCost -= (startupPages - path->indexinfo->rel->pages) * spc_seq_page_cost;
    DBUG_PRINT("pgvector", "remove cost of extra pages and indexStartupCost:%g", costs.indexStartupCost);
  }
  ...
  *indexPages = costs.numIndexPages;
  ...
  DBUG_PRINT("pgvector", "indexStartupCost:%g, indexTotalCost:%g, indexSelectivity:%g, indexCorrelation:%g, indexPages:%g",
...
hnswhandler(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgvector", "define index handler");
