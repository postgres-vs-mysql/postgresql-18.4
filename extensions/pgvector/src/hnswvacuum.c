...
RemoveHeapTids(HnswVacuumState * vacuumstate)
{
  ...
  DBUG_PRINT("pgvector", "remove deleted heap TIDs");
  ...
  if (tmp_trace_disabled) {
    ...
    DBUG_PRINT("pgvector", "...");
    DBUG_PRINT("pgvector", "similar things have been processed %lu times", count - min_trace_iterations);
...
NeedsUpdated(HnswVacuumState * vacuumstate, HnswElement element)
{
  ...
  DBUG_PRINT("pgvector", "check for deleted neighbors");
  ...
  for (int i = 0; i < ntup->count; i++) {
    ...
    if (DeletedContains(vacuumstate->deleted, indextid)) {
      DBUG_PRINT("pgvector", "check if in deleted list: true");
      needsUpdated = true;
      break;
    }
  }
...
RepairGraphElement(HnswVacuumState * vacuumstate, HnswElement element, HnswElement entryPoint)
{
  ...
  DBUG_PRINT("pgvector", "find neighbors for element, skipping itself");
  ...
  DBUG_PRINT("pgvector", "update neighbor tuple");
  /* Do this before getting page to minimize locking */
  HnswSetNeighborTuple(base, ntup, element, m);

  /* Get neighbor page */
  DBUG_PRINT("pgvector", "get neighbor page");
  buf = ReadBufferExtended(index, MAIN_FORKNUM, element->neighborPage, RBM_NORMAL, bas);
  LockBuffer(buf, BUFFER_LOCK_EXCLUSIVE);
  state = GenericXLogStart(index);
  page = GenericXLogRegisterBuffer(state, buf, 0);
...
RepairGraphEntryPoint(HnswVacuumState * vacuumstate)
{
  ...
  DBUG_PRINT("pgvector", "repair graph entry point");
...
RepairGraph(HnswVacuumState * vacuumstate)
{
  ...
  DBUG_PRINT("pgvector", "repair graph for all elements");
  /*
  ...
  DBUG_PRINT("pgvector", "repair entry point first");
  RepairGraphEntryPoint(vacuumstate);

  DBUG_PRINT("pgvector", "first blkno:%u", blkno);
  ...
  while (BlockNumberIsValid(blkno)) {
    ...
    blkno = HnswPageGetOpaque(page)->nextblkno;
    total_count++;
    DBUG_PRINT("pgvector", "next blkno:%u", blkno);
  ...
  DBUG_PRINT("pgvector", "blocks visited:%d", total_count);
}
...
MarkDeleted(HnswVacuumState * vacuumstate)
{
  ...
  DBUG_PRINT("pgvector", "mark items as deleted");
  /*
...
InitVacuumState(HnswVacuumState * vacuumstate, IndexVacuumInfo *info, IndexBulkDeleteResult *stats, IndexBulkDeleteCallback callback, void *callback_state)
{
  ...
  DBUG_PRINT("pgvector", "initialize the vacuum state");
               ...
               IndexBulkDeleteCallback callback, void *callback_state)
{
  ...
  DBUG_PRINT("pgvector", "bulk delete tuples from the index");
  InitVacuumState(&vacuumstate, info, stats, callback, callback_state);

  DBUG_PRINT("pgvector", "remove heap TIDs");
  /* Pass 1: Remove heap TIDs */
  RemoveHeapTids(&vacuumstate);

  DBUG_PRINT("pgvector", "repair graph");
  /* Pass 2: Repair graph */
  RepairGraph(&vacuumstate);

  DBUG_PRINT("pgvector", "mark as deleted");
...
hnswvacuumcleanup(IndexVacuumInfo *info, IndexBulkDeleteResult *stats)
{
  ...
  DBUG_PRINT("pgvector", "clean up after a VACUUM operation");
