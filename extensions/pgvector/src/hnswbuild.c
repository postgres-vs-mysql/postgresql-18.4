...
CreateMetaPage(HnswBuildState * buildstate)
{
  ...
  DBUG_PRINT("pgvector", "create the metapage");
...
InsertTuple(Relation index, Datum *values, bool *isnull, ItemPointer heaptid, HnswBuildState * buildstate)
{
  ...
  DBUG_PRINT("pgvector", "fForm index value");
  ...
  DBUG_PRINT("pgvector", "get datum size:%lu", valueSize);
  ...
  if (graph->flushed) {
    ...
    DBUG_PRINT("pgvector", "we are in the on-disk phase");
  ...
  LWLockInitialize(&element->lock, hnsw_lock_tranche_id, 0);
...
InitGraph(HnswGraph * graph, char *base, Size memoryTotal)
{
  ...
  LWLockInitialize(&graph->entryLock, hnsw_lock_tranche_id, 0);
  LWLockInitialize(&graph->entryWaitLock, hnsw_lock_tranche_id, 0);
  LWLockInitialize(&graph->allocatorLock, hnsw_lock_tranche_id, 0);
  LWLockInitialize(&graph->flushLock, hnsw_lock_tranche_id, 0);
}
...
{
  DBUG_TRACE;
  DBUG_PRINT("pgvector", "initialize the build state");
...
HnswParallelScanAndInsert(Relation heapRel, Relation indexRel, HnswShared * hnswshared, char *hnswarea, bool progress)
{
  ...
  DBUG_PRINT("pgvector", "perform a worker's portion of a parallel insert");
  /* Join parallel scan */
...
HnswParallelBuildMain(dsm_segment *seg, shm_toc *toc)
{
  ...
  DBUG_PRINT("pgvector", "perform work within a launched parallel process");
...
HnswBeginParallel(HnswBuildState * buildstate, bool isconcurrent, int request)
{
  ...
  DBUG_PRINT("pgvector", "begin parallel build");
...
hnswbuild(Relation heap, Relation index, IndexInfo *indexInfo)
{
  ...
  DBUG_PRINT("pgvector", "build the index for a logged table");
...
hnswbuildempty(Relation index)
{
  ...
  DBUG_PRINT("pgvector", "build the index for an unlogged table");
