...
InitCenters(Relation index, VectorArray samples, VectorArray centers, float *lowerBound)
{
  ...
  DBUG_PRINT("pgvector", "numCenters:%d, numSamples:%d", numCenters, numSamples);
...
RandomCenters(Relation index, VectorArray centers, const IvfflatTypeInfo * typeInfo)
{
  ...
  DBUG_PRINT("pgvector", "quick approach if we have no data");
...
ComputeNewCenters(VectorArray samples, float *agg, VectorArray newCenters, int *centerCounts, int *closestCenters, FmgrInfo *normprocinfo, Oid collation, const IvfflatTypeInfo * typeInfo)
{
  ...
  DBUG_PRINT("pgvector", "compute new centers(numCenters:%d, dimensions:%d)", numCenters, dimensions);
  ...
  DBUG_PRINT("pgvector", "increment sum of closest center");
  /* Increment sum of closest center */
  SumCenters(samples, agg, closestCenters, typeInfo);

  DBUG_PRINT("pgvector", "increment count of closest center(numSamples:%d)", numSamples);
...
ElkanKmeans(Relation index, VectorArray samples, VectorArray centers, const IvfflatTypeInfo * typeInfo)
{
  ...
  DBUG_PRINT("pgvector", "use Elkan for performance");
  DBUG_PRINT("pgvector", "this requires distance function to satisfy triangle inequality");
  ...
  DBUG_PRINT("pgvector", "pick initial centers");
  InitCenters(index, samples, centers, lowerBound);

  /* Assign each x to its closest initial center c(x) = argmin d(x,c) */
  DBUG_PRINT("pgvector", "assign each x to its closest initial center c(x) = argmin d(x,c)");
  DBUG_PRINT("pgvector", "numSamples:%d", numSamples);
  ...
  DBUG_PRINT("pgvector", "give 500 iterations to converge");
...
CheckElements(VectorArray centers, const IvfflatTypeInfo * typeInfo)
{
  ...
  DBUG_PRINT("pgvector", "ensure no NaN or infinite values");
...
CheckNorms(VectorArray centers, Relation index)
{
  ...
  DBUG_PRINT("pgvector", "ensure no zero vectors for cosine distance");
...
{
  DBUG_TRACE;
  DBUG_PRINT("pgvector", "detect issues with centers");
...
IvfflatKmeans(Relation index, VectorArray samples, VectorArray centers, const IvfflatTypeInfo * typeInfo)
{
  ...
  DBUG_PRINT("pgvector", "perform naive k-means centering");
  DBUG_PRINT("pgvector", "we use spherical k-means for inner product and cosine");
