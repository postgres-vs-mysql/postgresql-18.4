...
FindInsertPage(Relation index, Datum *values, BlockNumber *insertPage, ListInfo * listInfo)
{
  ...
  DBUG_PRINT("pgvector", "find the list that minimizes the distance function");
  ...
  DBUG_PRINT("pgvector", "search the first list page(blkno:%u)", nextblkno);
  ...
  while (BlockNumberIsValid(nextblkno)) {
    ...
    DBUG_PRINT("pgvector", "blkno:%u, FirstOffsetNumber:%u, maxoffno:%u", nextblkno, FirstOffsetNumber, maxoffno);
    ...
    for (OffsetNumber offno = FirstOffsetNumber; offno <= maxoffno; offno = OffsetNumberNext(offno)) {
      ...
      if (distance < minDistance || !BlockNumberIsValid(*insertPage)) {
        *insertPage = list->insertPage;
        ...
        DBUG_PRINT("pgvector", "set minimum distance: %g from page (blkno:%u), offset:%u", distance, nextblkno, offno);
      }
    }

    nextblkno = IvfflatPageGetOpaque(cpage)->nextblkno;
    DBUG_PRINT("pgvector", "search the next list page(blkno:%u)", nextblkno);
...
InsertTuple(Relation index, Datum *values, bool *isnull, ItemPointer heap_tid)
{
  ...
  DBUG_PRINT("pgvector", "find a page to insert the item");
