...
InitVector(int dim)
{
  ...
  result->dim = dim;
  DBUG_PRINT("pgvector", "dim:%d, vector size:%d", dim, size);
...
vector_in(PG_FUNCTION_ARGS)
{
  ...
  for (;;) {
    ...
    if (*pt == ',')
      ...
      DBUG_INSTANT_PRINT("pgvector", "invalid input syntax for type vector: \"%s\"", lit);
      ereport(ERROR,
              (errcode(ERRCODE_INVALID_TEXT_REPRESENTATION),
               errmsg("invalid input syntax for type vector: \"%s\"", lit)));
    }
...
vector_out(PG_FUNCTION_ARGS)
{
  ...
  *ptr = '\0';
  ...
  DBUG_PRINT("pgvector", "return '%s'", buf);
...
vector_typmod_in(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgvector", "convert type modifier(n:%d)", n);
  ...
  DBUG_PRINT("pgvector", "return reslt:%d", *tl);
  PG_RETURN_INT32(*tl);
}
...
vector(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgvector", "convert vector to vector");
...
array_to_vector(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgvector", "convert array to vector(dim:%d)", result->dim);
  PG_RETURN_POINTER(result);
}
...
vector_to_float4(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgvector", "convert vector(dim:%d) to float4[]", vec->dim);
...
VectorL2SquaredDistance(int dim, float *ax, float *bx)
{
  ...
  DBUG_PRINT("pgvector", "dim:%d, distance:%g", dim, distance);
  return distance;
}
...
l2_distance(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgvector", "get the L2 distance between vector:%g", result);
...
vector_l2_squared_distance(PG_FUNCTION_ARGS)
{
  ...
  result = (double) VectorL2SquaredDistance(a->dim, a->x, b->x);
  DBUG_PRINT("pgvector", "get the L2 squared distance between vectors:%g", result);
  PG_RETURN_FLOAT8(result);
}
...
VectorInnerProduct(int dim, float *ax, float *bx)
{
  ...
  DBUG_PRINT("pgvector", "distance:%g", distance);
  return distance;
}
...
inner_product(PG_FUNCTION_ARGS)
{
  ...
  result = (double) VectorInnerProduct(a->dim, a->x, b->x);
  DBUG_PRINT("pgvector", "get the inner product of two vectors:%g", result);
  PG_RETURN_FLOAT8(result);
}
...
vector_negative_inner_product(PG_FUNCTION_ARGS)
{
  ...
  result = (double) - VectorInnerProduct(a->dim, a->x, b->x);
  DBUG_PRINT("pgvector", "get the negative inner product of two vectors:%g", result);
  PG_RETURN_FLOAT8(result);
}
...
VectorCosineSimilarity(int dim, float *ax, float *bx)
{
  ...
  result = (double) similarity / sqrt((double) norma * (double) normb);
  DBUG_PRINT("pgvector", "cosine similarity:%g", result);
  return result;
}
...
cosine_distance(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgvector", "get the cosine distance between two vectors:%g", result);
...
vector_spherical_distance(PG_FUNCTION_ARGS)
{
  ...
  result = acos(distance) / M_PI;
  DBUG_PRINT("pgvector", "get the distance for spherical k-means:%g", result);
  PG_RETURN_FLOAT8(result);
}
...
l1_distance(PG_FUNCTION_ARGS)
{
  ...
  result = (double) VectorL1Distance(a->dim, a->x, b->x);
  DBUG_PRINT("pgvector", "get the L1 distance between two vectors:%g", result);
  PG_RETURN_FLOAT8(result);
}
...
vector_dims(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgvector", "get the dimensions of a vector:%d", a->dim);
...
vector_norm(PG_FUNCTION_ARGS)
{
  ...
  result = sqrt(norm);
  DBUG_PRINT("pgvector", "get the L2 norm of a vector:%g", result);
  PG_RETURN_FLOAT8(result);
}
...
l2_normalize(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("pgvector", "normalize a vector(dim:%d) with the L2 norm(%g)", a->dim, norm);
...
vector_lt(PG_FUNCTION_ARGS)
{
  ...
  Vector     *a = PG_GETARG_VECTOR_P(0);
  Vector     *b = PG_GETARG_VECTOR_P(1);
  bool result = vector_cmp_internal(a, b) < 0;
...
vector_cmp(PG_FUNCTION_ARGS)
{
  ...
  int32 result = vector_cmp_internal(a, b);

  DBUG_PRINT("pgvector", "return result:%d", result);
