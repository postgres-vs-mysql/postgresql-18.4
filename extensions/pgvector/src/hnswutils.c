...
HnswGetMetaPageInfo(Relation index, int *m, HnswElement * entryPoint)
{
  ...
  DBUG_PRINT("pgvector", "get the metapage info");
  ...
  if (entryPoint != NULL) {
    if (BlockNumberIsValid(metap->entryBlkno)) {
      *entryPoint = HnswInitElementFromBlock(metap->entryBlkno, metap->entryOffno);
      ...
      DBUG_PRINT("pgvector", "entry blkno:%u, level:%d", metap->entryBlkno, (*entryPoint)->level);
    } else
      *entryPoint = NULL;
  }
...
HnswLoadElementFromTuple(HnswElement element, HnswElementTuple etup, bool loadHeaptids, bool loadVec)
{
  ...
  DBUG_PRINT("pgvector", "load an element from a tuple and add heap TIDs(count:%d) to an element", count);
...
HnswGetDistance(Datum a, Datum b, HnswSupport * support)
{
  DBUG_TRACE;
  double result;
  result = DatumGetFloat8(FunctionCall2Coll(support->procinfo, support->collation, a, b));
  DBUG_PRINT("pgvector", "calculate the distance between values:%g", result);
  return result;
}
...
HnswLoadElementImpl(BlockNumber blkno, OffsetNumber offno, double *distance, HnswQuery * q, Relation index, HnswSupport * support, bool loadVec, double *maxDistance, HnswElement * element)
{
  ...
  DBUG_PRINT("pgvector", "load an element(blkno:%u, offset:%u) and optionally get its distance from q", blkno, offno);
  /* Read vector */
  ...
  if (distance != NULL) {
    ...
    if (maxDistance) {
      DBUG_PRINT("pgvector", "calculate distance:%g, maxDistance:%g", *distance, *maxDistance);
    } else {
      DBUG_PRINT("pgvector", "calculate distance:%g", *distance);
    }
  }
  ...
  if (distance == NULL || maxDistance == NULL || *distance < *maxDistance) {
    ...
    if (distance && maxDistance) {
      if (*distance > *maxDistance) {
        DBUG_PRINT("pgvector", "distance(%g) > maxDistance(%g)", *distance, *maxDistance);
      }
    } else {
      if (maxDistance) {
        DBUG_PRINT("pgvector", "maxDistance is nullptr");
      }
    }
  }
...
HnswEntryCandidate(char *base, HnswElement entryPoint, HnswQuery * q, Relation index, HnswSupport * support, bool loadVec)
{
  ...
  DBUG_PRINT("pgvector", "create a candidate for the entry point");

  if (inMemory) {
    distance = GetElementDistance(base, entryPoint, q, support);
    DBUG_PRINT("pgvector", "get the distance:%g", distance);
  } else {
...
HnswSearchLayer(char *base, HnswQuery * q, List *ep, int ef, int lc, Relation index, HnswSupport * support, int m, bool inserting, HnswElement skipElement, visited_hash * v, pairingheap **discarded, bool initVisited, int64 *tuples)
{
  ...
  bool    inMemory = index == NULL;
  int total_w = 0;
  int total_heavy_comparisons = 0;
  int total_comparisons = 0;

  DBUG_PRINT("pgvector", "algorithm 2 from paper(lc:%d, m:%d, lm:%d)", lc, m, lm);
  ...
  if (inMemory) {
    ...
    DBUG_PRINT("pgvector", "create local memory for neighborhood(neighborhoodSize:%lu)", neighborhoodSize);
    localNeighborhood = palloc(neighborhoodSize);
  } else {
    DBUG_PRINT("pgvector", "inMemory is false");
  }

  /* Add entry points to v, C, and W */
  DBUG_PRINT("pgvector", "add entry points to v, C, and W");
  ...
  while (!pairingheap_is_empty(C)) {
    ...
    if (c->distance > f->distance) {
      DBUG_PRINT("pgvector", "break if c->distance (%g) > f->distance (%g)", c->distance, f->distance);
      break;
    } else {
      DBUG_PRINT("pgvector", "c->distance (%g), f->distance (%g)", c->distance, f->distance);
    }
    ...
    DBUG_PRINT("pgvector", "unvisitedLength:%d for level:%d", unvisitedLength, lc);
    ...
    for (int i = 0; i < unvisitedLength; i++) {
      ...
      DBUG_PRINT("pgvector", "get a search candidate(distance:%g) as the basis for comparison for i:%d", f->distance, i);
      ...
      if (inMemory) {
        ...
        DBUG_PRINT("pgvector", "get the distance:%g", eDistance);
      } else {
        ...
        if (eElement == NULL) {
          DBUG_PRINT("pgvector", "eElement is nullptr, skipping to next comparison");
          continue;
        }
      }

      total_heavy_comparisons++;
      DBUG_PRINT("pgvector", "eDistance:%g, f->distance:%g", eDistance, f->distance);

      if (!(eDistance < f->distance || alwaysAdd)) {
        if (discarded != NULL) {
          /* Create a new candidate */
          DBUG_PRINT("pgvector", "create a new candidate(eDistance:%g, f->distance:%g)", eDistance, f->distance);
      ...
      /* Create a new candidate */
      DBUG_PRINT("pgvector", "create a new candidate(eDistance:%g)", eDistance);
      e = HnswInitSearchCandidate(base, eElement, eDistance);
  ...
  while (!pairingheap_is_empty(W)) {
    ...
    DBUG_PRINT("pgvector", "index:%d, distance:%g", total_w, sc->distance);
    total_w++;
    w = lappend(w, sc);
  }

  DBUG_PRINT("pgvector", "add each element of W(total:%d) to w", total_w);
  DBUG_PRINT("pgvector", "total lightweight comparisons for level %d: %d", lc, total_comparisons);
  DBUG_PRINT("pgvector", "total heavy comparisons for level %d: %d", lc, total_heavy_comparisons);
  return w;
}
...
CheckElementCloser(char *base, HnswCandidate * e, List *r, HnswSupport * support)
{
  ...
  foreach(lc2, r) {
    ...
    if (distance <= e->distance) {
      DBUG_PRINT("pgvector", "return false");
      return false;
    }
  }

  DBUG_PRINT("pgvector", "return true");
  return true;
}
...
SelectNeighbors(char *base, List *c, int lm, HnswSupport * support, bool *closerSet, HnswCandidate * newCandidate, HnswCandidate * *pruned, bool sortCandidates)
{
  ...
  DBUG_PRINT("pgvector", "algorithm 4 from paper(lm:%d)", lm);

  if (list_length(w) <= lm) {
    DBUG_PRINT("pgvector", "list_length(w):%u <= lm:%d", list_length(w), lm);
    return w;
  }
  ...
  if (sortCandidates) {
    DBUG_PRINT("pgvector", "ensure order of candidates is deterministic for closer caching");
  ...
  DBUG_PRINT("pgvector", "list_length(w):%u, list_length(r):%u, lm:%d", list_length(w), list_length(r), lm);
  ...
  while (list_length(w) > 0 && list_length(r) < lm) {
    ...
    if (mustCalculate) {
      DBUG_PRINT("pgvector", "use previous state of r and wd to skip work when possible");
...
HnswFindElementNeighbors(char *base, HnswElement element, HnswElement entryPoint, Relation index, HnswSupport * support, int m, int efConstruction, bool existing)
{
  ...
  DBUG_PRINT("pgvector", "algorithm 1 from paper(m:%d)", m);
  ...
  if (entryPoint == NULL) {
    DBUG_PRINT("pgvector", "no neighbors if no entry point");
    return;
  }
  ...
  DBUG_PRINT("pgvector", "1st phase: greedy search to insert level(entryLevel:%d, level:%d)", entryLevel, level);
  ...
  /* 2nd phase */
  DBUG_PRINT("pgvector", "2nd phase (level:%d)", level);
  ...
  for (int lc = level; lc >= 0; lc--) {
    ...
    DBUG_PRINT("pgvector", "convert search candidates to candidates");
    ...
    if (!inMemory) {
      DBUG_PRINT("pgvector", "elements being deleted or skipped can help with search");
      DBUG_PRINT("pgvector", "but should be removed before selecting neighbors");
...
HnswGetTypeInfo(Relation index)
{
  ...
  if (procinfo == NULL) {
    ...
    DBUG_PRINT("pgvector", "return default type(maxDimensions:%d, l2_normalize)", HNSW_MAX_DIM);
    return (&typeInfo);
  } else
