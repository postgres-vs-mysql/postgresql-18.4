...
inline static void print_gserialized_info(const GSERIALIZED *g, const char *extra) {
  if (gserialized_has_bbox(g)) {
    GBOX gbox;
    DBUG_PRINT("postgis", "check if %s has a bounding box without deserializing first ? Yes", extra);
    gserialized_get_gbox_p(g, &gbox); 
    DBUG_PRINT("postgis", "xmin:%g, xmax:%g, ymin:%g, ymax:%g",
         gbox.xmin, gbox.xmax, gbox.ymin, gbox.ymax);
  } else {
    DBUG_PRINT("postgis", "check if %s has a bounding box without deserializing first ? No", extra);
  }
}
...
Datum ST_Intersects(PG_FUNCTION_ARGS)
{
  ...
  if ( gserialized_is_empty(geom1) || gserialized_is_empty(geom2) ) {
    DBUG_PRINT("postgis", "A.Intersects(Empty) == FALSE");
    DBUG_PRINT("postgis", "ST_Intersects returns false");
  ...
  if ( gserialized_get_gbox_p(geom1, &box1) &&
    ...
    if ( gbox_overlaps_2d(&box1, &box2) == LW_FALSE ) {
      DBUG_PRINT("postgis", "if geom2 bounding box does not overlap geom1 bounding box we can return FALSE");
      DBUG_PRINT("postgis", "ST_Intersects returns false");
      PG_RETURN_BOOL(false);
    }
  }
  ...
  if ((is_point(geom1) && is_poly(geom2)) ||
    ...
    lwgeom_free(lwpt);
    DBUG_PRINT("postgis", "if the geoms are a point and a polygon, call the itree_pip_intersects function");
      
    if (result) {
      DBUG_PRINT("postgis", "ST_Intersects returns true");
    } else {
      DBUG_PRINT("postgis", "ST_Intersects returns false");
    }
...
Datum ST_Equals(PG_FUNCTION_ARGS)
{
  ...
  /* Empty == Empty */
  if ( gserialized_is_empty(geom1) && gserialized_is_empty(geom2) ) {
    DBUG_PRINT("postgis", "empty == empty");
    DBUG_PRINT("postgis", "ST_Equals returns true");
    PG_RETURN_BOOL(true);
  }
  ...
  if ( gserialized_get_gbox_p(geom1, &box1) &&
    ...
    if ( gbox_same_2d_float(&box1, &box2) == LW_FALSE ) {
      DBUG_PRINT("postgis", "if geom1 and geom2 do not have the same bounding box we can return FALSE");
      DBUG_PRINT("postgis", "ST_Equals returns false");
      PG_RETURN_BOOL(false);
    }
  }
  ...
  if (VARSIZE(geom1) == VARSIZE(geom2) && !memcmp(geom1, geom2, VARSIZE(geom1))) {
    DBUG_PRINT("postgis", "if geom1 and geom2 are binary-equivalent, we can return TRUE");
    DBUG_PRINT("postgis", "ST_Equals returns true");
    PG_RETURN_BOOL(true);
  }
...
Datum touches(PG_FUNCTION_ARGS)
{
  ...
  if ( gserialized_is_empty(geom1) || gserialized_is_empty(geom2) ) {
    DBUG_PRINT("postgis", "A.Touches(Empty) == FALSE");
    DBUG_PRINT("postgis", "touches returns false");
    PG_RETURN_BOOL(false);
  }
  ...
  if (result) {
    DBUG_PRINT("postgis", "touches returns true");
  } else {
    DBUG_PRINT("postgis", "touches returns false");
  }
  PG_RETURN_BOOL(result);
...
Datum disjoint(PG_FUNCTION_ARGS)
{
  ...
  if ( gserialized_is_empty(geom1) || gserialized_is_empty(geom2) ) {
    DBUG_PRINT("postgis", " A.Disjoint(Empty) == TRUE");
    DBUG_PRINT("postgis", "disjoint returns true");
    PG_RETURN_BOOL(true);
  }
  ...
  if ( gserialized_get_gbox_p(geom1, &box1) &&
       gserialized_get_gbox_p(geom2, &box2) ) {
    if ( gbox_overlaps_2d(&box1, &box2) == LW_FALSE ) {
      DBUG_PRINT("postgis", "if geom2 bounding box does not overlap geom1 bounding box we can return TRUE");
      DBUG_PRINT("postgis", "disjoint returns true");
      PG_RETURN_BOOL(true);
    }
  }
  ...
  if (result) {
    ...
    DBUG_PRINT("postgis", "disjoint returns false");
  }
  PG_RETURN_BOOL(result);
}
...
Datum overlaps(PG_FUNCTION_ARGS)
{
  ...
  if ( gserialized_is_empty(geom1) || gserialized_is_empty(geom2) ) {
    DBUG_PRINT("postgis", "A.Overlaps(Empty) == FALSE");
    DBUG_PRINT("postgis", "ST_Overlaps returns false");
    PG_RETURN_BOOL(false);
  }
  ...
  if (result) {
    DBUG_PRINT("postgis", "ST_Overlaps returns true");
  } else {
    DBUG_PRINT("postgis", "ST_Overlaps returns false");
  }
  PG_RETURN_BOOL(result);
...
Datum crosses(PG_FUNCTION_ARGS)
{
  ...
  if ( gserialized_is_empty(geom1) || gserialized_is_empty(geom2) ) {
    DBUG_PRINT("postgis", "A.Crosses(Empty) == FALSE");
    DBUG_PRINT("postgis", "ST_Crosses returns false");
    PG_RETURN_BOOL(false);
  }
  ...
  if (result) {
    DBUG_PRINT("postgis", "ST_Crosses returns true");
  } else {
    DBUG_PRINT("postgis", "ST_Crosses returns false");
  }
  PG_RETURN_BOOL(result);
...
Datum contains(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_is_empty(geom1) || gserialized_is_empty(geom2)) {
    DBUG_PRINT("postgis", "A.Contains(Empty) == FALSE");
    DBUG_PRINT("postgis", "ST_Contains returns false");
    PG_RETURN_BOOL(false);
  }

  POSTGIS_DEBUGF(3, "Contains: type1: %d, type2: %d", gserialized_get_type(geom1), gserialized_get_type(geom2));
  DBUG_PRINT("postgis", "Contains: type1: %d, type2: %d", gserialized_get_type(geom1), gserialized_get_type(geom2));
  ...
  */
  if (gserialized_get_gbox_p(geom1, &box1) &&
    ...
    if (!gbox_contains_2d(&box1, &box2)) {
      DBUG_PRINT("postgis", "if geom2 bounding box is not completely inside geom1 bounding box we can return FALSE");
      DBUG_PRINT("postgis", "ST_Contains returns false");
      PG_RETURN_BOOL(false);
    }
  }
  ...
  */
  if (is_poly(geom1) && is_point(geom2)) {
    ...
    lwgeom_free(lwpt);
    DBUG_PRINT("postgis", "if geom2 is a point and geom1 is a polygon call the point-in-polygon function");
    if (result) {
      DBUG_PRINT("postgis", "ST_Contains returns true");
    } else {
      DBUG_PRINT("postgis", "ST_Contains returns false");
    }
    PG_RETURN_BOOL(result);
...
Datum within(PG_FUNCTION_ARGS)
{
  ...
  /* A.Contains(Empty) == FALSE */
  if (gserialized_is_empty(geom1) || gserialized_is_empty(geom2)) {
    DBUG_PRINT("postgis", "ST_Within returns false");
    PG_RETURN_BOOL(false);
  }
  ...
  */
  if (gserialized_get_gbox_p(geom1, &box1) &&
    ...
    if (!gbox_within_2d(&box1, &box2)) {
      DBUG_PRINT("postgis", "if geom1 bounding box is not completely inside geom2 bounding box we can return FALSE");
      DBUG_PRINT("postgis", "ST_Within returns false");
      PG_RETURN_BOOL(false);
    }
  }
  ...
  */
  if (is_poly(geom2) && is_point(geom1)) {
    ...
    bool result = itree_pip_contains(itree, lwpt);
    lwgeom_free(lwpt);
    DBUG_PRINT("postgis", "if geom2 is a polygon and geom1 is a point call the point-in-polygon function");
    if (result) {
      DBUG_PRINT("postgis", "ST_Within returns true");
    } else {
      DBUG_PRINT("postgis", "ST_Within returns false");
    }
    PG_RETURN_BOOL(result);
...
Datum containsproperly(PG_FUNCTION_ARGS)
{
  ...
  if ( gserialized_is_empty(geom1) || gserialized_is_empty(geom2) ) {
    DBUG_PRINT("postgis", "A.ContainsProperly(Empty) == FALSE");
    DBUG_PRINT("postgis", "ST_ContainsProperly returns false");
    PG_RETURN_BOOL(false);
  }
  ...
  if (result) {
    DBUG_PRINT("postgis", "ST_ContainsProperly returns true");
  } else {
    DBUG_PRINT("postgis", "ST_ContainsProperly returns false");
  }
  PG_RETURN_BOOL(result);
...
Datum covers(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "covers: type1: %d, type2: %d", gserialized_get_type(geom1), gserialized_get_type(geom2));

  /* A.Covers(Empty) == FALSE */
  if ( gserialized_is_empty(geom1) || gserialized_is_empty(geom2) ) {
    DBUG_PRINT("postgis", "ST_Covers returns false");
    PG_RETURN_BOOL(false);
  }
  ...
  if (is_poly(geom1) && is_point(geom2)) {
    ...
    DBUG_PRINT("postgis", "if geom2 is a point and geom1 is a polygon call the point-in-polygon function");
    if (result) {
      DBUG_PRINT("postgis", "ST_Covers returns true");
    } else {
      DBUG_PRINT("postgis", "ST_Covers returns false");
    }
    PG_RETURN_BOOL(result);
...
Datum coveredby(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "CoveredBy: type1: %d, type2: %d", gserialized_get_type(geom1), gserialized_get_type(geom2));

  /* A.CoveredBy(Empty) == FALSE */
  if ( gserialized_is_empty(geom1) || gserialized_is_empty(geom2) ) {
    DBUG_PRINT("postgis", "ST_CoveredBy returns false");
    PG_RETURN_BOOL(false);
  }
  ...
  if (is_point(geom1) && is_poly(geom2)) {
    ...
    bool result = itree_pip_covers(itree, lwpt);
    lwgeom_free(lwpt);
    DBUG_PRINT("postgis", "if geom1 is a point and geom2 is a polygon call the point-in-polygon function");

    if (result) {
      DBUG_PRINT("postgis", "ST_CoveredBy returns true");
    } else {
      DBUG_PRINT("postgis", "ST_CoveredBy returns false");
    }
    PG_RETURN_BOOL(result);
...
Datum relate_pattern(PG_FUNCTION_ARGS)
{
  ...
  */
  ...
  if (result) {
    DBUG_PRINT("postgis", "relate_pattern returns false");
  } else {
    DBUG_PRINT("postgis", "relate_pattern returns false");
  }
  PG_RETURN_BOOL(result);
...
Datum relate_full(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "%s", GEOSGeomToWKT(g1));
  POSTGIS_DEBUGF(3, "%s", GEOSGeomToWKT(g2));
  DBUG_PRINT("postgis", "%s", GEOSGeomToWKT(g2));
...
Datum LWGEOM_dwithin(PG_FUNCTION_ARGS)
{
  ...
  print_gserialized_info(geom1, "geom1");
  print_gserialized_info(geom2, "geom2"); 
  /*
  ...
  if (gserialized_is_empty(geom1) || gserialized_is_empty(geom2)) {
    DBUG_PRINT("postgis", "LWGEOM_dwithin returns false");
    PG_RETURN_BOOL(false);
  }
   ...
   */
  if (use_prepared) {
    DBUG_PRINT("postgis", "only enter GEOS/PreparedGeometry code line if objects are big enough and cache is populated");
    initGEOS(lwpgnotice, lwgeom_geos_error);
    prep_cache = GetPrepGeomCache(fcinfo, shared_geom1, shared_geom2);
    ...
    if (prep_cache && prep_cache->prepared_geom) {
      ...
      DBUG_PRINT("postgis", "is_dwithin:%d, tolerance:%g", is_dwithin, tolerance);
  ...
  if (is_dwithin < 0) {
    ...
    DBUG_PRINT("postgis", "use the brute force implementation(tolerance:%g, mindist:%g)", tolerance, mindist);
  }

  if (is_dwithin) {
    DBUG_PRINT("postgis", "LWGEOM_dwithin returns true");
  } else {
    DBUG_PRINT("postgis", "LWGEOM_dwithin returns false");
  }
