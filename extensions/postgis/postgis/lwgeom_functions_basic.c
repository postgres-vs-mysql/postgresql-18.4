...
Datum LWGEOM_mem_size(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "find the size of geometry:%lu", size);
...
Datum ST_Distance(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "minimum 2d distance between objects in geom1 and geom2:%g", mindist);
  lwgeom_free(lwgeom1);
  lwgeom_free(lwgeom2);
...
Datum ST_3DDistance(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "minimum 2d distance between objects in geom1 and geom2 in 3D:%g", mindist);
  PG_FREE_IF_COPY(geom1, 0);
  PG_FREE_IF_COPY(geom2, 1);
...
Datum ST_3DIntersects(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "intersects3d through dwithin:%g", mindist);
...
Datum LWGEOM_dwithin3d(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "minimum 3d distance between objects in geom1 and geom2:%g", mindist);

  PG_FREE_IF_COPY(geom1, 0);
  PG_FREE_IF_COPY(geom2, 1);

  if (tolerance >= mindist) {
    DBUG_PRINT("postgis", "return true if minimum 3d distance is shorter than tolerance");
  } else {
    DBUG_PRINT("postgis", "return false if mindist > tolerance");
  }
...
Datum LWGEOM_dfullywithin3d(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "maximum 3d distance between objects in geom1 and geom2:%g", maxdist);
...
Datum LWGEOM_collect(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", " outtype = %d", outtype);
...
Datum LWGEOM_collect_garray(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "LWGEOM_collect_garray: array has %d elements", nelems);
  ...
  while (array_iterate(iterator, &value, &isnull)) {
    ...
    DBUG_PRINT("postgis", "%s: geom %d deserialized", __func__, count);
  ...
  DBUG_PRINT("postgis", "LWGEOM_collect_garray: outtype = %d", outtype);
...
Datum LWGEOM_makeline_garray(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "%s called", __func__);
  ...
  DBUG_PRINT("postgis", "%s: array has %d elements", __func__, nelems);
  ...
  while (array_iterate(iterator, &value, &isnull)) {
    ...
    DBUG_PRINT("postgis", "%s: element %d deserialized", __func__, ngeoms);
  }
  ...
  DBUG_PRINT("postgis", "LWGEOM_makeline_garray: elements: %d", ngeoms);
...
#pragma GCC diagnostic pop
  ...
  DBUG_PRINT("postgis", "%s", lwgeom_summary((LWGEOM *)outpoly, 0));
...
Datum LWGEOM_expand(PG_FUNCTION_ARGS)
{
  ...
  if (lwgeom_is_empty(lwgeom)) {
    DBUG_PRINT("postgis", "can't expand an empty");
    lwgeom_free(lwgeom);
    PG_RETURN_POINTER(geom);
  }

  /* Can't expand something with no gbox! */
  DBUG_PRINT("postgis", "calculate the gbox for this geometry, a cartesian box or geodetic box, depending on how it is flagged");
  ...
  if (PG_NARGS() == 2) {
    ...
    DBUG_PRINT("postgis", "expand the box the same amount in all directions");
    gbox_expand(&gbox, d);
  } else {
  ...
  /* Construct GSERIALIZED  */
  DBUG_PRINT("postgis", "construct GSERIALIZED");
...
Datum LWGEOM_to_BOX(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "convert geometry to BOX");
  /* Zero out flags */
  gbox_init(&gbox);

  /* Calculate the GBOX of the geometry */
  DBUG_PRINT("postgis", "calculate the GBOX of the geometry");
...
Datum LWGEOM_envelope(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "create new geometry of type polygon, 1 ring, 5 points");
  ...
  if (lwgeom_calculate_gbox(lwgeom, &box) == LW_FAILURE) {
    DBUG_PRINT("postgis", "must be the EMPTY geometry");
    /* must be the EMPTY geometry */
    PG_RETURN_POINTER(geom);
  }
  ...
  DBUG_PRINT("postgis", "alter envelope type so that a valid geometry is always returned depending upon the size of the geometry");

  if ((box.xmin == box.xmax) && (box.ymin == box.ymax)) {
    /* Construct and serialize point */
    LWPOINT *point = lwpoint_make2d(srid, box.xmin, box.ymin);
    DBUG_PRINT("postgis", "construct and serialize point");
    result = geometry_serialize(lwpoint_as_lwgeom(point));
    lwpoint_free(point);
  } else if ((box.xmin == box.xmax) || (box.ymin == box.ymax)) {
    LWLINE *line;
    /* Construct point array */
    DBUG_PRINT("postgis", "construct point array");
    pa = ptarray_construct_empty(0, 0, 2);

    /* Assign coordinates to POINT2D array */
    DBUG_PRINT("postgis", "assign coordinates to POINT2D array");
    pt.x = box.xmin;
    pt.y = box.ymin;
    ...
    DBUG_PRINT("postgis", "construct and serialize linestring");
    ...
    /* Construct polygon  */
    DBUG_PRINT("postgis", "construct polygon");
...
Datum ST_Normalize(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "Deserialized: %s", lwgeom_summary(lwgeom_in, 0));

  lwgeom_out = lwgeom_normalize(lwgeom_in);
  POSTGIS_DEBUGF(3, "Normalized: %s", lwgeom_summary(lwgeom_out, 0));
  DBUG_PRINT("postgis", "Normalized: %s", lwgeom_summary(lwgeom_out, 0));
...
Datum ST_TileEnvelope(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "boundsWidth:%g, boundsHeight:%g", boundsWidth, boundsHeight);
  ...
  DBUG_PRINT("postgis", "tileGeoSizeX:%g, tileGeoSizeY:%3f", tileGeoSizeX, tileGeoSizeY);
  ...
  DBUG_PRINT("postgis", "clip the final tile bounds to the bounds of the tile plane");
  ...
  DBUG_PRINT("postgis", "x1:%g, x2:%g, y1:%g, y2:%g", x1, x2, y1, y2);
  PG_RETURN_POINTER(
    geometry_serialize(
      lwpoly_as_lwgeom(
