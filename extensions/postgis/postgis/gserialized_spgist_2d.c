  ...
  for (quadrant = 0; quadrant < (uint8)in->nNodes; quadrant++) {
    ...
    for (i = 0; i < in->nkeys; i++) {
      ...
      if (DatumGetPointer(query) == NULL) {
        DBUG_PRINT("postgis", "[SPGIST] null query pointer (!?!), returning false");
      ...
      if (gserialized_datum_get_box2df_p(query, &query_gbox_index) == LW_FAILURE) {
        DBUG_PRINT("postgis", "[SPGIST] null query_gbox_index, returning false");
...
PGDLLEXPORT Datum gserialized_spgist_leaf_consistent_2d(PG_FUNCTION_ARGS)
{
  ...
  if (key == NULL) {
    DBUG_PRINT("postgis", "[SPGIST] null index entry, returning false");
...
PGDLLEXPORT Datum gserialized_spgist_compress_2d(PG_FUNCTION_ARGS)
{
  ...
  if (result == LW_FAILURE) {
    ...
    DBUG_PRINT("postgis", "[SPGIST] empty geometry!");
  ...
  if ((!isfinite(bbox_out->xmax) || !isfinite(bbox_out->xmin)) ||
    ...
    DBUG_PRINT("postgis", "[SPGIST] infinite geometry!");
