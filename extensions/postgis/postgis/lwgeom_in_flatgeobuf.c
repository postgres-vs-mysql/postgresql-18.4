...
Datum pgis_tablefromflatgeobuf(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "bytea data size is %lu", ctx->ctx->size);
  ...
  DBUG_PRINT("postgis", "found %d columns", ctx->ctx->columns_size);
  ...
  DBUG_PRINT("postgis", "sql: %s", sql.data);
...
Datum pgis_fromflatgeobuf(PG_FUNCTION_ARGS)
{
  ...
  if (SRF_IS_FIRSTCALL()) {
    ...
    DBUG_PRINT("postgis", "VARSIZE_ANY_EXHDR %lu", ctx->ctx->size);
    ...
    DBUG_PRINT("postgis", "header decoded now at offset %lu", ctx->ctx->offset);

    if (ctx->ctx->size == ctx->ctx->offset) {
      POSTGIS_DEBUGF(2, "no feature data offset %lld", ctx->ctx->offset);
      DBUG_PRINT("postgis", "no feature data offset %lu", ctx->ctx->offset);
      MemoryContextSwitchTo(oldcontext);
      SRF_RETURN_DONE(funcctx);
    }
