...
Datum postgis_index_supportfn(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "node's type:%d", nodeTag(rawreq));
  ...
  if (IsA(rawreq, SupportRequestSelectivity)) {
    ...
    if (req->is_join) {
      ...
      DBUG_PRINT("postgis", "is this a join? true");
      DBUG_PRINT("postgis", "%s: got selectivity %g", __func__, req->selectivity);
    } else {
      req->selectivity = gserialized_sel_internal(req->root, req->args, req->varRelid, 2);
      DBUG_PRINT("postgis", "is this a join? false");
      DBUG_PRINT("postgis", "%s: got selectivity %g", __func__, req->selectivity);
    }
    ...
    DBUG_PRINT("postgis", "rawreq is not a SupportRequestSelectivity");
  }
  ...
  */
  if (IsA(rawreq, SupportRequestIndexCondition)) {
    ...
    if (is_funcclause(req->node)) { /* ST_Something() */
      ...
      if (needsSpatialIndex(funcid, &idxfn)) {
        ...
        */
        ...
        DBUG_PRINT("postgis", "set the lossy field on the SupportRequestIndexCondition parameter");
        DBUG_PRINT("postgis", "to indicate that the index alone is not sufficient to evaluate the condition");
    ...
    DBUG_PRINT("postgis", "rawreq is not a SupportRequestIndexCondition");
  }
