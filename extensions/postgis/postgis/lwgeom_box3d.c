...
Datum BOX3D_to_LWGEOM(PG_FUNCTION_ARGS)
{
  ...
  /* BOX3D is a polygon in the X plane */
    ...
    points[0] = (POINT4D) {
      box->xmin, box->ymin, box->zmin, 0.0
    };
    points[1] = (POINT4D) {
...
Datum BOX3D_combine(PG_FUNCTION_ARGS)
{
  ...
  if (!box && !geom) {
    DBUG_PRINT("postgis", "can't do anything with null inputs");
    PG_RETURN_NULL();
  }
  /* Null geometry but non-null box, return the box */
  else if (!geom) {
    DBUG_PRINT("postgis", "null geometry but non-null box, return the box");
   ...
   */
  DBUG_PRINT("postgis", "deserialize geometry and calculate the box");
  ...
  if (rv == LW_FAILURE) {
    ...
    DBUG_PRINT("postgis", "if we couldn't calculate the box, return what we know");
  ...
  if (!box) {
    PG_FREE_IF_COPY(geom, 1);
    DBUG_PRINT("postgis", "null box and non-null geometry, just return the geometry box");
    result = box3d_from_gbox(&gbox);
    result->srid = srid;
    PG_RETURN_POINTER(result);
  }
