...
static void geohash_lwpgerror(char *msg, __attribute__((__unused__)) int error_code)
{
  ...
  DBUG_PRINT("postgis", "ST_Box2dFromGeoHash ERROR %i", error_code);
  lwpgerror("%s", msg);
}
...
parse_geohash(char *geohash, int precision)
{
  ...
  DBUG_PRINT("postgis", "ST_Box2dFromGeoHash sw: %.20f, %.20f", lon[0], lat[0]);
  POSTGIS_DEBUGF(2, "ST_Box2dFromGeoHash ne: %.20f, %.20f", lon[1], lat[1]);
  DBUG_PRINT("postgis", "ST_Box2dFromGeoHash ne: %.20f, %.20f", lon[1], lat[1]);
