...
static void inspect_table(struct flatgeobuf_agg_ctx *ctx)
{
  ...
  for (int i = 0; i < natts; i++) {
    ...
    DBUG_PRINT("postgis", "inspecting column definition for %s with oid %d", key, typoid);
    ...
    DBUG_PRINT("postgis", "creating column definition for %s with oid %d", key, typoid);
...
static void ensure_properties_size(struct flatgeobuf_agg_ctx *ctx, size_t size)
{
  ...
  if (ctx->ctx->properties_size == 0) {
    ...
    DBUG_PRINT("postgis", "flatgeobuf: properties buffer to size %d", ctx->ctx->properties_size);
  ...
  if (ctx->ctx->properties_size < size) {
    ...
    DBUG_PRINT("postgis", "flatgeobuf: reallocating properties buffer to size %d", ctx->ctx->properties_size);
...
static void ensure_items_len(struct flatgeobuf_agg_ctx *ctx)
{
  ...
  if (ctx->ctx->items_len < (ctx->ctx->features_count + 1)) {
    ...
    DBUG_PRINT("postgis", "flatgeobuf: reallocating items to len %lu", ctx->ctx->items_len);
...
static void encode_properties(flatgeobuf_agg_ctx *ctx)
{
  ...
  if (offset > 0) {
    ...
    DBUG_PRINT("postgis", "offset %ld", offset);
...
static void decode_properties(struct flatgeobuf_decode_ctx *ctx, Datum *values, bool *isnull)
{
  ...
  DBUG_PRINT("postgis", "flatgeobuf: decode_properties from byte array with length %d at offset %d", size, offset);
...
void flatgeobuf_decode_row(struct flatgeobuf_decode_ctx *ctx)
{
  ...
  DBUG_PRINT("postgis", "fid now %d", ctx->fid);

  if (ctx->ctx->offset == ctx->ctx->size) {
    POSTGIS_DEBUGF(3, "reached end at %lld", ctx->ctx->offset);
    DBUG_PRINT("postgis", "reached end at %lu", ctx->ctx->offset);
    ctx->done = true;
  }
}
...
uint8_t *flatgeobuf_agg_finalfn(struct flatgeobuf_agg_ctx *ctx)
{
  ...
  DBUG_PRINT("postgis", "called at offset %lu", ctx->ctx->offset);
