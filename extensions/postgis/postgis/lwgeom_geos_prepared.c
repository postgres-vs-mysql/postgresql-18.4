...
PreparedCacheDelete(void *ptr)
{
  ...
  DBUG_PRINT("postgis", "deleting geom object (%p) and prepared geom object (%p) with MemoryContext key (%p)", pghe->geom, pghe->prepared_geom, context);
...
PrepGeomCacheCleaner(GeomCache *cache)
{
  ...
  */
  ...
  DBUG_PRINT("postgis", "PrepGeomCacheFreeer: freeing %p argnum %d", prepcache, prepcache->gcache.argnum);
