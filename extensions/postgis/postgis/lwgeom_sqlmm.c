...
Datum LWGEOM_curve_segmentize(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "perQuad = %d", perQuad);
...
Datum ST_CurveToLine(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "tol = %g, typ = %d, flg = %d", tol, toltype, flags);
...
Datum ST_CurveN(PG_FUNCTION_ARGS)
{
  ...
  if (index < 1 || (uint32_t)index > lwcmp->ngeoms)
    PG_RETURN_NULL();

  subgeom = lwgeom_clone(lwcollection_getsubcurve(lwcmp, index - 1));
