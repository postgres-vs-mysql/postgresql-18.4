...
static int is_literal_valid(const char *literal)
{
  ...
  DBUG_PRINT("postgis", "is_literal_valid called (%s)", literal);
  ...
  if (literal[0] == 'N' || literal[0] == 'E' || literal[0] == 'S' || literal[0] == 'W' || literal[0] == '+' || literal[0] == '-') {
    ...
    if (literal_length < 4) {
      ...
      DBUG_PRINT("postgis", "  invalid literal length (%d): \"%s\"", literal_length, literal);
      return LW_FALSE;
    }
  ...
  for (int j = coord_start; j < literal_length; j++) {
    ...
    if (!isdigit(literal[j])) {
      ...
      if (j < 3) {
        ...
        DBUG_PRINT("postgis", "  invalid character '%c' at the degrees section: \"%s\"", literal[j], literal);
        return LW_FALSE;
      ...
      if (literal[j] == '.' || literal[j] == ',') {
        ...
        DBUG_PRINT("postgis", "  invalid character '%c' in %d: \"%s\"", literal[j], j, literal);
        return LW_FALSE;
  ...
  DBUG_PRINT("postgis", "=> is_literal_valid returns LW_TRUE for \"%s\"", literal);
  return LW_TRUE;
...
static double parse_geo_literal(char *literal)
{
  ...
  DBUG_PRINT("postgis", "parse_geo_literal called (%s)", literal);
  POSTGIS_DEBUGF(2, "  start character: %c", start_character);
  DBUG_PRINT("postgis", "  start character: %c", start_character);
  ...
  DBUG_PRINT("postgis", "    start_literal=%d", start_literal);
  ...
  if (strchr(literal, '.') == NULL && strchr(literal, ',') == NULL) {
    ...
    DBUG_PRINT("postgis", "    parsed degrees (lon/lat): %s", dgr);
    ...
    if (literal_length > (start_literal + numdigits_degrees)) {
      ...
      DBUG_PRINT("postgis", "    parsed minutes (lon/lat): %s", min);
      ...
      if (literal_length >= (start_literal + numdigits_degrees + numdigits_minutes)) {
        ...
        DBUG_PRINT("postgis", "    parsed seconds (lon/lat): %s", sec);
    ...
    if (strchr(literal, ',')) {
      ...
      DBUG_PRINT("postgis", "  decimal separator changed to '.': %s", literal);
    ...
    if (literal[start_literal + numdigits_degrees] == '.') {
      ...
      DBUG_PRINT("postgis", "    parsed decimal degrees: %s", dec);
      pfree(dec);
      ...
      DBUG_PRINT("postgis", "    parsed degrees: %s", dgr);
      POSTGIS_DEBUGF(2, "    parsed decimal minutes: %s", min);
      DBUG_PRINT("postgis", "    parsed decimal minutes: %s", min);
      ...
      DBUG_PRINT("postgis", "    parsed minutes: %s", min);
      POSTGIS_DEBUGF(2, "    parsed decimal seconds: %s", sec);
      DBUG_PRINT("postgis", "    parsed decimal seconds: %s", sec);
      pfree(min);
      pfree(sec);
  ...
  if (start_character == 'S' || start_character == 'W' || start_character == '-') {
    ...
    DBUG_PRINT("postgis", "  switching sign due to start character: '%c'", start_character);
    result = -result;
...
parse_marc21(xmlNodePtr xnode)
{
  ...
  DBUG_PRINT("postgis", "parse_marc21 called: root '<%s>'", xnode->name);
  ...
  for (datafield = xnode->children; datafield != NULL; datafield = datafield->next) {
    ...
    for (subfield = datafield->children; subfield != NULL; subfield = subfield->next) {
      ...
      DBUG_PRINT("postgis", "    subfield code '%s': %s", code, literal);
  ...
  if (ngeoms == 1) {
    ...
    DBUG_PRINT("postgis", "=> parse_marc21 returns single geometry: %s", lwtype_name(lwgeom_get_type(lwgeoms[0])));
    ...
    for (i = 0; i < ngeoms; i++) {
      ...
      DBUG_PRINT("postgis", "  adding geometry to result set: %s", lwtype_name(lwgeom_get_type(lwgeoms[i])));
    ...
    DBUG_PRINT("postgis", "=> parse_marc21 returns a collection: %s", lwtype_name(lwgeom_get_type(result)));
    return result;
