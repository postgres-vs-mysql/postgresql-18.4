...
gidx_copy(GIDX *b)
{
  ...
  DBUG_PRINT("postgis", "copied gidx (%p) to gidx (%p)", b, c);
...
gidx_validate(GIDX *b)
{
  ...
  DBUG_PRINT("postgis", "validating gidx (%s)", gidx_to_string(b));
...
gidx_merge(GIDX **b_union, GIDX *b_new)
{
  ...
  DBUG_PRINT("postgis", "dims_union:%d, dims_new:%d", dims_union, dims_new);
  ...
  if (dims_new < dims_union) {
    DBUG_PRINT("postgis", "shrink unshared dimensions");
...
gidx_volume(GIDX *a)
{
  ...
  DBUG_PRINT("postgis", "calculated volume of %s as %.8g", gidx_to_string(a), result);
  return result;
}
...
gidx_edge(GIDX *a)
{
  ...
  DBUG_PRINT("postgis", "calculated edge of %s as %.8g", gidx_to_string(a), result);
  return result;
}
...
gidx_union_volume(GIDX *a, GIDX *b)
{
  ...
  DBUG_PRINT("postgis", "calculate the volume of the union of the boxes");
  ...
  DBUG_PRINT("postgis", "ndims_a:%d, ndims_b:%d", ndims_a, ndims_b);
  ...
  DBUG_PRINT("postgis", "volume( %s union %s ) = %.12g", gidx_to_string(a), gidx_to_string(b), result);
...
gidx_union_edge(GIDX *a, GIDX *b)
{
  ...
  DBUG_PRINT("postgis", "calculate the edge of the union of the boxes");
  ...
  DBUG_PRINT("postgis", "edge( %s union %s ) = %.12g", gidx_to_string(a), gidx_to_string(b), result);
...
gidx_inter_volume(GIDX *a, GIDX *b)
{
  ...
  DBUG_PRINT("postgis", "calculate the volume of the intersection of the boxes");
  ...
  DBUG_PRINT("postgis", "volume( %s intersection %s ) = %.12g", gidx_to_string(a), gidx_to_string(b), result);
  return result;
}
...
gserialized_datum_predicate(Datum gs1, Datum gs2, gidx_predicate predicate)
{
  ...
  if ((gserialized_datum_get_gidx_p(gs1, gidx1) == LW_SUCCESS) &&
    ...
    DBUG_PRINT("postgis", "return true if they satisfy the predicate");
    return LW_TRUE;
  }

  DBUG_PRINT("postgis", "return false if they don't satisfy the predicate");
  return LW_FALSE;
}
...
gserialized_datum_predicate_gidx_geom(GIDX *gidx1, Datum gs2, gidx_predicate predicate)
{
  ...
  if ((gserialized_datum_get_gidx_p(gs2, gidx2) == LW_SUCCESS) && predicate(gidx1, gidx2)) {
    DBUG_PRINT("postgis", "got boxes %s and %s", gidx_to_string(gidx1), gidx_to_string(gidx2));
    POSTGIS_DEBUGF(3, "got boxes %s and %s", gidx_to_string(gidx1), gidx_to_string(gidx2));
    DBUG_PRINT("postgis", "return true");
    return LW_TRUE;
  }
...
gserialized_datum_predicate_geom_gidx(Datum gs1, GIDX *gidx2, gidx_predicate predicate)
{
  ...
  if ((gserialized_datum_get_gidx_p(gs1, gidx1) == LW_SUCCESS) && predicate(gidx1, gidx2)) {
    POSTGIS_DEBUGF(3, "got boxes %s and %s", gidx_to_string(gidx1), gidx_to_string(gidx2));
    DBUG_PRINT("postgis", "got boxes %s and %s", gidx_to_string(gidx1), gidx_to_string(gidx2));
    DBUG_PRINT("postgis", "build box for gs2 argument and predicate function to return true");
    return LW_TRUE;
  }
...
gidx_distance(const GIDX *a, const GIDX *b, int m_is_time)
{
  ...
  for (i = 0; i < ndims; ++i) {
    ...
    DBUG_PRINT("postgis", "A %g - %g", amin, amax);
    POSTGIS_DEBUGF(3, "B %g - %g", bmin, bmax);
    DBUG_PRINT("postgis", "B %g - %g", bmin, bmax);
    ...
    DBUG_PRINT("postgis", "dist %g, squared %g, grows sum to %g", d, d * d, sum);
  }

  result = sqrt(sum);
  DBUG_PRINT("postgis", "calculate the box->box distance:%g", result);
  return result;
}
...
Datum gserialized_overlaps(PG_FUNCTION_ARGS)
{
  ...
  if (gserialized_datum_predicate(PG_GETARG_DATUM(0), PG_GETARG_DATUM(1), gidx_overlaps)) {
    DBUG_PRINT("postgis", "return true if they overlap");
    PG_RETURN_BOOL(true);
  }

  DBUG_PRINT("postgis", "return false if they don't overlap");
  PG_RETURN_BOOL(false);
}
...
Datum gserialized_gist_compress(PG_FUNCTION_ARGS)
{
  ...
  */
  ...
  DBUG_PRINT("postgis", "[GIST] got entry_in->key: %s", gidx_to_string(bbox_out));
...
Datum gserialized_gist_decompress(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "we don't decompress. Just return the input");
...
gserialized_gist_consistent_leaf(GIDX *key, GIDX *query, StrategyNumber strategy)
{
  ...
  DBUG_PRINT("postgis", "[GIST] leaf consistent, strategy [%d]", strategy);

  switch (strategy) {
    case RTOverlapStrategyNumber:
      DBUG_PRINT("postgis", "[GIST] leaf consistent, strategy: RTOverlapStrategyNumber");
      retval = (bool)gidx_overlaps(key, query);
      break;

    case RTSameStrategyNumber:
      DBUG_PRINT("postgis", "[GIST] leaf consistent, strategy: RTSameStrategyNumber");
      retval = (bool)gidx_equals(key, query);
      break;

    case RTContainsStrategyNumber:
      DBUG_PRINT("postgis", "[GIST] leaf consistent, strategy: RTContainsStrategyNumber");
      retval = (bool)gidx_contains(key, query);
      break;

    case RTOldContainsStrategyNumber:
      DBUG_PRINT("postgis", "[GIST] leaf consistent, strategy: RTOldContainsStrategyNumber");
      retval = (bool)gidx_contains(key, query);
      break;

    case RTContainedByStrategyNumber:
      DBUG_PRINT("postgis", "[GIST] leaf consistent, strategy: RTContainedByStrategyNumber");
      retval = (bool)gidx_contains(query, key);
      break;

    case RTOldContainedByStrategyNumber:
      DBUG_PRINT("postgis", "[GIST] leaf consistent, strategy: RTOldContainedByStrategyNumber");
      retval = (bool)gidx_contains(query, key);
      break;
...
gserialized_gist_consistent_internal(GIDX *key, GIDX *query, StrategyNumber strategy)
{
  ...
  POSTGIS_DEBUGF(4,
                 ...
                 gidx_to_string(key));
  DBUG_PRINT("postgis",
                 "[GIST] internal consistent, strategy [%d], query[%s], key[%s]",
                 strategy,
                 gidx_to_string(query),
                 gidx_to_string(key));
  ...
  switch (strategy) {
    ...
    case RTContainedByStrategyNumber:
    case RTOldContainedByStrategyNumber:
      DBUG_PRINT("postgis", "[GIST] internal consistent, strategy [overlap], query[%s], key[%s]", gidx_to_string(query), gidx_to_string(key));
      retval = (bool)gidx_overlaps(key, query);
      break;
    ...
    case RTContainsStrategyNumber:
    case RTOldContainsStrategyNumber:
      DBUG_PRINT("postgis", "[GIST] internal consistent, strategy [contains], query[%s], key[%s]", gidx_to_string(query), gidx_to_string(key));
      retval = (bool)gidx_contains(key, query);
      break;
...
Datum gserialized_gist_consistent(PG_FUNCTION_ARGS)
{
  ...
  *recheck = false;
  ...
  DBUG_PRINT("postgis", "take in a query and an entry and see what the relationship is, based on the query strategy");
  ...
  if (gserialized_datum_get_gidx_p(PG_GETARG_DATUM(1), query_gbox_index) == LW_FAILURE) {
    DBUG_PRINT("postgis", "null box should never make this far");
...
Datum gserialized_gist_penalty(PG_FUNCTION_ARGS)
{
  ...
  *result = 0.0;
  ...
  DBUG_PRINT("postgis", "calculate the penalty cost of adding this entry into an existing entry:%g", *result);
...
Datum gserialized_gist_union(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "merge all the boxes in a page into one master box");
  ...
  *sizep = VARSIZE(box_union);
  ...
  DBUG_PRINT("postgis", "[GIST] union called, numranges(%i), pageunion %s", numranges, gidx_to_string(box_union));
...
Datum gserialized_gist_same(PG_FUNCTION_ARGS)
{
  ...
  *result = gidx_equals(b1, b2);
  ...
  if (*result) {
    DBUG_PRINT("postgis", "test equality of keys: true");
  } else {
    DBUG_PRINT("postgis", "test equality of keys: false");
  }
...
Datum gserialized_gist_geog_distance(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "[GIST] '%s' function called", __func__);
...
gserialized_gist_picksplit_badratio(int x, int y)
{
  ...
  DBUG_PRINT("postgis", "[GIST] checking split ratio (%d, %d)", x, y);
    ...
    GIDX **union2)
{
  ...
  if (v->spl_ldatum_exists || v->spl_rdatum_exists) {
    if (v->spl_ldatum_exists && v->spl_rdatum_exists) {
      ...
      DBUG_PRINT("postgis", "[GIST] sizeLR / sizeRL == %.12g / %.12g", sizeLR, sizeRL);
      ...
      DBUG_PRINT("postgis", "[GIST] p1 / p2 == %.12g / %.12g", p1, p2);
  ...
  DBUG_PRINT("postgis", "[GIST] firstToLeft == %d", firstToLeft);
...
Datum gserialized_gist_picksplit(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "split an overflowing node into two new nodes");
  ...
  */
  ...
  DBUG_PRINT("postgis", "[GIST] box_pageunion: %s", gidx_to_string(box_pageunion));
  ...
  if (all_entries_equal) {
    DBUG_PRINT("postgis", "[GIST] picksplit finds all entries equal!");
  ...
  DBUG_PRINT("postgis", "[GIST] ndims_pageunion == %d", ndims_pageunion);
  ...
  */
  ...
  DBUG_PRINT("postgis", "[GIST] 'picksplit' calculating best split axis (FirstOffsetNumber:%u, max_offset:%u)", FirstOffsetNumber, max_offset);
  ...
  */
  if (gserialized_gist_picksplit_badratios(pos, ndims_pageunion)) {
    ...
    */
    ...
    DBUG_PRINT("postgis", "[GIST] picksplit can't find good split axis, trying center point method");
    ...
    for (d = 0; d < ndims_pageunion; d++) {
      ...
      DBUG_PRINT("postgis", "[GIST] picksplit average center point[%d] = %.12g", d, avgCenter[d]);
    }
    ...
    if (gserialized_gist_picksplit_badratios(pos, ndims_pageunion)) {
      ...
      DBUG_PRINT("postgis", "[GIST] picksplit still cannot find a good split! just cutting the node in half");
      gserialized_gist_picksplit_fallback(entryvec, v);
      PG_RETURN_POINTER(v);
    }
  }
  ...
  */
  ...
  DBUG_PRINT("postgis", "[GIST] 'picksplit' splitting on axis %d", direction);
  ...
  DBUG_PRINT("postgis", "[GIST] spl_ldatum: %s", gidx_to_string((GIDX *)v->spl_ldatum));
  DBUG_PRINT("postgis", "[GIST] spl_rdatum: %s", gidx_to_string((GIDX *)v->spl_rdatum));
...
Datum gidx_in(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "function gidx_in not implemented");
