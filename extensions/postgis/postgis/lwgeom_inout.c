...
Datum LWGEOM_in(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "return a GSERIALIZED object");
...
Datum LWGEOM_to_latlon(PG_FUNCTION_ARGS)
{
  ...
  if (POINTTYPE != geom_type) {
    DBUG_INSTANT_PRINT("postgis", "only points are supported, you tried type %s", lwtype_name(geom_type));
    lwpgerror("Only points are supported, you tried type %s.", lwtype_name(geom_type));
  }

  /* Convert to LWGEOM type */
  DBUG_PRINT("postgis", "convert to LWGEOM type");
  ...
  DBUG_PRINT("postgis", "the input string supposedly will be in the database encoding, so convert to UTF-8");
  ...
  DBUG_PRINT("postgis", "convert the formatted string from UTF-8 back to database encoding");
  tmp = (char *)pg_do_encoding_conversion(
  ...
  DBUG_PRINT("postgis", "convert to the postgres output string type");
...
Datum LWGEOM_out(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "geom geometry to convert to HEXWKB");
...
Datum LWGEOM_addBBOX(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "put a bbox inside the geometry");
  lwgeom = lwgeom_from_gserialized(geom);
  lwgeom_add_bbox(lwgeom);
...
Datum LWGEOM_dropBBOX(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "remove a bbox from a geometry");
...
Datum parse_WKT_lwgeom(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "this just does the same thing as the _in function, except it has to handle a 'text' input");
