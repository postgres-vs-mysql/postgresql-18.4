...
static void interrupt_geos_callback(void)
{
  ...
  if (QueryCancelPending || ProcDiePending) {
    DBUG_PRINT("postgis", "if PgSQL global flags show interrupt, flip the pending flag in GEOS to end current query");
...
_PG_init(void)
{
   ...
   */
  DBUG_PRINT("postgis", "hook up interrupt checking to call back here and examine the PgSQL interrupt state variables");
  ...
  DBUG_PRINT("postgis", "install PostgreSQL error/memory handlers");
  pg_install_lwgeom_handlers();

#if POSTGIS_PROJ_VERSION > 60000
  /* Pass proj messages through the pgsql error handler */
  DBUG_PRINT("postgis", "pass proj messages through the pgsql error handler");
...
{
  DBUG_TRACE;
  DBUG_PRINT("postgis", "goodbye from PostGIS %s", POSTGIS_VERSION);
