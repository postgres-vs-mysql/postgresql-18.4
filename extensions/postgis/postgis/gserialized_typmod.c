...
Datum postgis_typmod_out(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "Got typmod(srid = %d, type = %d, hasz = %d, hasm = %d)", srid, type, hasz, hasm);
...
GSERIALIZED* postgis_valid_typmod(GSERIALIZED *gser, int32_t typmod)
{
  ...
  DBUG_PRINT("postgis", "Got geom(type = %d, srid = %d, hasz = %d, hasm = %d)", geom_type, geom_srid, geom_z, geom_m);
  POSTGIS_DEBUGF(3, "Got typmod(type = %d, srid = %d, hasz = %d, hasm = %d)", typmod_type, typmod_srid, typmod_z, typmod_m);
  DBUG_PRINT("postgis", "Got typmod(type = %d, srid = %d, hasz = %d, hasm = %d)", typmod_type, typmod_srid, typmod_z, typmod_m);
