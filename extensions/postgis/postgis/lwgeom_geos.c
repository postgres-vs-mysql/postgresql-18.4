  ...
  while (array_iterate(iterator, &value, &isnull)) {
    ...
    if ( gserialized_is_empty(gser_in) ) {
      ...
      if (gser_type > empty_type) {
        ...
        DBUG_PRINT("postgis", "empty_type = %d  gser_type = %d", empty_type, gser_type);
      }
    } else {
...
Datum convexhull(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "result: %s", GEOSGeomToWKT(g3));
...
Datum buffer(PG_FUNCTION_ARGS)
{
  ...
  if (VARSIZE_ANY_EXHDR(params_text) > 0) {
    ...
    for (param = params; ; param = NULL) {
      ...
      DBUG_PRINT("postgis", "Param: %s", param);
      ...
      *val = '\0';
      ...
      DBUG_PRINT("postgis", "Param: %s : %s", key, val);
  ...
  DBUG_PRINT("postgis", "endCap:%d joinStyle:%d mitreLimit:%g",
                 endCapStyle, joinStyle, mitreLimit);
...
Datum ST_OffsetCurve(PG_FUNCTION_ARGS)
{
  ...
  if ( nargs > 2 ) {
    ...
    DBUG_PRINT("postgis", "paramstr: %s", paramstr);
    ...
    DBUG_PRINT("postgis", "joinStyle:%d mitreLimit:%g", joinStyle, mitreLimit);
...
Datum ST_ClipByBox2d(PG_FUNCTION_ARGS)
{
  ...
  if (bbox2->xmin < bbox1.xmin && bbox2->xmax > bbox1.xmax &&
    ...
    DBUG_PRINT("postgis", "if bbox1 is strictly contained by bbox2, return input geometry");
    PG_RETURN_DATUM(PG_GETARG_DATUM(geom_idx));
  }

  /* If bbox1 outside of bbox2, return empty */
  if (!gbox_overlaps_2d(&bbox1, bbox2)) {
    DBUG_PRINT("postgis", "if  bbox1outside of bbox2, return empty");
...
GEOS2POSTGIS(GEOSGeom geom, uint8_t want3d)
{
  ...
  DBUG_PRINT("postgis", "%s: GEOS2LWGEOM returned a %s", __func__, lwgeom_summary(lwgeom, 0));
...
Datum polygonize_garray(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "polygonize_garray: number of non-null elements: %d", nelems);
...
Datum clusterintersecting_garray(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "clusterintersecting_garray: number of non-null elements: %d", nelems);
...
Datum cluster_within_distance_garray(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "cluster_within_distance_garray: number of non-null elements: %d", nelems);
