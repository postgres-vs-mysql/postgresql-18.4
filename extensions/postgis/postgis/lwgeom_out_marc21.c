...
lwgeom_to_marc21(const LWGEOM *geom, const char* format)
{
  ...
  DBUG_PRINT("postgis", "lwgeom_to_marc21 called: %s with format %s", lwtype_name(lwgeom_get_type(geom)), format);
  ...
  DBUG_PRINT("postgis", "opening MARC21/XML record: %s", lwtype_name(lwgeom_get_type(geom)));
  ...
  if (lwgeom_is_collection(geom)) {
    ...
    DBUG_PRINT("postgis", "collection detected: %s", lwtype_name(lwgeom_get_type(geom)));
    ...
    DBUG_PRINT("postgis", "calculating gbox: %s", lwtype_name(lwgeom_get_type(geom)));
    ...
    DBUG_PRINT("postgis", "creating MARC21/XML datafield: %s", lwtype_name(lwgeom_get_type(geom)));
  ...
  DBUG_PRINT("postgis", "closing MARC21/XML record");
...
static int corner_to_subfield_sb(stringbuffer_t *sb, double decimal_degrees, const char*format, char subfield)
{
  ...
  DBUG_PRINT("postgis", "corner_to_subfield_sb called with coordinates: %f and format: %s", decimal_degrees, format);
  ...
  if(format[3 + has_cardinal_direction] == '.' || format[3 + has_cardinal_direction] == ',' ) {
    ...
    DBUG_PRINT("postgis", "decimal degrees");
    ...
    DBUG_PRINT("postgis", "decimal minutes");
    ...
    DBUG_PRINT("postgis", "decimal seconds");
    ...
    DBUG_PRINT("postgis", "degrees/minutes/seconds (dddmmss)");
