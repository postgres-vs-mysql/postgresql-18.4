 ...
 * Copyright 2017-2025 Darafei Praliaskouski <me@komzpa.net>
 *
 **********************************************************************/
...
BOX2DF* box2df_copy(BOX2DF *b)
{
  ...
  DBUG_PRINT("postgis", "copied box2df (%p) to box2df (%p)", b, c);
  return c;
}
...
void box2df_merge(BOX2DF *b_union, BOX2DF *b_new)
{
  ...
  DBUG_PRINT("postgis", "merging %s with %s", box2df_to_string(b_union), box2df_to_string(b_new));
  ...
  DBUG_PRINT("postgis", "merge complete %s", box2df_to_string(b_union));
  return;
}
...
bool box2df_overlaps(const BOX2DF *a, const BOX2DF *b)
{
  ...
  if ( !a || !b || box2df_is_empty(a) || box2df_is_empty(b) ) {
    DBUG_PRINT("postgis", "result: false");
    return false;
  }

  DBUG_PRINT("postgis", "a:'%s', b:'%s'", box2df_to_string(a), box2df_to_string(b));
  ...
  DBUG_PRINT("postgis", "result: true");
  return true;
}
...
static double box2df_distance_leaf_centroid(const BOX2DF *a, const BOX2DF *b)
{
  ...
  result = sqrt((a_x - b_x) * (a_x - b_x) + (a_y - b_y) * (a_y - b_y));

  DBUG_PRINT("postgis", "calculate the centroid->centroid distance between the boxes:%g", result);
  return result;
}
...
static double box2df_distance(const BOX2DF *a, const BOX2DF *b)
{
  ...
  if ( box2df_overlaps(a, b) ) {
    DBUG_PRINT("postgis", "calculate the box->box distance:0.0 (overlap)");
    return 0.0;
  }

  if ( box2df_left(a, b) ) {
    if ( box2df_above(a, b) ) {
      distance = pt_distance(a->xmax, a->ymin, b->xmin, b->ymax);
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (left and above)", distance);
      return distance;
    }

    if ( box2df_below(a, b) ) {
      distance = pt_distance(a->xmax, a->ymax, b->xmin, b->ymin);
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (left and below)", distance);
      return distance;
    } else {
      distance = (double)b->xmin - (double)a->xmax;
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (left and (not above and below))", distance);
      return distance;
    }
  }
  ...
  if ( box2df_right(a, b) ) {
    if ( box2df_above(a, b) ) {
      distance = pt_distance(a->xmin, a->ymin, b->xmax, b->ymax);
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (right and above)", distance);
      return distance;
    }

    if ( box2df_below(a, b) ) {
      distance = pt_distance(a->xmin, a->ymax, b->xmax, b->ymin);
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (right and below)", distance);
      return distance;
    } else {
      distance = (double)a->xmin - (double)b->xmax;
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (right and (not above and below))", distance);
      return distance;
    }
  }
  ...
  if ( box2df_above(a, b) ) {
    if ( box2df_left(a, b) ) {
      distance = pt_distance(a->xmax, a->ymin, b->xmin, b->ymax);
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (above and left)", distance);
      return distance;
    }

    if ( box2df_right(a, b) ) {
      distance = pt_distance(a->xmin, a->ymin, b->xmax, b->ymax);
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (above and right)", distance);
      return distance;
    } else {
      distance = (double)a->ymin - (double)b->ymax;
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (above and (not left and right))", distance);
      return distance;
    }
  }
  ...
  if ( box2df_below(a, b) ) {
    if ( box2df_left(a, b) ) {
      distance = pt_distance(a->xmax, a->ymax, b->xmin, b->ymin);
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (below and left)", distance);
      return distance;
    }

    if ( box2df_right(a, b) ) {
      distance = pt_distance(a->xmin, a->ymax, b->xmax, b->ymin);
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (below and right)", distance);
      return distance;
    } else {
      distance = (double)b->ymin - (double)a->ymax;
      DBUG_PRINT("postgis", "calculate the box->box distance:%g (below and (not left and right))", distance);
      return distance;
    }
  }
...
gserialized_datum_get_internals_p(Datum gsdatum, GBOX *gbox, lwflags_t *flags, uint8_t *type, int32_t *srid)
{
  ...
  if (need_detoast) {
    DBUG_PRINT("postgis", "peek into a #GSERIALIZED datum to find its bounding box and some other metadata(need_detoast:true)");
    gpart = (GSERIALIZED *)PG_DETOAST_DATUM_SLICE(gsdatum, 0, gserialized_max_header_size());
  } else {
    DBUG_PRINT("postgis", "peek into a #GSERIALIZED datum to find its bounding box and some other metadata");
    gpart = (GSERIALIZED *)gsdatum;
  }

  if (!gserialized_has_bbox(gpart) && need_detoast && LWSIZE_GET(gpart->size) >= gserialized_max_header_size()) {
    DBUG_PRINT("postgis", "the headers don't contain a bbox and the object is larger than what we retrieved, so");
    DBUG_PRINT("postgis", "we now detoast it completely");
...
gserialized_datum_get_box2df_p(Datum gsdatum, BOX2DF *box2df)
{
  ...
  if (gserialized_has_bbox(gpart)) {
    ...
    if (need_detoast && LWSIZE_GET(gpart->size) >= gserialized_max_header_size()) {
      DBUG_PRINT("postgis", "the headers don't contain a bbox and the object is larger than what we retrieved, so");
      DBUG_PRINT("postgis", "we now detoast it completely and recheck");
...
gserialized_datum_predicate_2d(Datum gs1, Datum gs2, box2df_predicate predicate)
{
  ...
  if ( predicate(br1, br2) ) {
    ...
    DBUG_PRINT("postgis", "got boxes %s and %s", br1 ? box2df_to_string(&b1) : "(null)", br2 ? box2df_to_string(&b2) : "(null)");
    return LW_TRUE;
  }
...
gserialized_datum_predicate_box2df_geom_2d(const BOX2DF *br1, Datum gs2, box2df_predicate predicate)
{
  ...
  if ( predicate(br1, br2) ) {
    DBUG_PRINT("postgis", "got boxes %s", br2 ? box2df_to_string(&b2) : "(null)");
...
Datum gserialized_distance_centroid_2d(PG_FUNCTION_ARGS)
{
  ...
  if ( (gserialized_datum_get_box2df_p(gs1, &b1) == LW_SUCCESS) &&
    ...
    DBUG_PRINT("postgis", "got boxes %s and %s and distance:%g", box2df_to_string(&b1), box2df_to_string(&b2), distance);
...
Datum gserialized_distance_box_2d(PG_FUNCTION_ARGS)
{
  ...
  if ( (gserialized_datum_get_box2df_p(gs1, &b1) == LW_SUCCESS) &&
    ...
    POSTGIS_DEBUGF(3, "got boxes %s and %s", box2df_to_string(&b1), box2df_to_string(&b2));
    DBUG_PRINT("postgis", "got boxes %s and %s", box2df_to_string(&b1), box2df_to_string(&b2));
    PG_RETURN_FLOAT8(distance);
  }
...
Datum gserialized_gist_compress_2d(PG_FUNCTION_ARGS)
{
  ...
  */
  if ( ! entry_in->leafkey ) {
    DBUG_PRINT("postgis", "[GIST] non-leafkey entry, returning input unaltered");
    POSTGIS_DEBUG(4, "[GIST] non-leafkey entry, returning input unaltered");
    PG_RETURN_POINTER(entry_in);
  }

  DBUG_PRINT("postgis", "[GIST] processing leafkey input");
  ...
  */
  ...
  if (box2df_is_empty(&bbox_out)) {
    ...
    DBUG_PRINT("postgis", "[GIST] empty geometry!");
    POSTGIS_DEBUG(4, "[GIST] empty geometry!");
    PG_RETURN_POINTER(entry_out);
  }

  POSTGIS_DEBUGF(4, "[GIST] got entry_in->key: %s", box2df_to_string(&bbox_out));
  DBUG_PRINT("postgis", "[GIST] got entry_in->key: %s", box2df_to_string(&bbox_out));
  ...
  if ( ! isfinite(bbox_out.xmax) || ! isfinite(bbox_out.xmin) ||
    ...
    DBUG_PRINT("postgis", "[GIST] infinite geometry!");
  ...
  /* Return GISTENTRY. */
  DBUG_PRINT("postgis", "[GIST] compress' function complete");
...
Datum gserialized_gist_decompress_2d(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "we don't decompress and just return the input");
...
static inline bool gserialized_gist_consistent_leaf_2d(BOX2DF *key, BOX2DF *query, StrategyNumber strategy)
{
  ...
  DBUG_PRINT("postgis", "[GIST] leaf consistent, strategy [%d]",
                 strategy);
...
static inline bool gserialized_gist_consistent_internal_2d(BOX2DF *key, BOX2DF *query, StrategyNumber strategy)
{
  ...
  DBUG_PRINT("postgis", "strategy [%d], query[%s]", strategy, box2df_to_string(query));
  DBUG_PRINT("postgis", "key[%s]", box2df_to_string(key));
...
Datum gserialized_gist_consistent_2d(PG_FUNCTION_ARGS)
{
  ...
  *recheck = false;
  ...
  if ( DatumGetPointer(PG_GETARG_DATUM(1)) == NULL ) {
    DBUG_PRINT("postgis", "[GIST] null query pointer (!?!), returning false");
  ...
  if ( DatumGetPointer(entry->key) == NULL ) {
    DBUG_PRINT("postgis", "[GIST] null index entry, returning false");
  ...
  if ( gserialized_datum_get_box2df_p(PG_GETARG_DATUM(1), &query_gbox_index) == LW_FAILURE ) {
    DBUG_PRINT("postgis", "[GIST] null query_gbox_index, returning false");
  ...
  if (GIST_LEAF(entry)) {
    DBUG_PRINT("postgis", "treat leaf node tests different from internal nodes");
  ...
  if (result) {
    DBUG_PRINT("postgis", "gserialized_gist_consistent_2d returns true");
  } else {
    DBUG_PRINT("postgis", "gserialized_gist_consistent_2d returns false");
  }
...
Datum gserialized_gist_distance_2d(PG_FUNCTION_ARGS)
{
  ...
  *  and '14' as the gist distance-between-boxes <#> strategy number */
  ...
  if ( gserialized_datum_get_box2df_p(PG_GETARG_DATUM(1), &query_box) == LW_FAILURE ) {
    DBUG_PRINT("postgis", "[GIST] null query_gbox_index!");
    POSTGIS_DEBUG(4, "[GIST] null query_gbox_index!");
    PG_RETURN_FLOAT8(FLT_MAX);
  }
  ...
  DBUG_PRINT("postgis", "take in a query and an entry and return the distance between them:%g", distance);
  PG_RETURN_FLOAT8(distance);
}
...
Datum gserialized_gist_union_2d(PG_FUNCTION_ARGS)
{
  ...
  *sizep = sizeof(BOX2DF);
  ...
  DBUG_PRINT("postgis", "[GIST] 'union', numranges(%i), pageunion %s", numranges, box2df_to_string(box_union));
                     ...
                     float leftUpper, int maxLeftCount)
{
  ...
  DBUG_PRINT("postgis", "consider split: dimNum = %d, rightLower = %g, minLeftCount = %d, leftUpper = %g, maxLeftCount = %d",
             dimNum, rightLower, minLeftCount, leftUpper, maxLeftCount);
  ...
  DBUG_PRINT("postgis", "ratio of split - quotient between size of lesser group and tota entries count:%g", ratio);

  if (ratio > LIMIT_RATIO) {
    bool    selectthis = false;

    DBUG_PRINT("postgis", "the ratio is acceptable, so compare current split with previously selected one");
    ...
    if (context->first) {
      selectthis = true;
      DBUG_PRINT("postgis", "if there is no previous selection, select this");
      ...
      if (overlap < context->overlap ||
        ...
        selectthis = true;
        DBUG_PRINT("postgis", "within the same dimension, choose the new split if it has a smaller overlap, or same overlap but better ratio");
      }
    } else {
      /*
      ...
      if (non_negative(overlap) < non_negative(context->overlap) ||
        ...
        selectthis = true;
        DBUG_PRINT("postgis", "across dimensions, choose the new split if it has a smaller");
        DBUG_PRINT("postgis", "*non-negative* overlap, or same *non-negative* overlap but bigger range");
      }
    }
    ...
    if (selectthis) {
      ...
      context->dim = dimNum;
      DBUG_PRINT("postgis", "split selected");
...
Datum gserialized_gist_picksplit_2d(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "double sorting split algorithm");
   ...
   */
  DBUG_PRINT("postgis", "calculate the overall minimum bounding box over all the entries(maxoff:%u, nentries:%d", maxoff, nentries);
  ...
  POSTGIS_DEBUGF(4, "boundingBox is %s", box2df_to_string(
                   &context.boundingBox));
  DBUG_PRINT("postgis", "boundingBox is %s", box2df_to_string(&context.boundingBox));

  /*
   * Iterate over axes for optimal split searching.
   */
  DBUG_PRINT("postgis", "iterate over axes for optimal split searching");
   ...
   */
  if (context.first) {
    DBUG_PRINT("postgis", "no acceptable splits, trivial split");
  ...
  DBUG_PRINT("postgis", "split direction: %d", context.dim);
...
#define PLACE_RIGHT(box, off)         \
  ...
  DBUG_PRINT("postgis", "leftBox is %s", box2df_to_string(leftBox));
  POSTGIS_DEBUGF(4, "rightBox is %s", box2df_to_string(rightBox));
  DBUG_PRINT("postgis", "rightBox is %s", box2df_to_string(rightBox));
  ...
  DBUG_PRINT("postgis", "[GIST] 'picksplit' completed");
