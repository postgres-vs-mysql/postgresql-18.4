...
static uint32_t encode_ptarray(enum mvt_type type, POINTARRAY *pa, uint32_t *buffer, int32_t *px, int32_t *py)
{
  ...
  DBUG_PRINT("postgis", "loop points and add to buffer");
...
static void encode_feature_geometry(struct feature_builder *feature, LWGEOM *lwgeom)
{
  ...
  switch (type) {
    ...
    default:
      elog(ERROR, "encode_feature_geometry: '%s' geometry type not supported",
           lwtype_name(type));
  }
}
...
static void encode_values(mvt_agg_context *ctx)
{
  ...
  DBUG_PRINT("postgis", "encode_values n_values: %d", ctx->values_hash_i);
  ...
  { \
    ...
    { \
      ...
      { \
        ...
        DBUG_PRINT("postgis", "MVT_PARSE_VALUE new hash key: %d", ctx->values_hash_i); \
...
add_value_as_string_with_size(mvt_agg_context *ctx, struct feature_builder *feature, char *value, size_t size, uint32_t k)
{
  ...
  if (!kv) {
    ...
    POSTGIS_DEBUGF(4, "add_value_as_string new hash key: %d",
                   ctx->values_hash_i);
    DBUG_PRINT("postgis", "add_value_as_string new hash key: %d",
                   ctx->values_hash_i);
    kv->id = ctx->values_hash_i++;
    vector_tile__tile__value__init(kv->value);
...
parse_datum_as_string(mvt_agg_context *ctx, struct feature_builder *feature, Oid typoid, Datum datum, uint32_t k)
{
  ...
  DBUG_PRINT("postgis", "parse_value_as_string value: %s", value);
...
static void parse_values(mvt_agg_context *ctx, struct feature_builder *feature)
{
  ...
  DBUG_PRINT("postgis", "parse_values natts: %d", natts);
...
mvt_unsafe_clip_by_box(LWGEOM *lwg_in, GBOX *clip_box)
{
  ...
  DBUG_PRINT("postgis", "clip a geometry using lwgeom_clip_by_rect");
  gbox_init(&geom_box);
...
mvt_clip_and_validate(LWGEOM *lwgeom, uint8_t basic_type, uint32_t extent, uint32_t buffer, bool clip_geom)
{
  ...
  if (!clip_geom) {
    ...
    DBUG_PRINT("postgis", "with clipping disabled, we request a clip with the geometry bbox to force validation");
    DBUG_PRINT("postgis", "calculate the GBOX of the geometry");
                 ...
                 bool clip_geom)
{
  ...
  DBUG_PRINT("postgis", "transform a geometry into vector tile coordinate space");
...
void mvt_agg_init_context(mvt_agg_context *ctx)
{
  ...
  DBUG_PRINT("postgis", "initialize aggregation context");
...
void mvt_agg_transfn(mvt_agg_context *ctx)
{
  ...
  DBUG_PRINT("postgis", "aggregation step");
  DBUG_PRINT("postgis", "parse a row, turn it into a feature, and add it to the layer");
  ...
  DBUG_PRINT("postgis", "mvt_agg_transfn encoded feature count: %zd", layer->n_features);
  ...
  if (layer->n_features >= ctx->features_capacity) {
    ...
    DBUG_PRINT("postgis", "mvt_agg_transfn new_capacity: %zd", new_capacity);
  }
...
vectortile_tile_combine(VectorTile__Tile *tile1, VectorTile__Tile *tile2)
{
  ...
  DBUG_PRINT("postgis", "merge all matching layers(tile1 n_layers:%u and tile2 n_layers:%u) in the files", tile1->n_layers, tile2->n_layers);
