...
nd_box_from_gbox(const GBOX *gbox, ND_BOX *nd_box)
{
  ...
  DBUG_PRINT("postgis", "set the values of an #ND_BOX from a #GBOX:%s", gbox_to_string(gbox));
...
nd_box_intersects(const ND_BOX *a, const ND_BOX *b, int ndims)
{
  ...
  DBUG_PRINT("postgis", "ndims:%d", ndims);
  for ( d = 0; d < ndims; d++ ) {
    if ( (a->min[d] > b->max[d]) || (a->max[d] < b->min[d]) ) {
      DBUG_PRINT("postgis", "return false if #ND_BOX a does not overlap b");
      return false;
    }
  }

  DBUG_PRINT("postgis", "return true if #ND_BOX a overlaps b");
  return true;
}
...
nd_box_contains(const ND_BOX *a, const ND_BOX *b, int ndims)
{
  ...
  for ( d = 0; d < ndims; d++ ) {
    if ( ! ((a->min[d] < b->min[d]) && (a->max[d] > b->max[d])) ) {
      DBUG_PRINT("postgis", "return false if #ND_BOX a does not contain b");
      return false;
    }
  }

  DBUG_PRINT("postgis", "return true if #ND_BOX a contains b");
  return true;
}
...
nd_box_expand(ND_BOX *nd_box, double expansion_factor)
{
  ...
  int d;
  double size;
  DBUG_PRINT("postgis", "expand an #ND_BOX ever so slightly (expansion_factor:%g)", expansion_factor);
...
nd_box_overlap(const ND_STATS *nd_stats, const ND_BOX *nd_box, ND_IBOX *nd_ibox)
{
  ...
  DBUG_PRINT("postgis", "nd_box: %s", nd_box_to_json(nd_box, nd_stats->ndims));
  ...
  for ( d = 0; d < nd_stats->ndims; d++ ) {
    ...
    if (width < MIN_DIMENSION_WIDTH) {
      ...
      DBUG_PRINT("postgis", "stats: dim %d: min %g: max %g: width %g", d, smin, smax, width);
      POSTGIS_DEBUGF(5, "overlap: dim %d: (%d, %d)", d, nd_ibox->min[d], nd_ibox->max[d]);
      DBUG_PRINT("postgis", "overlap: dim %d: (%d, %d)", d, nd_ibox->min[d], nd_ibox->max[d]);
...
nd_box_array_distribution(const ND_BOX **nd_boxes, int num_boxes, const ND_BOX *extent, int ndims, double *distribution)
{
  ...
  DBUG_PRINT("postgis", "calculate how much a set of boxes is homogeneously distributed or concentrated within one dimension,");
  DBUG_PRINT("postgis", "returning the range_quintile of the overlap counts per cell in a uniform partition of the extent of the dimension");
  ...
  for ( d = 0; d < ndims; d++ ) {
    ...
    for ( i = 0; i < num_boxes; i++ ) {
      ...
      DBUG_PRINT("postgis", "dimension %d, feature %d: bin %d to bin %d", d, i, bmin, bmax);
    ...
    DBUG_PRINT("postgis", "dimension %d: range = %d", d, range);
    DBUG_PRINT("postgis", "dimension %d: average = %.6g", d, average);
    DBUG_PRINT("postgis", "dimension %d: stddev = %.6g", d, sdev);
    DBUG_PRINT("postgis", "dimension %d: stddev_ratio = %.6g", d, sdev_ratio);
...
pg_nd_stats_from_tuple(HeapTuple stats_tuple, int mode)
{
  ...
  /* Then read the geom status histogram from that */
  {
    ...
    if ( ! rv ) {
      ...
      DBUG_PRINT("postgis", "no slot of kind %d in stats tuple", stats_kind);
      return NULL;
    }
...
pg_get_nd_stats(const Oid table_oid, AttrNumber att_num, int mode, bool only_parent)
{
  ...
  if ( ! only_parent ) {
    rel_name1 = get_rel_name(table_oid);
    DBUG_PRINT("postgis", "searching whole tree stats for \"%s\"", rel_name1 ? rel_name1 : "NULL");
    POSTGIS_DEBUGF(2, "searching whole tree stats for \"%s\"", rel_name1 ? rel_name1 : "NULL");
    stats_tuple = SearchSysCache3(STATRELATTINH, ObjectIdGetDatum(table_oid), Int16GetDatum(att_num), BoolGetDatum(true));

    if ( stats_tuple ) {
      DBUG_PRINT("postgis", "found whole tree stats for \"%s\"",  rel_name1? rel_name1 : "NULL");
      POSTGIS_DEBUGF(2, "found whole tree stats for \"%s\"", rel_name1 ? rel_name1 : "NULL");
    }
  }
  ...
  if ( only_parent || ! stats_tuple ) {
    ...
    DBUG_PRINT("postgis", "searching parent table stats for \"%s\"", rel_name1 ? rel_name1 : "NULL");
    POSTGIS_DEBUGF(2, "searching parent table stats for \"%s\"", rel_name1 ? rel_name1 : "NULL");
    stats_tuple = SearchSysCache3(STATRELATTINH, ObjectIdGetDatum(table_oid), Int16GetDatum(att_num), BoolGetDatum(false));

    if ( stats_tuple ) {
      DBUG_PRINT("postgis", "found parent table stats for \"%s\"", rel_name1 ? rel_name1 : "NULL");
      POSTGIS_DEBUGF(2, "found parent table stats for \"%s\"", rel_name1 ? rel_name1 : "NULL");
    }
  }
  ...
  if ( ! stats_tuple ) {
    ...
    DBUG_PRINT("postgis", "stats for \"%s\" do not exist", rel_name1 ? rel_name1 : "NULL");
    POSTGIS_DEBUGF(2, "stats for \"%s\" do not exist", rel_name1 ? rel_name1 : "NULL");
    return NULL;
  }
  ...
  if ( ! nd_stats ) {
    ...
    DBUG_PRINT("postgis", "histogram for attribute %d of table \"%s\" does not exist?", att_num, rel_name1);
    POSTGIS_DEBUGF(2,
...
pg_get_nd_stats_by_name(const Oid table_oid, const text *att_text, int mode, bool only_parent)
{
  ...
  DBUG_PRINT("postgis", "pull the stats object from the PgSQL system catalogs");
  ...
  if ( att_text ) {
    ...
    DBUG_PRINT("postgis", "get the attribute number:%d for att:%s", att_num, att_name);
  } else {
...
estimate_join_selectivity(const ND_STATS *s1, const ND_STATS *s2)
{
  ...
  double val = 0;
  float8 selectivity;
  int loop1, loop2, loop_total = 0;

  DBUG_PRINT("postgis", "given two statistics histograms, what is the selectivity of a join driven by the && or &&& operator?");

  /* Drop out on null inputs */
  if ( ! ( s1 && s2 ) ) {
    DBUG_PRINT("postgis", "estimate_join_selectivity called with null inputs");
  ...
  DBUG_PRINT("postgis", "we need to know how many cells each side has");
  DBUG_PRINT("postgis", "ncells1:%d, ncells2:%d", ncells1, ncells2);
  ...
  DBUG_PRINT("postgis", "s1: %s", nd_stats_to_json(s1));
  DBUG_PRINT("postgis", "s2: %s", nd_stats_to_json(s2));
  ...
  DBUG_PRINT("postgis", "ntuples_max(%g) = ntuples_not_null1(%g) * ntuples_not_null2(%g)", 
      ntuples_max, ntuples_not_null1, ntuples_not_null2);
  ...
  if ( ! nd_box_intersects(&extent1, &extent2, ndims) ) {
    DBUG_PRINT("postgis", "relation stats do not intersect, returning");
  ...
  if ( ! nd_box_overlap(s1, &extent2, &ibox1) ) {
    DBUG_PRINT("postgis", "could not calculate overlap of relations");
  ...
  DBUG_PRINT("postgis", "for each affected cell of s1...");
  ...
  do {
    ...
    DBUG_PRINT("postgis", "at1 %d,%d  %s", at1[0], at1[1], nd_box_to_json(&nd_cell1, ndims1));
    ...
    DBUG_PRINT("postgis", "get the value at this cell:%g", val1);
    DBUG_PRINT("postgis", "for each overlapped cell of s2...");
    ...
    do {
      ...
      DBUG_PRINT("postgis", "at2 %d, %d  %s", at2[0], at2[1], nd_box_to_json(&nd_cell2, ndims2));

      /* Calculate overlap ratio of the cells */
      ratio2 = nd_box_ratio(&nd_cell1, &nd_cell2, Max(ndims1, ndims2));
      DBUG_PRINT("postgis", "calculate overlap ratio of the cells:%g", ratio2);
      ...
      DBUG_PRINT("postgis", "val(%g) += val1(%g) * (val2(%g) * ratio2(%g)", val, val1, val2, ratio2);
      loop2++;
      loop_total++;
    } while ( nd_increment(&ibox2, ndims2, at2) );

    DBUG_PRINT("postgis", "inner loop:%d, ndims2:%d", loop2, ndims2);

  } while( nd_increment(&ibox1, ndims1, at1) );
    
  DBUG_PRINT("postgis", "outer loop:%d, total loops:%d, ndims1:%d", loop1, loop_total, ndims1);

  POSTGIS_DEBUGF(3, "val of histogram = %g", val);
  DBUG_PRINT("postgis", "val of histogram = %g", val);
  /*
  ...
  DBUG_PRINT("postgis", "val scaled to full table size = %g", val);
  ...
  DBUG_PRINT("postgis", "calculate selectivity(%g) = val(%g) / ntuples_max(%g)", selectivity, val, ntuples_max);

  /* Guard against over-estimates and crazy numbers :) */
  if ( isnan(selectivity) || ! isfinite(selectivity) || selectivity < 0.0 ) {
    DBUG_PRINT("postgis", "guard against crazy numbers");
    selectivity = DEFAULT_ND_JOINSEL;
  } else if ( selectivity > 1.0 ) {
    DBUG_PRINT("postgis", "guard against over-estimates");
    selectivity = 1.0;
  }

  DBUG_PRINT("postgis", "selectivity:%g", selectivity);
  return selectivity;
}
...
gserialized_joinsel_internal(PlannerInfo *root, List *args, JoinType jointype, int mode)
{
  ...
  if (!IsA(arg1, Var) || !IsA(arg2, Var)) {
    ...
    DBUG_PRINT("postgis", "%s called with arguments that are not column references", __func__);
  ...
  if (!stats1) {
    char *rel_name = get_rel_name(relid2);
    POSTGIS_DEBUGF(2, "%s: cannot find stats for \"%s\"",  __func__, rel_name ? rel_name : "NULL");
    DBUG_PRINT("postgis", "%s: cannot find stats for \"%s\"",  __func__, rel_name ? rel_name : "NULL");
    return DEFAULT_ND_JOINSEL;
  } else if (!stats2) {
    char *rel_name = get_rel_name(relid2);
    POSTGIS_DEBUGF(2, "%s: cannot find stats for \"%s\"",  __func__, rel_name ? rel_name : "NULL");
  ...
  DBUG_PRINT("postgis", "got selectivity %g", selectivity);
  pfree(stats1);
  pfree(stats2);
  return selectivity;
}
...
Datum gserialized_gist_joinsel(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "join selectivity of the && operator");
  POSTGIS_DEBUGF(2, "%s: entered function", __func__);

  /* Check length of args and punt on > 2 */
  if (list_length(args) != 2) {
    POSTGIS_DEBUGF(2, "%s: got nargs == %d", __func__, list_length(args));
    DBUG_PRINT("postgis", "return default selectivity:%g", DEFAULT_ND_JOINSEL);
  ...
  result = gserialized_joinsel_internal(root, args, jointype, mode);
  DBUG_PRINT("postgis", "return selectivity:%d", result);
...
{
  DBUG_TRACE;
  DBUG_PRINT("postgis", "for (geometry &&& geometry) and (geography && geography), we call into the N-D mode");
...
{
  DBUG_TRACE;
  DBUG_PRINT("postgis", "for (geometry &&& geometry), we call into the 2-D mode");
  PG_RETURN_DATUM(DirectFunctionCall5(gserialized_gist_joinsel,
                                      PG_GETARG_DATUM(0),
                                      PG_GETARG_DATUM(1),
                                      PG_GETARG_DATUM(2),
                               ...
                               int sample_rows, double total_rows, int mode)
{
  ...
  DBUG_PRINT("postgis", "sample_rows: %d, estimate of total_rows: %.6g", sample_rows, total_rows);
  ...
  for ( i = 0; i < sample_rows; i++ ) {
    ...
    /* Skip all NULLs. */
    if ( is_null ) {
      DBUG_PRINT("postgis", "skip all NULLs");
    ...
    if (LW_FAILURE == gserialized_datum_get_gbox_p(datum, &gbox)) {
      /* Skip empties too. */
      DBUG_PRINT("postgis", "skipped empty geometry %d", i);
    ...
    if ( ! gbox_is_valid(&gbox) ) {
      DBUG_PRINT("postgis", "skipped infinite/nan geometry %d", i);
  ...
  DBUG_PRINT("postgis", "stats->attstattarget: %d", stats->attstattarget);
...
#else
  DBUG_PRINT("postgis", "stats->attr->attstattarget: %d", stats->attr->attstattarget);
  ...
  DBUG_PRINT("postgis", "target # of histogram cells: %d", histo_cells_target);

  /* If there's no useful features, we can't work out stats */
  if ( ! notnull_cnt ) {
    DBUG_PRINT("postgis", "if there's no useful features, we can't work out stats");
    stats->stats_valid = false;
    return;
  }

  DBUG_PRINT("postgis", "sample_extent: %s", nd_box_to_json(&sample_extent, ndims));
   ...
   */
  DBUG_PRINT("postgis", "second scan(ndims:%d): compute standard deviation", ndims);
   ...
   */
  DBUG_PRINT("postgis", "third scan: skip hard deviants and compute new histogram box");
  ...
  for ( i = 0; i < notnull_cnt; i++ ) {
    ...
    if ( ! nd_box_intersects(&histo_extent, ndb, ndims) ) {
      ...
      DBUG_PRINT("postgis", "feature %d is a hard deviant, skipped", i);
      sample_boxes[i] = NULL;
      continue;
    }
   ...
   */
  DBUG_PRINT("postgis", "expand the box slightly (1 percent) to avoid edge effects with objects that are on the boundary");
  ...
  DBUG_PRINT("postgis", "histo_ndims:%d", histo_ndims);
  ...
  if ( histo_ndims == 0 ) {
    ...
    DBUG_PRINT("postgis", "special case: no axes have variability");
    histo_cells_new = 1;
    ...
    for ( d = 0; d < ndims; d++ ) {
      ...
      DBUG_PRINT("postgis", "histo_size[%d]: %d", d, histo_size[d]);
      histo_cells_new *= histo_size[d];
    }

    POSTGIS_DEBUGF(3, " histo_cells_new: %d", histo_cells_new);
    DBUG_PRINT("postgis", "histo_cells_new: %d", histo_cells_new);
  } else {
    /*
     ...
     */
    DBUG_PRINT("postgis", "allocating histogram axes based on axis variability");
    ...
    DBUG_PRINT("postgis", "total_distribution: %.8g", total_distribution);
  ...
  DBUG_PRINT("postgis", "update histo_cells to the actual number of cells we need to allocate:%d", histo_cells);
  ...
  for ( i = 0; i < notnull_cnt; i++ ) {
    ...
    DBUG_PRINT("postgis", "feature %d: ibox (%d, %d, %d, %d) (%d, %d, %d, %d)", i,
     ...
     */
    DBUG_PRINT("postgis", "move through all the overlapped histogram cells values and add the box overlap proportion to them");
    ...
    do {
      ...
      DBUG_PRINT("postgis", "ratio (%.8g)  num_cells (%.8g) at (%d, %d, %d, %d)", ratio, num_cells, at[0], at[1], at[2], at[3]);
  ...
  DBUG_PRINT("postgis", "histogram_features: %d, sample_rows: %d, table_rows: %.6g", histogram_features, sample_rows, total_rows);
  ...
  if ( ! histogram_features ) {
    ...
    DBUG_PRINT("postgis", "no features lie in the stats histogram, invalid stats");
    stats->stats_valid = false;
    return;
  }
  ...
  DBUG_PRINT("postgis", "put this histogram data into the right slot/kind");
  ...
  DBUG_PRINT("postgis", " out: %s", nd_stats_to_json(nd_stats));
  DBUG_PRINT("postgis", "out: slot 0: kind %d (STATISTIC_KIND_ND)", stats->stakind[0]);
  DBUG_PRINT("postgis", "out: slot 0: op %d (InvalidOid)", stats->staop[0]);
  DBUG_PRINT("postgis", "out: slot 0: numnumbers %d", stats->numnumbers[0]);
  DBUG_PRINT("postgis", "out: null fraction: %g=%d/%d", stats->stanullfrac, null_cnt, sample_rows);
  DBUG_PRINT("postgis", "out: average width: %d bytes", stats->stawidth);
  DBUG_PRINT("postgis", "out: distinct values: all (no check done)");
  DBUG_PRINT("postgis", "out: %s", nd_stats_to_json(nd_stats));
  /*
  POSTGIS_DEBUGF(3, " out histogram:\n%s", nd_stats_to_grid(nd_stats));
  DBUG_PRINT("postgis", " out histogram:\n%s", nd_stats_to_grid(nd_stats));
  */
                          ...
                          int sample_rows, double total_rows)
{
  ...
  DBUG_PRINT("postgis", "call standard statistics calculation routine to fill in correlation for BRIN to work");
  stats->extra_data = extra_data->std_extra_data;
  extra_data->std_compute_stats(stats, fetchfunc, sample_rows, total_rows);
  stats->extra_data = extra_data;

  /* 2D Mode */
  DBUG_PRINT("postgis", "2D Mode");
  ...
  if (stats->stats_valid) {
    ...
    DBUG_PRINT("postgis", "ND Mode: Only computed if 2D was computed too (not NULL and valid)");
...
Datum gserialized_analyze_nd(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "this function will be called when the ANALYZE command is run on a column of the geometry or geography type");
...
estimate_selectivity(const GBOX *box, const ND_STATS *nd_stats, int mode)
{
  ...
  DBUG_PRINT("postgis", "calculate the overlap of the box on the histogram");

  /* Calculate the overlap of the box on the histogram */
  if ( ! nd_stats ) {
    DBUG_PRINT("postgis", "estimate_selectivity called with null input");
  ...
  DBUG_PRINT("postgis", "mode: %d", mode);

  if ( mode == 2 ) {
    DBUG_PRINT("postgis", "in 2d mode, stripping the computation down to 2d");
    POSTGIS_DEBUG(3, "in 2d mode, stripping the computation down to 2d");
    ndims_max = 2;
  }

  DBUG_PRINT("postgis", "nd_stats->extent: %s", nd_box_to_json(&(nd_stats->extent), nd_stats->ndims));
  DBUG_PRINT("postgis", "nd_box: %s", nd_box_to_json(&(nd_box), gbox_ndims(box)));
  ...
  if ( ! nd_box_intersects(&nd_box, &(nd_stats->extent), ndims_max) ) {
    DBUG_PRINT("postgis", "search box does not overlap histogram, returning 0");
  ...
  if ( nd_box_contains(&nd_box, &(nd_stats->extent), ndims_max) ) {
    DBUG_PRINT("postgis", "search box contains histogram, returning 1");
  ...
  if ( ! nd_box_overlap(nd_stats, &nd_box, &nd_ibox) ) {
    DBUG_PRINT("postgis", "search box overlap with stats histogram failed");
  ...
  for ( d = 0; d < nd_stats->ndims; d++ ) {
    ...
    DBUG_PRINT("postgis", "cell_size[%d] : %.9g", d, cell_size[d]);
  ...
  do {
    ...
    DBUG_PRINT("postgis", "cell (%d,%d), cell value %.6f, ratio %.6f", at[0], at[1], cell_count, ratio);
  } while ( nd_increment(&nd_ibox, nd_stats->ndims, at) );
  ...
  DBUG_PRINT("postgis", "scale by the number of features in our histogram to get the proportion");

  DBUG_PRINT("postgis", "nd_stats->histogram_features = %g", nd_stats->histogram_features);
  DBUG_PRINT("postgis", "nd_stats->histogram_cells = %g", nd_stats->histogram_cells);
  DBUG_PRINT("postgis", "sum(overlapped histogram cells) = %g", total_count);
  DBUG_PRINT("postgis", "selectivity = %g", selectivity);
  ...
  DBUG_PRINT("postgis", "prevent rounding overflows(selectivity:%g)", selectivity);
  return selectivity;
}
...
Datum _postgis_gserialized_sel(PG_FUNCTION_ARGS)
{
  ...
  POSTGIS_DEBUGF(3, " %s", gbox_to_string(&gbox));
  DBUG_PRINT("postgis", "%s", gbox_to_string(&gbox));
...
Datum _postgis_gserialized_joinsel(PG_FUNCTION_ARGS)
{
  ...
  pfree(nd_stats1);
  pfree(nd_stats2);
  DBUG_PRINT("postgis", "return the calculated join selectivity for a pair of tables:%g", selectivity);
  PG_RETURN_FLOAT8(selectivity);
}
...
gserialized_sel_internal(PlannerInfo *root, List *args, int varRelid, int mode)
{
  ...
  DBUG_PRINT("postgis", "should return an estimation of the number of rows returned by a query involving an overlap check");
  POSTGIS_DEBUGF(2, "%s: entered function", __func__);

  if (!get_restriction_variable(root, args, varRelid, &vardata, &other, &varonleft)) {
    POSTGIS_DEBUGF(2, "%s: could not find vardata", __func__);
    DBUG_PRINT("postgis", "could not find vardata");
    return DEFAULT_ND_SEL;
  }

  if (!IsA(other, Const)) {
    ReleaseVariableStats(vardata);
    DBUG_PRINT("postgis", "no constant argument, returning default selectivity %g", DEFAULT_ND_SEL);
  ...
  if ((!otherConst) || otherConst->constisnull) {
    ReleaseVariableStats(vardata);
    DBUG_PRINT("postgis", "constant argument is NULL");
  ...
  if (!gserialized_datum_get_gbox_p(otherConst->constvalue, &search_box)) {
    ReleaseVariableStats(vardata);
    DBUG_PRINT("postgis", "search box is EMPTY");
    POSTGIS_DEBUGF(2, "%s: search box is EMPTY", __func__);
    return 0.0;
  }

  if (!vardata.statsTuple) {
    DBUG_PRINT("postgis", "no statistics available on table. Empty? Need to ANALYZE?");
...
Datum gserialized_gist_sel(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "selectivity is %g", selectivity);
  PG_RETURN_FLOAT8(selectivity);
}
...
index_has_attr(Oid index_oid, Oid table_oid, int16 table_attnum)
{
  ...
  DBUG_PRINT("postgis", "index attnum:%d", index_attnum);
  return index_attnum;
}
...
index_get_am(Oid index_oid)
{
  ...
  DBUG_PRINT("postgis", "lookup for index %u", index_oid);
  ...
  DBUG_PRINT("postgis", "given an index return the access method:%d", index_am);
  return index_am;
}
...
index_get_keytype (Oid index_oid, int16 index_attnum)
{
  ...
  DBUG_PRINT("postgis", "given an index and index attribute, lookup the key type (box2df or gidx) of that index column");
...
table_get_spatial_index(Oid table_oid, int16 attnum, int *key_type, int16 *idx_attnum)
{
  ...
  DBUG_PRINT("postgis", "given a table and attribute number, find any 'spatial index' of that attribute");
  ...
  DBUG_PRINT("postgis", "for each index associated with this table...");
  ...
  foreach(lc, index_list) {
    ...
    DBUG_PRINT("postgis", "is our attribute indexed by this index?");
    *idx_attnum = index_has_attr(index_oid, table_oid, attnum);

    /* No, move on */
    if (*idx_attnum == InvalidAttrNumber) {
      DBUG_PRINT("postgis", "no, move on");
      continue;
    }

    /* We only handle GIST spatial indexes */
    if (index_get_am(index_oid) != GIST_AM_OID) {
      DBUG_PRINT("postgis", "we only handle GIST spatial indexes");
      DBUG_PRINT("postgis", "move on");
      continue;
    }
    ...
    if (atttypid == b2d_oid || atttypid == gdx_oid) {
      DBUG_PRINT("postgis", "spatial key found in this index");
      /* Spatial key found in this index! */
      *key_type = (atttypid == b2d_oid ? STATISTIC_KIND_2D : STATISTIC_KIND_ND);
      DBUG_PRINT("postgis", "return index oid:%u", index_oid);
      return index_oid;
    }
  }
...
spatial_index_read_extent(Oid idx_oid, int idx_att_num, int key_type)
{
  ...
  DBUG_PRINT("postgis", "given an index(%u) and indexed attribute(%d), look up the keys in the top page of the index, ", idx_oid, idx_att_num);
  DBUG_PRINT("postgis", "and using the appropriate key type, return a box that is the union of all those keys");
...
Datum _postgis_gserialized_index_extent(PG_FUNCTION_ARGS)
{
  ...
  if (!gbox)
    ...
    DBUG_PRINT("postgis", "gbox xmin:%g, xmax:%g, ymin:%g, ymax:%g, zmin:%g, zmax:%g, mmin:%g, mmax:%g",
               gbox->xmin, gbox->xmax, gbox->ymin, gbox->ymax, gbox->zmin, gbox->zmax, gbox->mmin, gbox->mmax);
...
get_attnum_attypid(Oid table_oid, const char *col, int16 *attnum, Oid *atttypid)
{
  ...
  DBUG_PRINT("postgis", "given a table and column name, look up the attribute number and type of that column");
  ...
  *attnum = att->attnum;
  DBUG_PRINT("postgis", "atttypid:%u, attnum:%d", *atttypid, *attnum);
  ReleaseSysCache(att_tuple);
  return true;
}
...
Datum gserialized_estimated_extent(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("postgis", "return the estimated extent of the table looking at gathered statistics");
  /* We need to initialize the internal cache to access it later via postgis_oid() */
  postgis_initialize_cache();
  ...
  if ((atttypid != geographyOid) && (atttypid != geometryOid)) {
    DBUG_INSTANT_PRINT("postgis", "column %s.\"%s\" must be a geometry or geography", nsp_tbl, col);
    elog(ERROR, "column %s.\"%s\" must be a geometry or geography", nsp_tbl, col);
  }

  DBUG_PRINT("postgis", "we can only do estimates on geograpy and geometry");
  ...
  if (idx_oid != InvalidOid) {
    ...
    DBUG_PRINT("postgis", "index for %s.\"%s\" exists, reading gbox from there", nsp_tbl, col);
  ...
  /* works if ANALYZE has been run */
    ...
    DBUG_PRINT("postgis", "index for %s.\"%s\" does not exist", nsp_tbl, col);
  ...
  if (atttypid == geographyOid) {
    ...
    DBUG_PRINT("postgis", "convert geocentric geography box into a planar box that users understand");
    gbox_geocentric_get_gbox_cartesian(gbox, gbox_planar);
    DBUG_PRINT("postgis", "gbox xmin:%g, xmax:%g, ymin:%g, ymax:%g, zmin:%g, zmax:%g, mmin:%g, mmax:%g",
               gbox_planar->xmin, gbox_planar->xmax, gbox_planar->ymin, gbox_planar->ymax, gbox_planar->zmin, gbox_planar->zmax,
               gbox_planar->mmin, gbox_planar->mmax);
