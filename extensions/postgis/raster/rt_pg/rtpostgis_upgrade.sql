...
--
-- UPGRADE SCRIPT TO PostGIS 3.7.0dev
--
...
BEGIN
    ...
    WITH versions AS (
      SELECT '3.7.0dev'::text as upgraded,
...
BEGIN
  ...
  THEN
      EXECUTE $postgis_type_upgrade_parsed_def$ CREATE TYPE raster (
...
CREATE OR REPLACE FUNCTION postgis_raster_scripts_installed() RETURNS text
       AS $$ SELECT trim('3.7.0dev'::text || $rev$ 3.6.0rc2-503-g707fb97 $rev$) AS version $$
...
BEGIN
  IF 201 > version_from_num
     FROM _postgis_upgrade_info()
  THEN
      EXECUTE $postgis_type_upgrade_parsed_def$ CREATE TYPE rastbandarg AS (
 $postgis_type_upgrade_parsed_def$;
  END IF;
END
$postgis_type_upgrade$;
...
BEGIN
  IF 200 > version_from_num
     FROM _postgis_upgrade_info()
  THEN
      EXECUTE $postgis_type_upgrade_parsed_def$ CREATE TYPE geomval AS (
 $postgis_type_upgrade_parsed_def$;
  END IF;
END
$postgis_type_upgrade$;
...
BEGIN
  IF 201 > version_from_num
     FROM _postgis_upgrade_info()
  THEN
      EXECUTE $postgis_type_upgrade_parsed_def$ CREATE TYPE addbandarg AS (
 $postgis_type_upgrade_parsed_def$;
  END IF;
END
$postgis_type_upgrade$;
...
OR version_from_num IN ( 201 )     FROM _postgis_upgrade_info()
  THEN
      EXECUTE $postgis_type_upgrade_parsed_def$ CREATE TYPE summarystats AS (
 $postgis_type_upgrade_parsed_def$;
  END IF;
END
$postgis_type_upgrade$;
...
BEGIN
  IF 202 > version_from_num
     FROM _postgis_upgrade_info()
  THEN
      EXECUTE $postgis_type_upgrade_parsed_def$ CREATE TYPE agg_count AS (
 $postgis_type_upgrade_parsed_def$;
  END IF;
END
$postgis_type_upgrade$;
...
BEGIN
  IF 200 > version_from_num
     FROM _postgis_upgrade_info()
  THEN
      EXECUTE $postgis_type_upgrade_parsed_def$ CREATE TYPE reclassarg AS (
 $postgis_type_upgrade_parsed_def$;
  END IF;
END
$postgis_type_upgrade$;
...
CREATE OR REPLACE FUNCTION ST_ReclassExact(rast raster,
	...
	RETURNS raster
	AS '$libdir/postgis_raster-3', 'RASTER_reclass_exact'
	LANGUAGE 'c' IMMUTABLE PARALLEL SAFE;
...
BEGIN
  IF 201 > version_from_num
     FROM _postgis_upgrade_info()
  THEN
      EXECUTE $postgis_type_upgrade_parsed_def$ CREATE TYPE agg_samealignment AS (
 $postgis_type_upgrade_parsed_def$;
  END IF;
END
$postgis_type_upgrade$;
...
CREATE OR REPLACE FUNCTION ST_IntersectionFractions(rast raster, geom geometry)
	RETURNS raster
	AS '$libdir/postgis_raster-3','RASTER_intersectionFractions'
	LANGUAGE 'c' IMMUTABLE STRICT PARALLEL SAFE;
...
BEGIN
  IF 201 > version_from_num
     FROM _postgis_upgrade_info()
  THEN
      EXECUTE $postgis_type_upgrade_parsed_def$ CREATE TYPE unionarg AS (
 $postgis_type_upgrade_parsed_def$;
  END IF;
END
$postgis_type_upgrade$;
...
END;
$BODY$ LANGUAGE 'plpgsql';
-- Aggregate ST_AsRasterAgg(geom geometry,val float8,ref raster,pixeltype text,nodataval float8,uniontype text,touched boolean) -- LastUpdated: 306
DO LANGUAGE 'plpgsql'
$postgis_proc_upgrade$
BEGIN
  IF pg_catalog.current_setting('server_version_num')::integer >= 120000
  THEN
    EXECUTE $postgis_proc_upgrade_parsed_def$ CREATE OR REPLACE AGGREGATE ST_AsRasterAgg(geom geometry, val float8, ref raster, pixeltype text, nodataval float8, uniontype text, touched boolean) (
 ...
 $postgis_proc_upgrade_parsed_def$;
  ELSIF 306 > version_from_num OR (
      306 = version_from_num AND version_from_isdev
    ) FROM _postgis_upgrade_info()
  THEN
    EXECUTE 'DROP AGGREGATE IF EXISTS ST_AsRasterAgg(geom geometry,val float8,ref raster,pixeltype text,nodataval float8,uniontype text,touched boolean)';
    EXECUTE $postgis_proc_upgrade_parsed_def$ CREATE AGGREGATE ST_AsRasterAgg(geom geometry, val float8, ref raster, pixeltype text, nodataval float8, uniontype text, touched boolean) (
	SFUNC = _ST_AsRasterAgg_transfn,
	STYPE = raster,
	parallel = safe,
	FINALFUNC = _ST_AsRasterAgg_finalfn
