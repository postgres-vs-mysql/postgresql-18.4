...
DROP AGGREGATE IF EXISTS ST_AsRasterAgg (geom geometry, val float8, ref raster, pixeltype text, nodataval float8, uniontype text, touched boolean);
...
DROP FUNCTION IF EXISTS ST_ReclassExact (rast raster,	inputvalues float8[], outputvalues float8[],	bandnumber integer ,	outputpixeltype text ,	nodatavalue float8 );
...
DROP FUNCTION IF EXISTS ST_IntersectionFractions (rast raster, geom geometry);
...
DROP FUNCTION IF EXISTS _ST_AsRasterAgg_transfn (staterast raster, geom geometry, val float8, rast raster, pixeltype text, nodataval float8, uniontype text, touched boolean);
DROP FUNCTION IF EXISTS _ST_AsRasterAgg_finalfn (rast raster);
