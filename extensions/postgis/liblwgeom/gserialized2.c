...
{
  DBUG_TRACE;
  int result = G2FLAGS_GET_BBOX(g->gflags);
  if (result) {
...
{
  DBUG_TRACE;
  int result = G2FLAGS_GET_EXTENDED(g->gflags);
  if (result) {
    DBUG_PRINT("postgis", "result: true");
  } else {
    DBUG_PRINT("postgis", "result: false");
...
{
  DBUG_TRACE;
  int result = G2FLAGS_GET_Z(g->gflags);
  if (result) {
    DBUG_PRINT("postgis", "result: true");
  } else {
    DBUG_PRINT("postgis", "result: false");
...
{
  DBUG_TRACE;
  int result = G2FLAGS_GET_M(g->gflags);
  if (result) {
    DBUG_PRINT("postgis", "result: true");
  } else {
    DBUG_PRINT("postgis", "result: false");
...
{
  DBUG_TRACE;
  int result = G2FLAGS_NDIMS(g->gflags);
  DBUG_PRINT("postgis", "result: %d",result);
  return result;
}
...
{
  DBUG_TRACE;
  int result = G2FLAGS_GET_GEODETIC(g->gflags);
...
int32_t gserialized2_get_srid(const GSERIALIZED *g)
{
  ...
  if (srid == 0) {
    DBUG_PRINT("postgis", "result: SRID_UNKNOWN");
    return SRID_UNKNOWN;
  } else {
    DBUG_PRINT("postgis", "result: %d", srid);
    return srid;
  }
}
...
{
  DBUG_TRACE;
  DBUG_PRINT("postgis", "called with srid = %d", srid);
...
int gserialized2_is_empty(const GSERIALIZED *g)
{
  ...
  if (isempty == LW_TRUE) {
    DBUG_PRINT("postgis", "isempty: LW_TRUE");
  } else if (isempty == LW_FALSE) {
    DBUG_PRINT("postgis", "isempty: LW_FALSE");
  }
  return isempty;
}
...
int gserialized2_read_gbox_p(const GSERIALIZED *g, GBOX *gbox)
{
  ...
  if (G2FLAGS_GET_BBOX(gflags)) {
    ...
    DBUG_PRINT("postgis", "has pre-calculated box");
...
gserialized2_peek_gbox_p(const GSERIALIZED *g, GBOX *gbox)
{
  ...
  if (type == POINTTYPE) {
    ...
    DBUG_PRINT("postgis", "boxes of points are easy peasy");
  ...
  /* We can calculate the box of a two-point cartesian line trivially */
    ...
    DBUG_PRINT("postgis", "we can calculate the box of a two-point cartesian line trivially");
  ...
  /* We can also do single-entry multi-points */
    ...
    DBUG_PRINT("postgis", "we can do single-entry multi-points");
  ...
  /* And we can do single-entry multi-lines with two vertices (!!!) */
    ...
    DBUG_PRINT("postgis", "we can do single-entry multi-lines with two vertices");
...
static size_t gserialized2_from_lwpoint_size(const LWPOINT *point)
{
  ...
  DBUG_PRINT("postgis", "point size = %zu", size);
  return size;
}
...
static size_t gserialized2_from_lwline_size(const LWLINE *line)
{
  ...
  DBUG_PRINT("postgis", "linestring size = %zu", size);
...
static size_t gserialized2_from_lwtriangle_size(const LWTRIANGLE *triangle)
{
  ...
  DBUG_PRINT("postgis", "triangle size = %zu", size);
...
static size_t gserialized2_from_lwpoly_size(const LWPOLY *poly)
{
  ...
  DBUG_PRINT("postgis", "polygon size = %zu", size);
...
static size_t gserialized2_from_lwcircstring_size(const LWCIRCSTRING *curve)
{
  ...
  DBUG_PRINT("postgis", "circstring size = %zu", size);
...
static size_t gserialized2_from_lwcollection_size(const LWCOLLECTION *col)
{
  ...
  for (i = 0; i < col->ngeoms; i++) {
    ...
    DBUG_PRINT("postgis", "lwcollection subgeom(%d) size = %zu", i, subsize);
  }

  LWDEBUGF(3, "lwcollection size = %zu", size);
  DBUG_PRINT("postgis", "lwcollection size = %zu", size);
...
{
  DBUG_TRACE;
  DBUG_PRINT("postgis", "input type: %s", lwtype_name(geom->type));
...
size_t gserialized2_from_lwgeom_size(const LWGEOM *geom)
{
  ...
  DBUG_PRINT("postgis", "size = %zu", size);
...
static size_t gserialized2_from_lwpoint(const LWPOINT *point, uint8_t *buf)
{
  ...
  /* Write in the type. */
  DBUG_PRINT("postgis", "write in the type(%d)", type);
  memcpy(loc, &type, sizeof(uint32_t));
  loc += sizeof(uint32_t);
  /* Write in the number of points (0 => empty). */
  DBUG_PRINT("postgis", "write in the number of points:%u", point->point->npoints);
  ...
  result = (size_t)(loc - buf);
  DBUG_PRINT("postgis", "total write size:%lu", result);
  return result;
}
...
static size_t gserialized2_from_lwline(const LWLINE *line, uint8_t *buf)
{
  ...
  DBUG_PRINT("postgis", "added npoints (%d)", line->points->npoints);
  ...
  DBUG_PRINT("postgis", "copied serialized_pointlist (%d bytes)", ptsize * line->points->npoints);
...
static size_t gserialized2_from_lwpoly(const LWPOLY *poly, uint8_t *buf)
{
  ...
  /* Write in the nrings. */
  DBUG_PRINT("postgis", "write in the nrings(%u)", poly->nrings);
  memcpy(loc, &(poly->nrings), sizeof(uint32_t));
  loc += sizeof(uint32_t);

  /* Write in the npoints per ring. */
  DBUG_PRINT("postgis", "write in the the npoints per ring");
  for (i = 0; i < poly->nrings; i++) {
  ...
  if (poly->nrings % 2) {
    ...
    loc += sizeof(uint32_t);
    DBUG_PRINT("postgis", "add in padding if necessary to remain double aligned");
  }

  /* Copy in the ordinates. */
    
  DBUG_PRINT("postgis", "copy in the ordinates");
  for (i = 0; i < poly->nrings; i++) {
...
static size_t gserialized2_from_lwtriangle(const LWTRIANGLE *triangle, uint8_t *buf)
{
  ...
  /* Write in the npoints. */
  DBUG_PRINT("postgis", "write in the npoints(%u)", triangle->points->npoints);
  memcpy(loc, &(triangle->points->npoints), sizeof(uint32_t));
  loc += sizeof(uint32_t);

  DBUG_PRINT("postgis", "added npoints (%d)", triangle->points->npoints);
  ...
  DBUG_PRINT("postgis", "copied serialized_pointlist (%d bytes)", ptsize * triangle->points->npoints);
...
static size_t gserialized2_from_lwcircstring(const LWCIRCSTRING *curve, uint8_t *buf)
{
  ...
  DBUG_PRINT("postgis", "write in the npoints(%u)", curve->points->npoints);
  loc += sizeof(uint32_t);
...
static size_t gserialized2_from_lwcollection(const LWCOLLECTION *coll, uint8_t *buf)
{
  ...
  DBUG_PRINT("postgis", "write in the number of subgeoms(%u)", coll->ngeoms);
  memcpy(loc, &coll->ngeoms, sizeof(uint32_t));
  loc += sizeof(uint32_t);

  /* Serialize subgeoms. */
  DBUG_PRINT("postgis", "serialize subgeoms");
...
static size_t gserialized2_from_lwgeom_any(const LWGEOM *geom, uint8_t *buf)
{
  ...
  DBUG_PRINT("postgis", "input type (%d) %s, hasz: %d hasm: %d", geom->type, lwtype_name(geom->type), 
...
static size_t gserialized2_from_gbox(const GBOX *gbox, uint8_t *buf)
{
  ...
  if (FLAGS_GET_GEODETIC(gbox->flags)) {
    ...
    DBUG_PRINT("postgis", "returning size %zu", return_size);
...
GSERIALIZED* gserialized2_from_lwgeom(LWGEOM *geom, size_t *size)
{
  ...
  */
  if ((!geom->bbox) && lwgeom_needs_bbox(geom) && (!lwgeom_is_empty(geom))) {
    DBUG_PRINT("postgis", "see if we need a bounding box, add one if we don't have one");
  ...
  */
  ...
  DBUG_PRINT("postgis", "set the SRID");
  /* Set the SRID! */
  ...
  */
  ...
  if (geom->bbox) {
    DBUG_PRINT("postgis", "write in the serialized form of the gbox");
    ptr += gserialized2_from_gbox(geom->bbox, ptr);
  }

  DBUG_PRINT("postgis", "write in the serialized form of the geometry");
  ...
  if (size) {/* Return the output size to the caller if necessary. */
    DBUG_PRINT("postgis", "return the output size to the caller:%lu", return_size);
    *size = return_size;
  }
...
lwpoint_from_gserialized2_buffer(uint8_t *data_ptr, lwflags_t lwflags, size_t *size, int32_t srid)
{
  ...
  DBUG_PRINT("postgis", "type:%d, npoints:%u", POINTTYPE, npoints);
  ...
  if (size) {
    *size = data_ptr - start_ptr;
    DBUG_PRINT("postgis", "return the size to the caller:%lu", *size);
  }
...
lwline_from_gserialized2_buffer(uint8_t *data_ptr, lwflags_t lwflags, size_t *size, int32_t srid)
{
  ...
  DBUG_PRINT("postgis", "type:%d, npoints:%u", LINETYPE, npoints);
  if (npoints > 0)
...
lwpoly_from_gserialized2_buffer(uint8_t *data_ptr, lwflags_t lwflags, size_t *size, int32_t srid)
{
  ...
  poly->nrings = nrings;
  LWDEBUGF(4, "type:%d, nrings = %d", POLYGONTYPE, nrings);
  ...
  for (i = 0; i < nrings; i++) {
    ...
    DBUG_PRINT("postgis", "read in the number of points:%u", npoints);
    data_ptr += 4;
...
lwcollection_from_gserialized2_buffer(uint8_t *data_ptr, lwflags_t lwflags, size_t *size, int32_t srid)
{
  ...
  DBUG_PRINT("postgis", "ngeoms:%u, collection type (%s)", ngeoms, lwtype_name(type));
...
lwgeom_from_gserialized2_buffer(uint8_t *data_ptr, lwflags_t lwflags, size_t *g_size, int32_t srid)
{
  ...
  DBUG_PRINT("postgis", "got type %d (%s), hasz=%d hasm=%d geodetic=%d hasbox=%d", type, lwtype_name(type),
...
LWGEOM* lwgeom_from_gserialized2(const GSERIALIZED *g)
{
  ...
  DBUG_PRINT("postgis", "got type %d (%s), srid=%d", lwtype, lwtype_name(lwtype), srid);
...
GSERIALIZED* gserialized2_set_gbox(GSERIALIZED *g, GBOX *gbox)
{
  ...
  DBUG_PRINT("postgis", "update the bounding box of a #GSERIALIZED");
...
GSERIALIZED* gserialized2_drop_gbox(GSERIALIZED *g)
{
  ...
  DBUG_PRINT("postgis", "remove the bounding box from a #GSERIALIZED");
