...
uint32_t gserialized_get_type(const GSERIALIZED *g)
{
  ...
  uint32_t result;
  if (GFLAGS_GET_VERSION(g->gflags)) {
    result = gserialized2_get_type(g);
    DBUG_PRINT("postgis", "result: %u", result);
    return result;
  } else {
    result = gserialized1_get_type(g);
    DBUG_PRINT("postgis", "result: %u", result);
    return result;
  }
}
...
uint32_t gserialized_max_header_size(void)
{
  ...
  result = sz1 > sz2 ? sz1 : sz2;
  DBUG_PRINT("postgis", "result: %lu", result);
...
int32_t gserialized_get_srid(const GSERIALIZED *g)
{
  ...
  int32_t result;
  if (GFLAGS_GET_VERSION(g->gflags)) {
    result = gserialized2_get_srid(g);
    DBUG_PRINT("postgis", "extract the SRID from the serialized form:%d", result);
    return result;
  } else {
    result = gserialized1_get_srid(g);
    DBUG_PRINT("postgis", "extract the SRID from the serialized form:%d", result);
    return result;
  }
}
...
int gserialized_is_empty(const GSERIALIZED *g)
{
  int result; 
  if (GFLAGS_GET_VERSION(g->gflags))
    result = gserialized2_is_empty(g);
  else
    result = gserialized1_is_empty(g);

  if (result) {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED is empty without deserializing first? Yes");
  } else {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED is empty without deserializing first? No");
  }
  return result;
}
...
int gserialized_has_bbox(const GSERIALIZED *g)
{
  int result;
  if (GFLAGS_GET_VERSION(g->gflags))
    result = gserialized2_has_bbox(g);
  else
    result = gserialized1_has_bbox(g);

  if (result) {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED has a bounding box without deserializing first? Yes");
  } else {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED has a bounding box without deserializing first? No");
  }
  return result;
}
...
int gserialized_has_z(const GSERIALIZED *g)
{
  int result;
  if (GFLAGS_GET_VERSION(g->gflags))
    result = gserialized2_has_z(g);
  else
    result = gserialized1_has_z(g);

  if (result) {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED has a Z ordinate? Yes");
  } else {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED has a Z ordinate? No");
  }
  return result;
}
...
int gserialized_has_m(const GSERIALIZED *g)
{
  ...
  if (GFLAGS_GET_VERSION(g->gflags))
    result = gserialized2_has_m(g);
  else
    result = gserialized1_has_m(g);

  if (result) {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED has a M ordinate? Yes");
  } else {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED has a M ordinate? No");
  }
  return result;
}
...
int gserialized_is_geodetic(const GSERIALIZED *g)
{
  ...
  int result;
  if (GFLAGS_GET_VERSION(g->gflags))
    result = gserialized2_is_geodetic(g);
  else
    result = gserialized1_is_geodetic(g);

  if (result) {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED is a geography? Yes");
  } else {
    DBUG_PRINT("postgis", "check if a #GSERIALIZED is a geography? No");
  }
  return result;
}
...
int gserialized_ndims(const GSERIALIZED *g)
{
  ...
  int result;
  if (GFLAGS_GET_VERSION(g->gflags))
    result = gserialized2_ndims(g);
  else
    result = gserialized1_ndims(g);

  DBUG_PRINT("postgis", "return the number of dimensions (2, 3, 4) in a geometry:%d", result);
  return result;
}
...
size_t gserialized_from_lwgeom_size(const LWGEOM *geom)
{
  ...
  size_t size = gserialized2_from_lwgeom_size(geom);
  DBUG_PRINT("postgis", "rReturn the memory size a GSERIALIZED will occupy for a given LWGEOM: %lu", size);
  return size;
}
...
int gserialized_cmp(const GSERIALIZED *g1, const GSERIALIZED *g2)
{
  ...
  if (bsz1 == bsz2 && cmp_srid == 0 && cmp == 0 && g1hasz == g2hasz && g1hasm == g2hasm) {
    ...
    if (g1_is_empty && !g2_is_empty) {
      DBUG_PRINT("postgis", "Empty < Non-empty and return -1");
      return -1;
    }

    /* Non-empty > Empty */
    if (!g1_is_empty && g2_is_empty) {
      DBUG_PRINT("postgis", "Non-empty > Empty and return 1");
      return 1;
    }
    ...
    if (!g1_is_empty && !g2_is_empty) {
      ...
      DBUG_PRINT("postgis", "using the boxes, calculate sortable hash key");

      if (hash1 > hash2) {
        DBUG_PRINT("postgis", "hash1 > hash2 and return 1");
        return 1;
      }
    ...
    if (bsz1 != bsz2 && cmp == 0) {
      DBUG_PRINT("postgis", "prefix comes before longer one");

      if (bsz1 < bsz2) {
        DBUG_PRINT("postgis", "bsz1 < bsz2 and return -1");
        return -1;
      }
    ...
    if (cmp_srid != 0) {
      DBUG_PRINT("postgis", "if SRID is not equal, sort on it");

      if (srid1 > srid2) {
        DBUG_PRINT("postgis", "srid1 > srid2 and return 1");
      } else {
        DBUG_PRINT("postgis", "srid1 < srid2 and return -1");
      }
    ...
    /* ZM flag sort*/
    if (g1hasz != g2hasz) {
      DBUG_PRINT("postgis", "ZM flag sort");

      if (g1hasz > g2hasz) {
        DBUG_PRINT("postgis", "g1hasz > g2hasz and return 1");
      } else {
        DBUG_PRINT("postgis", "g1hasz < g2hasz and return -1");
      }
    ...
    if (g1hasm != g2hasm) {
      if (g1hasm > g2hasm) {
        DBUG_PRINT("postgis", "g1hasm > g2hasm and return 1");
      } else {
        DBUG_PRINT("postgis", "g1hasm < g2hasm and return -1");
      }
    ...
    if (cmp > 0) {
      DBUG_PRINT("postgis", "cmp > 0 and return 1");
    } else {
      DBUG_PRINT("postgis", "cmp < 0 and return -1");
    }
