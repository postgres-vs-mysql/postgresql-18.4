...
int wkt_lexer_read_srid(char *str)
{
  ...
  DBUG_PRINT("postgis", "read the SRID number from an SRID=<> string:%d", srid);
  return srid;
}
...
static lwflags_t wkt_dimensionality(char *dimensionality)
{
  ...
  DBUG_PRINT("postgis", "dimensionality:'%s'", dimensionality);
...
static int wkt_pointarray_dimensionality(POINTARRAY *pa, lwflags_t flags)
{
  ...
  DBUG_PRINT("postgis", "dimensionality ndims == %d", ndims);
  DBUG_PRINT("postgis", "FLAGS_NDIMS(pa->flags) == %d", FLAGS_NDIMS(pa->flags));
  /*
...
POINT wkt_parser_coord_2(double c1, double c2)
{
  ...
  DBUG_PRINT("postgis", "build a 2d coordinate(point.x:%g, point.y:%g)", c1, c2);
  return p;
}
...
POINT wkt_parser_coord_3(double c1, double c2, double c3)
{
  ...
  FLAGS_SET_M(p.flags, 0);
  DBUG_PRINT("postgis", "build a 3d coordinate(point.x:%g, point.y:%g, point.z:%g)", c1, c2, c3);
  return p;
}
...
POINT wkt_parser_coord_4(double c1, double c2, double c3, double c4)
{
  ...
  DBUG_PRINT("postgis", "build a 4d coordinate(point.x:%g, point.y:%g, point.z:%g, point.m:%g)", c1, c2, c3, c4);
  return p;
}
...
POINTARRAY* wkt_parser_ptarray_add_coord(POINTARRAY *pa, POINT p)
{
  ...
  if (FLAGS_GET_Z(pa->flags)) {
    if( FLAGS_GET_M(pa->flags) ) {
      DBUG_PRINT("postgis", "add point(x:%g, y:%g, z:%g, m:%g)", pt.x, pt.y, pt.z, pt.m);
    } else {
      DBUG_PRINT("postgis", "add point(x:%g, y:%g, z:%g)", pt.x, pt.y, pt.z);
    }
  } else {
    DBUG_PRINT("postgis", "add point(x:%g, y:%g)", pt.x, pt.y);
  }
...
LWGEOM* wkt_parser_curvepolygon_add_ring(LWGEOM *poly, LWGEOM *ring)
{
  ...
  if( ! (ring && poly) ) {
    SET_PARSER_ERROR(PARSER_ERROR_OTHER);
    DBUG_PRINT("postgis", "inputs are null");
  ...
  if( FLAGS_NDIMS(poly->flags) != FLAGS_NDIMS(ring->flags) ) {
    ...
    DBUG_PRINT("postgis", "dimensionality does not match");
    lwgeom_free(ring);
    lwgeom_free(poly);
    SET_PARSER_ERROR(PARSER_ERROR_MIXDIMS);
    return NULL;
  ...
  if( (global_parser_result.parser_check_flags & LW_PARSER_CHECK_MINPOINTS) ) {
    ...
    vertices = lwgeom_count_vertices(ring);
    DBUG_PRINT("postgis", "lwgeom_count_vertices:%u, vertices_needed:%u", vertices, vertices_needed);
    if (vertices < vertices_needed) {
      LWDEBUG(4, "number of points is incorrect");
      DBUG_PRINT("postgis", "number of points is incorrect");
      lwgeom_free(ring);
      lwgeom_free(poly);
      SET_PARSER_ERROR(PARSER_ERROR_MOREPOINTS);
      return NULL;
  ...
  if( (global_parser_result.parser_check_flags & LW_PARSER_CHECK_CLOSURE) ) {
    ...
    DBUG_PRINT("postgis", "checking ring closure");
    switch ( ring->type ) {
      case LINETYPE:
    ...
    if ( ! is_closed ) {
      ...
      DBUG_PRINT("postgis", "ring is not closed");
      lwgeom_free(ring);
      lwgeom_free(poly);
      SET_PARSER_ERROR(PARSER_ERROR_UNCLOSED);
      return NULL;
  ...
  if( LW_FAILURE == lwcurvepoly_add_ring(lwgeom_as_lwcurvepoly(poly), ring) ) {
    ...
    DBUG_PRINT("postgis", "failed to add ring");
    lwgeom_free(ring);
    lwgeom_free(poly);
    SET_PARSER_ERROR(PARSER_ERROR_OTHER);
    return NULL;
...
LWGEOM* wkt_parser_collection_finalize(int lwtype, LWGEOM *geom, char *dimensionality)
{
  ...
  /* No geometry means it is empty */
  if( ! geom ) {
    DBUG_PRINT("postgis", "no geometry means it is empty");
...
{
  DBUG_TRACE;
  DBUG_PRINT("postgis", "srid:%d", srid);
  LWDEBUG(4, "entered");
...
LWGEOM *lwgeom_from_wkt(const char *wkt, const char check)
{
  ...
  DBUG_PRINT("postgis", "wkt:'%s'", wkt);
  if( LW_FAILURE == lwgeom_parse_wkt(&r, wkt, check) ) {
    lwerror("%s", r.message);
    return NULL;
  }
