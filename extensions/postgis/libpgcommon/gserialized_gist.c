...
GIDX* gidx_new(int ndims)
{
  ...
  DBUG_PRINT("postgis", "created new gidx of %d dimensions, size %d", ndims, (int)size);
  SET_VARSIZE(g, size);
  return g;
}
...
static int gidx_from_gbox_p(GBOX box, GIDX *a)
{
  ...
  DBUG_PRINT("postgis", "convert a double-based GBOX into a float-based GIDX, ensuring the float box is larger than the double box");
  ...
  DBUG_PRINT("postgis", "converted %s to %s", gbox_to_string(&box), gidx_to_string(a));
...
{
  DBUG_TRACE;
  DBUG_PRINT("postgis", "convert a gidx to a gbox");
...
gserialized_datum_get_gidx_p(Datum gsdatum, GIDX *gidx)
{
  ...
  DBUG_PRINT("postgis", "peek into a #GSERIALIZED datum to find the bounding box");

  /* Do we even have a serialized bounding box? */
  if (gserialized_has_bbox(gpart)) {
    DBUG_PRINT("postgis", "do we even have a serialized bounding box? Yes!");
    ...
    LWGEOM *lwgeom;
    GBOX gbox;
    DBUG_PRINT("postgis", "no, we need to calculate it from the full object");

    if (need_detoast && LWSIZE_GET(gpart->size) >= gserialized_max_header_size()) {
      /* If we haven't, read the whole gserialized object */
      DBUG_PRINT("postgis", "if we haven't, read the whole gserialized object");
    ...
    if (lwgeom_calculate_gbox(lwgeom, &gbox) == LW_FAILURE) {
      DBUG_PRINT("postgis", "could not calculate bbox, returning failure");
  ...
  DBUG_PRINT("postgis", "got gidx %s", gidx_to_string(gidx));
  return LW_SUCCESS;
}
