...
COMMENT ON FUNCTION topology.AddTopoGeometryColumn(name , name , name , name , varchar , integer ) IS 'args: topology_name, schema_name, table_name, column_name, feature_type, child_layer - Adds a topogeometry column to an existing table, registers this new column as a layer in topology.layer and returns the new layer_id.';
			
COMMENT ON FUNCTION topology.AddTopoGeometryColumn(name , regclass , name , integer , varchar , integer ) IS 'args: topology_name, tab, column_name, layer_id, feature_type, child_layer - Adds a topogeometry column to an existing table, registers this new column as a layer in topology.layer and returns the new layer_id.';
...
COMMENT ON FUNCTION topology.FixCorruptTopoGeometryColumn(name , name , name ) IS 'args: layerSchema, layerTable, layerColumn - Fixes topogeometry corruption caused by upgrade to postgis_topology 3.6.0 and higher';
...
COMMENT ON FUNCTION topology.ValidateTopologyPrecision(name , geometry, float8) IS 'args: toponame, bbox, gridSize - Returns non-precise vertices in the topology.';
			
COMMENT ON FUNCTION topology.MakeTopologyPrecise(name , geometry, float8) IS 'args: toponame, bbox, gridSize - Snap topology vertices to precision grid.';
...
COMMENT ON FUNCTION topology.TotalTopologySize(name ) IS 'args: toponame - Total disk space used by the specified topology, including all indexes and TOAST data.';
			
COMMENT ON FUNCTION topology.UpgradeTopology(name ) IS 'args: toponame - Upgrades the specified topology to support large ids (int8) for topology and primitive ids.';
			
COMMENT ON FUNCTION topology.CreateTopology(name , integer , double precision , boolean , integer , boolean ) IS 'args: topology_schema_name, srid, prec=0, hasz=false, topoid=0, useslargeids=false - Creates a new topology schema and registers it in the topology.topology table.';
...
COMMENT ON FUNCTION topology.TopoGeo_AddLineString(varchar , geometry , float8 , int ) IS 'args: atopology, aline, tolerance, max_edges - Adds a linestring to an existing topology using a tolerance and possibly splitting existing edges/faces.';
...
COMMENT ON FUNCTION topology.ST_AddIsoNode(varchar , bigint , geometry ) IS 'args: atopology, aface, apoint - Adds an isolated node to a face in a topology and returns the nodeid of the new node. If face is null, the node is still created.';
			
COMMENT ON FUNCTION topology.ST_AddIsoEdge(varchar , bigint , bigint , geometry ) IS 'args: atopology, anode, anothernode, alinestring - Adds an isolated edge defined by geometry alinestring to a topology connecting two existing isolated nodes anode and anothernode and returns the edge id of the new edge.';
			
COMMENT ON FUNCTION topology.ST_AddEdgeNewFaces(varchar , bigint , bigint , geometry ) IS 'args: atopology, anode, anothernode, acurve - Add a new edge and, if in doing so it splits a face, delete the original face and replace it with two new faces.';
			
COMMENT ON FUNCTION topology.ST_AddEdgeModFace(varchar , bigint , bigint , geometry ) IS 'args: atopology, anode, anothernode, acurve - Add a new edge and, if in doing so it splits a face, modify the original face and add a new face.';
			
COMMENT ON FUNCTION topology.ST_RemEdgeNewFace(varchar , bigint ) IS 'args: atopology, anedge - Removes an edge and, if the removed edge separated two faces,delete the original faces and replace them with a new face.';
			
COMMENT ON FUNCTION topology.ST_RemEdgeModFace(varchar , bigint ) IS 'args: atopology, anedge - Removes an edge, and if the edge separates two facesdeletes one face and modifies the other face to cover the space of both.';
			
COMMENT ON FUNCTION topology.ST_ChangeEdgeGeom(varchar , bigint , geometry ) IS 'args: atopology, anedge, acurve - Changes the shape of an edge without affecting the topology structure.';
			
COMMENT ON FUNCTION topology.ST_ModEdgeSplit(varchar , bigint , geometry ) IS 'args: atopology, anedge, apoint - Split an edge by creating a new node along an existing edge, modifying the original edge and adding a new edge.';
			
COMMENT ON FUNCTION topology.ST_ModEdgeHeal(varchar , bigint , bigint ) IS 'args: atopology, anedge, anotheredge - Heals two edges by deleting the node connecting them, modifying the first edgeand deleting the second edge. Returns the id of the deleted node.';
			
COMMENT ON FUNCTION topology.ST_NewEdgeHeal(varchar , bigint , bigint ) IS 'args: atopology, anedge, anotheredge - Heals two edges by deleting the node connecting them, deleting both edges,and replacing them with an edge whose direction is the same as the firstedge provided.';
			
COMMENT ON FUNCTION topology.ST_MoveIsoNode(varchar , bigint , geometry ) IS 'args: atopology, anode, apoint - Moves an isolated node in a topology from one point to another. If new apoint geometry exists as a node an error is thrown. Returns description of move.';
			
COMMENT ON FUNCTION topology.ST_NewEdgesSplit(varchar , bigint , geometry ) IS 'args: atopology, anedge, apoint - Split an edge by creating a new node along an existing edge, deleting the original edge and replacing it with two new edges. Returns the id of the new node created that joins the new edges.';
			
COMMENT ON FUNCTION topology.ST_RemoveIsoNode(varchar , bigint ) IS 'args: atopology, anode - Removes an isolated node and returns description of action. If the node is not isolated (is start or end of an edge), then an exception is thrown.';
			
COMMENT ON FUNCTION topology.ST_RemoveIsoEdge(varchar , bigint ) IS 'args: atopology, anedge - Removes an isolated edge and returns description of action. If the edge is not isolated, then an exception is thrown.';
...
COMMENT ON FUNCTION topology.ST_GetFaceEdges(varchar , bigint ) IS 'args: atopology, aface - Returns a set of ordered edges that bound aface.';
			
COMMENT ON FUNCTION topology.ST_GetFaceGeometry(varchar , bigint ) IS 'args: atopology, aface - Returns the polygon in the given topology with the specified face id.';
			
COMMENT ON FUNCTION topology.GetRingEdges(varchar , bigint , integer ) IS 'args: atopology, aring, max_edges=null - Returns the ordered set of signed edge identifiers met by walking on ana given edge side.';
			
COMMENT ON FUNCTION topology.GetNodeEdges(varchar , bigint ) IS 'args: atopology, anode - Returns an ordered set of edges incident to the given node.';
			
COMMENT ON FUNCTION topology.FindVertexSegmentPairsBelowDistance(varchar , float8 ) IS 'args: atopology, tolerance - Find pairs of topology vertex/segment that are closer than tolerated distance';
...
COMMENT ON FUNCTION topology.CreateTopoGeom(varchar , integer , integer, topoelementarray, bigint) IS 'args: toponame, tg_type, layer_id, tg_objs, tg_id - Creates a new topo geometry object from topo element array - tg_type: 1:[multi]point, 2:[multi]line, 3:[multi]poly, 4:collection';
...
COMMENT ON FUNCTION topology.toTopoGeom(geometry , varchar , integer, float8) IS 'args: geom, toponame, layer_id, tolerance=-1 - Converts a simple Geometry into a topo geometry.';
			
COMMENT ON FUNCTION topology.toTopoGeom(geometry , topogeometry , float8) IS 'args: geom, topogeom, tolerance=-1 - Converts a simple Geometry into a topo geometry.';
...
COMMENT ON FUNCTION topology.GetTopoGeomElementArray(varchar , integer , bigint) IS 'args: toponame, layer_id, tg_id - Returns a topoelementarray (an array of topoelements) containing the topological elements and type of the given TopoGeometry (primitive elements).';
...
COMMENT ON FUNCTION topology.GetTopoGeomElements(varchar , integer , bigint) IS 'args: toponame, layer_id, tg_id - Returns a set of topoelement objects containing the topological element_id,element_type of the given TopoGeometry (primitive elements).';
