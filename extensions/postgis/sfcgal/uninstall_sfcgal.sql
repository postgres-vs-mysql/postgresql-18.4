...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
--
-- Generated on: 2026-08-07 09:31:12
...
DROP FUNCTION IF EXISTS CG_3DBuffer (geom geometry, radius float8, segments integer, buffer_type integer);
DROP FUNCTION IF EXISTS CG_StraightSkeletonPartition (geom geometry, auto_orientation boolean);
DROP FUNCTION IF EXISTS CG_RotateX (geom geometry, angle float8);
DROP FUNCTION IF EXISTS CG_RotateY (geom geometry, angle float8);
DROP FUNCTION IF EXISTS CG_RotateZ (geom geometry, angle float8);
DROP FUNCTION IF EXISTS CG_Rotate (geom geometry, angle float8);
DROP FUNCTION IF EXISTS CG_2DRotate (geom geometry, angle float8, cx float8, cy float8);
DROP FUNCTION IF EXISTS CG_3DRotate (geom geometry, angle float8, ax float8, ay float8, az float8);
DROP FUNCTION IF EXISTS CG_Scale (geom geometry, factor float8);
DROP FUNCTION IF EXISTS CG_3DScaleAroundCenter (geom geometry, factorX float8, factorY float8, factorZ float8, centerX float8, centerY float8, centerZ float8);
DROP FUNCTION IF EXISTS CG_3DScale (geom geometry, factorX float8, factorY float8, factorZ float8);
DROP FUNCTION IF EXISTS CG_Translate (geom geometry, deltaX float8, deltaY float8);
DROP FUNCTION IF EXISTS CG_3DTranslate (geom geometry, deltaX float8, deltaY float8, deltaZ float8);
DROP FUNCTION IF EXISTS CG_Simplify (geom geometry, threshold float8, preserveTopology boolean );
DROP FUNCTION IF EXISTS CG_3DAlphaWrapping (geom geometry, relative_alpha int, relative_offset int );
