...
if [ ! -x "/opt/rh/devtoolset-11/root/usr/bin/nm" ];
then
...
exports=$(find /home/wangbin/work/timescaledb/build -not -path '*/\.*' -name '*.so' -print0 \
    | xargs -0 /opt/rh/devtoolset-11/root/usr/bin/nm ${DEFINED_ONLY} ${EXTERN_ONLY} \
