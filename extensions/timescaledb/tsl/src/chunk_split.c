               ...
               SplitRelationInfo *split_relations)
{
  ...
  DBUG_PRINT("timescaledb", "split a relation into 'split_factor(%d)' pieces", split_factor);
  ...
  for (unsigned int i = 0; i < split_factor; i++) {
     ...
     */
    if (sri->heap_swap) {
      DBUG_PRINT("timescaledb", "only reindex new chunks");
      DBUG_PRINT("timescaledb", "existing chunk will be reindexed during the heap swap");
...
chunk_split_chunk(PG_FUNCTION_ARGS)
{
  ...
  if (new_compressed_chunk) {
    ...
    csplit_relations[0] = (SplitRelationInfo) {
      .relid = compress_settings->fd.compress_relid,
      .chunk_id = chunk->fd.compressed_chunk_id,
      .heap_swap = true
    };
    csplit_relations[1] = (SplitRelationInfo) {
      .relid = new_compressed_chunk->table_id,
