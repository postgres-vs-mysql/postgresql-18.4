                   ...
                   Path *compressed_path)
{
  ...
  DBUG_PRINT("timescaledb", "calculate cost for ColumnarScanPath");
  ...
  DBUG_PRINT("timescaledb", "for the columnar scan, startup cost is composed of:");
  DBUG_PRINT("timescaledb", "1) cost before fetching the first compressed tuple:%g", path->startup_cost);

  /*
   * 2) cost of actually fetching the first compressed tuple.
   */
  tmp_cost =  (compressed_path->total_cost - compressed_path->startup_cost) / compressed_rows;
  path->startup_cost += tmp_cost;

  DBUG_PRINT("timescaledb", "2) cost before fetching the first compressed tuple:%g", tmp_cost);
  ...
  if (columnar_scan->enable_bulk_decompression) {
    tmp_cost = compression_info->compressed_batch_size * cpu_tuple_cost;
    path->startup_cost += tmp_cost;
    DBUG_PRINT("timescaledb", "3) in case of bulk decompression, cost of fully decompressing the first batch:%g", tmp_cost);
  }

  DBUG_PRINT("timescaledb", "startup_cost:%g", path->startup_cost);
  /*
  ...
  DBUG_PRINT("timescaledb", "estimate the resulting number of rows based on the batch size statistics:%g", path->rows);
  ...
  DBUG_PRINT("timescaledb", "decompression_cost_per_uncompressed_row:%g", decompression_cost_per_uncompressed_row);
  /*
  ...
  DBUG_PRINT("timescaledb", "total_cost(%g) is cost for fetching all tuples", path->total_cost);
#if PG18_GE
                        ...
                        ColumnarScanPath *dcpath, Path *compressed_path)
{
  ...
  DBUG_PRINT("timescaledb", "startup_cost(%g) is cost before fetching first tuple", dcpath->custom_path.path.startup_cost);
  ...
  DBUG_PRINT("timescaledb", "sort_path_cost_rest:%g, rows:%g, uncompressed_row_cost:%g",
             sort_path_cost_rest, dcpath->custom_path.path.rows, uncompressed_row_cost);
  DBUG_PRINT("timescaledb", "total_cost:%g", dcpath->custom_path.path.total_cost);;
#if PG18_GE
  /* PG18 changes the way we handle disabled nodes so we
   * need to take those into account as well.
   *
                       ...
                       const SortInfo *sort_info)
{
  ...
  if (parent_relindex) {
    ...
    DBUG_PRINT("timescaledb", "chunk_index:%d, startup_cost:%g, total_cost:%g", chunk_index, 
        sorted_path->startup_cost, sorted_path->total_cost);
  } else {
    DBUG_PRINT("timescaledb", "startup_cost:%g, total_cost:%g", 
        sorted_path->startup_cost, sorted_path->total_cost);
  }
                                ...
                                const Chunk *chunk)
{
  ...
  DBUG_PRINT("timescaledb", "add RangeTblEntry and RelOptInfo for compressed chunk");
  ...
  if (sort_info.use_compressed_sort) {
    DBUG_PRINT("timescaledb", "use compressed sort");
   ...
   */
  DBUG_PRINT("timescaledb", "estimate the size of the compressed chunk table");
  ...
  DBUG_PRINT("timescaledb", "estimate the size(%g) of the compressed batch from Postgres statistics", compression_info->compressed_batch_size);
  ...
  DBUG_PRINT("timescaledb", "estimate the size of decompressed chunk based on the compressed chunk");
  DBUG_PRINT("timescaledb", "new_row_estimate:%g, new_tuples_estimate:%g", new_row_estimate, new_tuples_estimate);
  if (!compression_info->single_chunk) {
    ...
    DBUG_PRINT("timescaledb", "adjust the hypertable estimate by the diff of new and old chunk estimate");
    /*
     * For appendrel, set tuples to the same value as rows,
     * like set_append_rel_size() does.
     */
    hypertable_rel->tuples += delta;
    DBUG_PRINT("timescaledb", "for appendrel, set tuples to the same value as rows");
  }
   ...
   */
  DBUG_PRINT("timescaledb", "create the paths for the compressed chunk table");
  ...
  DBUG_PRINT("timescaledb", "create non-parallel paths");
  foreach (compressed_cell, compressed_rel->pathlist) {
    ...
    DBUG_PRINT("timescaledb", "add the paths to the chunk relation");
    foreach (decompressed_cell, decompressed_paths) {
      Path *path = lfirst(decompressed_cell);
      add_path(root, chunk_rel, path);
    }
  }
  ...
  DBUG_PRINT("timescaledb", "create parallel paths");
  foreach (compressed_cell, compressed_rel->partial_pathlist) {
    ...
    foreach (decompressed_cell, decompressed_paths) {
      Path *path = lfirst(decompressed_cell);
      add_partial_path(root, chunk_rel, path);
    }
  }
                                ...
                                const CompressionInfo *compression_info)
{
  ...
  if (IsA(compressed_path, BitmapHeapPath) && compressed_path->param_info) {
    DBUG_PRINT("timescaledb", "we skip any BitmapScan parameterized paths here as supporting those would require fixing up the internal scan");
    return NIL;
  }
  ...
  if (!add_uncompressed_part) {
     ...
     */
    DBUG_PRINT("timescaledb", "if the chunk has only the compressed part, we're done");
                             ...
                             const CompressionInfo *compression_info, const SortInfo *sort_info)
{
  ...
  add_path(root, compressed_rel, compressed_path);
  ...
  if (compressed_rel->consider_parallel && required_outer == NULL) {
    ...
    if (parallel_workers > 0) {
      add_partial_path(root, compressed_rel,
