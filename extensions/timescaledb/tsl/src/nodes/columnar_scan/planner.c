...
{
  DBUG_TRACE;
  bool result = IsA(plan, CustomScan) &&
...
build_vector_attrs_array(const UncompressedColumnInfo *colinfo, const CompressionInfo *info)
{
  ...
  DBUG_PRINT("timescaledb", "attr number:%d", arrlen);
...
ts_label_sort_with_costsize(PlannerInfo *root, Sort *plan, double limit_tuples)
{
  ...
  DBUG_PRINT("timescaledb", "plan_rows:%g, plan_width:%d", plan->plan.plan_rows, plan->plan.plan_width);
  DBUG_PRINT("timescaledb", "startup_cost:%g, total_cost:%g", plan->plan.startup_cost, plan->plan.total_cost);
}
                          ...
                          List *output_targetlist, List *clauses, List *custom_plans)
{
  ...
  if (IsA(compressed_path, IndexPath)) {
    ...
    DBUG_PRINT("timescaledb", "check if any of the decompressed scan clauses are redundant with the compressed index scan clauses");
    foreach (lc, clauses) {
      ...
      if (indexclause_cell != NULL) {
        ...
        DBUG_PRINT("timescaledb", "we already have an index clause derived from same EquivalenceClass");
        continue;
      }
       ...
       */
      DBUG_PRINT("timescaledb", "we don't have this clause in the underlying index scan, add it to the decompressed scan");
  ...
  if (dcpath->batch_sorted_merge) {
    ...
    DBUG_PRINT("timescaledb", "build heap sort info for batch sorted merge");
    ...
    DBUG_PRINT("timescaledb", "now build the compressed batches sort node");
     ...
     */
    DBUG_PRINT("timescaledb", "add a sort if the compressed scan is not ordered appropriately");
