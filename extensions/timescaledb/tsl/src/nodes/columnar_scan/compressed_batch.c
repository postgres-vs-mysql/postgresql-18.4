                  ...
                  TupleTableSlot *compressed_slot, int i)
{
  ...
  DBUG_PRINT("timescaledb", "detoast the compressed datum");
  ...
  if (dcontext->enable_bulk_decompression && column_description->bulk_decompression_supported) {
    DBUG_PRINT("timescaledb", "decompress the entire batch if it is supported");
  ...
  if (arrow == NULL) {
    ...
    DBUG_PRINT("timescaledb", "as a fallback, decompress row-by-row");
                         ...
                         uint64 *restrict result)
{
  ...
  foreach (lc, quals) {
    ...
    if (get_vector_qual_summary(result, vqstate->num_results) == NoRowsPass) {
       ...
       */
      DBUG_PRINT("timescaledb", "exit early if no rows pass already");
      return;
    }
  }
}
                 ...
                 uint64 *restrict result)
{
  ...
  if (boolexpr->boolop == AND_EXPR) {
    DBUG_PRINT("timescaledb", "boolop: AND_EXPR");
  ...
  if (boolexpr->boolop == NOT_EXPR) {
    DBUG_PRINT("timescaledb", "boolop: NOT_EXPR");
  ...
  DBUG_PRINT("timescaledb", "boolop: OR_EXPR");
...
vector_qual_compute(VectorQualState *vqstate)
{
  ...
  if (n_rows % 64 != 0) {
    ...
    DBUG_PRINT("timescaledb", "we have to zero out the bits for past-the-end elements in the last bitmap word");
                                      ...
                                      TupleTableSlot *compressed_slot)
{
  ...
  for (int i = 0; i < dcontext->num_columns_with_metadata; i++) {
    ...
    switch (column_description->type) {
      case COMPRESSED_COLUMN: {
         ...
         */
        DBUG_PRINT("timescaledb", "skip decompressing some columns if the entire batch doesn't pass the quals");
  ...
  if (vector_qual_summary == NoRowsPass && !dcontext->batch_sorted_merge) {
     ...
     * needed for sorting.
     */
    DBUG_PRINT("timescaledb", "the entire batch doesn't pass the vectorized quals, so we might be able to avoid reading and decompressing other columns");
    ...
    DBUG_PRINT("timescaledb", "we have some rows in the batch that pass the vectorized filters, so");
    DBUG_PRINT("timescaledb", "we have to decompress the rest of the compressed columns(num_data_columns:%d)", num_data_columns);
    ...
    if (vector_qual_summary == AllRowsPass) {
      DBUG_PRINT("timescaledb", "if all rows pass, no need to test the vector qual for each row");
      batch_state->vector_qual_result = NULL;
...
vector_qual(DecompressBatchState *batch_state, uint16 arrow_row)
{
  ...
  result = arrow_row_is_valid(batch_state->vector_qual_result, arrow_row);

  if (result) {
    DBUG_PRINT("timescaledb", "vector_qual returns true");
  } else {
...
postgres_qual(DecompressContext *dcontext, DecompressBatchState *batch_state)
{
  ...
  result = ExecQual(dcontext->ps->qual, econtext);

  if (result ) {
    DBUG_PRINT("timescaledb", "perform the usual Postgres selection that returns true");
  } else {
    DBUG_PRINT("timescaledb", "perform the usual Postgres selection that returns false");
  }
  return result;
}
...
compressed_batch_advance(DecompressContext *dcontext, DecompressBatchState *batch_state)
{
  ...
  DBUG_PRINT("timescaledb", "total_batch_rows:%u, num_data_columns:%d", batch_state->total_batch_rows, num_data_columns);
  for (; batch_state->next_batch_row < batch_state->total_batch_rows;
    ...
    DBUG_PRINT("timescaledb", "output_row:%u, arrow_row:%u", output_row, arrow_row);
    if (!vector_qual(batch_state, arrow_row)) {
       ...
       */
      DBUG_PRINT("timescaledb", "this row doesn't pass the vectorized quals");
      DBUG_PRINT("timescaledb", "advance the iterated compressed columns if we have any");
      for (int i = 0; i < num_data_columns; i++) {
        CompressedColumnValues *column_values = &batch_state->compressed_columns[i];
    ...
    if (!postgres_qual(dcontext, batch_state)) {
       ...
       * iteration.
       */
      DBUG_PRINT("timescaledb", "the tuple didn't pass the qual, fetch the next one in the next iteration");
      InstrCountFiltered1(dcontext->ps, 1);
      continue;
    }

    DBUG_PRINT("timescaledb", "the tuple passed the qual");
  ...
  DBUG_PRINT("timescaledb", "check that the columns that we're decompressing row-by-row have also ended");
  for (int i = 0; i < num_data_columns; i++) {
    CompressedColumnValues *column_values = &batch_state->compressed_columns[i];
