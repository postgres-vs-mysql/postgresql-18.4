...
columnar_index_scan_plan_create(Agg *agg, CustomScan *cscan, List *rtable)
{
  ...
  DBUG_PRINT("timescaledb", "copy cost/parallel/param fields from the ColumnarScan");
  DBUG_PRINT("timescaledb", "startup_cost:%g, total_cost:%g", columnar_index_scan->scan.plan.startup_cost, columnar_index_scan->scan.plan.total_cost);
