                        ...
                        UpperRelationKind stage)
{
  ...
  foreach (lc, input_rel->pathlist) {
    ...
    add_path(root, output_rel, new_unique);
  }
}
...
skip_scan_path_create(PlannerInfo *root, Path *child_path, DistinctPathInfo *dpinfo)
{
  ...
  if (clauses_needing_scan == NULL && (Path *) index_path == child_path) {
    ...
    DBUG_PRINT("timescaledb", "we calculate SkipScan cost(startup_cost:%g, total_cost:%g)",
               skip_scan_path->cpath.path.startup_cost, skip_scan_path->cpath.path.total_cost);
  }
