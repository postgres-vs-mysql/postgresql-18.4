                            ...
                            bool *out_is_segmentby)
{
  ...
  if (!is_segmentby &&
    ...
    DBUG_PRINT("timescaledb", "vectorized aggregation not possible for this particular column");
    return false;
  }
...
custom_scan_to_uncompressed_chunk_attno(List *custom_scan_tlist, int custom_scan_attno)
{
  ...
  DBUG_PRINT("timescaledb", "map the custom scan attribute number to the uncompressed chunk attribute number");
  if (custom_scan_tlist == NIL) {
    DBUG_PRINT("timescaledb", "return attno:%d", custom_scan_attno);
  ...
  DBUG_PRINT("timescaledb", "return attno:%d", var->varattno);
  return var->varattno;
}
...
vectoragg_plan_columnar_scan(Plan *childplan, VectorQualInfo *vqi)
{
  ...
  DBUG_PRINT("timescaledb", "maxattno:%d", maxattno);
