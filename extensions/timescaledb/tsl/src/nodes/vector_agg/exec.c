                              ...
                              uint64 const *filter, List *args, Oid funcoid, Oid inputcollid)
{
  ...
  DBUG_PRINT("timescaledb", "total processed rows:%d", total_processed_rows);
...
vector_agg_begin(CustomScanState *node, EState *estate, int eflags)
{
  ...
  DBUG_PRINT("timescaledb", "first, count how many grouping columns(%d) and aggregate functions(%d) we have",
      grouping_column_counter, agg_functions_counter);
  ...
  for (int i = 0; i < tlist_length; i++) {
    ...
    if (IsA(tlentry->expr, Aggref)) {
      ...
      def->output_offset = i;
      DBUG_PRINT("timescaledb", "this is an aggregate function");
      ...
      DBUG_PRINT("timescaledb", "this is a grouping column");
  ...
  if (grouping_type == VAGT_Batch) {
     ...
     * Per-batch grouping.
     */
    DBUG_PRINT("timescaledb", "per-batch grouping");
     ...
     * Hash grouping.
     */
    DBUG_PRINT("timescaledb", "hash grouping");
    vector_agg_state->grouping =
...
vector_agg_exec(CustomScanState *node)
{
  ...
  if (have_partial) {
    DBUG_PRINT("timescaledb", "the grouping policy produced a partial aggregation result");
  ...
  if (vector_agg_state->input_ended) {
    DBUG_PRINT("timescaledb", "if the partial aggregation results have ended, and the input has ended, we're done");
    return NULL;
  }
  ...
  while (!grouping->gp_should_emit(grouping)) {
     ...
     */
    if (vector_agg_state->input_ended) {
      DBUG_PRINT("timescaledb", "exit if there is no more data");
      break;
    }
    ...
    DBUG_PRINT("timescaledb", "compute the vectorized filters for the aggregate function FILTER clauses(narrgs:%d)", naggs);
  ...
  if (have_partial) {
    DBUG_PRINT("timescaledb", "if we have partial aggregation results, start returning them");
  ...
  if (vector_agg_state->input_ended) {
     ...
     */
    DBUG_PRINT("timescaledb", "have no partial aggregation results and the input has ended, so we're done");
    return NULL;
  }
