...
FUNCTION_NAME(fill_offsets_impl)(BatchHashingParams params, int start_row, int end_row)
{
  ...
  for (int row = start_row; row < end_row; row++) {
    if (!arrow_row_is_valid(params.batch_filter, row)) {
      ...
      DBUG_PRINT("timescaledb", "row %d doesn't pass batch filter", row);
      continue;
    }
    ...
    if (unlikely(!key_valid)) {
      ...
      DBUG_PRINT("timescaledb", "row %d null key index %d", row, hashing->null_key_index);
...
#endif
      DBUG_PRINT("timescaledb", "row %d consecutive key index %d", row, previous_key_index);
    ...
    if (!found) {
      ...
      DBUG_PRINT("timescaledb", "row %d new key index %d", row, index);
      DEBUG_PRINT("%p: row %d new key index %d\n", hashing, row, index);
    } else {
      DBUG_PRINT("timescaledb", "row %d old key index %d", row, entry->key_index);
