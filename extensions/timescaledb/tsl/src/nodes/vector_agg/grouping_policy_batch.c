                         ...
                         MemoryContext agg_extra_mctx)
{
  ...
  if (arg_arrow != NULL) {
    /* Arrow argument. */
    DBUG_PRINT("timescaledb", "arrow argument");
    ...
    DBUG_PRINT("timescaledb", "scalar argument");
    if (n > 0) {
...
gp_batch_should_emit(GroupingPolicy *gp)
{
   ...
   * every batch.
   */
  bool result = policy->num_grouping_columns > 0 && policy->have_results;
  if (result) {
...
gp_batch_do_emit(GroupingPolicy *gp, TupleTableSlot *aggregated_slot)
{
  ...
  DBUG_PRINT("timescaledb", "naggs:%d", naggs);
  for (int i = 0; i < naggs; i++) {
    VectorAggDef *agg_def = &policy->agg_defs[i];
    void *agg_state = policy->agg_states[i];
  ...
  DBUG_PRINT("timescaledb", "ngrp:%d", ngrp);
  for (int i = 0; i < ngrp; i++) {
    GroupingColumn *col = &policy->grouping_columns[i];
    Assert(col->output_offset >= 0);
