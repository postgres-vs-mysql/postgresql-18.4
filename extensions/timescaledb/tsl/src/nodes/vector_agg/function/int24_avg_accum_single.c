                           ...
                           MemoryContext agg_extra_mctx)
{
  ...
  DBUG_PRINT("timescaledb", "n:%d, batch_count:%ld, batch_sum:%ld", n, batch_count, batch_sum);
  Int24AvgAccumState *state = (Int24AvgAccumState *) agg_state;
  state->count += batch_count;
  state->sum += batch_sum;
  DBUG_PRINT("timescaledb", "state->count:%ld, state->sum:%ld", state->count, state->sum);
}
