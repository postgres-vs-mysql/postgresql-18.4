                       ...
                       VectorAggGroupingType grouping_type)
{
  ...
  DBUG_PRINT("timescaledb", "copy the costs from the normal aggregation node, so that they show up in the EXPLAIN output");
  DBUG_PRINT("timescaledb", "startup_cost:%g, total_cost:%g", vector_agg->scan.plan.startup_cost, vector_agg->scan.plan.total_cost);
...
can_vectorize_aggref(const VectorQualInfo *vqi, Aggref *aggref)
{
  ...
  if (aggref->aggdirectargs != NIL) {
    ...
    DBUG_PRINT("timescaledb", "can't process ordered-set aggregates with direct arguments");
    return false;
  }

  if (aggref->aggorder != NIL) {
    DBUG_PRINT("timescaledb", "can't process aggregates with an ORDER BY clause");
    /* Can't process aggregates with an ORDER BY clause. */
    return false;
  }

  if (aggref->aggdistinct != NIL) {
    DBUG_PRINT("timescaledb", "can't process aggregates with DISTINCT clause");
  ...
  if (aggref->aggfilter != NULL) {
    ...
    if (aggfilter_vectorized == NULL) {
      DBUG_PRINT("timescaledb", "whether we can vectorize this particular aggregate? No");
      return false;
    }
  ...
  if (get_vector_aggregate(aggref->aggfnoid, aggref->inputcollid) == NULL) {
     ...
     * aggregate function.
     */
    DBUG_PRINT("timescaledb", "we don't have a vectorized implementation for this particular aggregate function");
    return false;
  }

  if (aggref->args == NIL) {
    DBUG_PRINT("timescaledb", "this must be count(*), we can vectorize it");
  ...
  bool result = is_vector_expr(vqi, argument->expr);
  if (result) {
    DBUG_PRINT("timescaledb", "whether we can vectorize this particular aggregate? Yes");
  } else {
    DBUG_PRINT("timescaledb", "whether we can vectorize this particular aggregate? No");
  }
  return result;
...
get_vectorized_grouping_type(const VectorQualInfo *vqinfo, Agg *agg, List *resolved_targetlist)
{
  ...
  DBUG_PRINT("timescaledb", "num_grouping_columns:%d, agg->numCols:%d", num_grouping_columns, agg->numCols);
  /*
   * We support vectorized aggregation without grouping.
   */
  if (num_grouping_columns == 0) {
    DBUG_PRINT("timescaledb", "we support vectorized aggregation without grouping");
    return VAGT_Batch;
  }
   ...
   */
  if (all_segmentby) {
    DBUG_PRINT("timescaledb", "we support grouping by any number of columns if all of them are segmentby");
    return VAGT_Batch;
  }
  ...
  if (num_grouping_columns == 1 && typlen != 0) {
    if (typbyval) {
      switch (typlen) {
        case 1:
          ...
          DBUG_PRINT("timescaledb", "return VAGT_HashSerialized");
        ...
        case 2:
          DBUG_PRINT("timescaledb", "return VAGT_HashSingleFixed2");
          return VAGT_HashSingleFixed2;

        case 4:
          DBUG_PRINT("timescaledb", "return VAGT_HashSingleFixed4");
          return VAGT_HashSingleFixed4;

        case 8:
          DBUG_PRINT("timescaledb", "return VAGT_HashSingleFixed8");
     ...
     */
      ...
      DBUG_PRINT("timescaledb", "return VAGT_HashSingleText");
...
vectoragg_plan_possible(Plan *childplan, VectorQualInfo *vqi)
{
  ...
  if (!IsA(childplan, CustomScan)) {
    DBUG_PRINT("timescaledb", "check if a VectorAgg is possible on top of the given child plan? No");
    return false;
  }

  if (childplan->qual != NIL) {
    /* Can't do vectorized aggregation if we have Postgres quals. */
    DBUG_PRINT("timescaledb", "can't do vectorized aggregation if we have Postgres quals");
    return false;
  }

  if (ts_is_columnar_scan_plan(childplan)) {
    vectoragg_plan_columnar_scan(childplan, vqi);
    DBUG_PRINT("timescaledb", "check if a VectorAgg is possible on top of the given child plan? Yes");
    return true;
  }
...
insert_vector_agg(Plan *plan, void *context)
{
  ...
  if (agg->aggsplit != AGGSPLIT_INITIAL_SERIAL && agg->aggsplit != AGGSPLIT_SIMPLE) {
    ...
    DBUG_PRINT("timescaledb", "can only vectorize partial or non-partial aggregation node");
    return plan;
  }

  if (agg->groupingSets != NIL) {
    /* No GROUPING SETS support. */
    DBUG_PRINT("timescaledb", "no GROUPING SETS support");
    return plan;
  }
  ...
  if (agg->plan.qual != NIL) {
     ...
     */
    DBUG_PRINT("timescaledb", "no HAVING support");
    return plan;
  }

  if (agg->plan.lefttree == NULL) {
    DBUG_PRINT("timescaledb", "not sure what this would mean, but check for it just to be on the safe side");
    /*
  ...
  if (!vectoragg_plan_possible(childplan, &vqi)) {
    DBUG_PRINT("timescaledb", "not a compatible vectoragg child node");
  ...
  if (grouping_type == VAGT_Invalid) {
    ...
    DBUG_PRINT("timescaledb", "the grouping is not vectorizable");
    return plan;
  }
  ...
  foreach (lc, resolved_targetlist) {
    ...
    if (IsA(target_entry->expr, Aggref)) {
      ...
      if (!can_vectorize_aggref(&vqi, aggref)) {
        ...
        DBUG_PRINT("timescaledb", "aggregate function not vectorizable");
        return plan;
      }
    }
  }
   ...
   * aggregation node.
   */
  DBUG_PRINT("timescaledb", "finally, all requirements are satisfied and we can vectorize this aggregation node");
  ...
  if (agg->aggsplit == AGGSPLIT_SIMPLE) {
    ...
    DBUG_PRINT("timescaledb", "convert a non-partial aggregation into a two-phase partial + finalize aggregation with VectorAgg performing the partial step");
     ...
     */
    DBUG_PRINT("timescaledb", "rebuild the plan output targetlist to reflect the updated types");
    vector_agg->scan.plan.targetlist =
      ts_build_trivial_custom_output_targetlist(vector_agg->custom_scan_tlist);
     ...
     */
    DBUG_PRINT("timescaledb", "set up the parent Agg to finalize the partial results from VectorAgg");
