...
FUNCTION_NAME(delta_delta_decompress_all, ELEMENT_TYPE)(Datum compressed, MemoryContext dest_mctx)
{
  ...
  DBUG_PRINT("timescaledb", "compressed len:%d", si.len);
  ...
  DBUG_PRINT("timescaledb", "pad the number of elements to multiple of 64 bytes if needed, so that we can work in 64-byte blocks");
  /*
...
#undef INNER_LOOP_SIZE
  ...
  DBUG_PRINT("timescaledb", "n_total:%u, n_notnull:%u", n_total, n_notnull);
  return result;
}
