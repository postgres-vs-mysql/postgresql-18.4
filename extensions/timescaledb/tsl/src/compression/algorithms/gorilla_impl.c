    ...
    MemoryContext dest_mctx)
{
  ...
  DBUG_PRINT("timescaledb", "n_total:%u, n_total_padded:%u", n_total, n_total_padded);
  ...
  DBUG_PRINT("timescaledb", "n_different:%u", n_different);
  ...
  DBUG_PRINT("timescaledb", "return the result(n_total:%u, n_notnull:%u)", n_total, n_notnull);
  return result;
}
