...
compression_get_default_algorithm(Oid typeoid)
{
  ...
  switch (typeoid) {
    ...
    case TIMESTAMPOID:
    case TIMESTAMPTZOID:
      DBUG_PRINT("timescaledb", "return a default compression algorithm suitable for the type:DELTADELTA");
      return COMPRESSION_ALGORITHM_DELTADELTA;

    case FLOAT4OID:
    case FLOAT8OID:
      DBUG_PRINT("timescaledb", "return a default compression algorithm suitable for the type:GORILLA");
      return COMPRESSION_ALGORITHM_GORILLA;

    case NUMERICOID:
      DBUG_PRINT("timescaledb", "return a default compression algorithm suitable for the type:ARRAY");
      return COMPRESSION_ALGORITHM_ARRAY;

    case BOOLOID:
      if (ts_guc_enable_bool_compression) {
        DBUG_PRINT("timescaledb", "return a default compression algorithm suitable for the type:BOOL");
    ...
    case UUIDOID:
      if (ts_guc_enable_uuid_compression) {
        DBUG_PRINT("timescaledb", "return a default compression algorithm suitable for the type:UUID");
        return COMPRESSION_ALGORITHM_UUID;
      } else {
        DBUG_PRINT("timescaledb", "return a default compression algorithm suitable for the type:DICTIONARY");
                          ...
                          ColumnAnalysis *candidates)
{
  ...
  for (int i = 0; i < in_desc->natts; i++) {
    ...
    /* Limit scan to first 10 eligible columns */
    if (n_candidates >= MAX_SEGMENTBY_CANDIDATES) {
      break;
    }
  }
...
analyze_and_get_segmentby(CompressionSettings *settings, RowCompressor *compressor)
{
  ...
  ColumnAnalysis *candidates = palloc0(sizeof(ColumnAnalysis) * MAX_SEGMENTBY_CANDIDATES);
