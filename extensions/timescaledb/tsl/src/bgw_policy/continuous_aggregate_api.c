...
policy_continuous_aggregate_get_mat_hypertable_id(const Jsonb *config)
{
  ...
  DBUG_PRINT("timescaledb", "return mat_hypertable_id:%d", mat_hypertable_id);
...
policy_refresh_cagg_get_buckets_per_batch(const Jsonb *config)
{
  ...
  DBUG_PRINT("timescaledb", "return result:%d", res);
  return res;
}
...
policy_refresh_cagg_exists(int32 materialization_id)
{
  ...
  if (jobs == NIL) {
    DBUG_PRINT("timescaledb", "return false if a policy could not be found");
    return false;
  }
                                       ...
                                       int32 existing_job_id)
{
  ...
  if (jobs == NIL) {
    DBUG_PRINT("timescaledb", "overlap_result:%d", overlap_result);
  ...
  DBUG_PRINT("timescaledb", "start_offset:%ld, end_offset:%ld", start_offset, end_offset);
  elog(DEBUG1,
  ...
  foreach (lc, jobs) {
    ...
    DBUG_PRINT("timescaledb", "start_offset_job:%ld, end_offset_job:%ld", start_offset, end_offset);
    elog(DEBUG1,
    ...
    if (start_offset == start_offset_job && end_offset == end_offset_job) {
      DBUG_PRINT("timescaledb", "if all arguments are the same, do nothing");
     ...
     * sorted by start/end offset. */
      ...
      DBUG_PRINT("timescaledb", "set overlap_result to POLICY_REFRESH_OFFSET_OVERLAP");
    }
  }
                                 ...
                                 NullableDatum refresh_newest_first)
{
  ...
  switch (res) {
    ...
    case POLICY_REFRESH_OFFSET_OVERLAP_EQUAL:
      if (if_not_exists) {
        char *rel_name = get_rel_name(cagg->relid);
        DBUG_PRINT("timescaledb", "continuous aggregate refresh policy already exists for '%s", rel_name);
        ereport(NOTICE,
                (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
  ...
  DBUG_PRINT("timescaledb", "return job_id:%d", job_id);
...
policy_refresh_cagg_remove_internal(Oid cagg_oid, bool if_exists)
{
  ...
  if (jobs == NIL) {
    if (!if_exists)
      ...
      char *rel_name = get_rel_name(cagg_oid);
      DBUG_PRINT("timescaledb", "continuous aggregate policy not found for '%s', skipping", rel_name);
