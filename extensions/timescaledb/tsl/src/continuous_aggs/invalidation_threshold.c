...
invalidation_threshold_scan_update(TupleInfo *ti, void *const data)
{
  ...
  if (invthresh->computed_invalidation_threshold > current_invalidation_threshold) {
    ...
    DBUG_PRINT("timescaledb", "hypertable %d existing watermark >= new invalidation threshold %ld %ld",
        invthresh->cagg->data.raw_hypertable_id, current_invalidation_threshold, invthresh->computed_invalidation_threshold);
    elog(DEBUG1,
...
invalidation_threshold_compute(const ContinuousAgg *cagg, const InternalTimeRange *refresh_window)
{
  ...
  if (max_refresh) {
    ...
    if (isnull) {
      ...
      DBUG_PRINT("timescaledb", "no data in hypertable");
      ...
      int64 bucket_start = ts_time_bucket_by_type_extended(bucket_width,
                           ...
                           offset,
                           origin);
      DBUG_PRINT("timescaledb", "add one bucket to get to the end of the last bucket");
...
invalidation_threshold_initialize(const ContinuousAgg *cagg)
{
  ...
  if (!found) {
    ...
    DBUG_PRINT("timescaledb", "get the MIN value for the partition type:%ld", min_value);
