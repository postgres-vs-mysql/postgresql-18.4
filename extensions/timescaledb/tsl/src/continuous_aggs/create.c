            ...
            ContinuousAggTimeBucketInfo *bucket_info, WithClauseResult *with_clause_options)
{
   ...
   */
  DBUG_PRINT("timescaledb", "step 1: create the materialization table");
  ts_catalog_database_info_become_owner(ts_catalog_database_info_get(), &sec_ctx);
   ...
   */
  DBUG_PRINT("timescaledb", "step 2: create view with select finalize from materialization table");
   ...
   */
  DBUG_PRINT("timescaledb", "step 3: create the internal view with select partialize(..)");
  ...
  DBUG_PRINT("timescaledb", "step 4: add catalog table entry for the objects we just created");
  ...
  if (IS_TIME_BUCKET_INFO_TIME_BASED(bucket_info->bf)) {
    /* Bucketing on time */
    DBUG_PRINT("timescaledb", "bucketing on time");
    ...
    DBUG_PRINT("timescaledb", "bucketing on integers");
                                    ...
                                    WithClauseResult *with_clause_options)
{
  ...
  if (OidIsValid(relid)) {
    if (stmt->if_not_exists) {
      DBUG_PRINT("timescaledb", "continuous aggregate \"%s\" already exists, skipping", stmt->into->rel->relname);
      ereport(NOTICE,
      ...
      DBUG_PRINT("timescaledb", "continuous aggregate \"%s\" already exists", stmt->into->rel->relname);
      DBUG_INSTANT_PRINT("timescaledb", "drop or rename the existing continuous aggregate first or use another name");
      ereport(ERROR,
              (errcode(ERRCODE_DUPLICATE_TABLE),
  ...
  if (!with_clause_options[CreateMaterializedViewFlagColumnstore].is_default) {
    DBUG_INSTANT_PRINT("timescaledb", "cannot enable compression while creating a continuous aggregate");
    ereport(ERROR,
