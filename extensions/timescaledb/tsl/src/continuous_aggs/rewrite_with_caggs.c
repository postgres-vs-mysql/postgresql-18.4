...
rewrite_query_with_caggs(Node *node, RewriteWithCaggsContext *context)
{
  ...
  DBUG_PRINT("timescaledb", "node tag:%d", nodeTag(node));
  ...
  if (IsA(node, RangeTblEntry)) {
    ...
    if (rte->rtekind == RTE_SUBQUERY &&
      ...
      if (rte->relkind == RELKIND_VIEW) {
        ...
        DBUG_PRINT("timescaledb", "view_name:'%s', view_schema:'%s'", view_name, view_schema);
        cagg = ts_continuous_agg_find_by_view_name(view_schema,
                              view_name,
                              ContinuousAggAnyView);

        if (cagg) {
          /* Enter Cagg view query*/
          DBUG_PRINT("timescaledb", "enter Cagg view query");
    ...
    DBUG_PRINT("timescaledb", "node is a CommonTableExpr");
    ...
    foreach (lc, query->rtable) {
      ...
      if (cagg_rewrite_ctx.eligible && rte->rtekind == RTE_RELATION) {
        ...
        if (ht) {
          DBUG_PRINT("timescaledb", "record hypertable in case query will be eligible for Cagg rewrites");
    ...
    if (cagg_rewrite_ctx.eligible && !cagg_rewrite_ctx.ht_rte) {
      ...
      if (ts_guc_cagg_rewrites_debug_info) {
        DBUG_PRINT("timescaledb", "at least one hypertable should be used in the query");
    ...
    DBUG_PRINT("timescaledb", "found a hypertable, check that time bucket is valid and on hypertable time dimension");
    if (cagg_rewrite_ctx.eligible) {
      /* "ht_rte" must be a Cagg, obtain its hypertable */
      if (!cagg_rewrite_ctx.ht) {
        DBUG_PRINT("timescaledb", "ht_rte must be a Cagg, obtain its hypertable");
      ...
      DBUG_PRINT("timescaledb", "record hypertable full name so we can release hcache");
    ...
    DBUG_PRINT("timescaledb", "validated all we could before retrieving CAggs for the hypertable,");
    DBUG_PRINT("timescaledb", "now we'll try to match available CAggs to the query");
    ...
    if (cagg_rewrite_ctx.cagg && ts_guc_enable_cagg_rewrites) {
      DBUG_PRINT("timescaledb", "rewritten_with_caggs: true");
    ...
    if (!OidIsValid(context->cagg_relid) && ts_guc_cagg_rewrites_debug_info) {
      ...
      if (cagg_rewrite_ctx.cagg) {
        if (ts_guc_enable_cagg_rewrites) {
          DBUG_PRINT("timescaledb", "%s was rewritten with CAggs \"%s.%s\"", query_label.data, NameStr(cagg_rewrite_ctx.cagg->data.user_view_schema),
          ...
          DBUG_PRINT("timescaledb", "%s can be rewritten with CAggs: \"%s.%s\"", query_label.data, NameStr(cagg_rewrite_ctx.cagg->data.user_view_schema),
              NameStr(cagg_rewrite_ctx.cagg->data.user_view_name));
          elog(INFO,
        ...
        DBUG_PRINT("timescaledb", "%s cannot be rewritten with CAggs: %s", query_label.data, cagg_rewrite_ctx.msg.data);
        elog(INFO,
...
continuous_agg_apply_rewrites(Query *parse)
{
  ...
  if (rewrite_with_caggs_context.rewritten_with_caggs) {
    ...
    DBUG_PRINT("timescaledb", "we need to rewrite the query to fire Cagg view rules if query was rewritten with Caggs");
