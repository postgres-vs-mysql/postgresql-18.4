...
constify_cagg_watermark_walker(Node *node, ConstifyWatermarkContext *context)
{
  ...
  if (IsA(node, FuncExpr)) {
    ...
    DBUG_PRINT("timescaledb", "node is a FuncExpr");

    /* Handle watermark function */
    if (watermark_function_oid == funcExpr->funcid) {
      DBUG_PRINT("timescaledb", "handle watermark function");
      ...
      if (context->parent_coalesce_expr == NULL || !IsA(linitial(funcExpr->args), Const) ||
        ...
        DBUG_PRINT("timescaledb", "no coalesce expression found so far or function parameter is not constant,");
        DBUG_PRINT("timescaledb", "we are not interested in this expression");
      ...
      if (context->parent_to_timestamp_func != NULL) {
        DBUG_PRINT("timescaledb", "only on time based hypertables, we have a to_timestamp function");
        ...
        if (coalesce_arg != node) {
          ...
          DBUG_PRINT("timescaledb", "check if the watermark function is wrapped into a cast function");
    ...
    if (list_member_oid(context->to_timestamp_func_oids, funcExpr->funcid)) {
      ...
      DBUG_PRINT("timescaledb", "capture the timestamp conversion function");
    ...
    DBUG_PRINT("timescaledb", "recurse into subselects");
    ...
    DBUG_PRINT("timescaledb", "node is a CoalesceExpr");
    DBUG_PRINT("timescaledb", "capture the CoalesceExpr");
    ...
    DBUG_PRINT("timescaledb", "node is a RangeTblEntry");

    if (rte->rtekind == RTE_RELATION) {
      DBUG_PRINT("timescaledb", "collect the Oid of the used range tables");
      context->relids = list_append_unique_oid(context->relids, rte->relid);
    }

    DBUG_PRINT("timescaledb", "allow range_table_walker to continue");
...
replace_watermark_with_const(ConstifyWatermarkContext *context)
{
  ...
  if (list_length(context->watermark_functions) < 1) {
    DBUG_PRINT("timescaledb", "we need to have at least one watermark value");
    return;
  }
  ...
  forboth (parent_lc,
    ...
    if (watermark_const == NULL) {
      DBUG_PRINT("timescaledb", "no constant created, it means the hypertable id used by the watermark function is not a range table");
      DBUG_PRINT("timescaledb", "and no invalidations would be processed. So, not replacing the function invocation");
      continue;
    }
    ...
    if (IsA(lfirst(parent_lc), FuncExpr)) {
      ...
      DBUG_PRINT("timescaledb", "replace cagg_watermark FuncExpr node by a Const node");
...
constify_cagg_watermark(Query *parse)
{
  ...
  if (parse->commandType != CMD_SELECT) {
    DBUG_PRINT("timescaledb", "process only SELECT queries");
    return;
  }
  ...
  DBUG_PRINT("timescaledb", "walk through the query and collect function information");
  ...
  if (context.valid_query) {
    DBUG_PRINT("timescaledb", "replace watermark functions with const value if the query might belong to the CAgg query");
...
cagg_sort_pushdown(Query *parse, int *cursor_opts)
{
  ...
  DBUG_PRINT("timescaledb", "push down ORDER BY and LIMIT into subqueries of UNION for realtime");
  DBUG_PRINT("timescaledb", "continuous aggregates when sorting by time");

  /* We dont optimize aggregations on top of caggs for now. */
  if (parse->groupClause) {
    DBUG_PRINT("timescaledb", "we don't optimize aggregations on top of caggs for now");
    return;
  }
  ...
  if (list_length(parse->rtable) != 1 || list_length(parse->sortClause) != 1 ||
    ...
    DBUG_PRINT("timescaledb", "nothing to do if we have no valid sort clause");
    return;
  }
  ...
  foreach (lc, parse->rtable) {
    ...
    if (!cagg || cagg->data.materialized_only) {
      DBUG_PRINT("timescaledb", "this optimization only applies to realtime caggs");
      continue;
    }
    ...
    if (!IsA(tle->expr, Var) || castNode(Var, tle->expr)->varattno != time_col) {
      DBUG_PRINT("timescaledb", "we only pushdown ORDER BY when it's single column ORDER BY on the time column");
      continue;
    }
