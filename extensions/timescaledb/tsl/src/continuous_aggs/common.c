                                        ...
                                        bool *custom_origin)
{
  ...
  switch (exprType((Node *) arg)) {
    /* Timezone as text */
    case TEXTOID:
      DBUG_PRINT("timescaledb", "timezone as text");
    ...
    case INTERVALOID:
      ...
      DBUG_PRINT("timescaledb", "bucket offset as interval");
      bf->bucket_time_offset = DatumGetIntervalP(arg->constvalue);
      break;

    case DATEOID:

      DBUG_PRINT("timescaledb", "bucket origin as Date");
    ...
    case TIMESTAMPOID:
      ...
      DBUG_PRINT("timescaledb", "bucket origin as Timestamp");
    ...
    case TIMESTAMPTZOID:
      ...
      DBUG_PRINT("timescaledb", "bucket origin as TimestampTZ");
                     ...
                     const bool for_rewrites)
{
  ...
  DBUG_PRINT("timescaledb", "query was OK and is supported");
...
get_bucket_width(ContinuousAggTimeBucketInfo bucket_info)
{
  ...
  DBUG_PRINT("timescaledb", "witdh:%ld", width);
  return width;
}
                    ...
                    const bool is_cagg_create)
{
   ...
   */
  DBUG_PRINT("timescaledb", "we need a GROUP By clause with time_bucket on the partitioning column of the hypertable");
...
cagg_get_boundary_converter_funcoid(Oid typoid)
{
  ...
  switch (typoid) {
    case DATEOID:
      DBUG_PRINT("timescaledb", "typoid:'DATEOID', function_name:'INTERNAL_TO_DATE_FUNCTION'");
      function_name = INTERNAL_TO_DATE_FUNCTION;
      break;

    case TIMESTAMPOID:
      DBUG_PRINT("timescaledb", "typoid:'TIMESTAMPOID', function_name:'INTERNAL_TO_TS_FUNCTION'");
      function_name = INTERNAL_TO_TS_FUNCTION;
      break;

    case TIMESTAMPTZOID:
      DBUG_PRINT("timescaledb", "typoid:'INTERNAL_TO_TSTZ_FUNCTION', function_name:'INTERNAL_TO_TSTZ_FUNCTION'");
  ...
  DBUG_PRINT("timescaledb", "get oid(%u) of function to convert from our internal representation to postgres representation", converter_oid);
