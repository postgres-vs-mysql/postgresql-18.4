                                   ...
                                   bool bucketing_refresh_window)
{
  ...
  DBUG_PRINT("timescaledb", "execute refreshes based on the processed invalidations");
...
continuous_agg_refresh(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("timescaledb", "refresh a continuous aggregate across the given window");
  ...
  if (!PG_ARGISNULL(1))
    ...
    DBUG_PRINT("timescaledb", "get min time for a cagg depending of the primary partition type");
...
emit_up_to_date_notice(const ContinuousAgg *cagg, const ContinuousAggRefreshContext context)
{
  ...
  switch (context.callctx) {
    ...
    case CAGG_REFRESH_CREATION:
      DBUG_PRINT("timescaledb", "continuous aggregate \"%s\" is already up-to-date", NameStr(cagg->data.user_view_name));
      elog(NOTICE,
                                ...
                                bool extend_last_bucket)
{
  ...
  PG_TRY();
  {
    ...
    DBUG_PRINT("timescaledb", "commit and start a new transaction");
    ...
    if (refresh_window.start < refresh_window.end &&
      ...
      DBUG_PRINT("timescaledb", "after_process_cagg_materializations");
                                    ...
                                    int32 buckets_per_batch, bool refresh_newest_first)
{
   ...
   *
   */
  DBUG_PRINT("timescaledb", "cap the refresh window to the min and max time of the hypertable");
  if (refresh_window.start_isnull) {
    ...
    if (NULL == slice || TS_TIME_IS_MIN(slice->fd.range_start, refresh_window.type) ||
      ...
      DBUG_PRINT("timescaledb", "no min slice range start for continuous aggregate \"%s.%s\", falling back to single batch processing",
          NameStr(cagg->data.user_view_schema), NameStr(cagg->data.user_view_name));
      elog(DEBUG1,
  ...
  DBUG_PRINT("timescaledb", "compute the inscribed bucket for the capped refresh window range:%ld", bucket_width);
  if (cagg->bucket_function->bucket_fixed_interval == false) {
    ts_compute_inscribed_bucketed_refresh_window_variable(&refresh_window.start,
        &refresh_window.end,
        cagg->bucket_function);
  ...
  DBUG_PRINT("timescaledb", "refresh_window_size:%ld, batch_size:%ld", refresh_window_size, batch_size);
  if (refresh_window_size <= batch_size) {
    ...
    DBUG_PRINT("timescaledb", "refresh window size (%s) is smaller than or equal to batch size (%s), falling back to single batch processing",
        ts_datum_to_string(refresh_size_interval, type), ts_datum_to_string(batch_size_interval, type));
    elog(DEBUG1,
  ...
  if (SPI_processed == 1) {
    DBUG_PRINT("timescaledb", "only one batch produced for continuous aggregate \"%s.%s\", falling back to single batch processing",
        NameStr(cagg->data.user_view_schema), NameStr(cagg->data.user_view_name));
    elog(DEBUG1,
  ...
  if (refresh_window_list == NIL) {
    DBUG_PRINT("timescaledb", "no valid batches produced for continuous aggregate \"%s.%s\", falling back to single batch processing",
        NameStr(cagg->data.user_view_schema), NameStr(cagg->data.user_view_name));
    elog(DEBUG1,
