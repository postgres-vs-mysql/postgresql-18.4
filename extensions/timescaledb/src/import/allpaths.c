...
set_plain_rel_pathlist(PlannerInfo *root, RelOptInfo *rel, RangeTblEntry *rte)
{
  ...
  DBUG_PRINT("timescaledb", "consider sequential scans");
  add_path(root, rel, create_seqscan_path(root, rel, required_outer, 0));

  /* If appropriate, consider parallel sequential scan */
  if (rel->consider_parallel && required_outer == NULL) {
    DBUG_PRINT("timescaledb", "consider parallel sequential scans");
    ts_create_plain_partial_paths(root, rel);
  }

  /* Consider index scans */
  DBUG_PRINT("timescaledb", "consider index scans");
  create_index_paths(root, rel);

  /* Consider TID scans */
  DBUG_PRINT("timescaledb", "consider TID scans");
                           ...
                           RangeTblEntry *parent_rte)
{
   ...
   * non-dummy children.
   */
  DBUG_PRINT("timescaledb", "generate access paths for each member relation, and remember the non-dummy children");
  foreach (l, root->append_rel_list) {
    ...
    DBUG_PRINT("timescaledb", "compute the child's access paths");
    ...
    if (IS_DUMMY_REL(child_rel)) {
      DBUG_PRINT("timescaledb", "if child is dummy, ignore it");
      continue;
    }
     ...
     */
    DBUG_PRINT("timescaledb", "child is live, so add it to the live_childrels list for use below");
    ...
    if (child_rel->consider_startup) {
      DBUG_PRINT("timescaledb", "if consider startup costs on chunks because we can apply SkipScan, should also consider");
      DBUG_PRINT("timescaledb", "startup costs on a hypertable");
...
set_rel_pathlist(PlannerInfo *root, RelOptInfo *rel, Index rti, RangeTblEntry *rte)
{
  ...
  DBUG_PRINT("timescaledb", "based on the function in allpaths.c, with the irrelevant branches removed");
  if (IS_DUMMY_REL(rel)) {
    ...
    switch (rel->rtekind) {
      case RTE_RELATION:
        if (rte->rtekind == RTE_RELATION) {
          ...
          DBUG_PRINT("timescaledb", "relation name: %s.%s, relid:%d", nspname, relname, rte->relid);
        }
        if (rte->relkind == RELKIND_FOREIGN_TABLE) {
          /* Foreign table */
          DBUG_PRINT("timescaledb", "foreign table");
          set_foreign_pathlist(root, rel, rte);
        } else if (rte->tablesample != NULL) {
          /* Sampled relation */
          DBUG_PRINT("timescaledb", "sampled relation");
          set_tablesample_rel_pathlist(root, rel, rte);
        } else {
          /* Plain relation */
          DBUG_PRINT("timescaledb", "plain relation");
  ...
  DBUG_PRINT("timescaledb", "now find the cheapest of the paths for this rel");
...
set_rel_consider_parallel(PlannerInfo *root, RelOptInfo *rel, RangeTblEntry *rte)
{
  ...
  switch (rte->rtekind) {
    case RTE_RELATION:
      ...
      if (get_rel_persistence(rte->relid) == RELPERSISTENCE_TEMP) {
        DBUG_PRINT("timescaledb", "currently, parallel workers can't access the leader's temporary tables");
        return;
      }
      ...
      if (rte->tablesample != NULL) {
        ...
        if (proparallel != PROPARALLEL_SAFE) {
          DBUG_PRINT("timescaledb", "proparallel != PROPARALLEL_SAFE");
          return;
        }
      ...
      if (rte->relkind == RELKIND_FOREIGN_TABLE) {
        ...
        if (!rel->fdwroutine->IsForeignScanParallelSafe) {
          DBUG_PRINT("timescaledb", "IsForeignScanParallelSafe is nullptr");
          return;
        }

        if (!rel->fdwroutine->IsForeignScanParallelSafe(root, rel, rte)) {
          DBUG_PRINT("timescaledb", "IsForeignScanParallelSafe returns false");
          return;
        }
      }
       ...
       */
    {
      ...
      if (limit_needed(subquery)) {
        DBUG_PRINT("timescaledb", "do we actually need a Limit plan node for this subquery? Yes");
        return;
      }
    }
    break;
    ...
    case RTE_FUNCTION:
      ...
      DBUG_PRINT("timescaledb", "check for parallel-restricted functions");
  ...
  if (!is_parallel_safe(root, (Node *) rel->baserestrictinfo)) {
    DBUG_PRINT("timescaledb", "if there's anything in baserestrictinfo that's parallel-restricted, we");
    DBUG_PRINT("timescaledb", "give up on parallelizing access to this relation");
    return;
  }
  ...
  if (!is_parallel_safe(root, (Node *) rel->reltarget->exprs)) {
    DBUG_PRINT("timescaledb", "if the relation's outputs are not parallel-safe, give up");
    return;
  }

  DBUG_PRINT("timescaledb", "consider_parallel: true");
  /* We have a winner. */
...
ts_set_rel_size(PlannerInfo *root, RelOptInfo *rel, Index rti, RangeTblEntry *rte)
{
  ...
  if (rel->reloptkind == RELOPT_BASEREL && relation_excluded_by_constraints(root, rel, rte)) {
    ...
    DBUG_PRINT("timescaledb", "it's an 'append relation', process accordingly");
    ...
    switch (rel->rtekind) {
      case RTE_RELATION:
        if (rte->relkind == RELKIND_FOREIGN_TABLE) {
           ...
           */
          DBUG_PRINT("timescaledb", "we could get here if asked to scan a partitioned table with ONLY");
          DBUG_PRINT("timescaledb", "in that case we shouldn't scan any of the partitions, so mark it as a dummy rel");
          set_dummy_rel_pathlist(root, rel);
        } else if (rte->tablesample != NULL) {
          /* Sampled relation */
          DBUG_PRINT("timescaledb", "sampled relation");
