                                   ...
                                   Index *childRTindex_p)
{
  ...
  if (childrte->relkind == RELKIND_PARTITIONED_TABLE) {
    DBUG_PRINT("timescaledb", "apartitioned child will need to be expanded further");
  ...
  *childRTindex_p = childRTindex;
   ...
   */
  DBUG_PRINT("timescaledb", "build an AppendRelInfo struct for each parent/child pair");
  ...
  DBUG_PRINT("timescaledb", "child_tupdesc->natts:%d", child_tupdesc->natts);
  for (int cattno = 0; cattno < child_tupdesc->natts; cattno++) {
    ...
    if (att->attisdropped) {
      ...
      attname = "";
      DBUG_PRINT("timescaledb", "always insert an empty string for a dropped column");
      ...
      DBUG_PRINT("timescaledb", "duplicate the query-assigned name('%s') for the parent column", attname);
    } else {
      /* New column, just use its real name */
      attname = NameStr(att->attname);
      DBUG_PRINT("timescaledb", "new column('%s'), just use its real name", attname );
    }
  ...
  if (top_parentrc) {
    ...
    DBUG_PRINT("timescaledb", "build a PlanRowMark if parent is marked FOR UPDATE/SHARE");
  ...
  if (bms_is_member(parentRTindex, root->all_result_relids)) {
    ...
    DBUG_PRINT("timescaledb", "OK, record the child as a result rel too");
    ...
    if (childrte->relkind != RELKIND_PARTITIONED_TABLE) {
      ...
      DBUG_PRINT("timescaledb", "register any row-identity columns needed by this child");
