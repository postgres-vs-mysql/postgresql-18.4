...
ts_chunk_scan_by_chunk_ids(const Hyperspace *hs, const List *chunk_ids, unsigned int *num_chunks)
{
  ...
  DBUG_PRINT("timescaledb", "scan for chunks matching a query");
  /*
  ...
  foreach (lc, chunk_ids) {
    ...
    DBUG_PRINT("timescaledb", "we found a chunk. First, try to lock it.");
  ...
  DBUG_PRINT("timescaledb", "chunks:%d, locked_chunk_count:%d", count, locked_chunk_count);
   ...
   */
  DBUG_PRINT("timescaledb", "fetch the chunk constraints");
   ...
   */
  DBUG_PRINT("timescaledb", "build hypercubes for the chunks by finding and combining the dimension slices that match the chunk constraints");
