...
ts_time_datum_get_nobegin(Oid timetype)
{
  ...
  switch (timetype) {
    case DATEOID:
      DBUG_PRINT("timescaledb", "timetype:DATEOID, returning DATEVAL_NOBEGIN");
      return DateADTGetDatum(DATEVAL_NOBEGIN);

    case TIMESTAMPOID:
      DBUG_PRINT("timescaledb", "timetype:TIMESTAMPOID, returning DT_NOBEGIN");
      return TimestampGetDatum(DT_NOBEGIN);

    case TIMESTAMPTZOID:
      DBUG_PRINT("timescaledb", "timetype:TIMESTAMPTZOID, returning DT_NOBEGIN");
...
ts_time_datum_get_noend(Oid timetype)
{
  ...
  switch (timetype) {
    case DATEOID:
      DBUG_PRINT("timescaledb", "timetype:DATEOID, returning DATEVAL_NOEND");
      return DateADTGetDatum(DATEVAL_NOEND);

    case TIMESTAMPOID:
      DBUG_PRINT("timescaledb", "timetype:TIMESTAMPOID, returning DT_NOEND");
      return TimestampGetDatum(DT_NOEND);

    case TIMESTAMPTZOID:
      DBUG_PRINT("timescaledb", "timetype:TIMESTAMPTZOID, returning DT_NOEND");
