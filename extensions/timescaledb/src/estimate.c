...
estimate_max_spread_var(PlannerInfo *root, Var *var)
{
  ...
  DBUG_PRINT("timescaledb", "estimate the max spread on a time var in terms of the internal time representation:%g", (double) (max - min));
...
estimate_max_spread_opexpr(PlannerInfo *root, OpExpr *opexpr)
{
  ...
  if (function_name[0] == '-' || function_name[0] == '+') {
    result = estimate_max_spread_expr(root, nonconst);
    DBUG_PRINT("timescaledb", "return cost:%g", result);
    return result;
  }
...
estimate_max_spread_expr(PlannerInfo *root, Expr *expr)
{
  ...
  switch (nodeTag(expr)) {
    case T_Var:
      result = estimate_max_spread_var(root, (Var *) expr);
      DBUG_PRINT("timescaledb", "estimate the max spread (max(value)-min(value)) of the expr:%g", result);
      return result;

    case T_OpExpr:
      result = estimate_max_spread_opexpr(root, (OpExpr *) expr);
      DBUG_PRINT("timescaledb", "estimate the max spread (max(value)-min(value)) of the expr:%g", result);
      return result;
...
ts_estimate_group_expr_interval(PlannerInfo *root, Expr *expr, double interval_period)
{
  ...
  result = clamp_row_est(max_period / interval_period);
  DBUG_PRINT("timescaledb", "return an estimate for the number of groups formed:%g", result);
  return result;
}
...
group_estimate_integer_division(PlannerInfo *root, Oid opno, Node *left, Node *right)
{
  ...
  if (function_name[0] == '/' && function_name[1] == '\0' && IsA(right, Const)) {
    ...
    result = ts_estimate_group_expr_interval(root, (Expr *) left, (double) c->constvalue);
    DBUG_PRINT("timescaledb", "cost:%g", result);
    return result;
  }
...
group_estimate_funcexpr(PlannerInfo *root, FuncExpr *group_estimate_func, double path_rows)
{
  ...
  if (func_est && func_est->group_estimate) {
    result = func_est->group_estimate(root, group_estimate_func, path_rows);
    DBUG_PRINT("timescaledb", "cost:%g", result);
    return result;
  }
...
group_estimate_expr(PlannerInfo *root, Node *expr, double path_rows)
{
  ...
  switch (nodeTag(expr)) {
    case T_FuncExpr:
      result = group_estimate_funcexpr(root, (FuncExpr *) expr, path_rows);
      DBUG_PRINT("timescaledb", "cost:%g", result);
      return result;

    case T_OpExpr:
      result = group_estimate_opexpr(root, (OpExpr *) expr, path_rows);
      DBUG_PRINT("timescaledb", "cost:%g", result);
      return result;
...
group_estimate_opexpr(PlannerInfo *root, OpExpr *opexpr, double path_rows)
{
  ...
  if (IsA(first, Const)) {
    result = group_estimate_expr(root, second, path_rows);
    DBUG_PRINT("timescaledb", "cost:%g", result);
    return result;
  ...
  if (IsA(second, Const)) {
    result = group_estimate_expr(root, first, path_rows);
    DBUG_PRINT("timescaledb", "cost:%g", result);
    return result;
  }
...
ts_estimate_group(PlannerInfo *root, double path_rows)
{
  ...
  result = clamp_row_est(d_num_groups);
  DBUG_PRINT("timescaledb", "estimate of the groups for which custom estimates don't exist:%g", result);
  return result;
}
