...
ts_chunk_id_find_in_subspace(Hypertable *ht, List *dimension_vecs)
{
  ...
  foreach (lc, dimension_vecs) {
    ...
    if (vec->dri->dimension->type == DIMENSION_TYPE_STATS) {
      ...
      DBUG_PRINT("timescaledb", "add these chunks to the context appropriately");
                             ...
                             bool fail_if_not_found)
{
  ...
  DBUG_PRINT("timescaledb", "schema:'%s', table:'%s'", schema, table);
...
ts_chunk_id_from_relid(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("timescaledb", "lookup a Chunk ID from a chunk's relid(%u): %d", relid, last_id);
...
ts_chunk_get_id(const char *schema, const char *table, int32 *chunk_id, bool missing_ok)
{
  ...
  if (NULL != chunk_id) {
    *chunk_id = form.id;
    DBUG_PRINT("timescaledb", "chunk_id:%d", *chunk_id);
  }
...
chunk_delete(ScanIterator *iterator, Oid relid, DropBehavior behavior, bool detach)
{
  ...
  DBUG_PRINT("timescaledb", "count:%d", count);
  return count;
}
...
ts_chunk_set_name(Chunk *chunk, const char *newname)
{
  ...
  DBUG_PRINT("timescaledb", "newname:%s", newname);
  return true;
}
...
ts_chunk_set_schema(Chunk *chunk, const char *newschema)
{
  ...
  DBUG_PRINT("timescaledb", "newschema:%s", newschema);
  found = lock_chunk_tuple(chunk->fd.id, &tid, &form);
  Assert(found);
...
ts_chunk_set_unordered(Chunk *chunk)
{
  ...
  bool result = ts_chunk_add_status(chunk, CHUNK_STATUS_COMPRESSED_UNORDERED);
...
{
  DBUG_TRACE;
  bool result = ts_chunk_add_status(chunk, CHUNK_STATUS_FROZEN);
...
{
  bool result = ts_flags_are_set_32(chunk->fd.status, CHUNK_STATUS_FROZEN);
...
ts_chunk_drop_single_chunk(PG_FUNCTION_ARGS)
{
  ...
  if (ts_chunk_contains_compressed_data(ch)) {
    DBUG_INSTANT_PRINT("timescaledb", "dropping compressed chunks not supported");
    ereport(ERROR,
            (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
...
ts_chunk_drop_chunks(PG_FUNCTION_ARGS)
{
  ...
  if (PG_ARGISNULL(0)) {
    DBUG_INSTANT_PRINT("timescaledb", "invalid hypertable or continuous aggregate");
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
...
{
  DBUG_TRACE;
  bool result = ts_flags_are_set_32(chunk->fd.status, CHUNK_STATUS_COMPRESSED);
...
{
  DBUG_TRACE;
  bool result = !ts_chunk_is_compressed(chunk);
...
ts_chunk_needs_recompression(const Chunk *chunk)
{
  ...
  bool result = ts_chunk_is_partial(chunk) || ts_chunk_is_unordered(chunk);
...
{
  DBUG_TRACE;
  bool result = ts_flags_are_set_32(chunk->fd.status, CHUNK_STATUS_COMPRESSED_PARTIAL);
...
get_chunk_operation_str(ChunkOperation cmd)
{
  ...
  switch (cmd) {
    case CHUNK_INSERT:
      DBUG_PRINT("timescaledb", "return 'Insert'");
      return "Insert";

    case CHUNK_DELETE:
      DBUG_PRINT("timescaledb", "return 'Delete'");
      return "Delete";

    case CHUNK_UPDATE:
      DBUG_PRINT("timescaledb", "return 'Update'");
      return "Update";

    case CHUNK_COMPRESS:
      DBUG_PRINT("timescaledb", "return 'compress_chunk'");
      return "compress_chunk";

    case CHUNK_DECOMPRESS:
      DBUG_PRINT("timescaledb", "return 'decompress_chunk'");
      return "decompress_chunk";

    case CHUNK_DROP:
      DBUG_PRINT("timescaledb", "return 'drop_chunk'");
      return "drop_chunk";

    default:
      DBUG_PRINT("timescaledb", "return 'Unsupported'");
      return "Unsupported";
  }
}
...
ts_chunk_get_osm_chunk_id(int hypertable_id)
{
   ...
   */
  DBUG_PRINT("timescaledb", "perform an index scan on hypertable ID + osm_chunk");
  ScanKeyInit(&scankey[0],
...
ts_chunk_vec_sort(ChunkVec **chunks)
{
  ...
  if (vec->num_chunks > 1) {
    DBUG_PRINT("timescaledb", "qsort chunks(num_chunks:%u)", vec->num_chunks);
                              ...
                              int64 start_value, StrategyNumber end_strategy, int64 end_value)
{
  ...
  DBUG_PRINT("timescaledb", "hypertable_id:%u, start_strategy:%u, start_value:%ld, end_strategy:%u, end_value:%ld",
      hypertable_id, start_strategy, start_value, end_strategy, end_value);
  it->ctx.index =
                                        ...
                                        ScanTupLock *tupLock)
{
  ...
  DBUG_PRINT("timescaledb", "start_strategy:%u, start_value:%ld, end_strategy:%u, end_value:%ld, limit:%d",
      start_strategy, start_value, end_strategy, end_value, limit);
  ...
  *num_chunks = chunk_vec->num_chunks;
  DBUG_PRINT("timescaledb", "num_chunks:%lu", *num_chunks);
                                  ...
                                  ScanTupLock *tupLock)
{
  ...
  DBUG_PRINT("timescaledb", "older_than:%ld, newer_than:%ld", older_than, newer_than);
  if (older_than <= newer_than)
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
  ...
  *num_chunks_returned = num_chunks;
  ...
  DBUG_PRINT("timescaledb", "num_chunks_returned:%ld", *num_chunks_returned);
  return chunks;
}
...
ts_chunk_detach_by_relid(Oid relid)
{
  ...
  DBUG_PRINT("timescaledb", "relid:%u, schema:'%s', table:'%s' and count:%d", relid, schema, table, count);
