...
{
  DBUG_TRACE;
  DBUG_PRINT("timescaledb", "sort(qsort) the hypercubes slices in ascending dimension ID order");
...
ts_hypercube_find_existing_slices(const Hypercube *cube, const ScanTupLock *tuplock)
{
  ...
  DBUG_PRINT("timescaledb", "num_slices:%d", cube->num_slices);
  ...
  DBUG_PRINT("timescaledb", "find slices:%d in the hypercube that already exists in metadata", num_found);
  return num_found;
}
...
ts_hypercube_calculate_from_point(const Hyperspace *hs, const Point *p, const ScanTupLock *tuplock)
{
  ...
  DBUG_PRINT("timescaledb", "cube's num_slices:%d", cube->num_slices);
