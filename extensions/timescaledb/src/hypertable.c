...
ts_hypertable_find_chunk_for_point(const Hypertable *h, const Point *point, LOCKMODE lockmode)
{
  ...
  if (chunk) {
    DBUG_PRINT("timescaledb", "find the chunk(id:%d, hypertable_id:%d) responsible for the given point", chunk->fd.id, chunk->fd.hypertable_id);
  }
  return chunk;
}
                              ...
                              text *target_size, Oid sizing_func, bool is_generic)
{
  ...
  DBUG_PRINT("timescaledb", "create a hypertable from an existing table");
...
ts_hypertable_create(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("timescaledb", "process create_hypertable parameters for time specific implementation");

  if (!OidIsValid(table_relid)) {
    DBUG_INSTANT_PRINT("timescaledb", "relation cannot be NULL");
    ereport(ERROR,
            (errcode(ERRCODE_INVALID_PARAMETER_VALUE), errmsg("relation cannot be NULL")));
  }

  if (get_rel_name(table_relid) == NULL) {
    DBUG_INSTANT_PRINT("timescaledb", "relation with oid %d not found", table_relid);
    ereport(ERROR,
            (errcode(ERRCODE_UNDEFINED_TABLE),
  ...
  if (!open_dim_name) {
    DBUG_INSTANT_PRINT("timescaledb", "partition column cannot be NULL");
    ereport(ERROR,
