...
get_timeout_millisec(TimestampTz by_time)
{
  ...
  if (TIMESTAMP_IS_NOBEGIN(by_time)) {
    DBUG_PRINT("timescaledb", "return result:0");
    return 0;
  }

  if (TIMESTAMP_IS_NOEND(by_time)) {
    DBUG_PRINT("timescaledb", "return result:PG_INT64_MAX");
    return PG_INT64_MAX;
  }
  ...
  result = (int64) ((timeout_sec * MILLISECS_PER_SEC) +
                  (((int64) timeout_usec) / USECS_PER_MILLISEC));
  DBUG_PRINT("timescaledb", "return result:%ld", result);
  return result;
}
