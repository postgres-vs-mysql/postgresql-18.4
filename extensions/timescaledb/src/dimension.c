...
ts_hyperspace_calculate_point(const Hyperspace *hs, TupleTableSlot *slot)
{
  ...
  DBUG_PRINT("timescaledb", "num_dimensions:%d, point's num_coords:%d", hs->num_dimensions, p->num_coords);
  return p;
}
...
ts_dimension_set_num_slices(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("timescaledb", "num_slices:%d", num_slices);
...
{
  DBUG_TRACE;
  DBUG_INSTANT_PRINT("timescaledb", "cannot construct type \"dimension_info\" from string");
  ereport(ERROR,
          (errcode(ERRCODE_FEATURE_NOT_SUPPORTED),
...
ts_dimension_info_out(PG_FUNCTION_ARGS)
{
  ...
  DBUG_PRINT("timescaledb", "%s", str.data);
