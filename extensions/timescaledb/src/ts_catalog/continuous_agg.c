...
generic_time_bucket(const ContinuousAggBucketFunction *bf, Datum timestamp)
{
  ...
  if (bf->bucket_time_timezone != NULL) {
    ...
    fcinfo->args[0] =
    (NullableDatum) {
      .value = IntervalPGetDatum(bf->bucket_time_width), .isnull = false
    };
    fcinfo->args[1] = (NullableDatum) {
      .value = timestamp, .isnull = false
    };
    fcinfo->args[2] = (NullableDatum) {
      .value = CStringGetTextDatum(bf->bucket_time_timezone),
      .isnull = false
    };

    if (TIMESTAMP_NOT_FINITE(bf->bucket_time_origin))
      fcinfo->args[3] = (NullableDatum) {
      .value = (Datum) 0, .isnull = true
    };
    else
      fcinfo->args[3] = (NullableDatum) {
      .value = TimestampTzGetDatum(bf->bucket_time_origin),
      .isnull = false
    };

    if (has_offset)
      fcinfo->args[4] = (NullableDatum) {
      .value = IntervalPGetDatum(bf->bucket_time_offset),
      .isnull = false
    };
    else
      fcinfo->args[4] = (NullableDatum) {
