...
ts_array_is_member(ArrayType *arr, const char *name)
{
  ...
  if (ret) {
    DBUG_PRINT("timescaledb", "return true for name:'%s'", name);
  } else {
    DBUG_PRINT("timescaledb", "return false for name:'%s'", name);
  }
  return ret;
}
...
ts_array_position(ArrayType *arr, const char *name)
{
  ...
  array_free_iterator(it);
  if (found) {
    DBUG_PRINT("timescaledb", "return pos:%d for name:'%s'", pos, name);
  } else {
    DBUG_PRINT("timescaledb", "return pos:0 for name:'%s'", name);
  }
  return found ? pos : 0;
}
