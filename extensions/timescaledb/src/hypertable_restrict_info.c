                                 ...
                                 DimensionValues *dimvalues)
{
  ...
  DBUG_PRINT("timescaledb", "add restriction for open (time) dimension");
  /*
  ...
  if (restriction_added) {
    DBUG_PRINT("timescaledb", "restriction_added: true");
  } else {
    DBUG_PRINT("timescaledb", "restriction_added: false");
  }
...
hypertable_restrict_info_get(HypertableRestrictInfo *hri, AttrNumber attno)
{
  ...
  for (i = 0; i < hri->num_dimensions; i++) {
    if (hri->dimension_restriction[i]->dimension->column_attno == attno) {
      DBUG_PRINT("timescaledb", "matched at i=%d (num_dimensions=%d attno=%d)", i, hri->num_dimensions, attno);
                                  ...
                                  bool use_or)
{
  ...
  if (proven_true_by_hri) {
    ...
    DBUG_PRINT("timescaledb", "return true if the restriction was accepted exactly");
  } else {
...
ts_hypertable_restrict_info_add_clause(HypertableRestrictInfo *hri, PlannerInfo *root, Expr *e)
{
  ...
  result = hypertable_restrict_info_add_expr(hri, root, var, arg_value, opno, value_func, use_or);
  if (result) {
    ...
    List **nested_oids, unsigned int *num_chunks)
{
  ...
  if (reverse) {
    DBUG_PRINT("timescaledb", "num_chunks:%u, reverse:true and qsort is called", *num_chunks);
    qsort((void *) chunks, *num_chunks, sizeof(Chunk *), chunk_cmp_reverse);
  } else {
    DBUG_PRINT("timescaledb", "num_chunks:%u, reverse:false and qsort is called", *num_chunks);
  ...
  DBUG_PRINT("timescaledb", "chunks appended:%d", added);
  return chunks;
}
