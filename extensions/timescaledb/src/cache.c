...
ts_cache_init(Cache *cache)
{
  ...
  DBUG_PRINT("timescaledb", "cache->name:'%s', cache->numelements:%ld, cache->refcount:%d", cache->name, cache->numelements, cache->refcount);
}
...
cache_destroy(Cache **cache_ptr)
{
  ...
  if (cache->refcount > 0) {
    ...
    DBUG_PRINT("timescaledb", "it will be destroyed later");
    return;
  }
...
remove_pin(Cache *cache, SubTransactionId subtxnid)
{
  ...
  foreach (lc, pinned_caches) {
    ...
    if (cp->cache == cache && cp->subtxnid == subtxnid) {
      ...
      DBUG_PRINT("timescaledb", "free cache memory and then remove the pin");
