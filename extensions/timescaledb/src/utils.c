...
ts_pg_timestamp_to_unix_microseconds(PG_FUNCTION_ARGS)
{
  ...
  if (TIMESTAMP_IS_NOBEGIN(timestamp)) {
    DBUG_PRINT("timescaledb", "return %ld", TS_TIME_NOBEGIN);
    PG_RETURN_INT64(TS_TIME_NOBEGIN);
  }

  if (TIMESTAMP_IS_NOEND(timestamp)) {
    DBUG_PRINT("timescaledb", "return %ld", TS_TIME_NOEND);
    PG_RETURN_INT64(TS_TIME_NOEND);
  }

  if (timestamp < TS_TIMESTAMP_MIN) {
    DBUG_INSTANT_PRINT("timescaledb", "timestamp out of range");
    ereport(ERROR,
  ...
  DBUG_PRINT("timescaledb", "return %ld", timestamp + TS_EPOCH_DIFF_MICROSECONDS);
...
ts_time_value_to_internal(Datum time_val, Oid type_oid)
{
  ...
  switch (type_oid) {
    ...
    case TIMESTAMPOID:
       ...
       * at UTC
       */
      DBUG_PRINT("timescaledb", "for timestamps, ignore timezones, make believe the timestamp is at UTC");
