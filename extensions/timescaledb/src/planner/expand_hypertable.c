...
is_timestamptz_op_interval(Expr *expr)
{
  ...
  DBUG_PRINT("timescaledb", "pre-check to determine if an expression is eligible for constification");
  ...
  result = (c1->consttype == TIMESTAMPTZOID && c2->consttype == INTERVALOID) ||
...
collect_quals_walker(Node *node, CollectQualCtx *ctx)
{
  ...
  if (IsA(node, FromExpr)) {
    ...
    if (IS_OUTER_JOIN(j->jointype)) {
      ctx->join_level++;
      result = expression_tree_walker(node, collect_quals_walker, ctx);
      ctx->join_level--;
                    ...
                    bool *reverse)
{
  ...
  result = ts_ordered_append_should_optimize(root, rel, ht, order_attno, reverse);
           ...
           List **quals_proven_true_by_hri_out)
{
  ...
  DBUG_PRINT("timescaledb", "get chunks from restrict info");
...
timebucket_annotate_walker(Node *node, CollectQualCtx *ctx)
{
  ...
  if (IsA(node, FromExpr)) {
    FromExpr *f = castNode(FromExpr, node);
    DBUG_PRINT("timescaledb", "it is a FromExpr");
    f->quals = timebucket_annotate(f->quals, ctx);
  } else if (IsA(node, JoinExpr)) {
    DBUG_PRINT("timescaledb", "it is a JoinExpr");
    JoinExpr *j = castNode(JoinExpr, node);
...
ts_plan_expand_timebucket_annotate(PlannerInfo *root, RelOptInfo *rel)
{
  ...
  DBUG_PRINT("timescaledb", "walk the tree and find restrictions or chunk exclusion functions");
                                 ...
                                 bool include_osm)
{
  ...
  if (!num_chunks) {
    DBUG_PRINT("timescaledb", "nothing to do here if we have no chunks");
    return;
  } else {
    DBUG_PRINT("timescaledb", "num_chunks:%u", num_chunks);
  }
