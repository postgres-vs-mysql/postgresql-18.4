...
excluded_by_constraint(PlannerInfo *root, RangeTblEntry *rte, Index rt_index, List *restrictinfos)
{
  ...
  result = relation_excluded_by_constraints(root, &rel, rte);
  if (result) {
...
{
  DBUG_TRACE;
  bool result = rte->rtekind == RTE_RELATION && rte->relkind == RELKIND_RELATION && !rte->inh &&
...
ts_constraint_aware_append_path_create(PlannerInfo *root, Path *subpath)
{
  ...
  DBUG_PRINT("timescaledb", "startup_cost:%g, total_cost:%g", path->cpath.path.startup_cost, path->cpath.path.total_cost);
