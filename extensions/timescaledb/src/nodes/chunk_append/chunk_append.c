...
{
  DBUG_TRACE;
  bool result = IsA(path, CustomPath) &&
...
{
  DBUG_TRACE;
  bool result = list_length(jointree->fromlist) != 1 || !IsA(linitial(jointree->fromlist), RangeTblRef);
  if (result) {
    DBUG_PRINT("timescaledb", "result: true");
  } else {
    DBUG_PRINT("timescaledb", "result: false");
                     ...
                     Relids required_outer, List *partitioned_rels, List **nested_children)
{
  ...
  if (list_length(group) > 1) {
    ...
    *nested_children = lappend(*nested_children, append);
    ...
    DBUG_PRINT("timescaledb", "if group only has 1 member we can add it directly");
...
ts_chunk_append_path_copy(ChunkAppendPath *ca, List *subpaths, PathTarget *pathtarget)
{
  ...
  foreach (lc, subpaths) {
    ...
    rows += child->rows;
    DBUG_PRINT("timescaledb", "child->total_cost:%g, child->rows:%g, total_cost:%g, rows:%g",
        child->total_cost, child->rows, total_cost, rows);
  }

  new->cpath.path.total_cost = total_cost;
  new->cpath.path.rows = rows;
  new->cpath.path.pathtarget = copy_pathtarget(pathtarget);
  DBUG_PRINT("timescaledb", "new->cpath.path.total_cost:%g, new->cpath.path.rows:%g", total_cost, rows);
                            ...
                            bool parallel_aware, bool ordered, List *nested_oids)
{
  ...
  DBUG_PRINT("timescaledb", "set pathtype:custom scan");
  ...
  foreach (lc, path->cpath.custom_paths) {
    ...
    if (!path->pushdown_limit || path->limit_tuples == -1 || rows < path->limit_tuples) {
      total_cost += child->total_cost;
      rows += child->rows;
      DBUG_PRINT("timescaledb", "child rows:%g, child total cost:%g", child->rows, child->total_cost);
    }
  }

  path->cpath.path.rows = rows;
  path->cpath.path.total_cost = total_cost;
  DBUG_PRINT("timescaledb", "rows:%g, total cost:%g", rows, total_cost);

  if (path->cpath.custom_paths != NIL) {
    path->cpath.path.startup_cost = ((Path *) linitial(path->cpath.custom_paths))->startup_cost;
    DBUG_PRINT("timescaledb", "startup cost:%g", path->cpath.path.startup_cost);
  }
