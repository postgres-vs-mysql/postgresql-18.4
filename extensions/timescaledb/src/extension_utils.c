...
extension_version(char const *const extension_name)
{
  ...
  DBUG_PRINT("timescaledb", "version:'%s'", sql_version);
  return sql_version;
}
...
extension_current_state(char const *const extension_name, char const *const schema_name,
   ...
   */
  DBUG_PRINT("timescaledb", "extension_name:'%s', schema_name:'%s', table_name:'%s'", extension_name,
      schema_name, table_name);
  if (!IsNormalProcessingMode() || !IsTransactionState() || !OidIsValid(MyDatabaseId)) {
    DBUG_PRINT("timescaledb", "we cannot be sure what the state is");
  ...
  if (extension_is_transitioning(extension_name)) {
    DBUG_PRINT("timescaledb", "want to be in TRANSITIONING state even before that table is created");
    DBUG_PRINT("timescaledb", "TRANSITIONING only occurs in the middle of a CREATE EXTENSION or ALTER EXTENSION UPDATE");
  ...
  if (OidIsValid(ts_get_relation_relid(schema_name, table_name, true)))
  {
    ...
    DBUG_PRINT("timescaledb", "the extension is created");
    return EXTENSION_STATE_CREATED;
  }

  DBUG_PRINT("timescaledb", "this backend knows that the extension is not present");
