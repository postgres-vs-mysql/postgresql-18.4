...
static int r_postlude(struct SN_env * z)
{
  ...
  while(1) {
    ...
    if (z->c >= z->l || z->p[z->c + 0] >> 5 != 2 || !((35652352 >> (z->p[z->c + 0] & 0x1f)) & 1)) among_var = 7;
    else
...
lab0:
    ...
    if (out_grouping_b_U(z, g_v, 97, 251, 0)) {
      z->lb = mlimit1;
      return 0;
    }
...
lab2:
        ...
        if (out_grouping_b_U(z, g_keep_with_s, 97, 232, 0)) {
          z->c = z->l - m1;
          goto lab0;
        }
      }
...
lab0:
...

  {
    ...
    if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((278560 >> (z->p[z->c - 1] & 0x1f)) & 1)) {
      z->lb = mlimit4;
      return 0;
    }
