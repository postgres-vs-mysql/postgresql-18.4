...
static int r_prelude(struct SN_env * z)
{
  ...
  while(1) {
    ...
    if (z->c + 1 >= z->l || (z->p[z->c + 1] != 163 && z->p[z->c + 1] != 181)) among_var = 3;
    else
...
static int r_postlude(struct SN_env * z)
{
  ...
  while(1) {
    ...
    if (z->c + 1 >= z->l || z->p[z->c + 1] != 126) among_var = 3;
    else
...
static int r_standard_suffix(struct SN_env * z)
{
  ...
  switch (among_var) {
...

    {
      ...
      if (z->c - 1 <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((4718616 >> (z->p[z->c - 1] & 0x1f)) & 1)) {
        z->c = z->l - m1;
        goto lab0;
      }
...
lab0:
...

    {
      ...
      if (z->c - 3 <= z->lb || (z->p[z->c - 1] != 101 && z->p[z->c - 1] != 108)) {
        z->c = z->l - m2;
        goto lab1;
      }

      if (!find_among_b(z, a_3, 3)) {
        z->c = z->l - m2;
        goto lab1;
      }
