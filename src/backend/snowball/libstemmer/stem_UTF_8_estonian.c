...
static int r_verb(struct SN_env * z)
{
...

  {
    ...
    if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((540726 >> (z->p[z->c - 1] & 0x1f)) & 1)) {
      z->lb = mlimit1;
      return 0;
    }
...
static int r_i_plural(struct SN_env * z)
{

  {
    ...
    if (z->c <= z->lb || z->p[z->c - 1] != 105) {
      z->lb = mlimit1;
      return 0;
    }

    if (!find_among_b(z, a_3, 1)) {
      z->lb = mlimit1;
      return 0;
    }
...
static int r_special_noun_endings(struct SN_env * z)
{
...

  {
    ...
    if (z->c - 3 <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((1049120 >> (z->p[z->c - 1] & 0x1f)) & 1)) {
      z->lb = mlimit1;
      return 0;
    }
...
static int r_case_ending(struct SN_env * z)
{
...

  {
    ...
    if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((1576994 >> (z->p[z->c - 1] & 0x1f)) & 1)) {
      z->lb = mlimit1;
      return 0;
    }
...
lab0:
    ...
    case 3: {
      ...
      if (z->c <= z->lb || (z->p[z->c - 1] != 115 && z->p[z->c - 1] != 116)) among_var = 2;
      else
...
static int r_degrees(struct SN_env * z)
{
...

  {
    ...
    if (z->c <= z->lb || z->p[z->c - 1] >> 5 != 3 || !((8706 >> (z->p[z->c - 1] & 0x1f)) & 1)) {
      z->lb = mlimit1;
      return 0;
    }
