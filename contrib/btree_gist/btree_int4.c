...
gbt_int4_consistent(PG_FUNCTION_ARGS)
{
  ...
  *recheck = false;
  ...
  DBUG_PRINT("btree_gist",
                   "consistent check: range=[%d,%d] query=%d strategy='%s'",
                   *((int32 *) key.lower),
                   *((int32 *) key.upper),
                   query,
  ...
  if (result) {
    DBUG_PRINT("btree_gist", "gbt_int4_consistent returns true");
  } else {
    DBUG_PRINT("btree_gist", "gbt_int4_consistent returns false");
  }
